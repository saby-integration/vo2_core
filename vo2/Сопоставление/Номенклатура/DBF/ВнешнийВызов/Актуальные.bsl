...
Функция НоменклатураПоставщика_МассовоеОбновлениеСервер(ПараметрыОбновитьВходящие)
	...
	Для Каждого СтрНоменклатураПоставщика Из ПараметрыОбновитьВходящие.Номенклатура Цикл
		...
		Если	СтрНоменклатураПоставщика.Свойство("НеобходимоСопоставление")
			...
			Продолжить;
		Иначе
			СтрНоменклатураПоставщика.НеобходимоСопоставление = Ложь;
		КонецЕсли;
...
Функция НоменклатураПоставщика_Список(ПараметрыОбновитьВходящие, ДопПараметры) Экспорт
	...
	НеобходимоСопоставление = Ложь;
	
	Если Не ДопПараметры = Неопределено Тогда
		Если ДопПараметры.Свойство("НеобходимоСопоставление") Тогда
			НеобходимоСопоставление = ДопПараметры.НеобходимоСопоставление;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Если ПараметрыОбновитьВходящие.Свойство("Фильтр") Тогда
			ПараметрыСпискаСервер.Вставить("Фильтр",	ПараметрыОбновитьВходящие.Фильтр);
		КонецЕсли;

		ДопПараметрыСервер = Новый Структура();
		ДопПараметрыСервер.Вставить("ТипыПолей1С", МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("ТипыПолей1С"));
		ДопПараметрыСервер.Вставить("НеобходимоСопоставление", НеобходимоСопоставление);
...
Функция	НоменклатураПоставщика_СписокСервер(ПараметрыСписка, ДопПараметры)
	...
	Если ПараметрыСписка.Свойство("Фильтр") Тогда
		...
		ПараметрыСписка.Фильтр.Свойство("НаименованиеСБИС", ФильтрНаименованиеСБИС);
	...
	Попытка
		ШаблонЗапроса = "Select {top} KEY_ID, NSB_ID, NSB_TYID, NSB_NM, CHRSB_ID from {base_sbis} {where} ORDER BY NSB_NM";
		...
		Иначе
			...
			Иначе
				ЧислоПропустить = 0;
			КонецЕсли;
			ЧислоПропустить = ЧислоПропустить + 1;
	...
	Пока Не Recordset.EOF Цикл
		...
		ИначеЕсли СчетчикПозиций = РазмерВыборки Тогда
			...
			Прервать;
		КонецЕсли;
		ДопПараметрыКласса = Новый Структура("НеобходимоСопоставление", ДопПараметры.НеобходимоСопоставление);
		СтрокаСопоставленияСБИС = МодульКлассаСервер.НовыйСтрокаСопоставленияСБИССервер(,ДопПараметрыКласса);
		
		КлючНоменклатурыСБИС	= ПолучитьЗначениеИзРекордСет(Recordset, "KEY_ID");
		...
		ХарактеристикаСБИС		= ПолучитьЗначениеИзРекордСет(Recordset, "CHRSB_ID");
		...
		МодульКлассаСервер.СтрокаСопоставленияСБИССервер_Вставить(СтрокаСопоставленияСБИС, "ИмяХарактеристики",	ХарактеристикаСБИС);
		
		СоответствиеПозицийИдСБИС.Вставить(КлючНоменклатурыСБИС, СтрокаСопоставленияСБИС);
		РезультатСписка.Список.Добавить(СтрокаСопоставленияСБИС);
		
		СписокКлючей = СписокКлючей + "'" + КлючНоменклатурыСБИС + "',";
	...
	Пока Не Recordset.EOF Цикл
		...
		Если Не ЗначениеЗаполнено(СсылкаНоменклатура1С) Тогда
			// Без ссылок на номенклатуру 1С не обрабатываем
			Recordset.MoveNext();
			Продолжить;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СписокКлючейСБИС) Тогда
		...
		Пока Не Recordset.EOF Цикл
			...
			Иначе
				СопоставлениеДляЕдиницы	= МодульОбъектаСервер.НовыйСопоставлениеДляЕдиницыСервер();
				МодульОбъектаСервер.СопоставлениеДляЕдиницыСервер_Вставить(СопоставлениеДляЕдиницы, "Владелец",	СсылкаНоменклатура);
				СсылкаНаЕдиницу1С = МодульОбъектаСервер.СопоставлениеДляЕдиницыСервер_Получить(СопоставлениеДляЕдиницы, "Ссылка");
				
				МодульОбъектаСервер.СопоставлениеДляЕдиницыСервер_Вставить(СопоставлениеДляЕдиницы, "ОКЕИ", 	СсылкаНаЕдиницу1С.Код);
				МодульОбъектаСервер.СопоставлениеДляЕдиницыСервер_Вставить(СопоставлениеДляЕдиницы, "Название",	СсылкаНаЕдиницу1С.Наименование);
