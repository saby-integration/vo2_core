...
&НаКлиенте
Функция НайтиНоменклатуруПоставщикаПоТабличнойЧасти(стрКонтрагент, мТаблДок, КаталогНастроек, Ини, ДопПараметры = Неопределено) Экспорт
...
Функция НайтиНоменклатуруПоставщикаПоТабличнойЧасти_СсылкаСервер(стрКонтрагент,  мТаблДок) Экспорт
	...
	Исключение
		МодульОбъектаСервер.ВызватьСбисИсключение(ИнформацияОбОшибке(), "СопоставлениеНоменклатуры_ДБФ.НайтиНоменклатуруПоставщикаПоТабличнойЧасти_Сервер");
	КонецПопытки;
	...
	Для Каждого СтрокаРезультат Из НоменклатураНайти Цикл
		...
		СтрокаПоиска.Вставить("Название",		МодульОбъектаСервер.СтрокаСопоставленияСБИССервер_Получить(СтрокаРезультат, "Наименование"));
		СтрокаПоиска.Вставить("Идентификатор",	МодульОбъектаСервер.СтрокаСопоставленияСБИССервер_Получить(СтрокаРезультат, "Код"));
...
&НаКлиенте
Функция НайтиНоменклатуруПоставщика(стрКонтрагент, СтрНоменклатураПоставщика, КаталогНастроек, Ини, ДопПараметры = Неопределено) Экспорт

	Если Не ПараметрыСоединения.НовоеСопоставление Тогда
		Возврат old_НайтиНоменклатуруПоставщика(стрКонтрагент, СтрНоменклатураПоставщика, КаталогНастроек, Ини, ДопПараметры);
	КонецЕсли;
...
Функция НайтиНоменклатуруПоставщика_СсылкаСервер(стрКонтрагент, СтрНоменклатураПоставщика)
	...
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
		МодульОбъектаСервер.ВызватьСбисИсключение(ИнфоОбОшибке, "СопоставлениеНоменклатуры_ДБФ.НайтиНоменклатуруПоставщикаПоТабличнойЧасти");
	КонецПопытки;
...
Функция ПолучитьИдентификаторНоменклатурыПоставщика_СсылкаСервер(стрКонтрагент, стрНоменклатура)
	...
	Попытка
		...
		Возврат МодульОбъектаСервер.СтрокаСопоставленияСБИССервер_Получить(НоменклатураНайти[0], "Код");
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
		МодульОбъектаСервер.ВызватьСбисИсключение(ИнфоОбОшибке, "СопоставлениеНоменклатуры_ДБФ.ПолучитьИдентификаторНоменклатурыПоставщика_Сервер");
	КонецПопытки;
