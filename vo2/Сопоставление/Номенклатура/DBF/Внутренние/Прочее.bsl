...
Функция ВосстановитьСсылкуИзСтрокиДБФ(RecordSetDBF, КлючТипа, КлючЗначения)
	...
	СсылкаНаСправочник = Справочники[СокрЛП(ТипСсылки)].ПолучитьСсылку(УИДНоменклатуры);
	
	Если Найти(Строка(СсылкаНаСправочник), "<Объект не найден>")
				ИЛИ Найти(Строка(СсылкаНаСправочник), "<Object not found>")
				ИЛИ Не ЗначениеЗаполнено(СсылкаНаСправочник) Тогда
		// Может найтись ссылка из другой конфы 1С или пустая ссылка из-за неправильной записи номенклатуры
		Возврат Неопределено;
	КонецЕсли;
