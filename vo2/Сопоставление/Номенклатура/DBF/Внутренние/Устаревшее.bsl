...
&НаСервере
Функция НайтиНоменклатуруПоставщикаНаСервере(стрКонтрагент, стрНоменклатураПоставщика, КаталогНастроек, ИниКонфигурация, Отказ, ДопПараметры = Неопределено)
	
	Если ИниКонфигурация.Свойство("КлассификаторЕдиницИзмерения") Тогда
		ИмяКлассификтораЕдИзм = СтрЗаменить(ИниКонфигурация.КлассификаторЕдиницИзмерения.Значение, "'","");
	ИначеЕсли ИниКонфигурация.Свойство("ТипКлассификатораЕдиниц") Тогда
	...
	Если стрКонтрагент.Свойство("ИдентификаторЛица")
			И ЗначениеЗаполнено(стрКонтрагент.ИдентификаторЛица) Тогда 
	...
	Попытка
		...
		Иначе
			...
			Попытка
				БД.Open(СтрокаПодключения);
			Исключение
				СтрокаПодключения = "Driver={Microsoft Access dBASE Driver (*.dbf, *.ndx, *.mdx)};DriverID=533;Dbq=" + ИмяКаталогаНоменклатурыПоставщика + ";";
				БД.Open(СтрокаПодключения);
			КонецПопытки;
		КонецЕсли;
	Исключение
	...
	Результат = Новый Структура("Номенклатура, Характеристика, ЕдИзм, ЕдИзмОрг, Коэффициент");
...
//Процедура приводит старый файл DBF кш "новому формату", с переносом старых данных
&НаСервереБезКонтекста
