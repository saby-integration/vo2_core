...
Функция НовыйНаборЗаписейДБФ(СоединениеСДБФ)
	...
	Возврат Новый Структура("_тип, СоединениеСДБФ, Номенклатура1С, НоменклатураСБИС, Единицы", "НаборЗаписейDBF", СоединениеСДБФ, Новый Массив, Новый Массив, Новый Массив);
...
Процедура НовыйНаборЗаписейДБФ_СформироватьНабор(НаборЗаписейДБФ, СтрокаСопоставленияСБИССервер);
	...
	Для Каждого КлючИЗначениеИС Из СопоставлениеНоменклатуры Цикл
		
		// Расклонировать по каждому элементу соответствия отдельную запись на ИД позиции СБИС
		ЗаписьДБФНоменклатура1С	= НовыйЗаписьДБФНоменклатура1С(НаборЗаписейДБФ.СоединениеСДБФ, СтрокаСопоставленияСБИССервер, СтруктураЗаписиСБИС, КлючИЗначениеИС.Ключ);
		НаборХарактеристик		= МодульОбъектаСервер.ОписаниеНоменклатуры1ССервер_Получить(КлючИЗначениеИС.Значение, "Характеристики");
		...
		ЕдиницыИзмерения1С	= МодульОбъектаСервер.ОписаниеНоменклатуры1ССервер_Получить(КлючИЗначениеИС.Значение, "Единицы");
...
Функция НовыйЗаписьДБФСБИС(СоединениеСДБФ, ЗаписьНоменклатурыПоставщика=Неопределено)
	...
	//		NSB_ID	- (Индекс) идентификатор номенклатуры поставщика.
	//  	NSB_NM	- название номенклатуры поставщика.
	...
	КонецЦикла;
	Результат	= Новый Структура("_тип, Поля, СоединениеСДБФ", "ЗаписьДБФСБИС", НаборПолей, СоединениеСДБФ);
...
Процедура ЗаписьДБФСБИС_Заполнить(ЗаписьДБФСБИС, ДанныеВходящие)
	...
	Если ДанныеВходящие.Свойство("_тип") Тогда
		Если ДанныеВходящие._тип = "СтрокаСопоставленияСБИС" Тогда
			ЗаписьДБФСБИС.Поля.NSB_ID	= МодульОбъектаСервер.СтрокаСопоставленияСБИССервер_Получить(ДанныеВходящие, "Код");
			ЗаписьДБФСБИС.Поля.NSB_TYID	= МодульОбъектаСервер.СтрокаСопоставленияСБИССервер_Получить(ДанныеВходящие, "ТипКода");
			ЗаписьДБФСБИС.Поля.NSB_NM	= МодульОбъектаСервер.СтрокаСопоставленияСБИССервер_Получить(ДанныеВходящие, "Наименование");
	...
	Иначе
		...
		ЗаписьДБФСБИС.Поля.NSB_ID = ДанныеВходящие.НоменклатураСБИС;
		Если ЗначениеЗаполнено(ДанныеВходящие.ИмяНоменклатурыСБИС) Тогда
			ЗаписьДБФСБИС.Поля.NSB_NM = ДанныеВходящие.ИмяНоменклатурыСБИС;
		КонецЕсли;
...
Функция ЗаписьДБФСБИС_КлючЗаписи(ЗаписьДБФ)
	...
	КонецЕсли;
	Результат = ЗаписьДБФ.Поля.NSB_ID;
...
Процедура ЗаписьДБФСБИС_Записать(СтруктураЗаписи)
	...
	Если ЗаписьДБФОбновить = Неопределено Тогда
		...
		Для Каждого КлючИЗначениеЗаписи Из СтруктураЗаписи.Поля Цикл
			...
			ШаблонДобавить = СтрЗаменить(ШаблонДобавить, "{" + КлючИЗначениеЗаписи.Ключ + "}", СтрЗаменить(КлючИЗначениеЗаписи.Значение, "'", "''"));
	...
	Иначе
		...
		ЗаписьДБФОбновить.Fields("NSB_ID").Value	= Лев(СтруктураЗаписи.Поля.NSB_ID,		240);
		ЗаписьДБФОбновить.Fields("NSB_TYID").Value	= Лев(СтруктураЗаписи.Поля.NSB_TYID,	240);
		ЗаписьДБФОбновить.Fields("NSB_NM").Value	= Лев(СтруктураЗаписи.Поля.NSB_NM,		240);
...
Функция ЗаписьДБФСБИС_НайтиСуществующую(СтруктураЗаписи, СоединениеСДБФ, ДопПараметры=Неопределено)
	...
	//		KEY_ID		- (Индекс, Ключ) идентификатор для связи с таблицами.
	//		NSB_ID		- (Индекс) идентификатор номенклатуры поставщика.
	//		NSB_TYID	- тип идентификатора номенклатуры поставщика.
	//  	NSB_NM		- название номенклатуры поставщика.
	//  	CHRSB_ID	- (Индекс) идентификатор характеристики номенклатуры поставщика.
...
Функция НовыйЗаписьДБФНоменклатура1С(СоединениеСДБФ, СтрокаСопоставленияСБИС=Неопределено, ЗаписьДБФСБИС=Неопределено, СсылкаНоменклатура=Неопределено)
	...
	Результат	= Новый Структура("_тип, Поля, СоединениеСДБФ", "ЗаписьДБФНоменклатура1С", НаборПолей);
	...
	Если Не СтрокаСопоставленияСБИС = Неопределено Тогда
		...
		ЗаписьДБФНоменклатура1С_Установить(Результат, "N1C_MAIN",		МодульОбъектаСервер.ОписаниеНоменклатуры1ССервер_Получить(ОписаниеНоменклатуры1С, "Основное"));
...
Процедура ЗаписьДБФНоменклатура1С_Установить(ЗаписьДБФНоменклатура1С, КлючЗаписать, ЗначениеВходящее)
	...
	Если		КлючЗаписать = "Номенклатура" Тогда
		Попытка
			ЗаписьДБФНоменклатура1С.Поля.N1C_ID	= Строка(ЗначениеВЗапись.УникальныйИдентификатор());
			ЗаписьДБФНоменклатура1С.Поля.N1C_TY	= ПредставлениеТипаПоСсылке(ЗначениеВЗапись, "Номенклатура");
		Исключение
			// Не ссылка
			ЗаписьДБФНоменклатура1С.Поля.N1C_ID	= "";
			ЗаписьДБФНоменклатура1С.Поля.N1C_TY	= "";
		КонецПопытки;
...
Процедура ЗаписьДБФНоменклатура1С_Записать(СтруктураЗаписи)
	...
	Если СтруктураЗаписи.Поля.N1C_MAIN Тогда
		// Тут же ищем и устанавливаем в ложь признак основной номенклатуры всем соответствиям с таким же ключем СБИС
		СписокКлючейСБИС = "'" + СтруктураЗаписи.Поля.KEY_ID + "'";
		СписокGTINСБИС = "'" + СтруктураЗаписи.Поля.GTIN + "'";
		ШаблонПоиска = ОписаниеФайлБД.Шаблоны.МассовыйПоиск;
		ШаблонПоиска = СтрЗаменить(ШаблонПоиска, "{KEY_ID_LIST}", СписокКлючейСБИС);
		ШаблонПоиска = СтрЗаменить(ШаблонПоиска, "{GTIN_LIST}", СписокGTINСБИС);
		
		RecordSet = Новый ComОбъект("ADODB.Recordset");
		RecordSet.CursorLocation = 3;
		RecordSet.Open(ШаблонПоиска, СоединениеБД, 2, 3);
		
		RecordSet.Find("N1C_MAIN = 'True'");
		
		Пока Не RecordSet.EOF Цикл
			Если ПолучитьЗначениеИзРекордСет(RecordSet, "N1C_MAIN", Ложь) Тогда 
				Recordset.Fields("N1C_MAIN").value = Ложь;
				RecordSet.Update();
			КонецЕсли;
			RecordSet.MoveNext();
		КонецЦикла;
	...
	Для Каждого КлючИЗначениеЗаписи Из СтруктураЗаписи.Поля Цикл
		ЗначениеЗаменить = ?(ЗначениеЗаполнено(КлючИЗначениеЗаписи.Значение), "= '" + КлючИЗначениеЗаписи.Значение + "'" , "is null");
		ШаблонПоиска = СтрЗаменить(ШаблонПоиска, "{" + КлючИЗначениеЗаписи.Ключ + "}", ЗначениеЗаменить);
	КонецЦикла;
	...
	Иначе
		...
		Recordset.Fields("N1C_TY").value	= СтруктураЗаписи.Поля.N1C_TY;
		Recordset.Fields("GTIN").value		= СтруктураЗаписи.Поля.GTIN;
		Recordset.Fields("N1C_MAIN").value	= СтруктураЗаписи.Поля.N1C_MAIN;
		RecordSet.Update();
	КонецЕсли;
	RecordSet.Close();
...
Функция НовыйЗаписьДБФЕдиница(СоединениеСДБФ, ЗаписьДБФСБИС, ИсточникДанныхЕдиницы=Неопределено)
	...
	Результат			= Новый Структура("_тип, Поля, СоединениеСДБФ", "ЗаписьДБФНоменклатура1С", НаборПолей, СоединениеСДБФ);
	
	Если Не ИсточникДанныхЕдиницы = Неопределено Тогда
		ЗаписьДБФЕдиница_Установить(Результат, ИсточникДанныхЕдиницы._тип, ИсточникДанныхЕдиницы);
	КонецЕсли;
...
Процедура ЗаписьДБФЕдиница_Установить(СтруктураЗаписиЕдиница, КлючЗаписать, ЗначениеВходящее)
	...
	ИначеЕсли	КлючЗаписать = "Коэффициент" Тогда
		...
		Если Не ЗначениеЗаполнено(ЗначениеВЗапись) Тогда
			ЗначениеВЗапись = "1";
		КонецЕсли;
		СтруктураЗаписиЕдиница.Поля.RATIO = СтрЗаменить(Строка(ЗначениеВЗапись), ",", ".");
	...
	ИначеЕсли	КлючЗаписать = "Владелец" Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ЗначениеВЗапись) Тогда
			Попытка
				...
				СтруктураЗаписиЕдиница.Поля.UOW1C_TY	= ПредставлениеТипаПоСсылке(ЗначениеВЗапись, "Номенклатура");
			Исключение
				// Не ссылка
				СтруктураЗаписиЕдиница.Поля.UNT1C_ID	= "";
				СтруктураЗаписиЕдиница.Поля.UOW1C_TY	= "";
			КонецПопытки;
		КонецЕсли;
