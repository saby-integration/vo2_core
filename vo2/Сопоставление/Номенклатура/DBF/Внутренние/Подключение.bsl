...
Функция ПолучитьСоединениеDBF(ПараметрыСоединенияВходящие=Неопределено, ДопПараметры=Неопределено)
	...
	Исключение
		...
		МодульОбъектаСервер.ВызватьСбисИсключение(ИнфоОбОшибке, "Статусы_ДБФ.ПолучитьСоединениеDBF")
	КонецПопытки;
...
Процедура ПроверитьФайлыСопоставленияДБФ(СоединениеСДБФ, СоединениеБД)
	...
	Если	ОписаниеФайлаНоменклатураСБИС.Есть
		...
		Если Не ОписаниеФайлаНоменклатураСБИС.Проверен Тогда 			
			...
			Если		Recordset.Fields.Count = 5 Тогда
				Попытка
					Если Recordset.EOF Тогда
						...
						Исключение
							ИнфоОбОшибке = ИнформацияОбОшибке();
							МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "СопоставлениеНоменклатуры_ДБФ.ПроверитьФайлыСопоставленияДБФ.СоздатьФайлСопоставления");
						КонецПопытки;
					Иначе
				...
				Исключение
					ИнфоОбОшибке = ИнформацияОбОшибке();
					МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "Recordset.Open", 717,, "Неизвестный формат файла сопоставления");
				КонецПопытки;
			...
			Иначе
				...
				МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "Recordset.Open", 717,, "Неизвестный формат DBF. Обратитесь в техподдержку!",, Новый Структура("Поля", СписокПолей));
			КонецЕсли;
...
Процедура УстановитьШаблоныРаботыССопоставлениемДБФ(СоединениеСДБФ, СоединениеБД)
	...
	|		OR (NOT {base_1C}.GTIN is null
	|			AND {base_1C}.GTIN {GTIN}))";
...
Процедура УстановитьПоляДБФ(СоединениеСДБФ, СоединениеБД)
	...
	СоответствиеОграничений.Вставить("UOW1C_TY",	Новый Структура("Длина, Значение",						100,	""));
	...
	СоответствиеОграничений = Новый Структура;
	СоответствиеОграничений.Вставить("KEY_ID",		Новый Структура("Длина, Значение, Индекс, Уникальный",	240,	"", Истина, Истина));
	СоответствиеОграничений.Вставить("N1C_ID",		Новый Структура("Длина, Значение, Индекс, Уникальный",	240,	"", Истина, Ложь));
	СоответствиеОграничений.Вставить("N1C_TY",		Новый Структура("Длина, Значение",						240,	""));
	...
	СоответствиеОграничений.Вставить("N1C_MAIN",	Новый Структура("Длина, Значение",						240,	Ложь));
	...
	СоответствиеОграничений = Новый Структура;
	СоответствиеОграничений.Вставить("KEY_ID",		Новый Структура("Длина, Значение, Индекс, Уникальный",	240,	"", Истина, Истина));
	СоответствиеОграничений.Вставить("NSB_ID",		Новый Структура("Длина, Значение, Индекс, Уникальный",	240,	"", Истина, Ложь));
	СоответствиеОграничений.Вставить("NSB_TYID",	Новый Структура("Длина, Значение",						240,	""));
	СоответствиеОграничений.Вставить("NSB_NM",		Новый Структура("Длина, Значение",						240,	""));
...
Процедура ЗаполнитьБазыXBaseДанными(СоединениеСДБФ)
	...
	Попытка
		Пока Не БазаXBaseСтарая.ВКонце() Цикл
			...
			БазаXBaseСБИС.NSB_ID		= БазаXBaseСтарая.ID;
			БазаXBaseСБИС.NSB_TYID		= "КодПокупателя";
			БазаXBaseСБИС.NSB_NM		= БазаXBaseСтарая.NAME;
			БазаXBaseСБИС.Записать();
			//Записи в базу 1С
			БазаXBase1С.Добавить();
			БазаXBase1С.KEY_ID		= БазаXBaseСтарая.ID;
			БазаXBase1С.N1C_ID		= БазаXBaseСтарая.IDNOM;
			БазаXBase1С.N1C_TY		= БазаXBaseСтарая.SPRNOM;
	...
	Исключение
		МодульОбъектаСервер().ВызватьСбисИсключение(ИнформацияОбОшибке(), "БазаXBaseСтарая.ОткрытьФайл", 773, "Не удалось обновить файл сопоставления");
	КонецПопытки;
...
Процедура ЗаполнитьБазыRecordsetДанными(СоединениеСДБФ,СоединениеБД)
	...
	Если Recordset.EOF Тогда
		...
		Возврат;
	Иначе
		ЗапросыКонвертации = Новый Массив;
		ЗапросыКонвертации.Добавить("INSERT INTO " + ОписаниеФайлаНоменклатураСБИС.ИмяБД + "(
		|KEY_ID,	NSB_ID,	NSB_TYID,			NSB_NM) SELECT
		|ID,		ID,		""КодПокупателя""	NAME FROM " + ОписаниеФайлаНоменклатураСтарое.ИмяБД);
		
		ЗапросНоменклатура1С = "INSERT INTO " + ОписаниеФайлаНоменклатура1С.ИмяБД + "(
		|KEY_ID,	N1C_ID,	N1C_TY,	CHR1C_ID,	CHR1C_TY,	GTIN,	N1C_MAIN) SELECT 
		|ID,		IDNOM,	SPRNOM,	IDCHAR,		SPRCHAR,	%GTIN%,	'TRUE' FROM " + ОписаниеФайлаНоменклатураСтарое.ИмяБД;
		
		Если		Recordset.Fields.Count = 7 Тогда
			// Есть GTIN, нет единиц
			ЗапросНоменклатура1С = СтрЗаменить(ЗапросНоменклатура1С, "%GTIN%", "GTIN");
			ЗапросыКонвертации.Добавить(ЗапросНоменклатура1С);
		ИначеЕсли	Recordset.Fields.Count = 10 Тогда
			// Есть GTIN, есть единицы
			ЗапросНоменклатура1С = СтрЗаменить(ЗапросНоменклатура1С, "%GTIN%", "GTIN");
			ЗапросыКонвертации.Добавить(ЗапросНоменклатура1С);
			
			ЗапросыКонвертации.Добавить("INSERT INTO " + ОписаниеФайлаЕдиниц.ИмяБД + "(
			|KEY_ID,	UNT1C_ID,	UNT1C_TY,									RATIO,	UOW1C_ID,	UOW1C_TY) 
			|SELECT ID,	IDUNTORG,	'" + ПараметрыСоединения.Типы.Единицы + "',	RATIO,	IDNOM,	 	SPRNOM
			|FROM " + ОписаниеФайлаНоменклатураСтарое.ИмяБД + " WHERE (NOT " + ОписаниеФайлаНоменклатураСтарое.ИмяБД + ".IDUNPOST is null)");
		Иначе	
		...
		Попытка
			...
			Для Каждого ЗапросВыполнить Из ЗапросыКонвертации Цикл
		...
		Исключение
			МодульОбъектаСервер().ВызватьСбисИсключение(ИнформацияОбОшибке(), "Recordset.Open", 773, "Не удалось обновить файл сопоставления");
		КонецПопытки;
	КонецЕсли;
...
//		N1C_MAIN		- признак основной номенклатуры при множественном сопоставлении
...
//		UOW1C_ID	- владелец. Значение N1C_ID, либо - при отсутствии владельца в ИС.
//		UOW1C_TY	- название справочника номенклатуры 1С (Номенклатура)
...
Процедура СоздатьФайлыСопоставления(СоединениеСДБФ, СоединениеБД)
	...
	Попытка
		Если Не ОписаниеФайлаНоменклатураСБИС.Есть Тогда
			...
			//СоединениеБД.Execute("Create Table " + ОписаниеФайлаНоменклатураСБИС.ИмяФайла + " (KEY_ID char(240), NSB_ID char(240), NSB_NM char(240), CHRSB_ID char(240))");
	...
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
		МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "СоединениеБД.Execute");
	КонецПопытки;
...
Процедура СоздатьИндексСопоставления(СоединениеСДБФ, СоединениеБД)
	...
	Если Не ФайлСтатусов.Существует() Тогда
		...
		Исключение
			ИнфоОбОшибке = ИнформацияОбОшибке();
			МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "Сопоставление_ДБФ.СоздатьИндексСопоставления");
		КонецПопытки;
	КонецЕсли;
	ОписаниеФайлаНоменклатура1С		= СоединениеСДБФ.Файлы.Получить("Номенклатура1С");
