	...
	Для Каждого Ключ Из КлючиКонтрагентов Цикл
		...
		Пока ПродолжитьПеренос Цикл
			...
			Попытка
				...
				ПорцияСопоставлений = ФормаДБФ.НоменклатураПоставщика_Список(ПараметрыСписка, Новый Структура("Кэш", Кэш));
...
Функция НоменклатураПоставщика_МассовоеОбновлениеВызовМетода(ПараметрыОбновитьВходящие, ДопПараметры)	
	...
	Для Каждого КлассСопоставления Из ПараметрыОбновитьВходящие.Номенклатура Цикл
		...
		//Сыч, запись трёх типов кода
		//ИмеющиесяКоды = Новый Массив;
		//ОсновнойКод = Новый Структура("Код, ТипКода", КлассСопоставления.НоменклатураСБИС.Код, КлассСопоставления.НоменклатураСБИС.ТипКода);
		//ИмеющиесяКоды.Добавить(ОсновнойКод);
		//
		//Если ЗначениеЗаполнено(КлассСопоставления.НоменклатураСБИС.КодПоставщика)
		//	И НЕ КлассСопоставления.НоменклатураСБИС.ТипКода = "КодПоставщика" Тогда
		//	КодПоставщика = Новый Структура("Код, ТипКода", КлассСопоставления.НоменклатураСБИС.КодПоставщика, "КодПоставщика");
		//	ИмеющиесяКоды.Добавить(КодПоставщика);	
		//КонецЕсли;
		//
		//Если ЗначениеЗаполнено(КлассСопоставления.НоменклатураСБИС.КодПокупателя)
		//	И НЕ КлассСопоставления.НоменклатураСБИС.ТипКода = "КодПокупателя" Тогда
		//	КодПокупателя = Новый Структура("Код, ТипКода", КлассСопоставления.НоменклатураСБИС.КодПокупателя, "КодПокупателя");
		//	ИмеющиесяКоды.Добавить(КодПокупателя);	
		//КонецЕсли;
		//
		//Если ЗначениеЗаполнено(КлассСопоставления.НоменклатураСБИС.Идентификатор)
		//	И НЕ КлассСопоставления.НоменклатураСБИС.ТипКода = "Идентификатор" Тогда
		//	Идентификатор = Новый Структура("Код, ТипКода", КлассСопоставления.НоменклатураСБИС.Идентификатор, "Идентификатор");
		//	ИмеющиесяКоды.Добавить(Идентификатор);	
		//КонецЕсли;
		
		Для Каждого Номенклатура Из КлассСопоставления.Номенклатура1С Цикл
			
			//Для Каждого КодСопоставления Из ИмеющиесяКоды Цикл
				...
				//СтруктураЗаписи.ContrCode				= КодСопоставления.Код;     Сыч, запись трёх типов кода
				//СтруктураЗаписи.CodeType				= КодСопоставления.ТипКода;
