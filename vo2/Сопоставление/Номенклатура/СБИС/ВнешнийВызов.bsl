...
Функция НоменклатураПоставщика_МассовоеОбновлениеВызовМетода(ПараметрыОбновитьВходящие, ДопПараметры)	
	...
	Для Каждого КлассСопоставления Из ПараметрыОбновитьВходящие.Номенклатура Цикл
		...
		Для Каждого Номенклатура Из КлассСопоставления.Номенклатура1С Цикл
			...
			|MeasureUnitCode, MeasureUnitQty, ContrName, ContrCode, CodeType, ContrGTIN, ContrCharacteristic, ContrMeasureUnitCode, 
		...
		Для Каждого Номенклатура Из КлассСопоставления.Номенклатура1С Цикл
			...
			Если Номенклатура.Значение.Характеристики.Количество() Тогда
				...
				Для Каждого Характеристика Из Номенклатура.Значение.Характеристики Цикл
					...
					КонецЕсли;
					Если ЗначениеЗаполнено(КлассСопоставления.НоменклатураСБИС.Характеристика.Идентификатор) Тогда
						СтруктураЗаписи.Вставить("ContrCharacteristic",	КлассСопоставления.НоменклатураСБИС.Характеристика.Идентификатор);
					КонецЕсли;
			...
			Иначе
				...
				КонецЕсли;
				Если ЗначениеЗаполнено(КлассСопоставления.НоменклатураСБИС.Характеристика.Идентификатор) Тогда
					СтруктураЗаписи.Вставить("ContrCharacteristic",	КлассСопоставления.НоменклатураСБИС.Характеристика.Идентификатор); // Сейчас характеристики на онлайне не работают, но когда-нибудь..
				КонецЕсли;
...
Функция	НоменклатураПоставщика_ОбогатитьПоКлючамСБИС(ПараметрыОбновитьВходящие, ДопПараметры) 
	...
	Для Каждого КлассСопоставления Из ПараметрыОбновитьВходящие.Номенклатура Цикл
		...
		Для Каждого ЕдиницаПоставщика Из КлассСопоставления.НоменклатураСБИС.Единицы Цикл
			...
			СтруктураЗаписи = Новый Структура("DocumentId, DocumentType, DocumentDate, DocumentNumber, ContrCode, CodeType, ContrCharacteristic, ContrGTIN, ContrMeasureUnitCode, ContrMeasureUnitName");		
			СтруктураЗаписи.ContrCode				= КлассСопоставления.НоменклатураСБИС.Код;
			СтруктураЗаписи.CodeType				= КлассСопоставления.НоменклатураСБИС.ТипКода;
