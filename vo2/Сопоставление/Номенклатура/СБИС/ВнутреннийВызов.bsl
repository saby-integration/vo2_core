...
Функция ПреобразоватьОтветОнлайна(Ответ, ДопПараметры = Неопределено)
	...
	Если ДопПараметры.Свойство("Результат") Тогда
		Результат = ДопПараметры.Результат;
		Результат.Навигация.ЕстьЕще = ОтветНавигация.HasMore;
	Иначе
	...
	Для Каждого СопоставлениеКлючЗначение Из НаборСопоставлений Цикл
		ДанныеЗаполнения				= Новый Структура("НазваниеСБИС, АртикулСБИС, ИдентификаторСБИС, БазоваяЕдиницаОКЕИ, 
														|GTIN_СБИС, ЕдИзмСБИС, Номенклатура, ИмяХарактеристикиСБИС");
		...
		Для Каждого Сопоставление Из СопоставлениеКлючЗначение.Значение Цикл
			...
			Если ЗаполнитьДанныеСБИС Тогда
				...
				Если Не ЗначениеЗаполнено(Сопоставление.ContrCode) Тогда
					Продолжить;
				КонецЕсли;
				
				ДанныеЗаполнения.НазваниеСБИС		= Сопоставление.ContrName;
				Если Сопоставление.Свойство("ContrChar") Тогда
					ДанныеЗаполнения.ИмяХарактеристикиСБИС = Сопоставление.ContrChar;
				КонецЕсли;
				...
				Если Сопоставление.Свойство("CodeType")
					...
					ДанныеЗаполнения.Вставить("ТипКода",				Сопоставление.CodeType);
					ДанныеЗаполнения.Вставить(Сопоставление.CodeType,	Сопоставление.ContrCode);
				Иначе
					ДанныеЗаполнения.Вставить("ТипКода",				"Идентификатор");
					ДанныеЗаполнения.Вставить("Идентификатор",			Сопоставление.ContrCode);
				КонецЕсли;
...
Функция ОбогатитьОтветСсылками(Ответ, ДопПараметры) Экспорт
	...
	Для СчетчикЦикла = 1 По Ответ.Количество() Цикл
		...
		Если Соответствие.Свойство("CatalogType") И ЗначениеЗаполнено(Соответствие.CatalogType) Тогда
			ДопПараметры.Вставить("ТипНоменклатуры", Соответствие.CatalogType);
		КонецЕсли;
...
Функция НоменклатураПоКлючуСопоставления(Ключ, ДопПараметры)
	...
	Если Найти(ДопПараметры.ТипНоменклатуры, "Справочник.") Тогда
		НазваниеСправочника = СтрЗаменить(ДопПараметры.ТипНоменклатуры,"Справочник.","");
	ИначеЕсли Найти(ДопПараметры.ТипНоменклатуры, "СправочникСсылка.") Тогда
		НазваниеСправочника = СтрЗаменить(ДопПараметры.ТипНоменклатуры,"СправочникСсылка.","");
	Иначе
		НазваниеСправочника = ДопПараметры.ТипНоменклатуры;
	КонецЕсли;
	
	НазваниеСправочника = СтрЗаменить(НазваниеСправочника, " ", ""); // TODO временный костыль, держать не дольше 2025 года
	
	ЕстьСправочникНоменклатуры = Ложь;
	
	Если ДопПараметры.Свойство("ДопустимыеТипыСправочниковНоменклатуры") Тогда
		
		Если ДопПараметры.ДопустимыеТипыСправочниковНоменклатуры.Свойство(НазваниеСправочника) Тогда
			ЕстьСправочникНоменклатуры = ДопПараметры.ДопустимыеТипыСправочниковНоменклатуры[НазваниеСправочника];
		Иначе
			ЕстьСправочникНоменклатуры = Метаданные.Справочники.Найти(НазваниеСправочника) <> Неопределено;
			ДопПараметры.ДопустимыеТипыСправочниковНоменклатуры.Вставить(НазваниеСправочника, ЕстьСправочникНоменклатуры);
		КонецЕсли;
		
	Иначе
		
		ЕстьСправочникНоменклатуры = Метаданные.Справочники.Найти(НазваниеСправочника) <> Неопределено;
		ДопПараметры.Вставить("ДопустимыеТипыСправочниковНоменклатуры", Новый Структура(НазваниеСправочника, ЕстьСправочникНоменклатуры));
		
	КонецЕсли;
	
	Если Не ЕстьСправочникНоменклатуры Тогда
		// Такого справочника может не быть, т.к. могли произвести сопоставление в другой конфигурации 1С
		Возврат Неопределено;
	КонецЕсли;
	
	СправочникМенеджер	= Справочники[НазваниеСправочника];
...
Функция ДопПараметрыСервер()
	...
	Если ИниКонфигурации.Свойство("Номенклатура") Тогда
		ДопПараметры.ТипНоменклатуры = ИниКонфигурации.Номенклатура.Значение;
	Иначе
	...
	Если ИниКонфигурации.Свойство("ЕдиницыИзмеренияНоменклатуры") Тогда
		ДопПараметры.ЕдиницыИзмеренияНоменклатуры = СтрЗаменить(ИниКонфигурации.ЕдиницыИзмеренияНоменклатуры.Значение,"'","");
	КонецЕсли;
	
	Если ИниКонфигурации.Свойство("ЕдиницыИзмеренияНоменклатуры_ЕдиницаПоКлассификатору") Тогда
			...
			Если ЗначениеЗаполнено(Номенклатура.Значение.Характеристики) Тогда
				Для Каждого Характеристика Из Номенклатура.Значение.Характеристики Цикл			
					Если ЗначениеЗаполнено(Характеристика) Тогда // может прийти пустая характеристика
						Характеристики.Вставить(Характеристика, Строка(Характеристика.УникальныйИдентификатор()));
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
// Процедура - Дозапросить обновить номенклатуру по дополнительному коду
// Если перенос был и он завершен, то будем дополнительно искать по коду с типом "КодПокупателя",
// т.к. после переноса старого сопоставления с ДБФ - все кода стали "КодПокупателя"
//
// Параметры:
//  ДанныеСопоставлений	 - Массив	 - массив полученных с онлайна сопоставлений
//  ДанныеНоменклатуры	 - Структура - подготовленные данные номенклатуры контрагента, по которым запрашиваем сопоставления: "Contractor", "Data"
//  КодДляПоиска		 - Строка	 - код, по которому будет произведен дополнительный поиск сопоставлений
//  ДопПараметры		 - Структура - набор доп параметров метода, должен включать "Кэш"
//
&НаКлиенте
Процедура ДозапроситьОбновитьНоменклатуруПоДополнительномуКоду(ДанныеСопоставлений, ДанныеНоменклатуры, КодДляПоиска, ДопПараметры)
	
	Кэш								= ДопПараметры.Кэш;
	МассивНоменклатуры				= ДанныеНоменклатуры.Data;
	...
	Если МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ПараметрыПереносаСопоставлений_Завершен") = Истина Тогда
		
		МассивНоменклатурыДоп = Новый Массив;
		МаппингИсхСтрок = Новый Соответствие;
		
		Инд	= 0;
		Для ИндИсх = 0 По ДанныеСопоставлений.ВГраница() Цикл
			
			Если 	Не ЗначениеЗаполнено(ДанныеСопоставлений[ИндИсх].Id)
				И	ДанныеСопоставлений[ИндИсх].CodeType <> "КодПокупателя" Тогда
				
				МассивНоменклатурыДоп.Добавить(МассивНоменклатуры[ИндИсх]);
				СтрНоменклатурыДоп = МассивНоменклатурыДоп[МассивНоменклатурыДоп.ВГраница()];
				СтрНоменклатурыДоп.Вставить("CodeType",				"КодПокупателя");
				
				МаппингИсхСтрок.Вставить(Инд, ИндИсх);
		...
		Если МассивНоменклатурыДоп.Количество() Тогда
			
			ДанныеНоменклатуры.Вставить("Data", МассивНоменклатурыДоп);
			Отказ = Ложь;
			ДанныеСопоставленийДоп = Кэш.Интеграция.ContractorNomenclatureCollation_SearchByContractorData(Кэш, ДанныеНоменклатуры, ДопПараметрыВнМетода, Отказ);
			Если Отказ Тогда
			...
			ДанныеДляЗаписиДоп = Новый Массив; 
			
			Для Инд = 0 По ДанныеСопоставленийДоп.ВГраница() Цикл
				
				ИндексИсходнойСтроки = МаппингИсхСтрок.Получить(Инд);
				СтрокаСопоставленияДоп = ДанныеСопоставленийДоп[Инд];
				
				Если ЗначениеЗаполнено(СтрокаСопоставленияДоп.Id) Тогда
					СтрокаСопоставленияДоп.CodeType = ДанныеСопоставлений[ИндексИсходнойСтроки].CodeType;
					ДанныеСопоставлений.Установить(ИндексИсходнойСтроки, СтрокаСопоставленияДоп);
					ДанныеДляЗаписиДоп.Добавить(СтрокаСопоставленияДоп);
				КонецЕсли;
				
			КонецЦикла;
			
			Если ДанныеДляЗаписиДоп.Количество() Тогда
				// Перезаписываем позиции с новым TypeCode
				ДанныеНоменклатуры.Вставить("Data", ДанныеДляЗаписиДоп);
				Кэш.Интеграция.ContractorNomenclatureCollation_MassUpdate(Кэш, ДанныеНоменклатуры, ДопПараметрыВнМетода, Отказ);
			КонецЕсли;
