...
Функция ОбогатитьОтветСсылками(Ответ, ДопПараметры) Экспорт
	...
	Для Каждого Соответствие Из Ответ Цикл
		...
		Соответствие.Вставить("Характеристика", ХарактеристикаПоКлючуСопоставления(Соответствие.Id,ДопПараметры));
		...
		Если	Соответствие.Свойство("MeasureUnit") И ЗначениеЗаполнено(Соответствие.MeasureUnit) Тогда
			...
			Для Каждого Единица Из Соответствие.MeasureUnit Цикл
				...
				Если ЗначениеЗаполнено(Единица.MeasureUnitCode) Тогда
					...
					Если 	ДопПараметры.ЕдиницыИзмеренияНоменклатуры = Неопределено 
						ИЛИ ДопПараметры.КлассификаторЕдиницИзмерения = Неопределено 
						ИЛИ ДопПараметры.ЕдиницыИзмеренияНоменклатуры_ЕдиницаПоКлассификатору = Неопределено Тогда
...
&НаСервереБезКонтекста
Функция ХарактеристикаПоКлючуСопоставления(Ключ, ДопПараметры)
	
	Если НЕ Найти(Ключ,"#") Тогда
	...
	Попытка
		Возврат СправочникМенеджер.ПолучитьСсылку(Новый УникальныйИдентификатор(Сред(Ключ,Найти(Ключ,"#")+1)));
	Исключение
		Возврат СправочникМенеджер.ПустаяСсылка();
	КонецПопытки;
...
Функция ДопПараметрыСервер()
	...
	Если ВладелецФормы.Кэш.ини.Конфигурация.Свойство("КлассификаторЕдиницИзмерения") Тогда
		ДопПараметры.КлассификаторЕдиницИзмерения = СтрЗаменить(ИниКонфигурации.КлассификаторЕдиницИзмерения.Значение,"'","");
	КонецЕсли;
