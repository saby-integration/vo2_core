...
Функция РезультатДействия_Новый(Кэш, ДопПараметры) Экспорт
	...
	Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("ПриличныйВидВО") Тогда
		ПараметрыМаппинга = Новый Структура("Назначение", "Документы");
		РезультатДействия.Вставить("ДанныеМаппинга", МодульОбъектаКлиент().НовыйСписокMappingObject(ПараметрыМаппинга));
	КонецЕсли;
...
Процедура РезультатДействия_СообщитьРезультат(РезультатДействия, ДопПараметры) Экспорт
	...
	Если Не ДопПараметры.Свойство("ПараметрыСообщить", ПараметрыСообщить) Тогда
		...
		ИначеЕсли	РезультатДействия.Свойство("Операция")
			...
			ПараметрыСообщить.Вставить("Ошибки",		Новый Структура("Текст, Статус, ВывестиСБИСИсключение", "%Название% завершено с ошибками. ", СтатусСообщения.Внимание, Истина));		
		Иначе
	...
	Если РезультатДействия.Свойство("ПрикладнаяСтатистика") Тогда
		ПараметрыОтправкиСтатистики = Новый Структура("Контекст", "Загрузка документов");
		МодульОбъектаКлиент().ПрикладнаяСтатистика_Отправить(РезультатДействия.ПрикладнаяСтатистика, ПараметрыОтправкиСтатистики);
	КонецЕсли;
	
	Если РезультатДействия.Свойство("ДанныеМаппинга") Тогда
	
		МодульОбъектаКлиент().СписокMappingObject_Записать(РезультатДействия.ДанныеМаппинга);
...
Процедура РезультатДействия_СообщитьДетализацию(ЭлементДетализации, ДопПараметры) 
	...
	Перем ВывестиСБИСИсключение, СтатусСообщений; 
	 
	Если Не ДопПараметры.Свойство("Статус", СтатусСообщений) Тогда
		СтатусСообщений = СтатусСообщения.Обычное;
	КонецЕсли; 
  
	Если Не ДопПараметры.Свойство("ВывестиСБИСИсключение", ВывестиСБИСИсключение) Тогда
		ВывестиСБИСИсключение = Ложь;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ЭлементДетализации Цикл
		Для Каждого СтрокаДетализации Из КлючИЗначение.Значение Цикл
			...
			Если Не ПустаяСтрока(СтрокаДетализации.Сообщение) Тогда
				Если ВывестиСБИСИсключение И МодульОбъектаКлиент().ЭтоТипСбис(СтрокаДетализации.Данные, "СбисИсключение") Тогда    
					МодульОбъектаКлиент().СбисСообщить(СтрокаДетализации.Данные);
					ВывестиСБИСИсключение = Ложь;	
				КонецЕсли;
...
Процедура РезультатДействия_ДобавитьВРасшифровку(Кэш, ТипДетализации, СтрокаДетализации, ПараметрыОбъекта) Экспорт
	...
	Если ПараметрыОбъекта.Свойство("Ошибка") Тогда
		
		СтрокаДобавить.Сообщение = МодульОбъектаКлиент().СбисИсключение_Выгрузить(ПараметрыОбъекта.Ошибка, "Сообщение");
