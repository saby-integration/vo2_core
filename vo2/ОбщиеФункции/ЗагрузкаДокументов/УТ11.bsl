...
&НаСервере
Функция ЗаполнитьЕдиницуИзмененияНоменклатурыУТ11НаСервере(Знач Контекст)
	...
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
		МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "ОбщиеФункции.ЗаполнитьЕдиницуИзмененияНоменклатурыУТ11НаСервере");
	КонецПопытки;
