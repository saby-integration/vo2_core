...
Функция ЗаполнитьДанныеДляЗагрузкиПоУмолчанию(Кэш, СоставПакета, Вложение, ДополнительныеПараметры=Неопределено) Экспорт     	
	...
	Если	ЗначениеЗаполнено(СтруктураИниФайла)
		И	ЗначениеЗаполнено(СтруктураФайла) Тогда
		
		СторонаНОНайдена = Ложь;
		Если Не ОрганизацияРоль = Неопределено Тогда
			
			Попытка
				ПутьОрганизации	= Вложение.СтруктураИниФайла.мСторона[ОрганизацияРоль].Сторона.Данные;
			Исключение
				ПутьОрганизации	= Вложение.ДанныеСтруктурыФайла.Поля.Получить("Организация");
			КонецПопытки;
			
			СтрОрганизации	= РассчитатьЗначениеИзСтруктуры(ПутьОрганизации, СтруктураФайла);
			Если Не СтрОрганизации = Неопределено Тогда
				СторонаНОНайдена = Истина;
			КонецЕсли;
		
			Если Не СторонаНОНайдена Тогда
