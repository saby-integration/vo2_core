...
Функция ЗаполнитьДанныеДляЗагрузкиПоУмолчанию(Кэш, СоставПакета, Вложение, ДополнительныеПараметры=Неопределено) Экспорт     	
	...
	Если	ЗначениеЗаполнено(СтруктураИниФайла)
		...
		Если Не ОрганизацияРоль = Неопределено Тогда
			...
			Исключение
				...
				МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке(), "РаботаСДокументами1С.ЗаполнитьДанныеДляЗагрузкиПоУмолчанию");
	...
	Если ПараметрыНоменклатуры.НадоСопоставлять 
		...
		Если ИспользоватьАвтоматическоеСопоставление Тогда   
			...
			Иначе 
				Разделитель = ",";
				ПорядокАвтоматическогоСопоставления = МодульОбъектаКлиент().СбисРазложитьСтрокуВМассивПодстрокНаКлиенте(ПорядокАвтоматическогоСопоставленияСтрока, Разделитель); 
			КонецЕсли;
