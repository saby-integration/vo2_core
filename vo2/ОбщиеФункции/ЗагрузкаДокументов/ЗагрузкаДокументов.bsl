...
Функция ПолучитьКонтрагентаВложения(Кэш, Вложение, СтрКонтрагента)
	...
	Возврат ?(ЗначениеЗаполнено(СтрКонтрагента), НайтиКонтрагентаИзДокументаСБИС(Кэш.Ини.Конфигурация, СтрКонтрагента, Вложение.СтруктураИниФайла), Ложь);
...
Функция ЗаполнитьДанныеДляЗагрузкиПоУмолчанию(Кэш, СоставПакета, Вложение, ДополнительныеПараметры=Неопределено) Экспорт     	
	...
	Если МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ИспользоватьВходящийКонтекст") Тогда
		...
		Если МодульОбъектаКлиент().ДоступнаПодстановкаСторонИзПакета(СоставПакета)
			И ДанныеСторон.ДанныеОрганизации.Свойство("Ссылка")
			И Не ЗначениеЗаполнено(ДанныеСторон.ДанныеОрганизации.Ссылка) Тогда
			
			ТекСторона = ПолучитьОрганизациюВложения(Кэш, Вложение, СтрОрганизации);
			Если ТекСторона <> Ложь Тогда
			
				ДанныеСторон.ДанныеОрганизации.Ссылка = ТекСторона;
				
				ДанныеДляОбновления = Новый Структура;
				МестныйКэш.ОбщиеФункции.сбисСкопироватьСтруктуруНаКлиенте(ДанныеДляОбновления, ДанныеСторон.ДанныеОрганизации);
				ДанныеДляОбновления.Вставить("Тип", "НашаОрганизация"); 
				ДанныеДляОбновления.Вставить("ТипДанныхСтороны", "ДанныеОрганизации");
				ДопПараметры = Новый Структура("СторонаСБИС", СоставПакета.НашаОрганизация);
				МодульОбъектаКлиент().Маппинг_ОбновитьСторону1С(ДанныеДляОбновления, ДопПараметры);
				
			КонецЕсли;
			
		ИначеЕсли Не МодульОбъектаКлиент().ДоступнаПодстановкаСторонИзПакета(СоставПакета)  Тогда
			ДанныеСторон.ДанныеОрганизации.Ссылка = ТипыДанныхСторон.ДанныеОрганизации;
		КонецЕсли;
		
		Если МодульОбъектаКлиент().ДоступнаПодстановкаСторонИзПакета(СоставПакета)
			И СоставПакета.Свойство("Контрагент")
			И ДанныеСторон.ДанныеКонтрагента.Свойство("Ссылка")
			И Не ЗначениеЗаполнено(ДанныеСторон.ДанныеКонтрагента.Ссылка)
			И Не МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ЗаполнениеКонтрагента1С") = "ГрузополучательСБИС" Тогда 
			
			ТекСторона = ПолучитьКонтрагентаВложения(Кэш, Вложение, СтрКонтрагента);
			Если ТекСторона <> Ложь Тогда
				
				ДанныеСторон.ДанныеКонтрагента.Ссылка = ТекСторона;
				
				ДанныеДляОбновления = Новый Структура;
				МестныйКэш.ОбщиеФункции.сбисСкопироватьСтруктуруНаКлиенте(ДанныеДляОбновления, ДанныеСторон.ДанныеКонтрагента);
				ДанныеДляОбновления.Вставить("Тип", "Контрагент"); 
				ДанныеДляОбновления.Вставить("ТипДанныхСтороны", "ДанныеКонтрагента");
				ДопПараметры = Новый Структура("СторонаСБИС", СоставПакета.Контрагент);
				МодульОбъектаКлиент().Маппинг_ОбновитьСторону1С(ДанныеДляОбновления, ДопПараметры);  
				
			КонецЕсли;
			
		ИначеЕсли Не МодульОбъектаКлиент().ДоступнаПодстановкаСторонИзПакета(СоставПакета)  Тогда
			ДанныеСторон.ДанныеКонтрагента.Ссылка = ТипыДанныхСторон.ДанныеКонтрагента;
		КонецЕсли;
	...
	Если ПараметрыНоменклатуры.НадоСопоставлять 
		...
		И НЕ ДополнительныеПараметры.Свойство("ДанныеНоменклатурДляРасширенногоСопоставления") Тогда
		
		ТаблДок = Неопределено;
		Если НЕ Вложение.Свойство("ТаблДокРасширениеФункционалаСопоставлений", ТаблДок) Тогда
		...
		Для Каждого СтрокаНоменклатуры Из ТаблДок Цикл
			...
			ЗаполнитьЗначенияСвойств(ДанныеНоменклатурыКонтрагента, СтрокаНоменклатуры);
		...
		Если ИспользоватьАвтоматическоеСопоставление И НЕ ОбогащенныеДанныеНоменклатурыДляСопоставления = Неопределено Тогда
			
			ОбогащенныеДанныеНоменклатурыДляСопоставления =	АвтоматическоеСопоставлениеНоменклатур(МестныйКэш, ОбогащенныеДанныеНоменклатурыДляСопоставления);		
		...
		// TO DO Ду-ду Класть ТаблДок на Вложение и проверять его наличие. Если есть - тянем его из Вложения и изменяем
		Результат.Вставить("ОбогащенныеДанныеНоменклатурыДляСопоставления", ОбогащенныеДанныеНоменклатурыДляСопоставления);

	ИначеЕсли ПараметрыНоменклатуры.НадоСопоставлять 
		И МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры")
		И СпособСопоставленияПоддерживается  
		И НЕ ДополнительныеПараметры = Неопределено
		И ДополнительныеПараметры.Свойство("ДанныеНоменклатурДляРасширенногоСопоставления") Тогда
		
		ПараметрыНоменклатуры.Сопоставлено = Истина;
		Результат.Статус = "Номенклатура сопоставлена. ";
		Результат.СтатусКартинка = 3;
		
		ДанныеНоменклатур = ДополнительныеПараметры.ДанныеНоменклатурДляРасширенногоСопоставления;
		
		ОбнаруженаНесопоставленнаяНоменклатура = Ложь;
		
		Для Каждого КлассНоменклатуры Из ДополнительныеПараметры.ДанныеНоменклатурДляРасширенногоСопоставления.ПодготовленныеДанныеНоменклатур Цикл 
		...
		Для Каждого Строка Из ТаблДок Цикл
			...
			Если НЕ ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур.Количество() Тогда
				Продолжить;
			КонецЕсли;
			
			//Пока у нас возможна только связка с одной номенклатурой, тут будет жить этот костыль (С)Сычев
			Для Каждого Номенклатура1С Из ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Номенклатура1С Цикл
				ТаблДок[СчетчикСтрок].Вставить("Номенклатура", Номенклатура1С.Ключ);
            	Прервать;
			КонецЦикла;
			
			Если ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Свойство("Характеристика")  
				И ЗначениеЗаполнено(ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Характеристика) Тогда
				ТаблДок[СчетчикСтрок].Вставить("ХарактеристикаНоменклатуры", ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Характеристика);
			КонецЕсли;
			Если    ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Свойство("ЕдИзмОрг")
				И	ЗначениеЗаполнено(ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].БазоваяЕдиницаОКЕИ) Тогда
				ТаблДок[СчетчикСтрок].Вставить("ЕдИзмОрг", ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].БазоваяЕдиницаОКЕИ);
			КонецЕсли;
			Если	ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Свойство("Коэффициент")
				И	ЗначениеЗаполнено(ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Коэффициент) Тогда
				ТаблДок[СчетчикСтрок].Вставить("Коэффициент", ДанныеНоменклатур.ПодготовленныеДанныеНоменклатур[СчетчикСтрок].Коэффициент);
			КонецЕсли;
...
Функция ЗагрузитьУпорядоченныйСоставПакетаВыбраннымСпособом(РезультатВопроса, ПараметрыЗагрузки) Экспорт
	...
	Для Каждого Вложение Из УпорядоченныйСоставПакета Цикл 
		...
		Иначе    
			...
			ТекущийДокумент1С = Неопределено;
			Если ИсходящиеДокументы1С.Количество() И РезультатВопроса = КодВозвратаДиалога.ОК Тогда
