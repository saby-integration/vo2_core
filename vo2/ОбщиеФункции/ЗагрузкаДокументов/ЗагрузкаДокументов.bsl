...
Функция ЗаполнитьДанныеДляЗагрузкиПоУмолчанию_ТаблДок(Вложение, ДопПараметры) Экспорт     	
	...
	ДопПараметры.Вставить("КлючРоли", "СопоставлениеНоменклатуры");
	
	Попытка
		КонтрагентДляСопоставления	= МодульТекущий.ВложениеСбис_Получить(Вложение, "СторонаПоРоли", ДопПараметры);
	Исключение
	...
	Если ЗначениеЗаполнено(ТаблДок) Тогда
		Попытка
			...
			Если НЕ МодульОбъектаКлиент().ИспользоватьНовоеСопоставлениеНоменклатуры() Тогда
		...
		Для Каждого СтрТабл Из ТаблДок Цикл
			...
			Иначе
				Попытка   
					...
					КонецЕсли;
				Исключение
					СообщениеИсключение = "Ошибка поиска сопоставленной номенклатуры";
					ДетализацияИсключение  = "Номенклатура с названием " + СтрТабл.Название + " не найдена";
					СбисИсключение = МодульТекущий.НовыйСбисИсключение(ИнформацияОбОшибке(), "ОбщиеФункции.ЗаполнитьДанныеДляЗагрузкиПоУмолчанию",, СообщениеИсключение, ДетализацияИсключение);
...
Функция СоздатьПерезаполнитьДокументы(Кэш, СоставПакета, СписокВложений, ПараметрыВходящие, ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаТаблДок из СписокВложений Цикл
		...
		Если МодульОбъектаКлиент().ИспользоватьНовоеСопоставлениеНоменклатуры() Тогда // TO DO Ду-ду (с)Сыч
	...
	Для Каждого Вложение Из УпорядоченныйСоставПакета Цикл 
		...
		ИначеЕсли ПараметрыВложения.РедактироватьТабЧасть Тогда
			...
			Иначе
				...
				ПараметрыОшибки = МодульОбъектаКлиент().НовыйСбисИсключение(723301,"РаботаСДокументами1С.ЗагрузитьУпорядоченныйСоставПакета_Обработать",
				,,СтрокаРезультат.Сообщение);
		...
		Если ДанныеДозаполнить.Отказ
			...
			ПараметрыОшибки = МодульОбъектаКлиент().НовыйСбисИсключение(723,"РаботаСДокументами1С.ЗагрузитьУпорядоченныйСоставПакета_Обработать",
				,,СтрокаРезультат.Сообщение);
			РезультатДействия_ДобавитьОшибку(Кэш, Результат, СтрокаРезультат, ПараметрыОшибки);
