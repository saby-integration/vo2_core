...
Функция ЗаполнитьДанныеДляЗагрузкиПоУмолчанию(Кэш, СоставПакета, Вложение, ДополнительныеПараметры=Неопределено) Экспорт     	
	...
	ИспользоватьНовоеСопоставление	= МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры")
									И	(	МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("СпособСопоставленияНоменклатуры") = 0
	...
	Если ПараметрыНоменклатуры.НадоСопоставлять Тогда
		...
		Если ИспользоватьНовоеСопоставление Тогда
			...
			Если Вложение.Свойство("КлассыСопоставленияНоменклатур") Тогда
				...
				ВходящиеПараметры = Новый Структура("ПараметрыНоменклатуры, Результат, Вложение");
				ВходящиеПараметры.ПараметрыНоменклатуры = ПараметрыНоменклатуры;
				ВходящиеПараметры.Результат = Результат;
				ВходящиеПараметры.Вложение = Вложение;
				
				РасширенныйФункционалСопоставления_ЕслиЕстьНесопоставленнаяНоменклатура(ВходящиеПараметры);
				
			Иначе
		
				// Убери ПутьТаблДок и ПараметрыНоменклатуры. Они берутся от вложения, пример смотри выше.
				ПараметрыОбработки = Новый Структура;
				ПараметрыОбработки.Вставить("Результат", Результат);
				ПараметрыОбработки.Вставить("Вложение", Вложение);
				ПараметрыОбработки.Вставить("СоставПакета", СоставПакета);
				ПараметрыОбработки.Вставить("ПараметрыНоменклатуры", ПараметрыНоменклатуры);
				ПараметрыОбработки.Вставить("ПутьТаблДок", ПутьТаблДок);
				
				РасширенныйФункционалСопоставления_ОбработкаСопоставленияПоТаблДоку(ПараметрыОбработки); 
		...
		Иначе
			...
			Попытка
				...
				ТаблДок = ЗаполнитьДанныеДляЗагрузкиПоУмолчанию_ТаблДок(Вложение, ПараметрыЗаполнить);
...
Функция ЗаполнитьДанныеДляЗагрузкиПоУмолчанию_ТаблДок(Вложение, ДопПараметры) Экспорт     	
	...
	Если ЗначениеЗаполнено(ТаблДок) Тогда
		Попытка
			...
			Если НЕ МодульТекущий.ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры") Тогда
...
Функция СоздатьПерезаполнитьДокументы(Кэш, СоставПакета, СписокВложений, ПараметрыВходящие, ДополнительныеПараметры) Экспорт
	...
	Для каждого СтрокаТаблДок из СписокВложений Цикл
		...
		Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры")
			...
			МодульОбъектаКлиент().РасширенноеСопоставление_ПодготовитьНоменклатурыКЗагрузкеВ1С(ПараметрыПодготовкиНоменклатуры);
		...
		ИначеЕсли		ПараметрыНоменклатуры.НадоСопоставлять
			...
			ПараметрыЗаполнения.Вставить("фрмНоменклатуры",			МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("МодульСопоставлениеНоменклатуры"));
