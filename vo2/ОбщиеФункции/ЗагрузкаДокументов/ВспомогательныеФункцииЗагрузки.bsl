...
Функция ОпределитьСторонуДокумента(Контекст) Экспорт  
	...
	// Получает имя реквизита стороны в документе по её роли
	Если Контекст.Ини.Свойство("мСторона") Тогда
		ОписаниеПоля = МодульОбъектаКлиент.НовыйОписаниеПоля1СВложения(, Роль + ".Сторона", Контекст.Ини.мСторона);
		Если ОписаниеПоля.Доступность Тогда
			Ключ = ОписаниеПоля.Путь;
		КонецЕсли;
	КонецЕсли;
	...
	БратьСтороныСПакета = ЗначениеЗаполнено(Ключ)
			...
			И	ВходящийКонтекст.РассчитанноеЗначение.Свойство("Организация")
			И	ВходящийКонтекст.РассчитанноеЗначение.Свойство("Контрагент")
			И	ЗначениеЗаполнено(ВходящийКонтекст.РассчитанноеЗначение.Организация)
			И	ЗначениеЗаполнено(ВходящийКонтекст.РассчитанноеЗначение.Контрагент);
	
	Если МодульОбъектаКлиент.ПолучитьЗначениеПараметраСБИС("ИспользоватьВходящийКонтекст")
		И БратьСтороныСПакета Тогда
