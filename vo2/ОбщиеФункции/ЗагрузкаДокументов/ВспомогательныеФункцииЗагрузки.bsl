...
Функция КорректировкаРеализации_ЗаполнитьТоварыПоИсходнымДанным_УТ11_Сервер(Контекст)  
	...
	Возврат МодульОбъектаСервер().СериализоватьОбъектДляВозвратаНаКлиент(Запрос.Выполнить());
	
КонецФункции

&НаСервере
Функция КорректировкаРеализации_ТаблицыПоИсходнымДанным_БУХ3_Сервер(Контекст)  
	
	// Заполнение корректируемых данных по основанию
	ПараметрыЗаполнить = Новый ФиксированнаяСтруктура("ДокументРеализации", Контекст.Основание);
	ТаблицыДокумента = Документы.КорректировкаРеализации.ДанныеЗаполненияПоДокументу(ПараметрыЗаполнить);
	Возврат МодульОбъектаСервер().СериализоватьОбъектДляВозвратаНаКлиент(ТаблицыДокумента);
...
Функция СбисНайтиДоговорНаСервере(Знач Контекст)
	...
	КонецЕсли;
	НаименованиеРеквизитаДоговорДата = "Дата";
	Если Метаданные.Справочники.ДоговорыКонтрагентов.Реквизиты.Найти("ДатаДоговора") <> Неопределено Тогда
		НаименованиеРеквизитаДоговорДата = "ДатаДоговора";
	КонецЕсли;
	НаименованиеРеквизитаДоговорНомер = "Номер";
	Если Метаданные.Справочники.ДоговорыКонтрагентов.Реквизиты.Найти("НомерДоговора") <> Неопределено Тогда
		НаименованиеРеквизитаДоговорНомер = "НомерДоговора";
	КонецЕсли;
	...
	Если Не	(	ДатаДоговора = Неопределено
		...
		|ГДЕ
		|	ДоговорыКонтрагентов." + НаименованиеРеквизитаДоговорДата + " = &Дата
		|	И ДоговорыКонтрагентов." + НаименованиеРеквизитаДоговорНомер + " = &Номер
	...
	НаименованиеПеречисленияВидДоговораКонтрагента = "ВидыДоговоров";
	Если Метаданные.Перечисления.Найти("ВидыДоговоровКонтрагентов") <> Неопределено Тогда
		НаименованиеПеречисленияВидДоговораКонтрагента = "ВидыДоговоровКонтрагентов";
	КонецЕсли;
	Если ВидДоговора = Неопределено Тогда
		СписокВидовДоговора.Добавить(Перечисления[НаименованиеПеречисленияВидДоговораКонтрагента].СПоставщиком);
	Иначе
...
Функция сбисПолучитьДоговорУТ11_Сервер(Контекст)
	...
	Если Контекст.Документ.Свойство("Контрагент") И НЕ Контекст.Документ.Контрагент = Неопределено Тогда 
		Контекст.Документ.Вставить("Партнер", Контекст.Документ.Контрагент.Партнер);
	КонецЕсли;   
...
Функция сбисПолучитьСтавкуНДС_Перечисление_НаСервере(Знач сбисСтавка)
	Если		(Найти(сбисСтавка,"22/122") = 1) Тогда
		Попытка
			Возврат Перечисления.СтавкиНДС.НДС22_122;
		Исключение
			Сообщить("На данной конфигурации не поддерживается ставка 22/122, загрузка может быть некорректной");
			Возврат Неопределено;
		КонецПопытки;
	ИначеЕсли	(Найти(сбисСтавка,"20/120") = 1) Или (сбисСтавка = "17") Тогда
	...
	ИначеЕсли	(Найти(сбисСтавка,"5/105") = 1) Тогда
		Попытка
			Возврат Перечисления.СтавкиНДС.НДС5_105;
		Исключение
			Сообщить("На данной конфигурации не поддерживается ставка 5/105, загрузка может быть некорректной");
			Возврат Неопределено;
		КонецПопытки;
	ИначеЕсли	(Найти(сбисСтавка,"7/107") = 1) Тогда
		Попытка
			Возврат Перечисления.СтавкиНДС.НДС7_107;
		Исключение
			Сообщить("На данной конфигурации не поддерживается ставка 7/107, загрузка может быть некорректной");
			Возврат Неопределено;
 		КонецПопытки;
	ИначеЕсли	Найти(сбисСтавка,"22") = 1 Тогда
		Попытка
			Возврат Перечисления.СтавкиНДС.НДС22;
		Исключение
			Сообщить("На данной конфигурации не поддерживается ставка 22%, загрузка может быть некорректной");
			Возврат Неопределено;
		КонецПопытки;
	...
	ИначеЕсли	Найти(сбисСтавка,"7") = 1 Тогда
		Попытка
			Возврат Перечисления.СтавкиНДС.НДС7; 
		Исключение
			Сообщить("На данной конфигурации не поддерживается ставка 7%, загрузка может быть некорректной");
			Возврат Неопределено;
 		КонецПопытки;
	ИначеЕсли	Найти(сбисСтавка,"5") = 1 Тогда 
		Попытка
			Возврат Перечисления.СтавкиНДС.НДС5; 
		Исключение
			Сообщить("На данной конфигурации не поддерживается ставка 5%, загрузка может быть некорректной");
			Возврат Неопределено;
		КонецПопытки;
...
Функция сбисПолучитьСтавкуНДС_Справочник_НаСервере(Знач сбисСтавка)
	Если		(Найти(сбисСтавка,"22/122") = 1) Тогда
		Возврат ПоискСтавкиНДСВСправочникеПоНазванию("22/122");
	ИначеЕсли		(Найти(сбисСтавка,"20/120") = 1) Или (сбисСтавка = "17") Тогда
	...
	ИначеЕсли	(Найти(сбисСтавка,"7/107") = 1) Тогда
		Возврат ПоискСтавкиНДСВСправочникеПоНазванию("7/107");
	ИначеЕсли	(Найти(сбисСтавка,"5/105") = 1) Тогда
		Возврат ПоискСтавкиНДСВСправочникеПоНазванию("5/105");		
	ИначеЕсли	Найти(сбисСтавка,"22") = 1 Тогда
		Возврат ПоискСтавкиНДСВСправочникеПоНазванию("22");		
	ИначеЕсли	Найти(сбисСтавка,"20") = 1 Тогда
	...
	ИначеЕсли	Найти(сбисСтавка,"7") = 1 Тогда
		Возврат ПоискСтавкиНДСВСправочникеПоНазванию("7");
	ИначеЕсли	Найти(сбисСтавка,"5") = 1 Тогда
		Возврат ПоискСтавкиНДСВСправочникеПоНазванию("5");
	ИначеЕсли	Найти(сбисСтавка,"0") = 1 Тогда
...
Функция ОпределитьСторонуДокумента(Контекст) Экспорт  
	Перем Результат, Роль, ОграниченияПоискСтороны;
	...
	// После того, как обработка пройдет через РассчитатьПараметрИни и в РассчитанноеЗначение ВМЕСТО узла ини встанут данные
	Контекст.ТекущееПолеИни.Свойство("РассчитанноеЗначение", Результат);
	
	Если	Не Контекст.Свойство("АргументыВходящие") 
		Или	Не Контекст.АргументыВходящие.Свойство("Параметр1", Роль) Тогда
	...
	Результат = ОпределитьСторонуДокумента_НайтиПоРолиИСтороне(Роль, Результат, Контекст);
...
&НаКлиенте
Функция ОпределитьСторонуДокумента_НайтиПоРолиИСтороне(Роль, ДанныеСтороны, Контекст) Экспорт  
	Перем ТипСтороны, Карточки1С, ОграниченияПоискСтороны;
	
	МодульОбъектаКлиент	= МодульОбъектаКлиент();
	Документ_Шаблон		= МодульОбъектаКлиент.ПолучитьФормуОбработки("Документ_Шаблон");
	ВложениеСбис		= Контекст.Вложение;

    Ключ		= ВложениеСбис.ДанныеСтруктурыФайла.Синонимы.Получить(Роль);
	Результат	= ДанныеСтороны;
	...
	БратьСтороныСПакета = ЗначениеЗаполнено(Ключ)
	            ...
	            И 	(Ключ = "Организация" ИЛИ Ключ = "Контрагент" ИЛИ Ключ = "Грузополучатель");
	...
	Иначе
		...
		// Ищем сторону запросом
		Если Контекст.ТекущееПолеИни.Свойство("Тип", ТипСтороны) Тогда
			
			ТипСтороны = МодульОбъектаКлиент.СсылочныйТипСтрокой(ТипСтороны);
		...
		УзелИниПоиска			= МодульОбъектаКлиент.ВложениеСБИС_Получить(ВложениеСбис, "СтруктураИниФайла");
		ПараметрыНайтиКарточки	= Новый Структура("ТипСтороны, КлючСтороны, УзелИниПоиска", ТипСтороны, Ключ, УзелИниПоиска);
		Если	Контекст.ТекущееПолеИни.Свойство("Ограничения", ОграниченияПоискСтороны)
			И	ЗначениеЗаполнено(ОграниченияПоискСтороны) Тогда
			
			// Необхобимо добавить на данные Стороны ограничение для поиска по организации в случае запроса.
			Результат.Вставить("Ограничения", ОграниченияПоискСтороны);
			
		КонецЕсли;
		
		Попытка
			
			Карточки1С = МодульОбъектаКлиент.Сторона_НайтиКарточки1С(Результат, ПараметрыНайтиКарточки);
			
		Исключение
			
			//МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке(), "РаботаСдокументами1С.ОпределитьСторонуДокумента");
			// Ничего не делаем. Пока пропускаем.
			
		КонецПопытки;
		
		Если НЕ Карточки1С = Неопределено
			И	Карточки1С.Количество() Тогда
			
			Результат = Карточки1С[0];
...
&НаКлиенте
Функция ДобавитьУпаковки(МестныйКэш, ФормаПросмотра) Экспорт
	
	СтраницаУпаковки = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ФормаПросмотра, "Контент.ТранспортныеУпаковки");
	СтраницаУпаковки.Видимость = Истина;
	
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ФормаПросмотра, "Контент").ТекущаяСтраница = СтраницаУпаковки;
	
	ФормаПросмотра.ЗаполнитьТранспортныеУпаковки();
