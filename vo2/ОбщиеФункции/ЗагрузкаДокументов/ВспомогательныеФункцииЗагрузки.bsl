...
Функция ОпределитьСторонуДокумента(Контекст) Экспорт  
	...
	Документ_Шаблон = МестныйКэш.ГлавноеОкно.СбисНайтиФормуФункции("РассчитатьЗначение", "Документ_Шаблон", "", МестныйКэш);
	Роль = Документ_Шаблон.РассчитатьЗначение("Роль", Контекст, МестныйКэш);
	Ключ = МодульОбъектаКлиент().СоставПакета_КлючСтороны(Контекст.СоставПакета, Роль);
