...
&НаКлиенте
Функция ОпределитьСторонуДокумента(Знач Контекст) Экспорт  
	Перем ВходящийКонтекст, Результат;
	
	МодульОбъектаКлиент = МодульОбъектаКлиент();
	фрм = МестныйКэш.ГлавноеОкно.СбисНайтиФормуФункции("РассчитатьЗначение", "Документ_Шаблон", "", МестныйКэш);
	Роль = фрм.РассчитатьЗначение("Роль", Контекст, МестныйКэш);
	
	Если МодульОбъектаКлиент.ПолучитьЗначениеПараметраСБИС("ИспользоватьВходящийКонтекст") Тогда
		
		Если 		Контекст.СоставПакета.Направление = "Входящий"
				И	Найти("Покупатель Отправитель", Роль)
			Или		Контекст.СоставПакета.Направление = "Исходящий"
				И	Найти("Поставщик Получатель", Роль) Тогда
			
			Ключ = "Контрагент";
		ИначеЕсли 	Контекст.СоставПакета.Направление = "Входящий"
				И	Найти("Поставщик Получатель", Роль)
			Или 	Контекст.СоставПакета.Направление = "Исходящий"
				И	Найти("Покупатель Отправитель", Роль) Тогда
			
			Ключ = "Организация"
		ИначеЕсли 	Контекст.СоставПакета.Направление = "Входящий"
			И		Роль = "Грузополучатель" Тогда
			
			Ключ = "Грузополучатель";
		КонецЕсли;
		
		Если 	ЗначениеЗаполнено(Ключ)
			И	Контекст.Ини.Свойство("ВходящийКонтекст", ВходящийКонтекст)
			И	ЗначениеЗаполнено(ВходящийКонтекст.РассчитанноеЗначение.Организация)
			И	ЗначениеЗаполнено(ВходящийКонтекст.РассчитанноеЗначение.Контрагент) Тогда
			
			Контекст.Ини.ВходящийКонтекст.РассчитанноеЗначение.Свойство(Ключ, Результат);
			
		Иначе 
			
			Контекст.Сторона.Сторона.Свойство("РассчитанноеЗначение", Результат);
