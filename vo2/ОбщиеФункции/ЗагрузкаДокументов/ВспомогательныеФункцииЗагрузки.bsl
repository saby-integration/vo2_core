...
Функция ОпределитьСторонуДокумента(Контекст) Экспорт  
	Перем ВходящийКонтекст, Результат, Роль, ТипСтороны, Карточки1С;
	...
	Если Не Контекст.АргументыВходящие.Свойство("Параметр1", Роль) Тогда
	...
	Иначе
		...
		Попытка
			...
			Карточки1С = МодульОбъектаКлиент().Сторона_НайтиКарточки1С(Результат, Новый Структура("ТипСтороны", ТипСтороны));
