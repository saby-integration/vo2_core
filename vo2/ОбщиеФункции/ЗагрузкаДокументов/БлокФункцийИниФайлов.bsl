...
Функция сбисСформироватьДанныеПоМаркировкеОбщее(Контекст, мШтрихкодыУпаковок, ДопПараметры = Неопределено) Экспорт       
	...
	Для Каждого Элемент Из мШтрихкодыУпаковок Цикл  
		...
		ВыгружаемКодУпаковки = Элемент.ТипУпаковки = "МультитоварнаяУпаковка"
					...
					И (ОсобенностьУчета = "ЛегкаяПромышленность"
		...
		Если Элемент.Свойство("ОбрезатьНулиSSCC")
				И Элемент.ОбрезатьНулиSSCC = Истина Тогда
			ОбрезатьНулиSSCC = Истина;
		Иначе 
			ОбрезатьНулиSSCC = Ложь;
		КонецЕсли;
		ДопПараметры.Вставить("ОбрезатьНулиSSCC", ОбрезатьНулиSSCC);
