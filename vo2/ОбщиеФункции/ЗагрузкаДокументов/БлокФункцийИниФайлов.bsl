...
Функция сбисСформироватьДанныеПоМаркировкеОбщее(Контекст, мШтрихкодыУпаковок, ДопПараметры = Неопределено) Экспорт  
	...
	Для Каждого Элемент Из мШтрихкодыУпаковок Цикл  
		...
		// Базовый сценарий
		ВыгружаемКодЕдиницы = Элемент.ТипУпаковки <> "МультитоварнаяУпаковка" 
			И Элемент.Свойство("ОсобенностьУчета") 
			И ЗначениеЗаполнено(Элемент.Номенклатура); 
			
		// На упаковке не заполнена номенклатура и требуется выгрузить код упаковки, а не её состав
		ВыгружаемКодУпаковкиПоСоставу = (РасширеннаяРаботаСМаркировкой 
			И Элемент.ТипУпаковки = "МультитоварнаяУпаковка")
			ИЛИ (НЕ ЗначениеЗаполнено(Элемент.Номенклатура) 
			И Элемент.ТипУпаковки = "МонотоварнаяУпаковка");
		...
		// Пробрасываем количество кодов маркировки, которое указано в элементе справочника ШтрихкодыУпаковокТоваров
		Если Элемент.Свойство("Количество") Тогда
			ДопПараметры.Вставить("Количество", Элемент.Количество);
		КонецЕсли;
		...
		Если ВыгружаемКодЕдиницы Тогда 
		...
		ИначеЕсли ВыгружаемКодУпаковкиПоСоставу Тогда
...
Функция сбисДобавитьДанныеШтрихкодаВСоответствие(Контекст, Маркировка, Элемент, ДанныеШтрихкода, ЕстьХарактеристики, ЕстьСерии) Экспорт  
	...
	ДанныеШтрихкодаДобавить = Новый Структура("ЗначениеШтрихкода, Количество", ДанныеШтрихкода.ЗначениеШтрихкода, ДанныеШтрихкода.Количество);
		
	Если НЕ ЕстьХарактеристики Тогда
		
		МодульОбъектаКлиент.НаборСредствИдентификацииТоваров_УстановитьДанные(ЭлемПоНоменклатуре,
			ДанныеШтрихкода.Вид, ДанныеШтрихкодаДобавить); 
...
Функция сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, ТипУпаковки, ЗначениеШтрихкода, ДопПараметры = Неопределено) Экспорт   
	...
	Если ДопПараметры.Свойство("Количество") Тогда
		Количество = ДопПараметры.Количество;
	Иначе
		Количество = 1;
	КонецЕсли;
	...
	МодульОбъектаКлиент.ДанныеШтрихкода_Установить(ДанныеШтрихкода,	"Количество",			Количество);
...
&НаКлиенте
Функция сбисПолучитьКодыМаркировокПоСтрокеВозврат(Контекст) Экспорт
	
	Попытка
		Количество = Число(Контекст.КоличествоВозвращается);
	Исключение
		Количество = 0;
	КонецПопытки;                                                                                     
...
Функция сбисЗаполнитьНомСредИдентТов(НомСредИдентТов, ДанныеПоМаркировке, Количество) Экспорт
	...
	МодульОбъектаКлиент.ЗаполнитьНомСредИдентТов_КолВедМарк		(НомСредИдентТов, ДанныеПоМаркировке, Количество);
