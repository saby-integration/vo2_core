	...
	Для Каждого Элемент Из мШтрихкодыУпаковок Цикл
		Если Элемент.Свойство("ТипУпаковки") Тогда  // Добавлено условие, так как встречаются случаи, когда в таб части ШтрихкодыУпаковок появляются пустые ссылки. Их нужно пропустить.
			Если Элемент.ТипУпаковки <> "МультитоварнаяУпаковка" И Элемент.Свойство("ОсобенностьУчета") Тогда   // добавлена проверка на ОсобенностьУчета, так как встречаются случаи, когда в монотоварной упаковке номенклатура не заполнена, тогда тоже смотрим на вложенные штрихкоды
				ОсобенностьУчета = РассчитатьЗначение("ОсобенностьУчета", Элемент);
			...
			Иначе
				Для Каждого ВложенныйЭлемент Из Элемент.мВложенныеШтрихкоды Цикл 
					ОсобенностьУчета = РассчитатьЗначение("ОсобенностьУчета", ВложенныйЭлемент);
					ДанныеШтрихкода = сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, Элемент.ТипУпаковки, Элемент.ЗначениеШтрихкода);
	...
	ИначеЕсли	(	ТипУпаковки = "МонотоварнаяУпаковка"
			...
			ИЛИ (	Лев(ЗначениеШтрихкода,2) = "02"
				...
				И (ОсобенностьУчета = "УпакованнаяВода" ИЛИ ОсобенностьУчета = "Молоко" ИЛИ ОсобенностьУчета = "БАДы" 
					ИЛИ ОсобенностьУчета = "Антисептики" ИЛИ ОсобенностьУчета = "СоковаяПродукция")) Тогда // объемно-сортовой учет
...
Функция ЗаполнитьEDIТЧ(Контекст) Экспорт
	Если	Не Контекст.СтруктураФайла.Файл.Документ.Покупатель.Свойство("GLN") 
		Или	Не ЗначениеЗаполнено(Контекст.СтруктураФайла.Файл.Документ.Покупатель.GLN) Тогда
		Возврат Неопределено;
	КонецЕсли;
