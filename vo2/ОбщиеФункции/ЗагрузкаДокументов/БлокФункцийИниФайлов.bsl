...
Функция сбисСтруктураАдрИно(КодСтраны, Представление, НаимСтран = "") Экспорт   // alo вставлена для УКД
	...
	АдрИно.Вставить("НаимСтран",?(ЗначениеЗаполнено(НаимСтран),КодСтраны,"РОССИЯ"));
...
Функция сбисВычислитьЦену(Контекст) Экспорт 	
	Если Контекст.СуммаВключаетНДС Тогда
		...
		Если Сумма = 0 И Контекст.Свойство("СуммаНДСДо") Тогда
			...
			Попытка
				Кол_воДо = ?(Число(Контекст.ПредСтрТабл_Кол_во) = 0, 1, Число(Контекст.ПредСтрТабл_Кол_во));
			Исключение
				Кол_воДо = 1;
			КонецПопытки;
			Попытка
				СуммаНДСДо = Число(Контекст.СуммаНДСДо);
			Исключение
				СуммаНДСДо = 0;
			КонецПопытки;
			Попытка
				СуммаСНДСДо = Число(Контекст.ПредСтрТабл_Сумма);
			Исключение
				СуммаСНДСДо = 0;
			КонецПопытки;
	...
	Иначе
		...
		Если Контекст.Цена = 0 Тогда
			...
			Попытка
				...
				КонецПопытки;
				Попытка
					СуммаДо = Число(Контекст.ПредСтрТабл_Сумма);
				Исключение
					СуммаДо = 0;
				КонецПопытки;
