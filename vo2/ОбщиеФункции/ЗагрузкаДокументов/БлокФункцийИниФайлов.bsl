...
Функция сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, ТипУпаковки, ЗначениеШтрихкода) Экспорт   
	...
	Если ТипУпаковки = "МаркированныйТовар" Тогда
		...
		ИначеЕсли ОсобенностьУчета = "Фотоаппараты" Тогда
			ЗначениеШтрихкода = Лев(ЗначениеШтрихкода, 38);
		КонецЕсли;
	ИначеЕсли	(	ТипУпаковки = "МонотоварнаяУпаковка"
		И	(	ОсобенностьУчета = "Табак" Или ОсобенностьУчета = "АльтернативныйТабак" Или ОсобенностьУчета = "НикотиносодержащаяПродукция")
		И	Лев(ЗначениеШтрихкода,2) = "01"
		И	Сред(ЗначениеШтрихкода, 17, 2) = "21"
		И	(	Сред(ЗначениеШтрихкода, 26, 4) = "8005"
		Или СтрДлина(СокрЛП(ЗначениеШтрихкода)) = 25)) // встретился у клиента непонятный штрихкод 01146033990011272001212005/05/22/2/3/0001, который не подходит ни под одно описание, но должен выгружаться в ИдентТрансУпак (как во входящем ЭД)
		
		Или (КодВФорматеОСУ
			И НЕ НовыйФорматУПД) Тогда // объемно-сортовой учет
...
Функция сбисПолучитьКодыМаркировокПоСтрокеОбщее(Контекст, КодыМаркировки, Количество) Экспорт 
	...
	Если ЗначениеЗаполнено(ДанныеПоМаркировке.GTIN) Тогда
		
		 Контекст.Вставить("GTIN", ДанныеПоМаркировке.GTIN)  
...
Функция сбисЗаполнитьНомСредИдентТов(НомСредИдентТов, ДанныеПоМаркировке, Количество) Экспорт
	Если ЗначениеЗаполнено(ДанныеПоМаркировке) Тогда  
		...
		Если ДанныеПоМаркировке.КолВедМарк.Количество()>0 Тогда
			...
			КонецЦикла;
		КонецЕсли;    
		Если ДанныеПоМаркировке.ПрПартМарк.Количество()>0 Тогда
...
&НаКлиенте
Функция сбисЗаполнитьАдресИзJSON(АдресЗаписьЗначение,Индекс,Регион,Район,Город,НаселПункт,Улица,УлицаДопИнфо,Дом,Корпус,Квартира,КодСтраны,НаимСтран,ЗаполненоЗначениеАдреса);
