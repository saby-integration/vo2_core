		...
		Если Элемент.Свойство("ТипУпаковки") Тогда  // Добавлено условие, так как встречаются случаи, когда в таб части ШтрихкодыУпаковок появляются пустые ссылки. Их нужно пропустить.
			...
			Иначе
				Для Каждого ВложенныйЭлемент Из Элемент.мВложенныеШтрихкоды Цикл 
					ОсобенностьУчета = РассчитатьЗначение("ОсобенностьУчета", НоменклатураДляРазбора);
					ДанныеШтрихкода = сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, Элемент.ТипУпаковки, ВложенныйЭлемент.ЗначениеШтрихкода);
...
Функция сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, ТипУпаковки, ЗначениеШтрихкода) Экспорт
	...
	ИначеЕсли	(	ТипУпаковки = "МонотоварнаяУпаковка"
		...
		Или (	Лев(ЗначениеШтрихкода,2) = "02"
		И	Сред(ЗначениеШтрихкода, 17, 2) = "37"
		И (ОсобенностьУчета = "УпакованнаяВода" Или ОсобенностьУчета = "Молоко" Или ОсобенностьУчета = "БАДы" 
		Или ОсобенностьУчета = "Антисептики" Или ОсобенностьУчета = "СоковаяПродукция"))
		Или ОсобенностьУчета = "ТабачноеСырье" Тогда // объемно-сортовой учет
