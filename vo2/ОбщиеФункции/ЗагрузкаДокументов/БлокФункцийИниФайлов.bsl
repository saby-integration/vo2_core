...
Функция сбисСформироватьДанныеПоМаркировкеОбщее(Контекст, мШтрихкодыУпаковок) Экспорт       
	...
	Для Каждого Элемент Из мШтрихкодыУпаковок Цикл 
		...
		Если Элемент.Свойство("ТипУпаковки") Тогда 
			...
			Если Элемент.ТипУпаковки <> "МультитоварнаяУпаковка" И Элемент.Свойство("ОсобенностьУчета") Тогда 
...
Функция сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, ТипУпаковки, ЗначениеШтрихкода) Экспорт   
	...
	УпакованнаяВодаНомУпак = (ОсобенностьУчета = "УпакованнаяВода"
								И НЕ Лев(ЗначениеШтрихкода,2) = "00");
...
Функция сбисПолучитьКодыМаркировокПоСтрокеОбщее(Контекст, КодыМаркировки, Количество) Экспорт 
	...
	Если ДанныеПоМаркировке.Свойство("GTIN") 
		И ЗначениеЗаполнено(ДанныеПоМаркировке.GTIN) Тогда 
...
// Получает ФиасИД со стороны Грузополучатель / Получатель, если он - ИП (нет КПП)
&НаКлиенте
Функция ПолучитьФиасИдГрузополучателяИП(Контекст) Экспорт
	
	Если Контекст.мСторона.Свойство("Грузополучатель")
			И Контекст.мСторона.Грузополучатель.Свойство("Сторона_КПП")
			И ЗначениеЗаполнено(Контекст.мСторона.Грузополучатель.Сторона_КПП) Тогда
		// Для Юр. лиц ФиасИд не получаем
		Возврат Неопределено;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(АдресДоставки) Тогда
		Если Контекст.мСторона.Свойство("Грузополучатель") Тогда
			АдресДоставки = РассчитатьЗначение("Сторона_Адрес", Контекст.мСторона.Грузополучатель);
		Иначе
			АдресДоставки = РассчитатьЗначение("Сторона_Адрес", Контекст.мСторона.Получатель);
		КонецЕсли;
	КонецЕсли;
	
	Если	ЗначениеЗаполнено(АдресДоставки)
			И АдресДоставки.Свойство("ФиасИд")
			И ЗначениеЗаполнено(АдресДоставки.ФиасИд) Тогда
...
// Получает ФиасИД со стороны Грузоотправитель / Отправитель, если он - ИП (нет КПП)
&НаКлиенте
Функция ПолучитьФиасИдГрузоотправителяИП(Контекст) Экспорт
	
	Если	Контекст.мСторона.Отправитель.Свойство("Сторона_КПП")
			И ЗначениеЗаполнено(Контекст.мСторона.Отправитель.Сторона_КПП) Тогда
		// Для Юр. лиц ФиасИд не получаем
		Возврат Неопределено;
	КонецЕсли;
	...
	Если Контекст.мСторона.Свойство("Грузоотправитель") Тогда
		АдресПродавца = РассчитатьЗначение("Сторона_Адрес", Контекст.мСторона.Грузоотправитель);
	ИначеЕсли Контекст.мСторона.Свойство("Отправитель") Тогда
		АдресПродавца = РассчитатьЗначение("Сторона_Адрес", Контекст.мСторона.Отправитель);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Если	ЗначениеЗаполнено(АдресПродавца)
			И АдресПродавца.Свойство("ФиасИд")
			И ЗначениеЗаполнено(АдресПродавца.ФиасИд) Тогда
		ФиасИд = АдресПродавца.ФиасИд;
	КонецЕсли;
...
Функция сбисВычислитьЦену(Контекст) Экспорт 	
	...
	Иначе
		...
		Если Число(РассчитатьЗначение("Цена", Контекст)) = 0 Тогда
			// В случае, когда количество в итоговой таблице обнуляется - приходится смотреть в изначальные строки
			Попытка
				Попытка
					Кол_воДо = ?(Число(РассчитатьЗначение("ПредСтрТабл_Кол_во", Контекст)) = 0,
