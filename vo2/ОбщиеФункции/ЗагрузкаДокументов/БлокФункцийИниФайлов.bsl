...
Функция ПолучитьКодСтраныПоНазванию(НазваниеСтраны) Экспорт
	...
	Если МодульОбъектаСерверБезКонтекста().СбисСуществуетОбъектКонфигурацииСервер("СтраныМира", ДопПараметры) Тогда
		Возврат Справочники.СтраныМира.НайтиПоНаименованию(СокрЛП(НазваниеСтраны), Истина).Код;	
	ИначеЕсли МодульОбъектаСерверБезКонтекста().СбисСуществуетОбъектКонфигурацииСервер("КлассификаторСтранМира", ДопПараметры) Тогда
...
Функция НайтиКонтрагентаИзДокументаСБИС(Ини, оУчастник, ИниДок = Неопределено, МножественныйВыбор = Ложь, СообщитьЕслиНеНайден = Ложь, ТолькоПоИНН = Ложь) Экспорт
	...
	Если оУчастник.Свойство("GLN") И ЗначениеЗаполнено(оУчастник.GLN) И Контрагент = Ложь И СообщитьЕслиНеНайден Тогда
