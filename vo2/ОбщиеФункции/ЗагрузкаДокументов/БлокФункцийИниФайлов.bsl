...
Функция сбисСформироватьДанныеПоМаркировкеОбщее(Контекст, мШтрихкодыУпаковок) Экспорт       
	...
	Для Каждого Элемент Из мШтрихкодыУпаковок Цикл  
		...
		Если Элемент.ТипУпаковки <> "МультитоварнаяУпаковка" 
			...
			ДанныеШтрихкода = сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, Элемент.ТипУпаковки, Элемент.ЗначениеШтрихкода);
		...
		Иначе
			...
			Для Каждого ВложенныйЭлемент Из Элемент.мВложенныеШтрихкоды Цикл  
				...
				ДанныеШтрихкода = сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, ВложенныйЭлемент.ТипУпаковки, ВложенныйЭлемент.ЗначениеШтрихкода);
				сбисДобавитьДанныеШтрихкодаВСоответствие(Контекст, Маркировка, ВложенныйЭлемент, ДанныеШтрихкода, ЕстьХарактеристики, ЕстьСерии);	
...
Функция сбисСформироватьДанныеШтрихкода(Контекст, ОсобенностьУчета, ТипУпаковки, ЗначениеШтрихкода) Экспорт   
	...
	НовыйФорматУПД = РассчитатьЗначение("_УПД24", Контекст) = Истина; 
	...
	КодВФорматеОСУ = МодульОбъектаКлиент().СбисЭтоКодОСУ(ЗначениеШтрихкода)
	...
	ТабачнаяПродукцияНомУпак = (ОсобенностьУчета = "Табак" 
	...
	ИначеЕсли КодВФорматеОСУ
		...
		ДанныеШтрихкода.Вид = "КолВедМарк"; 
	...
	Иначе
		...
		ШтрихкодБезСкобок = МодульОбъектаКлиент().ШтрихкодБезСкобок(ЗначениеШтрихкода, Новый Массив);  
		
		Если ШтрихкодБезСкобок.Количество() Тогда   
			
			ЗначениеШтрихкода = МодульОбъектаКлиент().СбисСтрСоединить(ШтрихкодБезСкобок, ""); 
			...
			ИначеЕсли ДанныеПоМаркировкеС = Неопределено И Контекст.Серия.Пустая() И ДанныеПоМаркировке.Количество() > 0 Тогда
				...
				Для	Каждого Элем Из ДанныеПоМаркировке Цикл
					...
					Для Каждого ИдентТрансУпак Из Элем.Значение.ИдентТрансУпак Цикл
						...
						МодульОбъектаКлиент.НаборСредствИдентификацииТоваров_УстановитьДанные(МаркировкаСПустойСерией, "ИдентТрансУпак", ИдентТрансУпак.Значение);
						
					КонецЦикла;  
					
					Для Каждого КолВедМарк Из Элем.Значение.КолВедМарк Цикл
						
						МодульОбъектаКлиент.НаборСредствИдентификацииТоваров_УстановитьДанные(МаркировкаСПустойСерией, "КолВедМарк", КолВедМарк.Значение);
						
					КонецЦикла;  
					
					Для Каждого ПрПартМарк Из Элем.Значение.ПрПартМарк Цикл
						
						МодульОбъектаКлиент.НаборСредствИдентификацииТоваров_УстановитьДанные(МаркировкаСПустойСерией, "ПрПартМарк", ПрПартМарк.Значение);
					...
					Для Каждого НомУпак Из Элем.Значение.НомУпак Цикл
						...
						МодульОбъектаКлиент.НаборСредствИдентификацииТоваров_УстановитьДанные(МаркировкаСПустойСерией, "НомУпак", НомУпак.Значение); 
						
					КонецЦикла; 
					
					Для Каждого КИЗ Из Элем.Значение.КИЗ Цикл 
						
						МодульОбъектаКлиент.НаборСредствИдентификацииТоваров_УстановитьДанные(МаркировкаСПустойСерией, "КИЗ", КИЗ.Значение); 	
...
Функция сбисЗаполнитьНомСредИдентТов(НомСредИдентТов, ДанныеПоМаркировке, Количество) Экспорт
	Если ЗначениеЗаполнено(ДанныеПоМаркировке) Тогда  
		...
		Если ДанныеПоМаркировке.Свойство("КолВедМарк")
			И ДанныеПоМаркировке.КолВедМарк.Количество()>0 Тогда
		...
		Если ДанныеПоМаркировке.Свойство("ПрПартМарк") 
			И ДанныеПоМаркировке.ПрПартМарк.Количество()>0 Тогда 
...
&НаКлиенте
Функция сбисСтруктураАдрИно(КодСтраны, Представление, НаимСтран = "") Экспорт   // alo вставлена для УКД
...
Функция ПолучитьАдресЗначениеПоКонтрагентуИПредставлениюАдресаУП_НаСервере(Контекст)
...
Функция ПолучитьФиасИдГрузополучателяИП(Контекст) Экспорт
	...
	ВключеноЗаполнениеФиасИдНаУПДСГенератором = МодульОбъектаКлиент().ПолучитьЗначениеФичи("ЗаполнениеФиасИдНаУПДСГенератором");
	
	Если ГенераторВключенДляДокумента И ВключеноЗаполнениеФиасИдНаУПДСГенератором Тогда
...
Функция ПолучитьАдресУПОбщая(АдресЗапись) Экспорт
	...
	Если	АдресЗапись.Свойство("Значение")
		...
		сбисЗаполнитьАдресИзJSON(АдресЗапись.Значение,
			Индекс,
			Регион,
			Район,
			Город,
		...
		Если Лев(АдресРег,21) = "<КонтактнаяИнформация" Тогда
				...
				Если ЗначениеСостав.Свойство("ДопАдрЭл") Тогда
					Для Каждого АдрЭл Из ЗначениеСостав.ДопАдрЭл Цикл
						Если АдрЭл.Свойство("ТипАдрЭл") и АдрЭл.Свойство("Значение") Тогда
							...
							КонецЕсли;
						ИначеЕсли	АдрЭл.Свойство("Номер")
								И	АдрЭл.Номер.Свойство("Тип")
								И	АдрЭл.Номер.Свойство("Значение") Тогда
								
							Если		АдрЭл.Номер.Тип = "1010" Тогда
								
								Дом = АдрЭл.Номер.Значение;
								
							ИначеЕсли	АдрЭл.Номер.Тип = "1015" Тогда							// Подвал
								
								Корпус = Корпус + "подв." + АдрЭл.Номер.Значение + ",";
								
							ИначеЕсли	АдрЭл.Номер.Тип = "1016" Тогда							// Котельная
								
								Корпус = Корпус + "кот." + АдрЭл.Номер.Значение + ",";
								
							ИначеЕсли	АдрЭл.Номер.Тип = "1017" Тогда							// Погреб

								Корпус = Корпус + "п-б" + АдрЭл.Номер.Значение + ",";
							...
							ИначеЕсли	АдрЭл.Номер.Тип = "1050"
									Или	АдрЭл.Номер.Тип = "1014" Тогда							// корпус
							...
							ИначеЕсли	АдрЭл.Номер.Тип = "1060"
									Или	АдрЭл.Номер.Тип = "1011" Тогда
								
								Корпус = Корпус + "стр." + строка(АдрЭл.Номер.Значение) + ",";	//Строение
								
							ИначеЕсли	АдрЭл.Номер.Тип = "1070"
									Или	АдрЭл.Номер.Тип = "1012" Тогда
								
								Корпус = Корпус + "соор." + строка(АдрЭл.Номер.Значение)+",";	//Сооружение
								
							ИначеЕсли	АдрЭл.Номер.Тип = "1080"
									Или	АдрЭл.Номер.Тип = "1090"
									Или	АдрЭл.Номер.Тип = "1013" Тогда   
								
								Корпус = Корпус + "лит." + строка(АдрЭл.Номер.Значение) + ",";	// литера или литер
			...
			ИначеЕсли ТипЗнч(ЗначениеСостав) = Тип("Строка") Тогда	 // иностранный адрес
				Попытка
					...
					НаимСтран = АдресСтруктураXML.КонтактнаяИнформация.Состав.Страна;
				Исключение
				КонецПопытки;
			КонецЕсли;
		Иначе
			...
			Пока сч<=СтрЧислоСтрок(АдресРег) Цикл
				...
				ИначеЕсли Найти(нрег(ЭлементАдреса), "страна=") Тогда
					НаимСтран = Сред(ЭлементАдреса, 8);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли; 
	КонецЕсли;
	...
	Попытка
		...
		сбисПроверитьПолеАдреса(АдрСтрук, "КодРегион",	КодРегион, 2);
		сбисПроверитьПолеАдреса(АдрСтрук, "НаимРегион",	Регион, 50);
	...
	Исключение
		Адрес.Вставить("АдрИно",сбисСтруктураАдрИно(КодСтраны, АдресЗапись.Представление, НаимСтран)); // alo УКД
	КонецПопытки;
	//  УКД alo >>
	
	Адрес.Вставить("ФиасИД", ФиасИД);
...
&НаКлиенте
Функция сбисЗаполнитьАдресИзJSON(АдресЗаписьЗначение,Индекс,Регион,Район,Город,НаселПункт,Улица,УлицаДопИнфо,Дом,Корпус,Квартира,КодСтраны,НаимСтран,ФиасИд,ЗаполненоЗначениеАдреса);
	...
	Попытка
		...
		Если АдресСтруктура.Свойство("houseType") и АдресСтруктура.Свойство("houseNumber") Тогда
			...
			ИначеЕсли нрег(АдресСтруктура.houseType) = "строение" Тогда  // строение
				Дом = "стр."+строка(АдресСтруктура.houseNumber); 
			КонецЕсли;
		...
		КонецЕсли;
		Если	АдресСтруктура.Свойство("buildings")
			И	ТипЗнч(АдресСтруктура.buildings) = Тип("Массив") Тогда
			...
			Для Каждого КорпусСтруктура Из АдресСтруктура.buildings Цикл
				...
				ИначеЕсли	ТипКорпуса = "литера"
						Или	ТипКорпуса = "литер" Тогда   // литера или литер
						
					Корпус = Корпус + "лит."	+ Строка(КорпусСтруктура.number) + ",";
						
				ИначеЕсли	ТипКорпуса = "подвал" Тогда
					
					Корпус = Корпус + "подв."	+ Строка(КорпусСтруктура.number) + ",";
					
				ИначеЕсли	ТипКорпуса = "котельная" Тогда
					
					Корпус = Корпус + "кот."	+ Строка(КорпусСтруктура.number) + ",";

				ИначеЕсли	ТипКорпуса = "погреб" Тогда
					
					Корпус = Корпус + "п-б"		+ Строка(КорпусСтруктура.number) + ",";
		...
		Если АдресСтруктура.Свойство("country") Тогда
			НаимСтран = АдресСтруктура.country;	
		КонецЕсли;
		Если АдресСтруктура.Свойство("houseId") Тогда
			ФиасИд = АдресСтруктура.houseId;
		КонецЕсли;
...
Функция сбисДатаИНомерЗаказаПоКонтуру(Контекст) Экспорт
	...
	Перем EDIПервичныйДокумент, Реализация; 
	
	Если МестныйКэш.ПараметрыСистемы.Конфигурация.Расширение.Свойство("КонтурEDI") Тогда
		Если Контекст.Свойство("EDIПервичныйДокумент") Тогда
			
			EDIПервичныйДокумент = Контекст.EDIПервичныйДокумент;
		...
		Возврат РассчитатьДатуНомерЗаказаПоКонтуру(EDIПервичныйДокумент, Реализация);
	КонецЕсли;
...
&НаСервере
Функция РассчитатьДатуНомерЗаказаПоКонтуру(Знач EDIПервичныйДокумент, Знач Реализация) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И КонтурEDI_Сообщения.Документ = &ДокументЗаказа
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |	И КонтурEDI_Сообщения.Документ = &ДокументОтгрузки
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |		ПО (ИСТИНА)";
	Если EDIПервичныйДокумент = Неопределено Тогда
		Возврат СтруктураДатаИНомерКонтура;	
	Иначе 
		Запрос.УстановитьПараметр("ДокументЗаказа", EDIПервичныйДокумент);
	КонецЕсли;
	
	Если Реализация = Неопределено Тогда
		Запрос.УстановитьПараметр("ДокументОтгрузки", Неопределено);
	Иначе 
		Запрос.УстановитьПараметр("ДокументОтгрузки", Реализация);
	КонецЕсли;
	...
	Пока РезультатЗапроса.Следующий() Цикл
		 ...
		 СтруктураДатаИНомерКонтура.Вставить("Дата", РезультатЗапроса.ДатаЗаказа);
		 СтруктураДатаИНомерКонтура.Вставить("Номер", РезультатЗапроса.НомерЗаказа);
		 СтруктураДатаИНомерКонтура.Вставить("ДатаПоставки", РезультатЗапроса.ДатаПоставки);
		 СтруктураДатаИНомерКонтура.Вставить("НомерДокументаОтгр", РезультатЗапроса.НомерДокумента);
		 СтруктураДатаИНомерКонтура.Вставить("ДатаДокументаОтгр", РезультатЗапроса.ДатаДокумента);
...
Функция РассчитатьЗначение(Параметр, Контекст, ВходящийКэш="") Экспорт //Контекст{ини,Документ,СтрТабл}
	...
	Попытка
		...
		ИначеЕсли Контекст.Свойство(Параметр, ЗнПер) Тогда
			
			Если	ТипЗнч(ЗнПер) = Тип("Структура")
				И	ЗнПер.Свойство("Значение") Тогда
				Если ЗнПер.Свойство("РассчитанноеЗначение") Тогда
					
					Возврат ЗнПер.РассчитанноеЗначение;
	...
	Если Лев(ЗнПер, 1)="!" и Найти(ЗнПер,"{")>1 и Прав(ЗнПер, 1)="}" Тогда // кешируемая функция
		...
		Если КэшироватьФункцию = "Организация" Тогда 
			...
			Если МестныйКэш.КэшЗначенийИни.Организации.Свойство("к"+ИНН+КПП+"_"+Параметр) Тогда
				Возврат МестныйКэш.КэшЗначенийИни.Организации["к"+ИНН+КПП+"_"+Параметр];
			КонецЕсли;
		Иначе	
			Если МестныйКэш.КэшЗначенийИни.ТекущийПакет.Свойство(КэшироватьФункцию+"_"+Параметр) Тогда
				Возврат МестныйКэш.КэшЗначенийИни.ТекущийПакет[КэшироватьФункцию+"_"+Параметр];
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	Если Найти(ЗнПер,"{")=1 Тогда // функция
		...
		Попытка
			ГлавноеОкно = Неопределено;
			Если МестныйКэш <> "" Тогда
				ГлавноеОкно = МестныйКэш.ГлавноеОкно;
			ИначеЕсли МестныйКэш = Неопределено
				Или Не МестныйКэш.Свойство("ГлавноеОкно", ГлавноеОкно) Тогда 
				ГлавноеОкно = сбисПолучитьФорму("ФормаГлавноеОкно");
			КонецЕсли;
			фрм = ГлавноеОкно.сбисНайтиФормуФункции(ИмяФункции,"РаботаСДокументами1С","", МестныйКэш);
			Результат=вычислить("фрм."+ИмяФункции+"(Контекст)");
			Если КэшироватьФункцию<>Ложь Тогда
				Если КэшироватьФункцию = "Организация" Тогда
					МестныйКэш.КэшЗначенийИни.Организации.Вставить("к"+ИНН+КПП+"_"+Параметр, Результат);
				Иначе
					МестныйКэш.КэшЗначенийИни.ТекущийПакет.Вставить(КэшироватьФункцию+"_"+Параметр, Результат);
				КонецЕсли;
			КонецЕсли;  
			
			Если ТипЗнч(Контекст[Параметр]) = Тип("Структура") 
				И Контекст[Параметр].Свойство("Формат") Тогда
				
				ОписаниеЧисло = Новый ОписаниеТипов("Число");
				РезультатПриведенияТипов = ОписаниеЧисло.ПривестиЗначение(Результат); 
				Если Не РезультатПриведенияТипов = 0 Тогда
					Результат = РезультатПриведенияТипов;
				КонецЕсли;
				Результат = Формат(Результат, Контекст[Параметр].Формат);
			КонецЕсли;    
...
Функция сбисВычислитьФормулу(ЗнПер,ПараметрИни,Контекст)
	Попытка
		...
		ИначеЕсли ИмяФормулы = "!=" Тогда
			...
			КонецЕсли;
		ИначеЕсли ИмяФормулы = "+" Тогда
			Если ПараметрИни.Свойство("Тип") Тогда
				ОписаниеТипа = Новый ОписаниеТипов(ПараметрИни.Тип);
				Результат = ОписаниеТипа.ПривестиЗначение();
			Иначе
				Результат = "";
			КонецЕсли;
			Для Каждого Элемент Из МассивПараметров Цикл
				Элемент = РассчитатьЗначениеНаСервере(Элемент, Контекст);
				Если ПараметрИни.Свойство("Тип") И ПараметрИни.Тип <> "Строка" Тогда
		...
		ИначеЕсли ИмяФормулы = "-" Тогда
			...
			КонецЦикла;
			Попытка
				Уменьшаемое = ОписаниеТипа.ПривестиЗначение(МассивПараметров[0]);
			Исключение
				Уменьшаемое = 0;
			КонецПопытки;
			Попытка
...
Функция сбисАвтоматическиеОперации(Контекст) экспорт
	...
	Если УстановленаОбработка Тогда	
		...
		фрм.Открыть();   
	Иначе
		ТекстПредупреждение = "Работа с ""Автоматическими операциями"" возможна только при добавленнии обработки в
