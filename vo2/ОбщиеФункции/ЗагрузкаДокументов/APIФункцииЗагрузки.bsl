...
&НаКлиенте
Функция СбисЗаполнитьДанныеИсходногоДокументаПоПакету(Контекст) Экспорт
	
	ПараметрыПОискаОснований = Новый ФиксированнаяСтруктура("ВложениеСБИС", Контекст.Вложение);
	Возврат МодульОбъектаКлиент().СоставПакета_Получить(Контекст.СоставПакета, "ДокументОснованиеДляВложения", ПараметрыПОискаОснований);
