...
Функция ЗаполнитьДоговорАА66(Контекст) Экспорт
	...
	ДопПараметры = Новый Структура("ТипПараметра", "Сеанс");
	ОсобенностиКонфигурации = МодульОбъектаКлиент().ПараметрSABY("ОсобенностиКонфигурации", ДопПараметры);
	...
	Попытка
		...
		НовыйДоговор = ПолучитьДоговорАльфы6СложнымПутем(ПараметрыЗапроса, ОсобенностиКонфигурации);
		Возврат НовыйДоговор;
	Исключение
...
&НаСервереБезКонтекста
Функция ПолучитьДоговорАльфы6СложнымПутем(ПараметрыЗапроса, ОсобенностиКонфигурации)
	...
	ОсобенностиРассчетаДоговора = ОсобенностиКонфигурации.ОсобенностиРассчетаДоговора;
	
	Для Каждого Особенность Из ОсобенностиРассчетаДоговора Цикл
		
		Если Особенность.Ключ = "РеквизитСервиса" И НЕ Особенность.Значение = "ДляАвтосервиса" Тогда
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДляАвтосервиса", Особенность.Значение); 
		КонецЕсли;
...
&НаКлиенте
Функция НовыеМетаданныеДоговораАльфаАвто6(Контекст) Экспорт
	Возврат НовыеМетаданныеДоговораАльфаАвто6ВызовСервера();	
КонецФункции 

&НаСервере
Функция НовыеМетаданныеДоговораАльфаАвто6ВызовСервера()
	
	СтруктураИзмененныхМетаданных = Новый Структура;
	Если Метаданные.Справочники.ДоговорыВзаиморасчетов.Реквизиты.Найти("ДляАвтосервиса") = Неопределено Тогда
		СтруктураИзмененныхМетаданных.Вставить("РеквизитСервиса", "ДляСервиса");
	Иначе
		СтруктураИзмененныхМетаданных.Вставить("РеквизитСервиса", "ДляАвтосервиса");
	КонецЕсли;
	
	Возврат СтруктураИзмененныхМетаданных;	
...
Функция ЗначениеИТипСтавки(Ставка) Экспорт
	...
	СтавкаСиноним = НРег(СтрЗаменить(СтавкаСиноним, "%", ""));   
	СтавкаСиноним = СтрЗаменить(СтавкаСиноним, "ставка", "");
	СтавкаСиноним = СокрЛП(СтрЗаменить(СтавкаСиноним, " ", ""));
...
Функция сбисРассчитатьЦену(Контекст) Экспорт
	...
	ЦенаВключаетНДС  = НЕ МодульРассчитать.РассчитатьЗначение("[Документ].СуммаВключаетНДС", Контекст) = Ложь; 
	ПараметрыРасчета.Вставить("ЦенаВключаетНДС", ЦенаВключаетНДС);
...
Функция сбисРассчитатьПредЦену(Контекст) Экспорт
	...
	МодульРассчитать = МодульОбъектаКлиент().ПолучитьФормуОбработки("Документ_Шаблон");
	ЦенаВключаетНДС  = НЕ МодульРассчитать.РассчитатьЗначение("[Документ].СуммаВключаетНДС", Контекст) = Ложь; //МодульРассчитать.РассчитатьЗначение("Документ_СуммаВключаетНДС",	Контекст)
	ПараметрыРасчета.Вставить("ЦенаВключаетНДС", ЦенаВключаетНДС);
...
Функция ПолучитьНомераИДатыПлатежноРасчетныхДокументов(ПараметрыЗапроса)
	...
	Запрос.Текст = 
		...
		|
		|{ДополнительныйЗапросДляВзаимозачета}
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	МетаданныеОперацийВзаимозачета = Метаданные.Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ЗначенияПеречисления;
	Если МетаданныеОперацийВзаимозачета.Найти("ОтражениеОплатыЧерезКомиссионера") <> Неопределено Тогда
		ДополнительныйЗапросДляВзаимозачета = 
		"ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Иначе
		ДополнительныйЗапросДляВзаимозачета = "";
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "{ДополнительныйЗапросДляВзаимозачета}", ДополнительныйЗапросДляВзаимозачета);
...
Функция ПолучитьПлатежноРасчетныеДокументыУТ11(ПараметрыЗапроса) Экспорт
	...
	НоваяАрхитектураВзаиморасчетов	= Метаданные.ФункциональныеОпции.Найти("НоваяАрхитектураВзаиморасчетов");
	ТолькоАвансы					= Ложь;
	...
	Если НоваяАрхитектураВзаиморасчетов = Неопределено Тогда
	...
	ИначеЕсли Метаданные.Справочники.Найти("ОбъектыРасчетов") <> Неопределено Тогда
	...
	Иначе
		...
		Запрос.Текст = 
		   ...
		   |		РегистрНакопления.РасчетыСКлиентамиПоСрокам КАК Расчеты
		   |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаОтбораОснованийИДокументовОплаты КАК ТаблицаОтбораОснованийИДокументовОплаты
		   |			ПО Расчеты.ОбъектРасчетов = ТаблицаОтбораОснованийИДокументовОплаты.ДокументОтбора
		   |	ГДЕ
		   ...
		   |		РегистрНакопления.РасчетыСКлиентамиПоДокументам КАК Расчеты
		   |			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаОтбораОснованийИДокументовОплаты КАК ТаблицаОтбораОснованийИДокументовОплаты
		   |			ПО Расчеты.ЗаказКлиента = ТаблицаОтбораОснованийИДокументовОплаты.ДокументОтбора
		   |	ГДЕ
		   |		Расчеты.Регистратор = Расчеты.РасчетныйДокумент
		   |		И Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
		   |		И Расчеты.Период <= &ДатаОкончания
