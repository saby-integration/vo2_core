...
Функция ЗаполнитьДоговорУТ11(Контекст) Экспорт
	...
	сбисКонтекстСервера	= Новый Структура("Документ", Новый Структура);
	сбисСкопироватьСтруктуруНаКлиенте(сбисКонтекстСервера.Документ, Контекст.Документ);
	Если НЕ сбисКонтекстСервера.Документ.Свойство("Партнер") Тогда
		сбисКонтекстСервера.Документ.Вставить("Партнер", Неопределено)
	КонецЕсли;
...
Функция сбисРассчитатьЦену(Контекст) Экспорт
	...
	Попытка
		...
		СуммаСНДС = Число(?(СтрТабл.Свойство("СуммаСНДС") И ЗначениеЗаполнено(СтрТабл.СуммаСНДС), СтрТабл.СуммаСНДС, 
							?(СтрТабл.Свойство("Сумма") И ЗначениеЗаполнено(СтрТабл.Сумма), СтрТабл.Сумма, 
								?(СтрТабл.Свойство("СуммаБезНал") И ЗначениеЗаполнено(СтрТабл.СуммаБезНал), СтрТабл.СуммаБезНал, 
									0))));
