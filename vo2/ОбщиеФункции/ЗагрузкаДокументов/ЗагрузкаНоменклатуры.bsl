...
&НаКлиенте
Функция АвтоматическоеСопоставлениеНоменклатур(МестныйКэш, ДанныеНоменклатурДляРасширенногоСопоставления) Экспорт 
	...
	Счетчик = 0; 
	Пока Счетчик < ДанныеНоменклатурДляРасширенногоСопоставления.Количество() Цикл
		
		Если НЕ ДанныеНоменклатурДляРасширенногоСопоставления[Счетчик].Номенклатура1С.Количество() Тогда
			    НесопоставленнаяНоменклатура.Вставить(Счетчик, ДанныеНоменклатурДляРасширенногоСопоставления[Счетчик]);
				Счетчик = Счетчик + 1;
				Продолжить;
		КонецЕсли; 
			
		Номенклатуры1С = ДанныеНоменклатурДляРасширенногоСопоставления[Счетчик].Номенклатура1С;			
		
		Для Каждого Номенклатура1С Из Номенклатуры1С Цикл
			
			Если НЕ ЗначениеЗаполнено(Номенклатура1С.Ключ) Тогда
				
				НесопоставленнаяНоменклатура.Вставить(ДанныеНоменклатурДляРасширенногоСопоставления[Счетчик].НоменклатураСБИС.Наименование,
													  ДанныеНоменклатурДляРасширенногоСопоставления[Счетчик]);
				Счетчик = Счетчик + 1;									  
	...
	Пока НесопоставленнаяНоменклатура.Количество() И СчетчикКоличестваВыполненныхЗапросов < КоличествоПолейСопоставления Цикл
		...
		ЗапроситьИСопоставитьНоменклатуруПоДанным1С(НесопоставленнаяНоменклатура, ДанныеДляПоиска, ДанныеНоменклатурДляРасширенногоСопоставления, ИмяСправочникаНоменклатуры);
				
		СчетчикКоличестваВыполненныхЗапросов = СчетчикКоличестваВыполненныхЗапросов + 1;
	КонецЦикла;
	
	Возврат ДанныеНоменклатурДляРасширенногоСопоставления;
КонецФункции

&НаСервере
Процедура ЗапроситьИСопоставитьНоменклатуруПоДанным1С(НесопоставленнаяНоменклатура, ДанныеДляПоиска, ДанныеНоменклатурДляРасширенногоСопоставления, ИмяСправочникаНоменклатуры)
	...
	Если НайденнаяНоменклатура.Количество() Тогда
		СопоставитьНоменклатуруИПересмотретьНесопоставленныеПозиции(НесопоставленнаяНоменклатура, ДанныеНоменклатурДляРасширенногоСопоставления, НайденнаяНоменклатура, ДанныеДляПоиска.ПараметрПоиска); 
	КонецЕсли;
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
&НаКлиенте
Функция ОбогащенныеЗначенияПоиска(НесопоставленнаяНоменклатуры)
	...
	Для Каждого РеквизитСопоставления Из ПорядокАвтоматическогоСопоставления Цикл
		...
		Для Каждого ДанныеНоменклатуры Из НесопоставленнаяНоменклатуры Цикл
		...
		Если Значения.Количество() Тогда  
			Значения = СвернутьМассив(Значения);
			ОбогащенныеЗначенияПоиска.Вставить(РеквизитСопоставления, Значения);
		КонецЕсли;
...
Функция СвернутьМассив(МассивДляСворачивания)
	...
	СвернутыйМассив = МодульОбъектаСервер().СвернутьМассивЧерезТаблицуЗначений(МассивДляСворачивания);
...
Процедура СопоставитьНоменклатуруИПересмотретьНесопоставленныеПозиции(НесопоставленнаяНоменклатура, ОбогащенныеДанныеНоменклатур, НайденнаяНоменклатура, ПараметрПоиска)
	...
	НайденнаяНоменклатура.Свернуть("ЗначениеПараметраСопоставления", "Номенклатура1ССсылка");		
...
Процедура ПривестиНоменклатуруККлассуСопоставлений(НесопоставленнаяНоменклатура, НайденнаяНоменклатура, ПараметрПоиска)
	...
	Для Каждого Номенклатура1С Из НайденнаяНоменклатура Цикл
		...
		СопоставлениеДляЕдиницы = Новый Структура("Владелец, Единица", Номенклатура1С.Ссылка, Неопределено); 
		КлючПоиска = "Единица";
		...
		Номенклатура1С.Вставить("ОписаниеНоменклатуры1С", ОписаниеНоменклатуры1С);
