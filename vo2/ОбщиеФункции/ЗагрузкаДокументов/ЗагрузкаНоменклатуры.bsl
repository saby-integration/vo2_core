...
Функция СопоставитьРеквизитыПоискаСМетаданными(ПорядокАвтоматическогоСопоставления, ИмяСправочникаНоменклатуры)
	...
	Для Каждого ПараметрПоиска Из ПорядокАвтоматическогоСопоставления Цикл
		...
		Если ПараметрыСопоставленияПоРеквизитам.Найти("Код") = Неопределено 
			И (ПараметрПоиска = "КодПокупателя" ИЛИ ПараметрПоиска = "КодПоставщика" ИЛИ ПараметрПоиска = "Идентификатор") Тогда
...
&НаКлиенте
Процедура ПересчетПоКоэффициентамЕдиницИзмерения(ПодготовленныеДанныеНоменклатур) Экспорт
	...
	Для Каждого КлассСопоставления Из ПодготовленныеДанныеНоменклатур Цикл
		...
		Для Каждого Номенклатура1С Из КлассСопоставления.Номенклатура1С Цикл
			...
			Если НЕ Коэффициент1С = КоэффициентСБИС Тогда	
...
Процедура ПересчетНДСПоДаннымСтрокиСопоставления(ПодготовленныеДанныеНоменклатур) Экспорт
	...
	Для Каждого КлассСопоставления Из ПодготовленныеДанныеНоменклатур Цикл
		...
		Иначе
			СуммаНДС 	= НоменклатураСБИС.СуммаНДС;
			СтавкаНДС 	= НоменклатураСБИС.СтавкаНДС;
			Сумма 		= НоменклатураСБИС.Сумма;
			СуммаБезНал = НоменклатураСБИС.СуммаБезнал;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(СуммаНДС) 
			...
			СтавкаНДС 			 	 = СтавкаНДС;
		ИначеЕсли СуммаБезНал = Сумма Тогда
			СтавкаНДС = ""; 
