...
Функция сбисПрименитьXSLT(Кэш, ШаблонXML, ИмяXSLT, ДопПараметры, Отказ) Экспорт
	...
	ШаблонXSLT = МодульОбъектаКлиент().XSLTПоПараметрам(ИмяXSLT);
	Если Не ЗначениеЗаполнено(ШаблонXSLT) Тогда
	...
	Исключение
		...
		Отказ = Истина;
		РезультатПреобразования = МодульОбъектаКлиент().НовыйСбисИсключение(770103, "ОбщиеФункции.сбисПрименитьXSLT",,,
		"Ошибка разбора XML. Возможно файл не валиден, либо поврежден",
		Новый Структура("ИмяXSLT, ШаблонXSLT, СовместимостьМетодов", ИмяXSLT, ШаблонXSLT, Кэш.СовместимостьМетодов));
...
Функция сбисПолучитьXMLФайлаИзСтруктуры(Кэш, Вложение, Отказ=Ложь) Экспорт
	...
	ИменаXSLT = сбисИмяXSLTДляВложения(Кэш, СтруктураФайла.Файл, Вложение);
	СтруктураФайла.Файл.Вставить("ВерсПрог", "VO2_xslt");	
	
	Для НомеИмени = 0 По ИменаXSLT.ВГраница() Цикл
		
		ИмяXSLT         = ИменаXSLT[НомеИмени];
		...
		// Если xslt нет с таким именем, и есть еще, то на сервер пока не лезем
		сбисДопПараметры.Вставить("Преобразовать", НомеИмени = ИменаXSLT.ВГраница());
		
		РезультатПреобразования = сбисПолучитьXMLФайлаИзСтруктурыПоИмениXSLT_Клиент(Кэш, СтруктураФайла, ИмяXSLT, сбисДопПараметры, Отказ);
		Если Отказ Тогда
			...
			ВызватьИсключение МодульОбъектаКлиент().СбисИсключение_Выгрузить(РезультатПреобразования);
		КонецЕсли;
...
Функция сбисПолучитьXMLФайлаИзСтруктурыПоИмениXSLT_Клиент(Кэш, СтруктураФайла, ИмяXSLT, сбисДополнительныеПараметры, Отказ) Экспорт
	Перем XSLTПреобразованиеФайлИмя, ИмяXSLTФайлИмя;
	
	XSLTПреобразование	= МодульОбъектаКлиент().XSLTПоПараметрам(ИмяXSLT);
	ЕстьXSLT			= ЗначениеЗаполнено(XSLTПреобразование);
	
	//KES МОТП ИСМП Имя файла через xslt -->
	Если	сбисДополнительныеПараметры.Свойство("ИмяXSLTФайлИмя", ИмяXSLTФайлИмя)
		 И	ЗначениеЗаполнено(ИмяXSLTФайлИмя) Тогда
		XSLTПреобразованиеФайлИмя = МодульОбъектаКлиент().XSLTПоПараметрам(ИмяXSLTФайлИмя);
	КонецЕсли;

	ЕстьXSLTФайлИмя = ЗначениеЗаполнено(XSLTПреобразованиеФайлИмя);
	...
	Если НЕ ЕстьXSLT 
		И сбисДополнительныеПараметры.Свойство("Преобразовать") 
		И НЕ сбисДополнительныеПараметры.Преобразовать Тогда
		Возврат "";
	КонецЕсли;
	...
	Если Кэш.Парам.РежимОтладки Тогда//В режиме отладки запишем данные по XML в выбранный каталог
		...
		Если сбисДополнительныеПараметры.Свойство("Название") Тогда
			сбисНазваниеПапки = МодульОбъектаКлиент().ПрименитьФорматКСтроке(сбисНазваниеПапки, сбисДополнительныеПараметры.Название, "Каталог");
		КонецЕсли;
...
Функция сбисПолучитьXMLФайлаИзСтруктурыПоИмениXSLT_Сервер(Знач СтруктураФайла, Знач ШаблонXSLT, Знач ИмяXSLT, Знач сбисДополнительныеПараметры, Отказ) Экспорт
	...
	Если	Отказ
		Или	Не ЗначениеЗаполнено(ШаблонXSLT) Тогда 
	...
	Исключение
		Отказ = Истина;
		РезультатПреобразования = МодульОбъектаСервер().НовыйСбисИсключение(770103, "ОбщиеФункции.сбисПрименитьXSLT",,,
		"Ошибка разбора XML. Возможно файл не валиден, либо поврежден");
...
Функция сбисИмяXSLTДляВложения(Кэш, СтруктураФайла, сбисДополнительныеПараметры) Экспорт
	...
	ВерсияФорматаВложения = МодульОбъектаКлиент().ВложениеСБИС_Получить(сбисДополнительныеПараметры, "ВерсияФормата");
	ВложениеСБИС_ВерсияФормата = сбисЗаменитьНедопустимыеСимволы(ВерсияФорматаВложения);
	...
	КонецЕсли;
	// Новый формат с исходной версией и итоговой
	ИмяXSLT1 = СбисФорматСтроки(Файл_Формат + "_" + ПодТип + "_" + Файл_ВерсияФормата + "_" + ВложениеСБИС_ВерсияФормата, "КлючСтруктуры");
	// Старый формат с исходной версией формата
	ИмяXSLT2 = СбисФорматСтроки(Файл_Формат + "_" + ПодТип + "_" + Файл_ВерсияФормата, "КлючСтруктуры"); 
	ИмяXSLT3 = Неопределено; 
	
	Если Файл_ВерсияФормата = "3_03" ИЛИ Файл_ВерсияФормата = "3_01" Тогда
		ИмяXSLT3 = "Шаблон__" + Файл_ВерсияФормата + "_" + Файл_ВерсияФормата;
	КонецЕсли;
		
	Результат.Добавить(ИмяXSLT1);  
	Результат.Добавить(ИмяXSLT2);
	Если НЕ (ИмяXSLT3 = Неопределено) Тогда
		Результат.Добавить(ИмяXSLT3);	
	КонецЕсли;
	
	Для Каждого Имя Из Результат Цикл
