...
Функция ПерейтиВРазделВО2(РазделВО2Перейти, ДопПараметры=Неопределено)
	...
	Если Отказ Тогда 
		...
		Попытка
			...
			ПерейтиВРазделВО2_ВосстановитьПредыдущийРаздел(Выбор, ДопПараметры);
			
		Исключение
			
			ТекстОшибки		= "Ошибка перехода в раздел.";
			ОшибкаПерехода	= МодульОбъектаКлиент.НовыйСбисИсключение(ИнформацияОбОшибке(), "ФормаГлавноеОкно.ПерейтиВРазделВО2",,ТекстОшибки);
			МодульОбъектаКлиент.СбисСообщить(ОшибкаПерехода);
...
&НаКлиенте
Процедура ПерейтиВРазделВО2_ВосстановитьПредыдущийРаздел(РазделГО, ДопПараметры=Неопределено)
