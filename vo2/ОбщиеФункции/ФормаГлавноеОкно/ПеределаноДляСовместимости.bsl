...
Функция ПерейтиВРазделВО2(СбисИмяРаздела, ДопПараметры=Неопределено)
	...
	Если ДопПараметры.Свойство("ПараметрыРаздела") Тогда
		Для каждого ПараметрРаздела Из ДопПараметры.ПараметрыРаздела Цикл
			Выбор.ПараметрыРаздела.Вставить(ПараметрРаздела.Ключ, ПараметрРаздела.Значение);
		КонецЦикла;
	КонецЕсли;
	...
	фрм = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки("АннулированиеНаПМО", Выбор.ФормыРаздела);
    Если фрм = Ложь Тогда
        МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "МассовыеОперацииАннулировать").Видимость = Ложь;
    Иначе
        фрм.АннулированиеНаПМО(Кэш);
    КонецЕсли;
