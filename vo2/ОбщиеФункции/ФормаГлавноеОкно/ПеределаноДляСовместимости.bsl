...
Процедура ОткрытьДокументОнлайнПоПакетуЗавершение(Результат, ДопПараметры) Экспорт
	...
	ИначеЕсли Не ЗначениеЗаполнено(Результат.Ссылка) Тогда
		Сообщить("Вероятно, документ был удален в СБИС");
		Возврат;
	КонецЕсли;	
...
Функция ПерейтиВРазделВО2(РазделВО2Перейти, ДопПараметры=Неопределено)
	...
	Если МодульОбъектаКлиент.ЭтоТипСбис(РазделВО2Перейти, "СтруктураРазделаГО") Тогда
	...
	Иначе
		...
		Выбор = МодульОбъектаКлиент.НовыйСтруктураРазделаГО(РазделВО2Перейти, ДопПараметры);
		
	КонецЕсли;
	
	Если МодульОбъектаКлиент.СтруктураРазделаГО_Пустой(Выбор)  Тогда
	...
	Попытка
		...
		МодульОбъектаКлиент.СтруктураРазделаГО_Подготовить(Выбор);
	...
	МодульОбъектаКлиент.СтруктураРазделаГО_СохранитьФильтр(Предыдущий);
	...
	//Дополнительный аккордеон для заказов. Пока не должен быть виден (24.5200)
	//фрм = сбисНайтиФормуФункции("ВывестиДополнительныйАккордеон", "Раздел_" + Кэш.Разделы["р" + Выбор.Раздел] + "_" + 
	//							Выбор.ТипДок, "Раздел_" + Кэш.Разделы["р" + Выбор.Раздел] + "_Шаблон", Кэш);
	//Если фрм <> Ложь Тогда
	//	фрм.ВывестиДополнительныйАккордеон(Кэш); 
	//Иначе  
	//	Если Не Кэш.ПараметрыСистемы.Клиент.УправляемоеПриложение Тогда
	//		СкрытьДополнительныйАккордеон();
	//	Иначе
	//	 	сбисЭлементФормы(ЭтаФорма, "АккордеонДополнительныйHTML").Видимость = Ложь;	
	//	КонецЕсли;
	//	
	//КонецЕсли;
...
&НаКлиенте
Процедура ПерейтиВРазделВО2_ОФ(СтруктураРазделаГО, ДопПараметры=Неопределено)
	...
	Предыдущий = СтруктураРазделаГО.ПредыдущийРаздел;
...
&НаКлиенте
Процедура ПерейтиВРазделВО2_ВосстановитьПредыдущийРаздел(СтруктураРазделаГО, ДопПараметры=Неопределено)
	...
	Если СтруктураРазделаГО.ПредыдущийРаздел = Неопределено Тогда
		
		РазделПерехода	= МодульОбъектаКлиент().НовыйСтруктураРазделаГО("АккордеонПолученные11");
		
	Иначе 
		
		РазделПерехода	= СтруктураРазделаГО.ПредыдущийРаздел;
		
	КонецЕсли;
	
	СформироватьСтруктуруАккордеона(Аккордеон, Новый Структура("Раздел", СтруктураРазделаГО.ПредыдущийРаздел));
