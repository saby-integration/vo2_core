...
Функция ПерейтиВРазделВО2(РазделВО2Перейти, ДопПараметры=Неопределено)
	...
	Попытка
		...
		СбисПрименитьФильтр(Выбор.Фильтр);
	...
	Исключение
		...
		СбисИсключение = МодульОбъектаКлиент.НовыйСбисИсключение(	ИнформацияОбОшибке(),
																	...
																	773,
																	,
																	"Произошла ошибка при переходе в раздел " + Выбор.ОписаниеРаздела.Имя + ". Переход прерван.");
	...
	Если Не фрм = Ложь Тогда
		...
		Исключение
			...
			СбисИсключение = МодульОбъектаКлиент.НовыйСбисИсключение(	ИнформацияОбОшибке(),
																		...
																		773,
																		,
																		"Произошла ошибка при подготовке раздела " + Выбор.ОписаниеРаздела.Имя + ". Переход прерван.");
			МодульОбъектаКлиент.СообщитьСбисИсключение(СбисИсключение);
			Отказ = Истина;
	...
	Если Отказ Тогда 
		...
		ПерейтиВРазделВО2_ВосстановитьПредыдущийРаздел(Выбор);
		// Не получилось перейти в новый раздел, выйти, вернуть неудачу
		Возврат Ложь;
...
&НаКлиенте
Процедура ПерейтиВРазделВО2_НачатьВосстановлениеРаздела(РазделВО2Перейти, ДопПараметры)
	...
	ОтложенноПерейти = МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("ПерейтиВРазделВО2_ВосстановитьПредыдущийРаздел", ЭтаФорма, ДопПараметры);
	МодульОбъектаКлиент().ПодключитьОтложенноеДействие(ОтложенноПерейти, РазделВО2Перейти);
		
КонецПроцедуры

&НаКлиенте
Процедура ПерейтиВРазделВО2_ВосстановитьПредыдущийРаздел(РазделГО, ДопПараметры=Неопределено) Экспорт

	Если Не ДопПараметры = Неопределено
		И	ДопПараметры.Свойство("ВосстановлениеРаздела")
		И	ДопПараметры.ВосстановлениеРаздела Тогда
		
		МодульОбъектаКлиент().СбисСообщить("Произошла ошибка при восстановлении раздела. Переход прерван");
		Возврат;
		
	КонецЕсли;
	
	ПараметрыПерехода = Новый Структура("ВосстановлениеРаздела", Истина);
	// Если не смогли обновить таблицу - откатываем назад
	Если РазделГО.ПредыдущийРаздел = Неопределено Тогда
		
		РазделПерехода	= МодульОбъектаКлиент().НовыйРазделГО("АккордеонПолученные11");
		
	Иначе 
		
		РазделПерехода	= РазделГО.ПредыдущийРаздел;
		
	КонецЕсли;
	
	СформироватьСтруктуруАккордеона(Аккордеон, Новый Структура("Раздел", РазделПерехода));
	ПерейтиВРазделВО2(РазделПерехода, ПараметрыПерехода);
