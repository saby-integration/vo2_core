...
Функция ПерейтиВРазделВО2(РазделВО2Перейти, ДопПараметры=Неопределено)
	...
	Попытка
		...
		СбисВосстановитьФильтр(Кэш, Выбор.Фильтр);
...
Процедура ПерейтиВРазделВО2_ВосстановитьПредыдущийРаздел(СтруктураРазделаГО, ДопПараметры=Неопределено)
	...
	Если СтруктураРазделаГО.ПредыдущийРаздел = Неопределено Тогда
		...
		ПерейтиВРазделВО2("АккордеонПолученные11", ПараметрыПерехода);
		
	Иначе 
		
		ПерейтиВРазделВО2(СтруктураРазделаГО.ПредыдущийРаздел, ПараметрыПерехода);
