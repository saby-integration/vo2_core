...
Функция ПерейтиВРазделВО2(СбисИмяРаздела, ДопПараметры=Неопределено)
	...
	Выбор = МодульОбъектаКлиент.НовыйСтруктураРазделаГО(СбисИмяРаздела, ДопПараметры);
		
	Если Выбор.Форма = Ложь Тогда 
		
		//нет отдельной формы функции для этого раздела
		Возврат Ложь;
		
	КонецЕсли;
		
	Если ЗначениеЗаполнено(Выбор.ПредыдущийРаздел) Тогда  
	...
	НазваниеРаздела		= Выбор.ОписаниеРаздела.Имя;
	...
	Исключение
		...
		СбисИсключение = МодульОбъектаКлиент.НовыйСбисИсключение(ИнформацияОбОшибке(), "ФормаГлавноеОкно.ОбновитьКонтент");
	...
	Попытка
		...
		Если СтруктураДляОбновленияФормы = Ложь Тогда //если не смогли обновить таблицу - откатываем назад
			Если Не ЗначениеЗаполнено(Предыдущий.ОписаниеРаздела.Имя) Тогда
	...
	Если Не Кэш.ПараметрыСистемы.Клиент.УправляемоеПриложение Тогда
		...
		Если	ЗначениеЗаполнено(Предыдущий) 
			...
			ФормаДопФильтраПред = МодульОбъектаКлиент.НайтиФункциюСеансаОбработки("сбисСписокДопПараметровФильтра", Предыдущий.ОписаниеРаздела.ФормыФильтра);
		...
		ФормаДопФильтра = МодульОбъектаКлиент.НайтиФункциюСеансаОбработки("сбисСписокДопПараметровФильтра", Выбор.ОписаниеРаздела.ФормыФильтра);
