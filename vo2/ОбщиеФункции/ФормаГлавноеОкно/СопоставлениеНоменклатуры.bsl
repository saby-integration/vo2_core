...
Процедура УдалитьСоответствиеНоменклатуры(Команда)
	...
	фрмСопоставлениеНоменклатуры = МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("МодульСопоставлениеНоменклатуры");
...
Процедура сбисЗаполнитьСписокСправочникаНоменклатурыКонтрагентов(СписокВходящий) Экспорт
	...
	Для Каждого Соответствие Из Список Цикл
		...
		СтрокаТаблицы.Характеристика			= Соответствие.НоменклатураСБИС.Характеристика.Наименование;
...
Процедура КонтрагентНоменклатурыПриИзменении(Элемент)
	...
	ОбновитьКонтент(, Новый Структура("ОчиститьФильтры", Истина));
