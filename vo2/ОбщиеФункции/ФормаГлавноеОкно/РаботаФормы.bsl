...
Процедура ФильтрМаскаПриИзменении(Элемент)
	...
	Если МодульФормированияФильтра <> Ложь Тогда
		...
		ПараметрыФормирования = Новый Структура("ФильтрМаска", ФильтрМаска);
		МодульФормированияФильтра.СформироватьСтруктуруФильтра(Кэш, ПараметрыФормирования);
...
Функция сбисПослеУстановкиФильтра(Результат, Параметры) Экспорт
	Если Результат<>Неопределено Тогда
		...
		Если ТипЗнч(Параметры) = Тип("Структура")
			И Параметры.Свойство("ФормаФильтраРазделаПольз") Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ПараметрыФильтра);
...
Процедура ПоказатьСогласованиеЦен(ЛокальныйКэш = Неопределено, сбисПараметрыОбработчика = Неопределено) Экспорт
	...
	Документ1С.Вставить("Дата", ДатаЦен); 
	Документ1С.Вставить("Номер", ""); 
	Документ1С.Вставить("Контрагент", );  
	Документ1С.Вставить("Организация", ВладелецКаталога);
	Документ1С.Вставить("ТипЦен", ТипЦен); 
	...
	МодулиРаздела = Кэш.Текущий.ФормыРаздела;
	МодулиРаздела.Вставить(0, "Раздел_" + ТекРаздел + "_" + Кэш.Текущий.ТипДок);
	
	Модуль = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки("ПоказатьДокумент", МодулиРаздела);
	
	Попытка
		Модуль.ПоказатьДокумент(Кэш, ИнформацияОДокументе);
	Исключение
		МодульОбъектаКлиент().СообщитьСбисИсключение(ИнформацияОбОшибке(), "ФормаГлавноеОкно.ПоказатьСогласованиеЦен");
	КонецПопытки;
