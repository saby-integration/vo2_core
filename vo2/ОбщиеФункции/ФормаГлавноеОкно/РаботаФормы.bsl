...
Функция ОтправитьПрайс(ЛокальныйКэш = Неопределено, сбисПараметрыОбработчика = Неопределено) Экспорт
	...
	ПараметрыФормы = Новый Структура("ФормаЗаполнения", КонструкторФорм);
	...
	ФормаЗаполненияДанных.ПоказатьПоверх(ПараметрыФормы);
...
Процедура ПоказатьСогласованиеЦен_Завершить(Результат, ДопПараметры) Экспорт
	...
	Если НЕ Результат.Отправлено Тогда
		...
		Кэш.Вставить("РезультатОтправки", МодульОбъектаКлиент().РезультатОтправки_Скопировать(Результат, Истина));
		
		фрм = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки("ПоказатьРезультатОтправки", "ФормаРезультатОтправки");
