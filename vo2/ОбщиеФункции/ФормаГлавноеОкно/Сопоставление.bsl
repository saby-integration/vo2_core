...
Функция ТаблДокПолучитьВыбранныеСтроки() Экспорт
	...
	Если Кэш.ТаблДок = Неопределено
		ИЛИ Кэш.ТаблДок = "" Тогда
		Возврат СписокСтрок;
	КонецЕсли;
		...
		Для Каждого Строка из СписокСтрок Цикл
			...
			Если Кэш.Текущий.Раздел = "3" или Кэш.Текущий.Раздел = "4" Тогда
				Если МассивСтатусРегламент.Количество()>0 Тогда  // для разделов Продажа/Покупка мы не знаем для какого регламента удалить сопоставление
					СбисСообщение = "Удаление сопоставления недоступно. Перейдите в доп.сведения документа 1С и удалите значения полей в открывшемся окне";
					СбисИсключение = МодульОбъектаКлиент().НовыйСбисИсключение(, "ГлавноеОкно.УдалитьСопоставление", 719102, СбисСообщение);
