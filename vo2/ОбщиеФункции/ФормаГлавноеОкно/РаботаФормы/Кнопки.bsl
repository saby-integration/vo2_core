...
Процедура сбисИнформацияДляТП(Команда)
	...
	МодульОбъектаКлиент().НачатьСборТехИнформации();
...
&НаКлиенте
Процедура ЗагрузитьТестовыйЗаказ(Команда)
	
	
	 ПараметрыПоказать	= Новый Структура("Кэш", Кэш); 
	 фрм = СбисПолучитьФорму("ФормаЗагрузитьТестовыйЗаказ", , , ЭтаФорма);
	 
	 ОбработчикПослеЗагрузкиТестовыхЗаказов = МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("ГлавноеОкно_ПослеЗагрузкиТестовыхЗаказов", ЭтаФорма);
	 фрм.ОписаниеОповещенияОЗакрытии = ОбработчикПослеЗагрузкиТестовыхЗаказов;
	 
	 фрм.ПоказатьПоверх(ПараметрыПоказать);
	
КонецПроцедуры 

&НаКлиенте
Процедура ГлавноеОкно_ПослеЗагрузкиТестовыхЗаказов(РезультатИзменения, ДопПараметры) Экспорт
	...
	Если РезультатИзменения.СписокДокументовПолучен Тогда
		...
		Исключение
			...
			МодульОбъектаКлиент().СообщитьСбисИсключение(ИнформацияОбОшибке(), "ФормаГлавноеОкно.ЗагрузитьТестовыйЗаказ");
