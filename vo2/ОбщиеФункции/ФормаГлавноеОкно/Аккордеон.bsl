...
Процедура  СформироватьСтруктуруАккордеона(Аккордеон, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли СпособОбмена = 1 Тогда
		ВыбранныйРаздел =  "АккордеонПродажаДок033";
	ИначеЕсли ДопПараметры.Свойство("ТекущийРазделУдален")
		И НЕ ДопПараметры.ТекущийРазделУдален Тогда
		ВыбранныйРаздел = Кэш.Текущий.Имя;
	Иначе
