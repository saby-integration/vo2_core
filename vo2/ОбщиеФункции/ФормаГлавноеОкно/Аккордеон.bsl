...
&НаКлиенте
Процедура  СформироватьСтруктуруАккордеона(АккордеонВходящий, ДопПараметры = Неопределено) Экспорт
	...
	Если ДопПараметры.Свойство("Раздел") Тогда
		
		//позиционирование на разделе кодом
		ВыбранныйРаздел = ДопПараметры.Раздел.Имя;
		
	КонецЕсли;
	
	АккордеонЭДО = Новый Структура("Пункты, Ини", Новый Массив, "Blockly_AccordionVO2");
	СтруктураАккордеона = "{""accordion"": [";
	
	Если ЗначениеЗаполнено(АккордеонВходящий) Тогда
		Для Каждого КлючИЗначениеАккордеон Из АккордеонВходящий Цикл
	...
	СтатусВерсии = 0;
	Если НЕ Кэш.ПараметрыСистемы.Обработка.Свойство("СтатусВерсии", СтатусВерсии) Тогда
		СтатусВерсии = 5;
	КонецЕсли;
	
	ВерсияПользователя = "";
	Если НЕ Кэш.ПараметрыСистемы.Обработка.Свойство("Версия", ВерсияПользователя) Тогда
		ВерсияПользователя = "";
	КонецЕсли;
	...
	СтруктураАккордеона = СтруктураАккордеона + ", ""productVersionStatus"":{";      //версия продукта
	СтруктураАккордеона = СтруктураАккордеона + """currentVersionNumber"":""v. " + ВерсияПользователя + """";
	СтруктураАккордеона = СтруктураАккордеона + ", ""clientVersionStatus "":" + СтатусВерсии + "}}";
...
#Область include_local_ЗаполнитьАккордеонВО3
#КонецОбласти
	...
	Если ИмяДействия = "ADDON_NAVIGATE" Тогда  //обработать выбор раздела аккордеона
		Попытка 
			Если ПустаяСтрока(ИДКнопки) Тогда
				//Возврат ;
			КонецЕсли;   
			ФлагПереходПоРазделуОбновлениеРаздела = Истина;
			ПерейтиВРаздел(ИДКнопки); 
			ФлагПереходПоРазделуОбновлениеРаздела = Ложь;
		Исключение
...
#Область include_core2_vo2_ОбщиеФункции_ФормаГлавноеОкно_Аккордеон_ДополнительныйАккордеон
#КонецОбласти
