...
Процедура ОтправитьВыбранныеДокументы(Ответ, СбисДополнительныеПараметры=Неопределено) Экспорт
	...
	Для Каждого СтрокаВыбрана Из СписокОтмеченныхДокументов Цикл
		...
		Если	СтрокаВыбрана.Значение.Статус = 0
			Или	СтрокаВыбрана.Значение.Статус = 3 Тогда
			...
			Иначе
				...
				// Костылищще с получением ИД документа СБИС. Перенести в построение реестра документов.
			...
			Если ЗначениеЗаполнено(ИдПакета) Тогда
				...
				СтрокаДобавить.Вставить("ИдентификаторСБИС",	ИдПакета);
				СтрокаДобавить.Вставить("Операция",				"Черновик");
	...
	ПараметрыОтправки = Новый Структура("Реестр, ИмяРеестра", Кэш.Текущий, Кэш.Текущий.ТипДок);
	МодульОбъектаКлиент().ЗапуститьМассовуюОтправкуДокументов(МассивПакетов, ПараметрыОтправки);
...
Процедура ОбновитьСтатусы(Команда)
	...
	фрм = сбисНайтиФормуФункции("сбисПараметрыСписокИзменений", "Раздел_" + ТекРаздел + "_" + ТипДокумента,"Раздел_" + ТекРаздел + "_Шаблон", Кэш);
	Если фрм <> Ложь Тогда
