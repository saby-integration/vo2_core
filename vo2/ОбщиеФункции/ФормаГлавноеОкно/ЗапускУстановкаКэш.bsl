...
Процедура ПроверитьИУстановитьКэш(СбисДополнительныеПараметры = Неопределено)
	...
	УстановитьОсновныеМодули(СбисДополнительныеПараметры);
...
&НаКлиенте
Процедура УстановитьОсновныеМодули(СбисДополнительныеПараметры = Неопределено) Экспорт
	...
	Исключение
		...
		ВызватьИсключение(ТекущийМодульОбъекта.СбисИсключение_Выгрузить(ОшибкаУстановкиКэш, "ПолныйТекст"));
...
Процедура УстановитьСтартовыеПараметрыПользователя(СбисДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РежимОтладкиПереключить = Истина Тогда
		ДопПараметры = Новый Структура("Параметры", Новый Структура("ИзменитьПараметрВО3", Истина));
		ТекущийМодульОбъекта.ИзменитьПараметрSABY("РежимОтладки",		Истина,, ДопПараметры);
...
Функция ОпределитьИнтеграциюРабочиеФормы(Кэш, ПараметрыИнтеграции, СбисДополнительныеПараметры=Неопределено)	Экспорт
	...
	Если СбисДополнительныеПараметры.ВключатьОбмен Тогда
		...
		ПользовательскаяТочкаВхода = МодульОбъектаКлиент().ПользовательскаяТочкаВхода("Включить", ИмяИнтеграции);
		Если Не ПользовательскаяТочкаВхода = Неопределено Тогда
			
			ДопПараметрыВызова	= Новый ФиксированнаяСтруктура("ЕстьРезультат", Истина);
			АргументыВызова		= Новый ФиксированнаяСтруктура("Кэш, ПараметрыИнтеграции", Кэш, ПараметрыИнтеграции);
			Попытка
				
				ОбменВключен = МодульОбъектаКлиент().ВызватьПользовательскуюТочкуВхода(ПользовательскаяТочкаВхода, АргументыВызова, ДопПараметрыВызова);
				
			Исключение
				
				МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке(), НазваниеКратко() + ".ОпределитьИнтеграциюРабочиеФормы");
				ОбменВключен = Ложь;
...
Процедура ВосстановитьПараметрыПользователя(СбисДополнительныеПараметры = Неопределено) Экспорт
	Перем ПараметрыПользователяВходящие, ВосстановленныйПараметрПользователя, ЗначенияПараметровЧтоНеобходимоПереопределить;
	...
	// Описание параметров, которые при определенных условиях должны быть переопределены
	НеприемлемыеПараметры = ПереопределяемыеПараметрыИИхНеприемлемыеЗначения();
	...
	Для Каждого КлючИЗначениеДефолт Из ЗначПоУмолчанию Цикл
		...
		Если ВосстановленныйПараметрПользователя = Неопределено 
			ИЛИ (НеприемлемыеПараметры.Свойство(КлючИЗначениеДефолт.Ключ, ЗначенияПараметровЧтоНеобходимоПереопределить) 
			И НЕ ЗначенияПараметровЧтоНеобходимоПереопределить.Найти(ВосстановленныйПараметрПользователя) = Неопределено) Тогда
...
&НаКлиенте
Функция ПереопределяемыеПараметрыИИхНеприемлемыеЗначения()
	ПереопределяемыеПараметры = Новый Структура;
	
	МассивЗначенийЗаписейНаСтранице1С = Новый Массив;
	МассивЗначенийЗаписейНаСтранице1С.Добавить("все");
	МассивЗначенийЗаписейНаСтранице1С.Добавить("0");

	ПереопределяемыеПараметры.Вставить("ЗаписейНаСтранице1С", МассивЗначенийЗаписейНаСтранице1С);
	
	Возврат ПереопределяемыеПараметры;
КонецФункции
