...
Процедура ВосстановитьПараметрыПользователя(СбисДополнительныеПараметры = Неопределено) Экспорт
	...
	ЗначПоУмолчанию.Вставить("АдресСервера", МодульТекущий.СписокДоступныхСерверовСБИС()[0].Значение);
	...
	ЗначПоУмолчанию.Вставить("РеквизитСопоставленияНоменклатуры", МодульТекущий.РеквизитСопоставленияНоменклатурыПоУмолчанию());
	...
	ЗначПоУмолчанию.Вставить("UserAgent",					Кэш.ПараметрыСистемы.Обработка.UserAgent);
