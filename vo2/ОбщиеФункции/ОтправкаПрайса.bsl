...
Функция сбисСформироватьДанныеПоНоменклатуре(Номенклатура,Знач ИниНоменклатура, ДанныеПоНоменклатуре, Предложения, СтруктураПараметров)
	...
	Контекст.Ини			= ГрязныйИни;
	Контекст.Переменные		= Новый Структура;
	Контекст.ДанныеИсточник	= Новый Структура("ОсновнойОбъект", Номенклатура);
...
Функция ПодготовитьСтруктуруПрайса(Кэш=Неопределено, ПараметрыФомирования=Неопределено, Отказ=Ложь) Экспорт
	...
	Если ТипЗнч(СбисПрайс) = Тип("Строка") Тогда
		...
		ОписаниеФайлаПрайса = сбисЗаписатьФайл_Клиент(Кэш, ПараметрыФайлаПрайса, Отказ);
		Если Отказ Тогда
			Возврат ОписаниеФайлаПрайса;
		КонецЕсли;
	Иначе
		СтруктураВложения = Новый Структура("СтруктураДокумента, ОписаниеФайла, ПодТип, ПодВерсияФормата, Параметры", сбисПрайс, ПараметрыФайлаПрайса);
		СтруктураВложения.Параметры = Новый Структура("ИспользоватьГенератор, ИспользоватьСопоставлениеНоменклатуры, НаборМодулейДляФормата", Ложь, Ложь, "");
		...
		Если ПараметрыФомирования.ПараметрыПрайса.Свойство("Тип") Тогда
			...
			Если ПараметрыФомирования.ПараметрыПрайса.Тип = "СогласованиеЦен" Тогда
				...
				МодульОбъектаКлиент().ВложениеСБИС_Заполнить(СтруктураВложения, Истина, "ИспользоватьГенератор");
