	...
	Для Каждого Элемент из Раздел Цикл
		...
		Если Элемент.Значение.Свойство("Значение", ЗначениеЭлемента) Тогда
			...
			Если СписокДопустимыхТипов.НайтиПоЗначению(ТипЗнч(ЗначениеПараметра)) <> Неопределено Тогда
				...
				Если   Не	(	ТипЗнч(ЗначениеПараметра) = Тип("Структура")
							И	Результат[Элемент.Ключ] = ЗначениеПараметра) Тогда	
...
Функция РассчитатьЗначениеНаСервере(Параметр,Контекст) Экспорт
	...
	Если ПервыйСимвол="[" Тогда  // ссылка на объект (другую переменную)
		...
		Попытка
			...
			Если ИмяПеременнойВПараметре Тогда
				...
				Объект1С = РассчитатьЗначениеНаСервере_ПривестиФормат(Объект1С, ПараметрИни, Контекст);
	...
	ИначеЕсли ПервыйСимвол="'" Тогда	 // строка
		...
		КонецЕсли;
	ИначеЕсли РассчитатьЗначениеНаСервере_ЭтоФункция(ЗнПер, Контекст) Тогда 
	...
	ИначеЕсли ПервыйСимвол="^" Тогда  // формула, которую необходимо вычислить
		...
		ЗнПер	= РассчитатьЗначениеНаСервере_ПривестиФормат(ЗнПер, ПараметрИни, Контекст);
	...
	ИначеЕсли ПозТочки>0 Тогда
		...
		Если ПервыйСимвол = "Справочник" Тогда	// ссылка на справочник
			...
			КонецЕсли;
		ИначеЕсли ПервыйСимвол = "Документ" Тогда // ссылка на документ. Если без отбора, то ссылка на текущий документ
		...
		ИначеЕсли ПервыйСимвол = "РегистрыСведений" Тогда	// ссылка на регистр сведений
			...
			Если ПараметрИни.Свойство("СрезПоследних") Тогда
				...
				ДатаСреза = ТипДата.ПривестиЗначение(ДатаСреза);
				Попытка
		...
		КонецЕсли;
		возврат Неопределено;
	ИначеЕсли ЗнПер = "Структура" И ТипЗнч(Контекст.Документ) = Тип("Структура") Тогда
		Возврат Контекст.Документ;
...
&НаСервереБезКонтекста
Функция РассчитатьЗначениеНаСервере_ПривестиФормат(РасчитанноеЗначение, УзелИни, Контекст)
	
	Если Не УзелИни.Свойство("Формат") Тогда
		
		Возврат РасчитанноеЗначение;
		
	КонецЕсли;
	
	Если РассчитатьЗначениеНаСервере_ЭтоФункция(РасчитанноеЗначение, Контекст) Тогда
		
		//УзелИни.Вставить("Значение", РасчитанноеЗначение);
		ПриведенныйРезультат = УзелИни;
...
&НаСервереБезКонтекста
Функция РассчитатьЗначениеНаСервере_ЭтоФункция(Параметр, Контекст)
...
Процедура РасчитатьОтборНаСервере_ДобавитьОтбор(Контекст, Запрос, Элемент)
	...
	ИмяПараметраОтбор	= Элемент.Ключ;
	ПараметрОтбор		= СтрЗаменить(Элемент.Ключ, "_", ".");
	
	ШаблонТекстОтбор = "ОбъектОтбора.{0} = &{1}
						|	И	";
	ШаблонТекстОтбор = СтрЗаменить(СтрЗаменить(ШаблонТекстОтбор, "{0}", ПараметрОтбор), "{1}", ИмяПараметраОтбор);
	
	Запрос.Текст = Запрос.Текст + ШаблонТекстОтбор;
	
	Запрос.Параметры.Вставить(ИмяПараметраОтбор, РассчитатьЗначениеНаСервере(Элемент.Значение, Контекст));
