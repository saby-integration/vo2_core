...
Функция ПодготовитьСтруктуруДокумента1С(СтрокаСпискаДокументов, Кэш) Экспорт
	...
	Для Каждого Элемент из СтрокаСпискаДокументов.СоставПакета Цикл		
		...
		Если Кэш.ини.Свойство(ИниДокумента) Тогда
			...
			ИдентификаторДокумента = МодульОбъектаКлиент().СбисИдентификаторДокумента1С(Документ);
	...
	МодульРаздела = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки("ТипФормируемогоДокумента", Кэш.Текущий.ФормыРаздела);  	
	...
	Иначе
		ТипФормируемогоДокумента = МодульОбъектаКлиент().ТипФормируемогоДокументаПоУмолчанию(ПараметрыРасчетаТипа);  
	КонецЕсли;
...
Функция СгенерироватьИсходящийПакет(ПакетВходящий, ПараметрыГенерацииПакета)
	...
	ОрганизацияСторона	= МодульОбъектаКлиент().ВложениеСБИС_Получить(ОснованиеДляПакета, "СторонаПоРоли", "Отправитель");
	КонтрагентСторона	= МодульОбъектаКлиент().ВложениеСБИС_Получить(ОснованиеДляПакета, "СторонаПоРоли", "Получатель");
	Если	ОрганизацияСторона = Неопределено Тогда
		
		ОшибкаГенерацияПакета = МодульОбъектаКлиент().НовыйСбисИсключение(724, "МодульОбъектаКлиент.СгенерироватьИсходящийПакет");
	...
	Если	КонтрагентСторона = Неопределено Тогда
		...
		ОшибкаГенерацияПакета = МодульОбъектаКлиент().НовыйСбисИсключение(725, "МодульОбъектаКлиент.СгенерироватьИсходящийПакет");
		Результат.Ошибки.Вставить(ОснованиеПакета, ОшибкаГенерацияПакета);
		Возврат Результат;
