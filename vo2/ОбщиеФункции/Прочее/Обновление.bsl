...
Процедура ОбновитьОбработкуНаНовуюВерсию(Ответ, ПараметрыОбновления) Экспорт
	...
	КонецЕсли;
	Если сбисРежимИспользованияОбработки = "Справочник" Тогда
		//Обновим справочник
		Если Кэш.ПараметрыСистемы.Клиент.УправляемоеПриложение Тогда
		...
		Если ЗначениеЗаполнено(ОбработкаСБИССсылка) Тогда
			РезультатОбновления = ЗаписатьОбработкуВСправочник(ПараметрыОбновления.ПолнаяВерсия, ОбработкаСБИССсылка, Новый ДвоичныеДанные(ИмяВременногоФайлаНовойВерсии), Отказ);
			Если Отказ Тогда
			...
			КонецЕсли;
		КонецЕсли;	
	ИначеЕсли сбисРежимИспользованияОбработки = "Файл" Тогда
		//Обновим файл
	...
	Иначе
		...
		Сообщить("Обновление обработки в режиме встроенной в конфигурацию не поддерживается!");
		Отказ = Истина;
		Возврат;
...
Функция ЗаписатьОбработкуВСправочник(Версия, ОбработкаСБИССсылка, Знач ДД, Отказ)
	...
	Если		Не сбисОбработкаОбъект.Метаданные().Реквизиты.Найти("Версия") = Неопределено Тогда
		
		сбисОбработкаОбъект.Версия = Версия;
		
	ИначеЕсли	Не сбисОбработкаОбъект.Метаданные().Реквизиты.Найти("Комментарий") = Неопределено Тогда
		
		сбисОбработкаОбъект.Комментарий = Версия;
		
	Иначе
		
		//Версию некуда прописать
	...
	Исключение
		...
		СбисИсключение = МодульОбъектаСервер().НовыйСбисИсключение(ИнфОбОшибке, "РаботаСДокументами1С.ЗаписатьОбработкуВСправочник", 709101, "Не удалось записать обработку");
		Возврат МодульОбъектаСервер().СбисИсключение_Выгрузить(СбисИсключение);
...
Функция СбисПроверитьНаличиеОбновленияОбработки(Кэш, ПараметрыОбновления, Отказ)
	Если Найти(Кэш.Парам.АдресСервера, "-online") И Не Кэш.Парам.РежимОтладки Тогда    //Не обновляем с тестовых стендов не в режиме отладки
		Кэш.ГлавноеОкно.ОбновитьАккордеон(Новый Структура("ОбновитьИнформациюОВерсии", Истина), Кэш);
		Возврат Неопределено;
	КонецЕсли;
	...
	ИнформацияОТекущейВерсии.Вставить("product",		Кэш.ПараметрыСистемы.Обработка.ИмяПродукта);
	...
	сбисПараметрыОбновления.Вставить("ПолнаяВерсия",	сбисВерсияНаСервере);    
	сбисПараметрыОбновления.Вставить("URL",				URL);
	
	ДопПараметрыСообщенияОбОбновлении = Новый Структура("СтатусВерсииПользователя, НоваяВерсия", СтатусВерсииПользователя, НоваяВерсияБезБилда);
	Кэш.ГлавноеОкно.сбисУстановитьОформлениеГиперссылокОбновления("Предложить обновление", ДопПараметрыСообщенияОбОбновлении);
...
Функция сбисПолучитьНомерВерсииОбработкиПоПараметрам(Кэш, Отказ, сбисПараметрыОбновления = Неопределено) Экспорт
	Если сбисПараметрыОбновления = Неопределено Тогда
		сбисПараметрыОбновления = сбисПараметрыСервисаОбновления(Кэш);
		сбисПараметрыОбновления.Вставить("ИмяФайла", СбисПолучитьИмяВременногоФайлаКлиент("tmp"));
