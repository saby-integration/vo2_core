...
&НаКлиенте
Функция СбисСформироватьТитулы(Кэш, СоставПакета, Действие, ДопПараметры, Отказ) Экспорт
	...
	ПараметрыДляТитулов = Новый Структура("ЕстьРасхождения, Шифрование", ЕстьРасхождения);
	Если Не ДопПараметры.Свойство("Шифрование", ПараметрыДляТитулов.Шифрование) Тогда
		
		ПараметрыДляТитулов.Шифрование = Ложь;
		
	КонецЕсли;
	
	Если ДопПараметры.Свойство("РезультатПодготовки") Тогда
		
		ПараметрыДляТитулов.Вставить("РезультатПодготовки", ДопПараметры.РезультатПодготовки);
	...
	Для Каждого Вложение Из СоставПакета.Вложение Цикл
		...
		Если Зашифрован Или ЕстьРасхождения Тогда
			...
			ПараметрыДляТитулов.Шифрование = Зашифрован;
		Иначе
		...
		Если фрм = Ложь Тогда
			Если Зашифрован Тогда
				...
				Если фрм = Ложь Тогда
					Продолжить;
				КонецЕсли;				
				сбисТекст	= Вычислить("фрм." + ИмяФункции + "(Кэш, СоставПакета, Вложение, Действие, ПараметрыДляТитулов)");
		...
		КонецЕсли;
		Попытка
			сбисРезультат = Вычислить("фрм."+ИмяФункции+"(Кэш, СоставПакета, Вложение, Действие, ПараметрыДляТитулов)");
		Исключение
...
Функция СбисСформироватьТитулПолучателяАктаНакладной_5_01(Кэш, СоставПакета, Вложение, НовоеВложение, Действие, ИмяXSLT, ПрефиксФайла, ДопПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДопПараметры) И ДопПараметры.Свойство("Шифрование") И ДопПараметры.Шифрование Тогда
		...
		КонецЕсли;
		XMLФайла = МодульОбъектаКлиент().ПрочитатьЗначениеПоBase64Клиент(ДвоичныеДанныеBase64);
	Иначе
...
#Область include_core2_vo2_ОбщиеФункции_Прочее_Титулы_5_03
#КонецОбласти
