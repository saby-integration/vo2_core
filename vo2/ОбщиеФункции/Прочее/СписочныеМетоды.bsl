...
//Обрабатывает список документов по событиям с online.saby.ru для реестра События
&НаКлиенте
Функция ОбработатьСписокСобытий(Кэш, СписокСобытий) Экспорт
	СтруктураДляОбновленияФормы = Новый Структура;
	ГлавноеОкно = Кэш.ГлавноеОкно;
	Если ТипЗнч(СписокСобытий.Навигация.ЕстьЕще) = Тип("Булево") Тогда
		ГлавноеОкно.ФильтрЕстьЕще = СписокСобытий.Навигация.ЕстьЕще;
	Иначе
...
Функция ОбработатьСписокДокументов(Кэш, СписокДокументов) Экспорт
	...
	КонецЕсли;
	Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("ПриличныйВидВО")
		И ГлавноеОкно.ФильтрЕстьЕще Тогда
		СтрокаЗагрузитьЕще = Новый Структура("Проведен, Статус, ЗагрузитьЕще", -1, -1);
		СтрокаЗагрузитьЕще.Вставить("Номер", "Загрузить еще");
		МассивДокументов.Добавить(СтрокаЗагрузитьЕще);
	КонецЕсли;
	ГлавноеОкно.сбисСпрятатьСостояние(ГлавноеОкно);
...
//Обрабатывает список документов определенного типа с online.saby.ru для реестра События	
&НаКлиенте
...
Функция ОбработатьСобытиеВРеестр(Кэш, оДокумент, ДопПараметры) Экспорт
	Перем СбисРасширение, СвязанныеДокументыSaby;
	...
	Если оДокумент.Свойство("LinkedOutflows", СвязанныеДокументыSaby)
		...
		МодульОбъектаКлиент().СоставПакета_Обновить(оДокумент, Новый Структура("ДопПоля", ДопПоля));  
	...
	НоваяСтр.Вставить("Статус",		МодульОбъектаКлиент().СоставПакета_Получить(оДокумент, "СтатусДокумента")); 
	НоваяСтр.Вставить("Дата",		МодульОбъектаКлиент().СоставПакета_Получить(оДокумент, "Дата"));
	НоваяСтр.Вставить("Номер",		МодульОбъектаКлиент().СоставПакета_Получить(оДокумент, "Номер"));
	...
	Если	оДокумент.Свойство("Расширение",	СбисРасширение) Тогда
		...
		НоваяСтр.Вставить("СтатусВГосСистеме", МодульОбъектаКлиент().СоставПакета_Получить(оДокумент, "СтатусГосСистемы"));
	...
	НоваяСтр.Вставить("ТекущийЭтап", МодульОбъектаКлиент().СоставПакета_Получить(оДокумент, "ТекущийЭтап"));
	
	НоваяСтр.Вставить("Комментарий", МодульОбъектаКлиент().СоставПакета_Получить(оДокумент, "Комментарий"));  
...
Функция ПолучитьФильтрСобытий(Кэш, ДопПараметры) Экспорт
	...
	ФильтрРаздела = СкопироватьОбъектСПараметрамиКлиент(МодульОбъектаКлиент().РазделГО_Получить(Кэш.Текущий, "Фильтр"));
	...
	Возврат МодульОбъектаКлиент().СформироватьФильтрДляРеестраОнлайна(ФильтрРаздела, Новый Структура("ДопФильтры", ДопПараметры));
...
Функция СбисПолучитьСписокЗадач(Кэш) Экспорт
	...
	Для сч = 0 По Размер - 1 Цикл
		...
		СуммаДокумента = 0;
		Если оДокумент.Свойство("Сумма") И ЗначениеЗаполнено(оДокумент.Сумма) Тогда
			СуммаДокумента = Число(оДокумент.Сумма);
		КонецЕсли;
		НоваяСтр.Вставить("Сумма",			СуммаДокумента);
