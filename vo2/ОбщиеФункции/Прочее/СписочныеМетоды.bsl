...
Функция ОбработатьСписокДокументов(Кэш, СписокДокументов) Экспорт
	...
	ПредставлениеСервера	= Кэш.СБИС.ПараметрыИнтеграции.ПредставлениеСервера;
	...
	Для сч = 0 По Размер - 1 Цикл
		ГлавноеОкно.сбисПоказатьСостояние("Получение данных с " + ПредставлениеСервера, ГлавноеОкно, Мин(100, Окр((сч + 1) * 100 / Размер)));
...
Функция ОбработатьСобытиеВРеестр(Кэш, оДокумент, ДопПараметры) Экспорт
	...
	НоваяСтр = Новый Структура("Проведен, Контрагент, Лицо2, Срок, АдресДоставки, " + ДопПараметры.КлючОрганизации, -1);   // alo Меркурий 
	...
	Если	оДокумент.Свойство("Участники")
		...
		НоваяСтр.АдресДоставки = МодульОбъектаКлиент().СторонаSABY_Получить(оДокумент.Участники.Лицо2, "Адрес");
