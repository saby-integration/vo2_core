...
Функция ПолучитьФильтрСобытий(Кэш, ДопПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ГлавноеОкно.ФильтрОрганизация) Тогда
		...
		Если СтрДлина(ИННОрг) = 12 Тогда
			...
			Если Кэш.ини.Конфигурация.Свойство("мСторона") и Кэш.ини.Конфигурация.мСторона.Свойство("НашаОрганизация") и Кэш.ини.Конфигурация.мСторона.НашаОрганизация.Свойство("Сторона_КодФилиала") Тогда 
				Контекст = Новый Структура("Документ, Ини, ТекущийРазделИни",ГлавноеОкно.ФильтрОрганизация, Кэш.ини.Конфигурация, Кэш.ини.Конфигурация.мСторона.НашаОрганизация);
				СвФЛ.Вставить( "КодФилиала", СокрЛП(РассчитатьЗначениеНаСервере("Сторона_КодФилиала", Контекст))); 
			КонецЕсли;
			org.Вставить( "СвФЛ", СвФЛ );	
		Иначе
			СвЮЛ = Новый Структура;
			...
			Исключение
				...
				Если Кэш.ини.Конфигурация.Свойство("мСторона") и Кэш.ини.Конфигурация.мСторона.Свойство("НашаОрганизация") Тогда 
					Контекст = Новый Структура("Документ, Ини, ТекущийРазделИни",ГлавноеОкно.ФильтрОрганизация, Кэш.ини.Конфигурация, Кэш.ини.Конфигурация.мСторона.НашаОрганизация);
					СвЮЛ.Вставить( "КПП", СокрЛП(РассчитатьЗначениеНаСервере("Сторона_КПП", Контекст))); 
				КонецЕсли;
			КонецПопытки;
			Если Кэш.ини.Конфигурация.Свойство("мСторона") и Кэш.ини.Конфигурация.мСторона.Свойство("НашаОрганизация") и Кэш.ини.Конфигурация.мСторона.НашаОрганизация.Свойство("Сторона_КодФилиала") Тогда 
				Контекст = Новый Структура("Документ, Ини, ТекущийРазделИни",ГлавноеОкно.ФильтрОрганизация, Кэш.ини.Конфигурация, Кэш.ини.Конфигурация.мСторона.НашаОрганизация);
				СвЮЛ.Вставить( "КодФилиала", СокрЛП(РассчитатьЗначениеНаСервере("Сторона_КодФилиала", Контекст))); 
			КонецЕсли;
			org.Вставить( "СвЮЛ", СвЮЛ );
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ГлавноеОкно.ФильтрКонтрагент) Тогда
		...
		Если ЗначениеЗаполнено(ИННКонтр) Тогда // если выбрана папка контрагентов, то ИНН получается NaN
			Если СтрДлина(ИННКонтр) = 12 Тогда
				...
				Если Кэш.ини.Конфигурация.Свойство("мСторона") и Кэш.ини.Конфигурация.мСторона.Свойство("Контрагент") и Кэш.ини.Конфигурация.мСторона.Контрагент.Свойство("Сторона_КодФилиала") Тогда
					Контекст = Новый Структура("Документ, Ини, ТекущийРазделИни",ГлавноеОкно.ФильтрКонтрагент, Кэш.ини.Конфигурация, Кэш.ини.Конфигурация.мСторона.Контрагент);
					СвФЛ.Вставить( "КодФилиала", СокрЛП(РассчитатьЗначениеНаСервере("Сторона_КодФилиала", Контекст)));     
				КонецЕсли;
				kontr.Вставить( "СвФЛ", СвФЛ );	
			Иначе
				...
				Исключение
					// если КПП живет в другом реквизите или вообще не в реквизите, а в регистре сведений 
					Если Кэш.ини.Конфигурация.Свойство("мСторона") и Кэш.ини.Конфигурация.мСторона.Свойство("Контрагент") Тогда
						Контекст = Новый Структура("Документ, Ини, ТекущийРазделИни",ГлавноеОкно.ФильтрКонтрагент, Кэш.ини.Конфигурация, Кэш.ини.Конфигурация.мСторона.Контрагент);
						СвЮЛ.Вставить( "КПП", СокрЛП(РассчитатьЗначениеНаСервере("Сторона_КПП", Контекст)));     
					КонецЕсли;
				КонецПопытки;
...
#Область include_core_vo2_ОбщиеФункции_Прочее_СписочныеМетоды_Реестр1С
#КонецОбласти
