			...
			Иначе
				...
				Если Не сбисФайлТест.Существует() Тогда
					...
					Исключение
						...
						Возврат МодульОбъектаКлиент().НовыйСбисИсключение(772102, "РаботаСДокументами1С.сбисСохранитьФайлыНаСервере", ,
						,"Проверьте права на каталог по пути " + сбисФайл.ПутьКуда, сбисПараметрыКопирования);
	...
	Для Каждого сбисФайл Из сбисПараметрыКопирования.Файлы Цикл
		...
		Если Не сбисФайлТест.Существует() Тогда
			...
			Исключение
				...
				Возврат МодульОбъектаСервер().НовыйСбисИсключение(772102, "РаботаСДокументами1С.сбисСохранитьФайлыНаСервере", ,,"Проверьте права на каталог по пути " + сбисФайл.ПутьКуда);
			КонецПопытки;
		КонецЕсли;
		Попытка
		...
		Исключение
			Отказ = Истина;
			Возврат МодульОбъектаСервер().НовыйСбисИсключение(, "РаботаСДокументами1С.сбисСохранитьФайлыНаСервере", 777, "Вложения не найдены", ОписаниеОшибки());
		КонецПопытки;
		Попытка
