...
Функция СбисСформироватьТитулПолучателяАктаНакладной_5_03(Кэш, СоставПакета, Вложение, НовоеВложение, Действие, ИмяXSLT, ПрефиксФайла, ДопПараметры = Неопределено) Экспорт
	Перем СертификатПодписи, ИмяФайла, ДатаДок, ВремДок;
	...
	Если СтруктураФайла.Файл.Документ.Свойство("СвСчФакт")
		И СтруктураФайла.Файл.Документ.СвСчФакт.Свойство("ДатаДок", ДатаДок) Тогда
		// Дата найдена в файле
		Если Не СтруктураФайла.Файл.Документ.СвСчФакт.Свойство("ВремДок", ВремДок) Тогда
			// Времени нет → ставим 00:00 от найденной даты
			ВремДок = Дата(1,1,1); 
		КонецЕсли;
	Иначе
		ДатаДок = ТекущаяДата();
	...
	ДанныеПолучателя.Вставить("ВремДок",	формат(ВремДок,"ДФ=HH.mm.ss; ДП=00.00.00"));
