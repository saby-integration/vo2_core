...
Функция ВыполнитьОбработкуАпи3Объекта(Кэш, ПараметрыОбработкиОбъекта, Отказ) Экспорт 
	...
	Если Отказ Тогда
		ДанныеОшибки = Новый Структура("Объект", ПараметрыОбработкиОбъекта);
		Результат = МодульОбъектаКлиент().НовыйСбисИсключение(Результат, "РаботаСДокументами1С.ВыполнитьОбработкуАпи3Объекта",,,,ДанныеОшибки);
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Функция СбисПолучитьЗначениеОбъекта(Знач СбисОписаниеОбъекта, Отказ)	
	...
	Если СбисОбъектЗначение = Неопределено Тогда
		...
		Исключение
			Отказ = Истина;
			Возврат Новый Структура("code,message,details", 773, "Ошибка поиcка ссылки на объект", ОписаниеОшибки());
		КонецПопытки;
...
Функция СбисПолучитьСтруктуруОбъекта1С(ОбъектСсылка, Знач СтруктураДокумента, Отказ) Экспорт
	Попытка
		...
		СтруктураОбъекта.Вставить("ИдИС",				Строка(ОбъектСсылка.УникальныйИдентификатор()));
		...
		ИначеЕсли Найти("Справочники.Контрагенты Справочники.Организации Справочники.Фирмы", СтруктураОбъекта.ИмяИС)  Тогда
			...
			Если ЗначениеЗаполнено(ОбъектСсылка.Родитель) Тогда
				...
				СтруктураОбъекта.Вставить("Родитель", Строка(ОбъектСсылка.Родитель.УникальныйИдентификатор()));
			КонецЕсли;
		...
		ИначеЕсли Найти(СтруктураОбъекта.ИмяИС, "Документы.")  Тогда
			
			СтруктураОбъекта.Вставить("Название",		Строка(ОбъектСсылка));
			СтруктураОбъекта.Вставить("Номер",			ОбъектСсылка.Номер);
			СтруктураОбъекта.Вставить("Дата", 			Формат(ОбъектСсылка.Дата, "ДФ=dd.MM.yyyy"));
			СтруктураОбъекта.Вставить("Организация",	Строка(ОбъектСсылка.Организация.УникальныйИдентификатор()));
			СтруктураОбъекта.Вставить("Контрагент",		Строка(ОбъектСсылка.Контрагент.УникальныйИдентификатор()));
			...
			Иначе
				Если КлючИЗначениеСтруктуры.Ключ = "ЭтоГруппа" или КлючИЗначениеСтруктуры.Ключ = "IsFolder" Тогда
					Если КлючИЗначениеСтруктуры.Значение <> Объект1С[КлючИЗначениеСтруктуры.Ключ] Тогда
						Отказ = Истина;
						Возврат Новый Структура("code,message,details,dump", 773, "Ошибка заполнения структуры объекта", "Значения реквизита ЭтоГруппа объекта 1С не соовтетсвует значению реквизита объекта Saby", Новый Структура("Ключ", КлючИЗначениеСтруктуры.Ключ));
					КонецЕсли;
				Иначе
