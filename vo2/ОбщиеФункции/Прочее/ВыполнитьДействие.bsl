...
Функция СбисУтвердитьОтклонить(Кэш,СписокОтмеченныхДокументов, ИмяКоманды) Экспорт
	...
	Если Кэш.Текущий.ТипДок <> "ЗаказВх"
		И СписокОтмеченныхДокументов.Количество() > 1
		...
		Иначе  // ввели пин-коды для всех сертов
			...
			Для Каждого Строка из СписокОтмеченныхДокументов Цикл        
				...
				ИначеЕсли Строка.Значение.Статус = 6 Тогда //Если уже утвержден
					...
					Продолжить;   
				ИначеЕсли МодульОбъектаКлиент().СоставПакета_Получить(Строка.Значение.СоставПакета[0].Значение, "КодСостояния") = "22" Тогда              
					// аннулировано по соглашению сторон
					Сообщить(ПолучитьПредставлениеПакетаДляОшибки(Строка) + "Невозможно выполнить " + ТекстСообщения + ". Документ аннулирован.");			
					Продолжить;
				КонецЕсли;	
				
				Попытка
					
					РезультатОперации = Кэш.Интеграция.сбисВыполнитьКоманду(Кэш, Строка.Значение.СоставПакета[0].Значение.Идентификатор,ИмяКоманды, ПолучитьПредставлениеПакетаДляОшибки(Строка));	
					
				Исключение
					
					МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке(), "РаботаСДокументами1С.СбисУтвердитьОтклонитьЗавершение");
					
				КонецПопытки;  
				
				Если РезультатОперации = Ложь Тогда
					Если Не МодульОбъектаКлиент().ПараметрSaby("ОбменВключен") 
						И МодульОбъектаКлиент().ПараметрSaby("Авторизован") Тогда
						Прервать;
					КонецЕсли;
				КонецЕсли;
...
&НаКлиенте
Процедура СбисАннулироватьЗаказОтправленный(СписокОтмеченныхДокументов, Комментарий) Экспорт
	...
	Для Каждого Строка Из СписокОтмеченныхДокументов Цикл        
		...
		Если Не МодульОбъектаКлиент().СоставПакета_Получить(СоставПакета, "МожноАннулировать") Тогда     
		...
		РезультатВыбора = Новый Структура("Комментарий", Комментарий);
		Отказ = Ложь;
	...
	МодульОбъектаКлиент().СбисСообщить("Документов аннулировано: " + УспешноОтправленных);
	ГлавноеОкно.сбисСпрятатьСостояние(ГлавноеОкно);
	ГлавноеОкно.ОбновитьКонтент();
