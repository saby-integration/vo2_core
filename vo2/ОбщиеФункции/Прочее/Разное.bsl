...
Функция сбисЗаписатьФайл_Клиент(Кэш, сбисОписаниеФайла, Отказ) Экспорт
	...
	Иначе
		...
		Исключение
			Отказ = Истина;
			Возврат Новый Структура("code,message,details", 772, "Ошибка работы с файловой системой",
																	"У вас нет прав доступа для работы с выбранным каталогом.
																	| Обратитесь к администратору для настройки прав, либо смените каталог.");
		КонецПопытки;
	КонецЕсли;
	...
	Результат.Вставить("ПолноеИмяФайла",	Результат.ПолноеИмя);
...
Функция СбисФорматСтроки(СбисСтрока, Режим="Убрать", ФорматнаяСтрока="@/-?=\& ""'#№*(:|;).") Экспорт
	...
	Возврат МодульОбъектаКлиент().ПрименитьФорматКСтроке(СбисСтрока, ФорматнаяСтрока, Режим);
...
Функция СбисЗавершениеРаботы(Кэш, Отказ) Экспорт
	...
	Если Кэш.ТекущийСеанс.Свойство("ВременныеФайлы") Тогда 
		...
		МодульОбъектаКлиент().ПроверитьОчиститьВременныеФайлы();
	КонецЕсли;
...
Функция СбисПолучитьИмяВременногоФайлаКлиент(Расширение="tmp") Экспорт
	...
	Возврат МодульОбъектаКлиент().НовыйВременныйФайлКлиент(, Расширение)
...
Процедура СбисОчиститьВременныйФайлКлиент(ИмяФайла) Экспорт
	...
	МодульОбъектаКлиент().ОсвободитьВременныйФайл(ИмяФайла);
...
&НаКлиенте                                  
Функция СериализоватьВерсию(Версия, Направление="ЧислоВСтроку",ДопПараметры=Неопределено) Экспорт
	
	Если ДопПараметры = Неопределено	Тогда
		ПараметрыПривести = Новый Структура("ВариантПриведения", Направление);
	Иначе
		ПараметрыПривести = ДопПараметры;
	КонецЕсли;
	ПараметрыПривести.Вставить("ВариантПриведения", Направление);
	
	Возврат МодульОбъектаКлиент().ПривестиВерсию(Версия, ПараметрыПривести);
...
&НаКлиенте
Функция СбисСформироватьОбщуюИнформациюДляТП(Кэш, ПараметрыВыгрузки) Экспорт
	...
	ТекстДок.ДобавитьСтроку("   ФормаРаботыСНоменклатурой: " + МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("ИмяМодуляСопоставленияНоменклатуры"));           
	ТекстДок.ДобавитьСтроку("   ФормаРаботыСоСтатусами: " + Кэш.ФормаРаботыСоСтатусами);
	ТекстДок.ДобавитьСтроку("   Идентификатор Аккаунта: " + Кэш.СБИС.ПараметрыИнтеграции.ИдАккаунта);
...
Функция СбисСформироватьИнформациюПоКонстантам() Экспорт
	...
	Для Каждого ОчереднаяКонстанта Из Метаданные.Константы Цикл
		
		Попытка
			
			ИмяКонст = ОчереднаяКонстанта.Имя;
			ЗначениеКонст = Константы[ИмяКонст].Получить();
...
&НаКлиенте
Функция СбисСформироватьИнформациюПоДокументам1С(Кэш, ПутьККаталогу, Документы1С) Экспорт
	
	АдресХранилища = сбисПолучитьСтруктурыДокументов1С(Документы1С, Кэш.Ини.Конфигурация);
	СведенияОДанных = ПолучитьИзВременногоХранилища(АдресХранилища);
	Если СведенияОДанных.Тип = "ДвоичныеДанные"
		И СведенияОДанных.ДопСведения.Архив = "ZIP" Тогда
		
		КаталогРаспаковки = МодульОбъектаКлиент().ПрименитьФорматКСтроке(ПутьККаталогу, "Документы1С", "Каталог");
		ЧтениеZipФайла = Новый ЧтениеZipФайла;
		ЧтениеZipФайла.Открыть(СведенияОДанных.Значение.ОткрытьПотокДляЧтения());
		ЧтениеZipФайла.ИзвлечьВсе(КаталогРаспаковки);
...
Функция СбисПолучитьСтруктурыДокументов1С(Документы1С, ИниКонфигурация)
	...
	Для Каждого Док1С Из Документы1С Цикл  
		МассивСсылок.Добавить(Док1С.Значение);
	КонецЦикла;
	
	ПараметрыВыгрузки = Новый Структура("ZIP", Истина);
	Если Документы1С.Количество() Тогда
		ПараметрыВыгрузки.Вставить("ИмяФайла", СокрЛП(СтрЗаменить(Строка(Документы1С[0].Значение), ":" , "_")) + ".json");
	КонецЕсли;
	
	Возврат МодульОбъектаСервер().ВыгрузитьОбъектыВJSON(МассивСсылок, ПараметрыВыгрузки);
...
//
&НаСервереБезКонтекста
Функция ОпределитьИмяТипаОбъекта(ТипОбъекта) Экспорт
