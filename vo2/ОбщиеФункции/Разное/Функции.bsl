...
Функция ПолучитьСторону(Кэш, Контекст) Экспорт  
	...
	Если РежимФормированияСтороны = "Полный" Тогда
		...
		ИначеЕсли ЭтоФизЛицо Тогда
			ВидСтороны = "Фл";
		КонецЕсли;
		Участник = МодульОбъектаКлиент().НовыйПрикладнойОбъектSABY("СторонаSABY");
		Участник.Вставить("Вид", ВидСтороны);// юл, фл, ин, ип
		МодульОбъектаКлиент().ПрикладнойОбъектSaby_Установить(Участник, "ИностраннаяОрганизация", ЭтоИностраннаяОрганизация);
		МодульОбъектаКлиент().ПрикладнойОбъектSaby_Установить(Участник, "ИдентификационныйНомер", ИдентификационныйНомер);
...
Функция ПолучитьСторону_ФизЛицо(Кэш, Контекст, Участник)
	...
	Если Роль = "ОтпускПроизвёл" Тогда
		...
		Если ЗначениеЗаполнено(ФИО) Тогда
			...
			ОтпустилГруз = Новый Структура("Должность, Имя, Отчество, Фамилия", Должность, СвФЛ.Имя, СвФЛ.Фамилия, СвФЛ.Отчество);
