...
Функция сбисТекстИзBase64(ДвоичныеДанныеBase64, ДополнительныеПараметры=Неопределено) Экспорт
	ИмяВремФайла = МестныйКэш.ОбщиеФункции.СбисПолучитьИмяВременногоФайлаКлиент("xml");
...
Функция ПолучитьСторону(Кэш, знач Контекст) Экспорт
	...
	Инн							= СокрЛП(РассчитатьЗначение("Сторона_ИНН",Контекст, Кэш));
	СбисКодСтраны				= "643";
	...
	Если ЗначениеЗаполнено(ТекКодСтраны) Тогда
		...
		ЭтоИностраннаяОрганизация	= ЭтоИностраннаяОрганизация Или Не (СбисКодСтраны = "643");
	КонецЕсли;
...
#Область include_core_vo2_ОбщиеФункции_ОтборПоМассивуСтрок
#КонецОбласти

#Область include_core_vo2_ОбщиеФункции_Статусы
#КонецОбласти
