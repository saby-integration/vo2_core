...
Функция ПолучитьСторону(Кэш, знач Контекст) Экспорт  
	...
	Если ЭтоИностраннаяОрганизация Тогда
		...
		Участник.СвЮЛ.Вставить("ИНН",		ИдентификационныйНомер);
	...
	// Код филиала стороны отправителя
	...
	// Сертификат стороны
	СертификатСтороны = РассчитатьЗначение("Сертификат", Контекст);
	Если ЗначениеЗаполнено(СертификатСтороны) Тогда
		ДопПараметрыДляСторны.Вставить("Сертификат", СертификатСтороны);
	КонецЕсли;
