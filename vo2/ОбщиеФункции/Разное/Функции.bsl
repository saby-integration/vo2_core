...
Функция ПолучитьРеквизитМетаданныхОбъекта(Объект1С, ИмяРеквизита) Экспорт
	...
	Если ТипЗнч(Объект1С) <> Тип("Структура") Тогда
		Результат = ПолучитьРеквизитМетаданныхОбъектаСервер(Объект1С, ИмяРеквизита);
	ИначеЕсли Объект1С.Свойство(ИмяРеквизита) Тогда 
		Результат = Объект1С[ИмяРеквизита];
	КонецЕсли;
...
Функция ПолучитьРеквизитМетаданныхОбъектаСервер(Объект1С, ИмяРеквизита) Экспорт
...
Функция ПодготовитьСтруктуруДокументаСбис(СтрокаСпискаДокументов, Кэш) Экспорт
	...
	Для каждого Строка из СтрокаСпискаДокументов.СоставПакета Цикл
		...
		Если Не Строка.Значение.Свойство("ИдСвязанногоДокументаSaby", ИдентификаторПакета) Тогда 
			
			ИдентификаторПакета = Строка.Значение.Идентификатор;  
			
		КонецЕсли;
		
		ОписаниеДокумент	= Новый ФиксированнаяСтруктура("Идентификатор", ИдентификаторПакета);
		ДопПараметры		= Новый Структура("ДокументСБИС", Документ);
		
		Если Документ.Свойство("ПерегенерироватьВложения") И Документ.ПерегенерироватьВложения Тогда
			
			ДопПараметры.Вставить("ПерегенерироватьВложения", Документ.ПерегенерироватьВложения);
			
		КонецЕсли;
		
		Попытка
			
			ПолныйСоставПакета = МодульОбъектаКлиент().ПрочитатьДокументСБИС(ОписаниеДокумент, ДопПараметры);
			
		Исключение
			
			МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке(), "РаботаСДокументами1С.ПодготовитьСтруктуруДокументаСбис");
			Продолжить;
			
		КонецПопытки;
		ПолныйСоставПакета.Вставить("Параметры", Новый Структура);
		
		Если ПолныйСоставПакета.Свойство("Вложение") Тогда
			
			фрм.НайтиДокументы1СПоПакетуСБИС(ПолныйСоставПакета, Кэш.Ини, Кэш.Парам.КаталогНастроек);
			МассивПакетов.Добавить(ПолныйСоставПакета);
			ПолныйСоставПакета.Параметры.Вставить("ПроведенПоискДокументов1С", Истина);
...
&НаКлиенте
Процедура ЗаполнитьСтруктуруЭлементаСправочника(Кэш, СтрТабл, СтруктураФайла, СтруктураНоменклатура)  Экспорт
	...
	Для Каждого Элемент Из Кэш.Ини.Конфигурация Цикл
		...
		Если НРег(Лев(Элемент.Ключ, 5)) = "выбор" Тогда
			
			Для каждого ВеткаВыбора Из Элемент.Значение Цикл
				
				ДобавитьУзел = Ложь;
				
				Для Каждого ЭлементВыбора Из ВеткаВыбора.Значение Цикл
					Если Лев(ЭлементВыбора.Ключ, 13) = КлючНоменклатура + "_" Тогда
	...
	СоответствиеДанныеКлюч.Вставить(КлючНоменклатура, "ДанныеРезультат");
...
Функция сбисОбновитьРеестрДокументов1С(Ини, Кэш) Экспорт
	...
	//ПараметрыФильтра = МодульОбъектаКлиент().РазделГО_Получить(Кэш.Текущий, "Фильтр");
	...
	Если МассивДокументов1С = Ложь Тогда
		
		МассивДокументов1С = Неопределено;
...
Функция ЗаполнитьСтруктуруФайла(СтрокаXML) Экспорт
	...
	сбисОписаниеОбъекта				= Новый Структура("Тип, Ключ, Значение", "ПользовательскиеЗначения.ПравилаXMLДокумента"); 
	...
	Если сбисОписаниеОбъекта.Значение = Неопределено Тогда
		...
		УзелКакМассив = Новый Массив;
		УзелКакМассив.Добавить("СтрТабл");
		УзелКакМассив.Добавить("ИтогТабл");
		УзелКакМассив.Добавить("Основание");
		УзелКакМассив.Добавить("ДопСведения");
		УзелКакМассив.Добавить("ДопАдрЭл");
		УзелКакМассив.Добавить("НомСредИдентТов");
		УзелКакМассив.Добавить("НомУпак");
		УзелКакМассив.Добавить("КИЗ");
		УзелКакМассив.Добавить("СведПрослеж");
		УзелКакМассив.Добавить("НомерГТД");

		СворачиватьПараметр	= Новый Структура("Параметр", Новый ФиксированнаяСтруктура("Ключ, Значение", "Имя", "Значение"));
		
		ПравилаXML = Новый Структура("УзелКакМассив, Свернуть", УзелКакМассив, СворачиватьПараметр);
		
		сбисОписаниеОбъекта.Значение = Новый Структура("Правила", ПравилаXML);
	...
	#Если ВебКлиент Тогда
		...
		Возврат ЗаполнитьСтруктуруФайла_Сервер(СтрокаXML, сбисОписаниеОбъекта.Значение);
		
	#Иначе 
		
		ЧтениеXML = МодульОбъектаКлиент().НовыйЧтениеXMLSABY(СтрокаXML, сбисОписаниеОбъекта.Значение);
		Возврат МодульОбъектаКлиент().ЧтениеXMLSABY_Прочитать(ЧтениеXML);
...
&НаСервере
Функция ЗаполнитьСтруктуруФайла_Сервер(Знач СтрокаXML, Знач ПравилаЧтенияXML)
	
	ЧтениеXML = МодульОбъектаСервер().НовыйЧтениеXMLSABY(СтрокаXML, ПравилаЧтенияXML);
	Возврат МодульОбъектаСервер().ЧтениеXMLSABY_Прочитать(ЧтениеXML);

КонецФункции

// Функция формирует структуру Стороны (отправителя, получателя, поставщика, покупателя) в универсальном формате (3.01).	
&НаКлиенте
Функция ПолучитьСторону(Кэш, Контекст) Экспорт  
	Перем РежимФормированияСтороны, ВидСтороны;
	
	ДопПараметрыДляСторны	= Новый Соответствие;
	
	Если Не Контекст.Свойство("РежимФормированияСтороны", РежимФормированияСтороны)	Тогда
		
		РежимФормированияСтороны = "Общий";
	...
	Если  ТипИдентификационногоНомера.БИН Тогда
		...
		ДопПараметрыДляСторны.Вставить("КлючИдентификатора", "БИН");
	ИначеЕсли  ТипИдентификационногоНомера.ИИН Тогда
		ИИН	= ИдентификационныйНомер;
		ДопПараметрыДляСторны.Вставить("КлючИдентификатора", "ИИН");
	Иначе
		ИНН	= ИдентификационныйНомер;  
	КонецЕсли;	

	СбисКодСтраны				= "643";
	СбисНаимСтраны				= "Российская Федерация";
	ЭтоИностраннаяОрганизация	= РассчитатьЗначение("ИностраннаяОрганизация",	Контекст) = Истина;
	РасширенныыйАдрес			= РассчитатьЗначение("РасширенныйАдрес",		Контекст) = Истина;
	ЭтоФизЛицо 					= РассчитатьЗначение("ЭтоФизЛицо", 				Контекст) = Истина;
	
 	ТекКодСтраны	= РассчитатьЗначение("КодСтраны",	Контекст);
	ТекНаимСтраны	= РассчитатьЗначение("НаимСтраны",	Контекст);
	Если ЗначениеЗаполнено(ТекКодСтраны) Тогда
		СбисКодСтраны				= ТекКодСтраны;
		СбисНаимСтраны				= ТекНаимСтраны;
		ЭтоИностраннаяОрганизация	= ЭтоИностраннаяОрганизация Или Не (СбисКодСтраны = "643" Или СбисКодСтраны = "398");
		ДопПараметрыДляСторны.Вставить("ИностраннаяОрганизация", ЭтоИностраннаяОрганизация)
	КонецЕсли;
	
	Если РежимФормированияСтороны = "Полный" Тогда
		Если ЭтоИностраннаяОрганизация Тогда
			ВидСтороны = "Ин";
		ИначеЕсли ЭтоФизЛицо Тогда
			ВидСтороны = "Фл";
		КонецЕсли;
		Участник = МодульОбъектаКлиент().Saby_НовыйПрикладнойОбъектSABY("СторонаSABY");
		Участник.Вставить("Вид", ВидСтороны);// юл, фл, ин, ип
		МодульОбъектаКлиент().Saby_ПрикладнойОбъектSABY_Установить(Участник, "ИностраннаяОрганизация", ЭтоИностраннаяОрганизация);
		МодульОбъектаКлиент().Saby_ПрикладнойОбъектSABY_Установить(Участник, "ИдентификационныйНомер", ИдентификационныйНомер);
		
	ИначеЕсли	Не ЗначениеЗаполнено(ИдентификационныйНомер)
		И	Не ЭтоИностраннаяОрганизация 
		И Не ЭтоФизЛицо Тогда
	...
	Если ЭтоФизЛицо Тогда
		Возврат ПолучитьСторону_ФизЛицо(Кэш, Контекст, Участник);
	КонецЕсли;
	
	ИспользоватьКакРоль = Ложь;
	
	Если Контекст.Свойство("Роль") Тогда
		
		Роль = РассчитатьЗначение("Роль", Контекст);
		
	Иначе
		
		Роль = РассчитатьЗначение("Сторона_Роль", Контекст);
	...
	Если	Не	ЭтоФизЛицо
		И	Не	ЭтоИностраннаяОрганизация
		И		Контекст.Свойство("Сторона_ИНН")  Тогда
		
		Контекст.Сторона_ИНН = ИдентификационныйНомер;
		Если СтрДлина(СокрЛП(Инн)) <> 12 Тогда
			
			КПП = РассчитатьЗначение("Сторона_КПП", Контекст);
			Если		Контекст.Свойство("КПППодразделения")
				И		ЗначениеЗаполнено(Контекст.КПППодразделения) // в счет-фактуре КПП контрагента можно указать прямо на документе (если у контрагента есть филиалы)
				И Не	ИспользоватьКакРоль Тогда
					
				КПП = РассчитатьЗначение("КПППодразделения",Контекст);
				
			КонецЕсли;
			
			Контекст.Вставить("Сторона_КПП", КПП);
	...
	ПолучитьСторону_ФизЛицо(Кэш, Контекст, Участник);
	
	// VAAl>>> Формирование СвЮЛ и СвФЛ
	СведенияОЛице = Новый Структура;
	Если ЭтоИностраннаяОрганизация
		ИЛИ НЕ СтрДлина(СокрЛП(ИНН))=12 Тогда
	
		СведенияОЛице.Вставить("Название",		Название);
		
	Иначе
		
		ФИОСтруктура = сбисПолучитьФИОИзНазвания(Название);
		
		СведенияОЛице.Вставить("Фамилия",	ФИОСтруктура.Фамилия);
		СведенияОЛице.Вставить("Имя",		ФИОСтруктура.Имя);
		СведенияОЛице.Вставить("Отчество",	ФИОСтруктура.Отчество);
		
	КонецЕсли;
	
	ЗаполнятьКПП = Ложь;
	
	Если СтрДлина(СокрЛП(ИНН)) <> 12
		И ЗначениеЗаполнено(БИН) Тогда
	
		СведенияОЛице.Вставить("БИН", БИН);
	ИначеЕсли СтрДлина(СокрЛП(ИНН)) <> 12
		И ЗначениеЗаполнено(ИИН) Тогда
		
		СведенияОЛице.Вставить("ИИН", ИИН);
	Иначе
		СведенияОЛице.Вставить("ИНН", ИНН);
		ЗаполнятьКПП = НЕ ЭтоИностраннаяОрганизация;
	КонецЕсли;
	
	Если ЗаполнятьКПП Тогда
		СведенияОЛице.Вставить("КПП", КПП);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СбисКодСтраны)
		И СбисКодСтраны <> "643" Тогда
	
		СведенияОЛице.Вставить("КодСтраны", СбисКодСтраны);
	КонецЕсли;
	
	Если РасширенныыйАдрес
		И ЗначениеЗаполнено(СбисНаимСтраны)
		И СбисНаимСтраны <> "Российская Федерация" Тогда
	
		СведенияОЛице.Вставить("НаимСтраны",	СбисНаимСтраны);
		СведенияОЛице.Вставить("ИдСтат",		"ИО");
	КонецЕсли;
	
	ЗаполнитьАтрибуты(Кэш, "Код", Контекст, СведенияОЛице);
	
	Если ЭтоИностраннаяОрганизация Тогда 
		Участник.Вставить("СвИН", СведенияОЛице);
		
	ИначеЕсли СтрДлина(СокрЛП(ИНН)) = 12
		ИЛИ ЗначениеЗаполнено(ИИН) Тогда
		Участник.Вставить("СвФЛ", СведенияОЛице);
		ВидСтороны = "Ип";
	Иначе
		Участник.Вставить("СвЮЛ", СведенияОЛице);
		ВидСтороны = "Юл";
	КонецЕсли;
	// <<<VAAl
	
	ДопПараметрыДляСторны.Вставить("Идентификатор",  РассчитатьЗначение("Сторона_Идентификатор", Контекст, Кэш));
	...
	// Запрет редакции для стороны получателя
	ЗапретРедакций = РассчитатьЗначение("ЗапретРедакций", Контекст);
	Если Не ЗапретРедакций = Неопределено Тогда
		
		ДопПараметрыДляСторны.Вставить("ЗапретРедакций", ЗапретРедакций);
		
	КонецЕсли;
	
	МодульОбъектаКлиент().СторонаSaby_Установить(Участник, "ДополнительныеДанные",	ДопПараметрыДляСторны);
	МодульОбъектаКлиент().СторонаSaby_Установить(Участник, "Вид",					ВидСтороны);
	МодульОбъектаКлиент().СторонаSaby_Установить(Участник, "Роль",					Роль);
	
	Если РежимФормированияСтороны = "Общий" Тогда
		
		Если Контекст.Свойство("Сторона_Роль") Тогда
			
			// Старый костыль - свойство попадает на участник.
			Участник.Вставить("Роль", Роль);
...
&НаКлиенте
Функция ПолучитьСторону_ФизЛицо(Кэш, Контекст, Участник)
	
	Представители = Новый Структура;
	Роль = РассчитатьЗначение("Роль", Контекст); 
	Если Роль = "ОтпускПроизвёл" Тогда
		ФИО = Кэш.ОбщиеФункции.РассчитатьЗначение("Сторона_ФИО", Контекст, Кэш);
		Должность=РассчитатьЗначение("Должность",Контекст,Кэш);
		РаботникИнойОрганизации=РассчитатьЗначение("РаботникИнойОрганизации",Контекст,Кэш);
		Если ЗначениеЗаполнено(ФИО) Тогда
			СвФЛ = Кэш.ОбщиеФункции.сбисПолучитьФИО(ФИО);
			ОтпустилГруз = Новый Структура("Должность, Имя, Фамилия, Отчество", Должность, СвФЛ.Имя, СвФЛ.Фамилия, СвФЛ.Отчество);
			Участник.Вставить("ОтпустилГруз", ОтпустилГруз); 
		КонецЕсли;
		Участник.Вставить("Роль", Роль);
		Участник.Вставить("РаботникИнойОрганизации", РаботникИнойОрганизации);
