...
// Особенность документа в том, что по одному документу формируется несколько пакетов
	...
	Для Каждого Файл Из Контекст.ДокументДанные.мФайл Цикл
		...
		Файл.Свойство("НаимДокОтгр", НаимДокОтгр);
		Стр = 0;
				...
				Если фрм.ПолучитьДанныеИзДокумента1С(Кэш,Контекст) Тогда //если хотябы что-то не выгрузилось - отбой
					Если Документ.Свойство("СчетФактура") и ЗначениеЗаполнено(Документ.СчетФактура) Тогда
						...
						Если Кэш.ини.Свойство(ИмяДокумента) Тогда
							...
							ИспользоватьШтрихкодыНоменклатурыКонтрагентов = МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ИспользоватьШтрихкодыНоменклатурыКонтрагентов");
							ЗначениеИни.Вставить("ИспользоватьШтрихкодыНоменклатурыКонтрагентов",	Новый Структура("Значение,РассчитанноеЗначение",
																													ИспользоватьШтрихкодыНоменклатурыКонтрагентов, ИспользоватьШтрихкодыНоменклатурыКонтрагентов));
								...
								Если Контекст.ФайлДанные.мОснование.Свойство("ДокПодтвОтгр") Тогда
									Если ТипЗнч(Контекст.ФайлДанные.мОснование.ДокПодтвОтгр) = Тип("Массив") Тогда
										Для каждого СтрДокПодтвОтгр Из Контекст.ФайлДанные.мОснование.ДокПодтвОтгр Цикл
											...
											Если ЗначениеЗаполнено(НаимДокОтгр) Тогда
												СтрДокПодтвОтгр.Вставить("Основание_НаимДокОтгр", НаимДокОтгр);
											КонецЕсли;
										КонецЦикла;
									Иначе
										...
										Если ЗначениеЗаполнено(НаимДокОтгр) Тогда
											Контекст.ФайлДанные.мОснование.ДокПодтвОтгр.Вставить("Основание_НаимДокОтгр", НаимДокОтгр);
										КонецЕсли;
									КонецЕсли;
								КонецЕсли;
								//--- МАИ 14.09.2021
