	...
	Если Кэш.ини.Свойство("СчетФактураВыданныйПоставщику") Тогда
		...
		Для Каждого Стр Из ТаблДокПоставщики Цикл
			Если ЗначениеЗаполнено(Стр.ИсхСФ) Тогда
				...
				Если Контекст.ДокументДанные.мФайл.СчФктр.мОснование.Свойство("ДокПодтвОтгр") 
					И ТипЗнч(Контекст.ДокументДанные.мФайл.СчФктр.мОснование.ДокПодтвОтгр) = Тип("Массив") 
					И Стр.мОснование.Свойство("ПоступлениеПоставщика") Тогда
					Контекст.ДокументДанные.мФайл.СчФктр.мОснование.ДокПодтвОтгр.Очистить();
				КонецЕсли;   
				
				// Необходимо переопределить узел "...мОснование.ДокПодтвОтгр", для того, чтобы брались данные исходного документа, а не из Отчета комитенту
				
				Для Каждого СтрокаПоставщики Из ТаблДокПоставщики Цикл
					Если	Контекст.ДокументДанные.мФайл.СчФктр.мОснование.Свойство("ДокПодтвОтгр")
						И	СтрокаПоставщики.мОснование.Свойство("ПоступлениеПоставщика") 
						И   СтрокаПоставщики.ИсхСФ = Стр.ИсхСФ Тогда  
						
						СтрокаДокПодтвОтгр = Новый Структура("Основание_Название, Основание_НаимДокОтгр, Основание_Дата, Основание_Номер, ПоступлениеДата, ПоступлениеДатаВх",
						"ДокПодтвОтгр",
						"Реализация (акт, накладная, УПД)",
						СтрокаПоставщики.мОснование.ПоступлениеПоставщика.Основание_Дата,
						СтрокаПоставщики.мОснование.ПоступлениеПоставщика.Основание_Номер,
						СтрокаПоставщики.мОснование.ПоступлениеПоставщика.ПоступлениеДата,
						СтрокаПоставщики.мОснование.ПоступлениеПоставщика.ПоступлениеДатаВх);
						Если ТипЗнч(Контекст.ДокументДанные.мФайл.СчФктр.мОснование.ДокПодтвОтгр) = Тип("Массив") Тогда
							Контекст.ДокументДанные.мФайл.СчФктр.мОснование.ДокПодтвОтгр.Добавить(СтрокаДокПодтвОтгр); 
						Иначе
							Контекст.ДокументДанные.мФайл.СчФктр.мОснование.ДокПодтвОтгр = СтрокаДокПодтвОтгр;
						КонецЕсли;
