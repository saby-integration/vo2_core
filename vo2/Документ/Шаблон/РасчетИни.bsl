...
Функция ЗаполнитьДанныеДокумента1С(Кэш, ини, СтруктураФайла, МассивОснований, СоставПакета, Вложение, Документ1С = Неопределено, ДополнительныеДанные = Неопределено) Экспорт
	...
	КонецЕсли;	
	АвтоматическоеПроведениеЗаказов = МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("АвтоматическоеПроведениеЗаказов");
	ТипОбъекта = СтрПолучитьСтроку(СтрЗаменить(ТипОбъекта, ".", Символы.ПС), 1);
	сбисДополнительныеПараметры = Новый Структура(	"ОбъектПерезаполнитьСсылка, ТипОбъекта, ПодТипОбъекта,	ДополнятьТабЧасть, РедактироватьТабЧасть, Ини, ПерезаполнятьТолькоНепроведенные, ЗаполняемыеТабЧасти, АвтоматическоеПроведениеЗаказов",
													Документ1С,					ТипОбъекта, ТипДокумента,	ДополнятьТабЧасть, РедактироватьТабЧасть, ини, Кэш.Парам.ПерезаполнятьТолькоНепроведенные, ЗаполняемыеТабЧасти, АвтоматическоеПроведениеЗаказов);
