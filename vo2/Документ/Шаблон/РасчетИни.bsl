...
Процедура ЗаполнитьДаннымиСтруктурыФайлаСтруктуруДокументаНаСервере(ОбъектЗаполнить, Знач ПараметрыОбработчика)
	...
	СбисКонтекст.ДанныеИсточник.Вставить("Документ",		ОбъектЗаполнить.Документ); // Для забора посчитанных значений
...
Функция ВычислитьОтборНаСервере(Аргумент, АргументыРассчитать)
	...
	СписокОтборов = Новый Структура;
	Для Каждого ЭлементОтбора Из СбисПоле.Отбор Цикл
		СписокОтборов.Вставить(ЭлементОтбора.Ключ, МодульОбъектаСервер().РассчитатьЗначениеSABY(ЭлементОтбора.Значение, АргументыРассчитать.Контекст));
	КонецЦикла;
	Если Не СбисПоле.Свойство("Выбрать", СбисВыбрать) Тогда
		СбисВыбрать = "все";
	Иначе
...
Функция ЗаполнитьДанныеДокумента1С(Кэш, ини, СтруктураФайла, МассивОснований, СоставПакета, Вложение, Документ1С = Неопределено, ДополнительныеДанные = Неопределено) Экспорт
	...
	ПараметрыЗаполнить = Новый Структура;
	ПараметрыЗаполнить.Вставить("РассчитываемыйОбъект", СтруктураДокОбъект);
	ПараметрыЗаполнить.Вставить("ПоляРассчета",	Новый Структура("Режим", "Загрузка"));
	ПараметрыЗаполнить.Вставить("ЗаполнитьКонтекст", МодульОбъектаКлиент().СоставПакета_Получить(СоставПакета, "ПерезаполнятьВходящийКонтекст"));
	ПараметрыЗаполнить.Вставить("СоставПакета", СоставПакета);
	МодульОбъектаКлиент().ВложениеСБИС_ЗаполнитьПараметрыПередРасчетом(Вложение, Новый ФиксированнаяСтруктура(ПараметрыЗаполнить));
	...
	сбисДополнительныеПараметры = Новый Структура("ОбъектПерезаполнитьСсылка, ТипОбъекта, ПодТипОбъекта");
	сбисДополнительныеПараметры.ОбъектПерезаполнитьСсылка			= Документ1С;
	сбисДополнительныеПараметры.ТипОбъекта							= ТипОбъекта;
	сбисДополнительныеПараметры.ПодТипОбъекта						= ТипДокумента;

	сбисДополнительныеПараметры.Вставить("ДополнятьТабЧасть",					ДополнятьТабЧасть);
	сбисДополнительныеПараметры.Вставить("РедактироватьТабЧасть",				РедактироватьТабЧасть);
	сбисДополнительныеПараметры.Вставить("Ини",									ини);
	сбисДополнительныеПараметры.Вставить("ЗаполняемыеТабЧасти",					ЗаполняемыеТабЧасти);
	сбисДополнительныеПараметры.Вставить("ПерезаполнятьТолькоНепроведенные",	МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("ПерезаполнятьТолькоНепроведенные"));
	сбисДополнительныеПараметры.Вставить("АвтоматическоеПроведениеЗаказов",		МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("АвтоматическоеПроведениеЗаказов"));
	
	Если	СтруктураДокОбъект.Свойство("ОтказОтОбработкиДокумента")
		И	ЗначениеЗаполнено(СтруктураДокОбъект.ОтказОтОбработкиДокумента)	Тогда
		
		ОшибкаСозданияДокумента = МодульОбъектаКлиент().НовыйСбисИсключение(
			СтруктураДокОбъект.ОтказОтОбработкиДокумента,
			"Документ_Шаблон.ЗаполнитьДанныеДокумента1С");
			
		Отказ						= Истина;
		РезультатЗагрузки			= Новый Структура("Тип, Ссылка, Состояние, Ошибка");
		РезультатЗагрузки.Тип		= ТипОбъекта + "." + ТипДокумента;
		РезультатЗагрузки.Ссылка	= Документ1С; 
		РезультатЗагрузки.Ошибка	= ОшибкаСозданияДокумента; 
			
	Иначе
		
		РезультатЗагрузки = МодульОбъектаКлиент().СоздатьОбъектИзСтруктуры(СтруктураДокОбъект, сбисДополнительныеПараметры, Отказ);
	...
	ТочкаВходаДопРасчеты = МодульОбъектаКлиент().ПользовательскаяТочкаВхода("ДопРасчетыПриЗагрузке", "Документ_" + ТипДокумента + ",Документ_Шаблон");
	
	Если Не ТочкаВходаДопРасчеты = Неопределено Тогда
		
		КонтекстТочкиВхода	= Новый Структура("Кэш, СтруктураФайла, Вложение, СоставПакета", Кэш, СтруктураФайла, Вложение, СоставПакета);
		АргументыТочкиВхода	= Новый ФиксированнаяСтруктура("Ссылка, Контекст", РезультатЗагрузки.Ссылка, КонтекстТочкиВхода);
		
		Попытка
			
			МодульОбъектаКлиент().ВызватьПользовательскуюТочкуВхода(ТочкаВходаДопРасчеты, АргументыТочкиВхода);
			// Обновить переменные после вызова
			СоставПакета				= АргументыТочкиВхода.Контекст.СоставПакета;
			РезультатЗагрузки.Ссылка	= АргументыТочкиВхода.Ссылка;

		Исключение
			
			МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке(), "Документ_Шаблон.ЗаполнитьДанныеДокумента1С");
...
Функция ПолучитьПриемникЗаполненияРеквзиита(ЗначениеПараметра, СбисКонтекст, Постфикс = "")
	...
	Иначе
		...
		КлючЗаполнить	= КлючПоля;
		Результат		= СбисКонтекст.ДанныеРезультат;
...
Функция СбисВычислитьФункциюНаКлиенте(Кэш, ЗнПер, СбисКонтекст, ДопПараметры)
	Перем СтрТабл, Переменные, ОбъектДляФункций, КлючОбъектДляФункций, ОшибкаВызова;
	
	НовыйФормат			= Истина;
	ОбъектФункцийВложен	= Ложь;
	
	СтрокаВычислить		= Сред(ЗнПер, 2, СтрДлина(ЗнПер) - 2);
	РазбивкаФункции		= СтрЗаменить(СтрЗаменить(СокрЛП(СтрокаВычислить), "(", Символы.ПС), ")", Символы.ПС);
	...
	Если Не СбисКонтекст.Свойство("ИмяОбъекта", КлючОбъектДляФункций) Тогда
		
		КлючОбъектДляФункций = "Документ";
		
	КонецЕсли;

	//Поддержка старых функций
	Если		СбисКонтекст.Свойство(КлючОбъектДляФункций, ОбъектДляФункций) Тогда
		СбисКонтекст.Свойство("СтрТабл", СтрТабл);
		Контекст	= СбисКонтекст;
		НовыйФормат	= Ложь;
	Иначе
		Если	СбисКонтекст.Свойство("ДанныеРезультат", ОбъектДляФункций)
		    И	ОбъектДляФункций.Свойство(КлючОбъектДляФункций) Тогда
			
			// Взять объект для определения значения Документ
			ОбъектДляФункций	= ОбъектДляФункций[КлючОбъектДляФункций];
			ОбъектФункцийВложен = Истина;
		...
		Контекст.Вставить(КлючОбъектДляФункций,	ОбъектДляФункций);
	...
	// Для совместимости с функциями, вызываемыми через ини с обращением к "Документ"
	Документ	= ОбъектДляФункций;
	
	Если Найти(ИмяФункции, ".") Тогда
		
		ИмяМодуля	= "Документ_Шаблон";
		ЭтоВызовВФК	= Ложь;
		
	Иначе
		
		ИмяМодуля		= "РаботаСДокументами1С";
		фрм				= МодульОбъектаКлиент().НайтиФункциюСеансаОбработки(ИмяФункции, ИмяМодуля);
		СтрокаВычислить = "фрм." + ИмяФункции + "(Контекст)";
	...
	Если МодульОбъектаКлиент().ПараметрSaby("РежимОтладки") Тогда
		...
		ДанныеДляЛога	= Новый Структура("СтрТабл, Переменные, АргументыВходящие, Документ, ТекущееПолеИни", СтрТабл);
		ЗаполнитьЗначенияСвойств(ДанныеДляЛога, Контекст);
		ВремяВызова		= ТекущаяДата();
		Если ЭтоВызовВФК Тогда
			
			ИмяМодуля = "ВФК_" + ИмяМодуля;
			
		КонецЕсли;
		
		// Создадим строку с переадресацией на модуль вызова
		НаборЗаписей.Добавить(МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("CALL", "СбисВычислитьФункциюНаКлиенте", ВремяВызова, ИмяМодуля, СтрокаВычислить, "Документ_Шаблон"));
		НаборЗаписей.Добавить(МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("CALL", "Документ_Шаблон.СбисВычислитьФункциюНаКлиенте", ВремяВызова, ДанныеДляЛога, СтрокаВычислить, ИмяМодуля));
		МодульОбъектаКлиент().СохранитьОтладочныеДанныеСБИС(Новый Структура("Log", НаборЗаписей));
	...
	Исключение
		...
		КонецЕсли;
		ОшибкаВызова = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), НазваниеКратко() + "_СбисВычислитьФункциюНаКлиенте", КодОшибки);
		МодульОбъектаКлиент().СбисСообщить(ОшибкаВызова);
		
	КонецПопытки;
	
	Если МодульОбъектаКлиент().ПараметрSaby("РежимОтладки") Тогда
		
		Если ОшибкаВызова = Неопределено Тогда
			
			ЗаписьВЛог = МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("RESULT", "Документ_Шаблон.СбисВычислитьФункциюНаКлиенте",,Результат, СтрокаВычислить, ИмяМодуля);

		Иначе
			
			ЗаписьВЛог = МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("ERROR", "Документ_Шаблон.СбисВычислитьФункциюНаКлиенте",,ОшибкаВызова, СтрокаВычислить, ИмяМодуля);
	...
	Если	НовыйФормат Тогда
		...
		// Т.к. функции пишутся хз как и контекст может переехать целиком (!) на сервер и обратно, то надо восстановить объект
		// с учетом его вероятного изменения на сервере в ВФК
		Если ОбъектФункцийВложен Тогда
			
			СбисКонтекст.ДанныеРезультат[КлючОбъектДляФункций] = Контекст[КлючОбъектДляФункций];
			
		Иначе
			
			СбисКонтекст.ДанныеРезультат = Контекст[КлючОбъектДляФункций];
...
&НаКлиенте
Функция РассчитатьПараметрИни(Кэш, ПараметрИмя="", СбисПоле=Неопределено, СбисКонтекстВходящий, ДопПараметры) Экспорт
	...
	СбисКонтекст		= Кэш.ОбщиеФункции.СкопироватьОбъектСПараметрамиКлиент(СбисКонтекстВходящий,,Ложь);	
	...
	СбисКонтекст.Вставить("ТекущееПолеИни",		СбисПолеРасчет);
	СбисКонтекст.Вставить("ТекущееПолеКлюч",	ПараметрИмя);
	...
	//Если источник не данные, то поставим отбор и приведем тип. Для данных Отбор применяется сразу, при расчёте.
	Если Не ПараметрыРасчета.Источник = "Данные" Тогда
		...
		Если			СбисПолеРасчет.Свойство("Тип")
			И	(	Не	ДопПараметры.Свойство("ПриводитьТип")
				Или		ДопПараметры.ПриводитьТип)	Тогда
			
			ЗначениеРеквизита = МодульОбъектаКлиент().ПривестиТипСбис(ЗначениеРеквизита, СбисПолеРасчет.Тип);
...
Функция РассчитатьПоЗначениюПараметра_ДляСервера(Кэш, СбисПараметр, СбисПоле, СбисКонтекст, ДопПараметры)
	...
	КонтекстДляСервера.ДанныеИсточник.Вставить("СтруктураФайла",		СбисКонтекст.ДанныеИсточник);
 	КонтекстДляСервера.ДанныеИсточник.Вставить	(СбисКонтекст.ИмяОбъекта, СбисКонтекст.ДанныеРезультат[СбисКонтекст.ИмяОбъекта]);
	КонтекстДляСервера.ДанныеРезультат.Вставить	(СбисКонтекст.ИмяОбъекта, СбисКонтекст.ДанныеРезультат[СбисКонтекст.ИмяОбъекта]);
	...
	КонецЕсли;
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		ЗначениеРеквизита = РассчитатьЗначениеSABY(СбисПараметр, КонтекстДляСервера);
	#Иначе
...
Функция РассчитатьПоЗначениюПараметра_ДляСервераВызов(СбисПараметр, Знач СбисКонтекст)
	...
	Возврат МодульОбъектаСервер().РассчитатьЗначениеSABY(СбисПараметр, СбисКонтекст);
...
Функция РассчитатьПоЗначениюПараметра_Клиент(Кэш, СбисПараметр, СбисПоле, СбисКонтекст, ДопПараметры)
	...
	Попытка
		Если		СбисПервыйСимвол = "'"	Тогда
			...
			Если	СбисПоле = Неопределено
				ИЛИ (ДопПараметры.Источник <> "Значение"
					...
					И	Не СбисПоле.Свойство("ВычислитьНаКлиенте")
					И 	НЕ (СбисПоле.Свойство("Тип") И СбисПоле.Тип = "Запрос")) Тогда
		...
		ИначеЕсли	СбисПервыйСимвол = "[" Тогда
			...
			ИначеЕсли Найти(СбисПараметр, ".") Тогда
				
				// Есть точка, это обращение к данным
				ЗначениеПосчитано = СбисВычислитьОбращениеКДаннымНаКлиенте(Кэш, СбисПараметр, СбисПоле, СбисКонтекст, ДопПараметры);
				
			Иначе
				
				// Это запись формата [УзелИниНаКоторыйСсылка]
				СбисПараметрТест		= Сред(СбисПараметр, 2, СтрДлина(СбисПараметр) - 2);
				Если ЭтоСсылкаНаУзелИни(Кэш, СбисПараметрТест, УзелИниЗначение, СбисКонтекст, ДопПараметры) Тогда
					
					ДопПараметры.Источник	= "ПоСсылкеНаУзел";
					ЗначениеПосчитано		= РассчитатьПараметрИни(Кэш, СбисПараметрТест, УзелИниЗначение, СбисКонтекст, ДопПараметры);
					ДопПараметры.Вставить("Посчитано", Истина);
					
				Иначе
					
					// Невалидная запись - переадресация на узел, которого нет.
...
Функция РассчитатьПоДаннымПараметра(Кэш, СбисПараметр, СбисПоле, СбисКонтекст, ДопПараметры)
	...
	Если СбисПоле.Свойство("Отбор") Тогда
		...
		ПараметрыРасчётаОтбор	= Новый Структура("Источник", "Отбор");
		АргументРасчитать		= Новый Структура("Значение, РассчитанноеЗначение", СбисПоле, РезультатРасчета);
		ПараметрыОтбора			= Новый Структура("Кэш, Контекст, ДополнительныеПараметры", Кэш, СбисКонтекст, ДопПараметры);
		РезультатРасчета		= ВычислитьОтборНаКлиенте(АргументРасчитать, ПараметрыОтбора);
		
	КонецЕсли;
	
	// Данные приводят только простые типы
	ПриводитьТип =		СбисПоле.Свойство("Тип")
				И (		СбисПоле.Тип = "Число"
					Или СбисПоле.Тип = "Булево"
					Или СбисПоле.Тип = "Дата");
	Если ПриводитьТип Тогда
		
		РезультатРасчета = МодульОбъектаКлиент().ПривестиТипСбис(РезультатРасчета, СбисПоле.Тип);
...
Функция РассчитатьЗначениеНаСервере(СбисПараметр, СбисКонтекст) Экспорт
	...
	КонтекстДляСервера.ДанныеРезультат.Вставить	(КонтекстДляСервера.ИмяОбъекта, СбисКонтекст.Документ);
 	КонтекстДляСервера.ДанныеИсточник.Вставить	(КонтекстДляСервера.ИмяОбъекта, СбисКонтекст.Документ);
	...
	КонецЕсли;
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		Возврат РассчитатьЗначениеSABY(СбисПараметр, КонтекстДляСервера);
	#Иначе
...
Функция СоздатьНаборСвойствПоМассиву(РассчитанныйНаборСвойств, МассивКлючей, ИниКонфигурация, Отказ) Экспорт
	Перем ОбъектПерезаполнитьСсылка;
	...
	ИмяСправочникаСвойствОбъектов = ИмяСправочникаСвойствОбъектов(ИниКонфигурация);
	...
	Для Каждого Ключ ИЗ МассивКлючей Цикл
		...
		Если	СтруктураОбъекта.Свойство("ОбъектПерезаполнитьСсылка", ОбъектПерезаполнитьСсылка) И ОбъектПерезаполнитьСсылка <> Неопределено
				Или Не ЗначениеЗаполнено(ДопСведение) Тогда
			ДополнительныеПараметры = Новый Структура();
			ДополнительныеПараметры.Вставить("ОбъектПерезаполнитьСсылка", ОбъектПерезаполнитьСсылка);
			...
			Результат = МодульОбъектаСервер().СоздатьОбъект1СИзСтруктурыНаСервере(СтруктураОбъекта, ДополнительныеПараметры, Отказ);
			
			Если СтруктураОбъекта.Свойство("мЗначенияСвойства") Тогда
				
				СоздатьЗначениеВыбораДляДопСвойства(СтруктураОбъекта.мЗначенияСвойства, ИмяСправочникаСвойствОбъектов, Результат.Ссылка);
		...
		Если ИмяНаборыДополнительныхРеквизитовИСведений <> Неопределено 
			...
			Если НЕ ЗначениеЗаполнено(НаборДопСведений) Тогда
				...
				Результат = МодульОбъектаСервер().СоздатьОбъект1СИзСтруктурыНаСервере(СтруктураСправочника, ДополнительныеПараметры, Отказ);
...
&НаСервере
Функция СоздатьЗначениеВыбораДляДопСвойства(СтруктураЗначений, ИмяСправочникаСвойствОбъектов, Владелец)
	
	Для Каждого Элемент Из СтруктураЗначений Цикл

		ЗначениеСпр = Справочники[ИмяСправочникаСвойствОбъектов].СоздатьЭлемент();
		ЗначениеСпр.Владелец = Владелец;
		ЗначениеСпр.Наименование = Элемент.Значение.Свойство_Значения;
