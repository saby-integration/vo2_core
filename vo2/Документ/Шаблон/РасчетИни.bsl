...
Функция ЗаполнитьДанныеДокумента1С(Кэш, ини, СтруктураФайла, МассивОснований, СоставПакета, Вложение, Документ1С = Неопределено, ДополнительныеДанные = Неопределено) Экспорт
	...
	Если Не Документ1С = Неопределено Тогда
		...
		// Структура с названиями полей, как у создаваемого документа 1С
		...
		РедактироватьТабЧасть = Кэш.ОбщиеФункции.РассчитатьЗначение("РедактироватьТабЧасть", Ини) = Истина
							Или	(	Вложение.Свойство("Параметры")
								И	Вложение.Параметры.РедактироватьСуществующий);
	...
	Если СтруктураДокОбъект = Неопределено Тогда
		...
		// Структура с названиями полей, как у создаваемого документа 1С
		СтруктураДокОбъект		= Новый Структура;
		РедактироватьТабЧасть	= Ложь;//Документ 1С не определен.
	...
	//ТипОбъекта = МодульОбъектаКлиент().ВложениеСБИС_Получить(Вложение, "ТипОбъекта", Новый ФиксированнаяСтруктура("СоставПакета", СоставПакета));
...
Функция ЗаполнитьРучныеИзмененияДокумента1С(Кэш, Вложение, СтруктураДокОбъект) Экспорт
	Перем РедактируемыеПоля;
	
	РучныеПараметрыВложения = МодульОбъектаКлиент().ВложениеСБИС_Получить(Вложение, "Просмотр");
	
	Если Не РучныеПараметрыВложения = Неопределено
		И	РучныеПараметрыВложения.Свойство("РедактируемыеПоля", РедактируемыеПоля) Тогда
		Для Каждого РучноеПоле Из РедактируемыеПоля Цикл
			...
			СтруктураДокОбъект.Вставить(РучноеПоле.Значение.Путь, РучноеПоле.Значение.Значение);
			
		КонецЦикла;
		
	ИначеЕсли Вложение.Свойство("РучноеИзменение") И Вложение.РучноеИзменение = Истина Тогда
