	...
	Если Не ПараметрыРасчета.Источник = "Данные" Тогда
		...
		Если СбисПолеРасчет.Свойство("Тип") Тогда
			...
			Исключение
				...
				ОшибкаПреобразованитя = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "Документ_Шаблон.РассчитатьПараметрИни", 612,,,СбисПолеРасчет);
				ВызватьИсключение МодульОбъектаКлиент().СбисИсключение_ВСтроку(ОшибкаПреобразованитя);
