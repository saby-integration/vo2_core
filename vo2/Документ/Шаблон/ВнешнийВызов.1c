...
Функция СоздатьДокумент(Кэш, Вложение, Ини, СоставПакета, МассивОснований, Документ1С = Неопределено) Экспорт
	...
	Если СписокДокументов.Количество()=1 Тогда
		...
		// Если документ уже загружен в рамках текущего пакета, то надо дополнить его данными текущего вложения (например, загружаем акт и накладную в одно поступление)
	...
	Для каждого ОснованиеСтруктура Из МассивОснований Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Если НЕ ДокументНайденВМассивеОснований Тогда // если документ уже загружен по текущему пакету, то не перепривязываем его к новому вложению (иначе будут проблемы при повторной загрузке)
		...
		фрм.ЗаписатьПараметрыДокументаСБИС(СтруктураСвойств, Документ1С, Кэш.Ини.Конфигурация, ГлавноеОкно.Кэш.Парам.КаталогНастроек, ДопПараметрыАккаунта);
