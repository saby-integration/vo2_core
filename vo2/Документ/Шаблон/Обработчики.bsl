...
&НаКлиенте
Функция Обработчик_мСторона_НайтиСсылкуСтороны(мСторонаИни, КонтекстРасчета, ДопПараметры)
	Перем ФункцияРасчетаНаСтороне;
	
	Кэш = МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш();
	
	//Возьмём шапку, дополним ТабЧастью. Это наш контекст строки
	КонтекстИниСтроки = Новый Структура(КонтекстРасчета.Ключи.Шапка);
	ЗаполнитьЗначенияСвойств(КонтекстИниСтроки, КонтекстРасчета.Ини);
	Кэш.ОбщиеФункции.СбисСкопироватьСтруктуруНаКлиенте(КонтекстИниСтроки, мСторонаИни);
	
	//Заполним в контекст Сторона данные стороны, чтобы правильно заполнить значение.
	КонтекстИниСтроки.Сторона.Вставить("РассчитанноеЗначение", мСторонаИни);
	
	КонтекстРасчета.Вставить("Ини", КонтекстИниСтроки);
	КонтекстРасчета.ТекущееПолеКлюч	= "Сторона";
	КонтекстРасчета.ТекущееПолеИни	= КонтекстИниСтроки[КонтекстРасчета.ТекущееПолеКлюч];
	Если ДопПараметры.Свойство("Ограничения") Тогда
		
		КонтекстРасчета.ТекущееПолеИни.Вставить("Ограничения", ДопПараметры.Ограничения);
		
	КонецЕсли;

	ДопПараметрыРасчета	= Новый Структура("ВызовСервера,БратьПосчитанное,ДопАргументыДляФункции", Истина, Ложь);
	ДопПараметрыРасчета.ДопАргументыДляФункции = Новый Структура("Сторона", мСторонаИни);

	ЗначениеПараметра	= РассчитатьПараметрИни(Кэш,
												КонтекстРасчета.ТекущееПолеКлюч,
												КонтекстРасчета.ТекущееПолеИни,
												КонтекстРасчета,
												ДопПараметрыРасчета);

	Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда  // структура в формате СБИС  
		
		ФункцияПоискСтороны = "{ОпределитьСторонуДокумента}";
		Если	(	КонтекстРасчета.ТекущееПолеИни.Свойство("ВычислитьНаКлиенте",	ФункцияРасчетаНаСтороне)
				Или	КонтекстРасчета.ТекущееПолеИни.Свойство("Вычислить",			ФункцияРасчетаНаСтороне))
			И	ФункцияРасчетаНаСтороне = ФункцияПоискСтороны Тогда
			
			ЗначениеПараметра = Неопределено;
			
		Иначе
			
			// Поиск был не ОпределитьСторонуДокумента. Проверить сторону новой функцией
			ЗначениеПараметра = СбисВычислитьФункциюНаКлиенте(Кэш, ФункцияПоискСтороны, КонтекстРасчета, ДопПараметрыРасчета);
	...
	Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда
		...
		// В качестве стороны все еще структура, значит ссылку так и не нашли
		ЗначениеПараметра = Неопределено;
...
Процедура Обработчик_ВыборСервер(Аргумент, ПараметрыОбработчика)
	...
	КлючиПеренестиКакЕсть.Добавить("РассчитанноеЗначение");
	
	КонтекстОбъединения			= Новый ФиксированнаяСтруктура("КлючиПеренестиКакЕсть", КлючиПеренестиКакЕсть);
...
Процедура Обработчик_мРасхожденияОбъектов(Аргумент, ПараметрыОбработчика) Экспорт
	...
	Для Каждого Расхождение Из Аргумент.Значение Цикл  
		...
		КонтекстЗаполнения		= Новый Структура("Ини, ДанныеИсточник, ДанныеРезультат", Ини, СбисКонтекст.ДанныеИсточник, СбисКонтекст.ДанныеРезультат);
		...
		Для Каждого ЭлементОбъекта Из СтруктураОбъекта Цикл
			Если	ТипЗнч(ЭлементОбъекта.Значение) = Тип("Массив")
				Или	Найти("ОписаниеОшибки,ОписаниеКратко,ЗначениеСБИС,Значение1С", ЭлементОбъекта.Ключ) Тогда
...
Процедура Обработчик_мТаблДокДляРасхождений(Аргумент, ПараметрыОбработчика) Экспорт
	...
	ТекущееЗначение = Аргумент.Значение;
	КлючТаблДок		= "ТаблДокРасхождения";

	СопоставлениеДанныеКлючиИни = Новый Соответствие;
   	СопоставлениеДанныеКлючиИни.Вставить(КлючТаблДок, "ДанныеРезультатЛокальные");
	...
	Для Каждого ТабЧасть Из ТекущееЗначение Цикл
		...
		мТаблДокРассчитанноеЗначение.Вставить(ТабЧасть.Ключ, ТабЧастьРассчитанноеЗначение);
		//Определим узел с описанием ТЧ
		Если ТабЧасть.Значение.Свойство(КлючТаблДок, УзелТаблДок) Тогда
			//Создаём копию, чтобы рассчитанное значение данных ТЧ не попадало в расчитанную ини, т.к. иначе замедлим передачу данных на сервер
			УзелТаблДок = Кэш.ОбщиеФункции.СкопироватьОбъектСПараметрамиКлиент(УзелТаблДок,, Ложь);
		Иначе
			УзелТаблДок = Новый Структура;
		...
		Если Не УзелТаблДок.Свойство("Данные") Тогда
			...
			УзелТаблДок.Вставить("Данные", "Файл.Документ.ТаблДок.СтрТабл");
		КонецЕсли;
		ПутьКТаблДок	= СтрПолучитьСтроку(СтрЗаменить(УзелТаблДок.Значение, ".", Символы.ПС), 1);
		ПутьКТаблДок	= СтрЗаменить(СтрЗаменить(ПутьКТаблДок, "]", ""), "[", "");
		
		ТаблДок			= РассчитатьПараметрИни(Кэш, КлючТаблДок, УзелТаблДок, СбисКонтекст, Новый Структура("ЗаполнитьДанные", Ложь));
		Если Не ЗначениеЗаполнено(ТаблДок) Тогда
			//При заполнении пустого документа
			Продолжить;
		КонецЕсли;
		...
		СбисКонтекстРассчета= Кэш.ОбщиеФункции.СкопироватьОбъектСПараметрамиКлиент(СбисКонтекст,			, Ложь);
		СбисКонтекстРассчета.Вставить("Переменные",					ПеременныеДляСтроки);		
		СбисКонтекстРассчета.Вставить("СопоставлениеДанныеКлючиИни",СопоставлениеДанныеКлючиИни);		
		...
		Для Каждого Строка Из ТаблДок Цикл
			...
			//	Продолжить;
			//Иначе
			Если	Не СбисКонтекстРассчета.ДанныеРезультат[ПутьКТаблДок].Свойство(ИмяТЧ) Тогда
				СбисКонтекстРассчета.ДанныеРезультат[ПутьКТаблДок].Вставить(ИмяТЧ, Новый массив);
			КонецЕсли;
			ТабЧастьРассчитанноеЗначение.Добавить(КонтекстИниСтроки);
			...
			Для Каждого Элемент Из ТабЧастьЗначение Цикл
				Обработчик_УзелИни(Новый Структура("Ключ, Значение", Элемент.Ключ, КонтекстИниСтроки[Элемент.Ключ]), АргументыВызова);
			КонецЦикла;
			СбисКонтекст.ДанныеРезультат[ПутьКТаблДок][ИмяТЧ].Добавить(СбисКонтекстРассчета.ДанныеРезультатЛокальные);
