...
Процедура Обработчик_мСторона(Аргумент, ПараметрыОбработчика) Экспорт
	...
	Если ЗначениеЗаполнено(ОрганизацияРоль) И ТекущееЗначение.Свойство(ОрганизацияРоль) Тогда
		...
		ЗначениеПараметра = Обработчик_мСторона_НайтиСсылкуСтороны(Сторона, СбисКонтекстРассчета, ДопПараметрыРасчета);
		...
		Если	Кэш.Ини.Конфигурация.Свойство("ЗапросКонтрагента")
			...
			ДопПараметрыРасчета.Вставить("Ограничения", Новый Структура("Организация", ЗначениеПараметра));
	...
	Для Каждого Сторона Из ТекущееЗначение Цикл
		...
		ЗначениеПараметра = Обработчик_мСторона_НайтиСсылкуСтороны(Сторона.Значение, СбисКонтекстРассчета, ДопПараметрыРасчета);
...
Функция Обработчик_мСторона_НайтиСсылкуСтороны(мСторонаИни, КонтекстРасчета, ДопПараметры)
	...
	ДопПараметрыРасчета	= Новый Структура("ВызовСервера, БратьПосчитанное, ПриводитьТип, ДопАргументыДляФункции", Истина, Ложь, Ложь);
	...
	Если ТипЗнч(ЗначениеПараметра) = Тип("Структура") Тогда  // структура в формате СБИС  
		...
		Иначе
			...
			ДопПараметрыРасчета.Удалить("ПриводитьТип");
