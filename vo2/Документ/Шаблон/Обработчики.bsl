...
Процедура Обработчик_УзелИниСервер(Аргумент, ПараметрыРассчитать)
	...
	ИначеЕсли 	ЕстьПараметрВычислитьНаСервере(Аргумент.Значение, лПараметрВычислить) Тогда
		РезультатРасчета = МодульОбъектаСервер().РассчитатьЗначениеSABY(лПараметрВычислить, СбисКонтекст);
...
Процедура Обработчик_мТаблДок_ЗаполнитьВТаблицу_Сервер(Аргумент, СбисКонтекст)
	...
	Если ТекущийУзелИни.Свойство("ПоискСтроки", КлючПоискаСтроки) Тогда
		...
		Для НомерСтроки = 1 По СтрЧислоСтрок(КлючиПоискаРазбить) Цикл
			...
			ЗначениеДляСтруктуры	= МодульОбъектаСервер.РассчитатьЗначениеSABY(КлючПоиска, СбисКонтекст);
...
Процедура Обработчик_ЭлементСложногоУзлаСервер(Аргумент, ПараметрыОбработчика)
	...
	Для Каждого Элемент Из СбисПоле Цикл
		...
		ИначеЕсли 	ЕстьПараметрВычислитьНаСервере(Элемент.Значение, лПараметрВычислить) Тогда
			...
			РассчитанноеЗначение = МодульОбъектаСервер().РассчитатьЗначениеSABY(лПараметрВычислить, СбисКонтекст);
