...
&НаКлиенте
Функция ПрочитатьДокумент(Кэш, Контекст, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого Файл Из Контекст.ДокументДанные.мФайл Цикл
		...
		Если фрм = Ложь Тогда
			...
			ТекстОшибки				= "Для документа "+ Контекст.Документ + " не найдена форма формирования электронного документа в формате "+Файл_Формат+", версии "+Файл.Файл_ВерсияФормата;
			ОшибкаЧТенияДокумента	= МодульОбъектаКлиент().НовыйСбисИсключение(, "Документ_Шаблон.ПрочитатьДокумент", 715202, , ТекстОшибки);
			Контекст.СоставПакета.Вставить("Ошибка", ОшибкаЧТенияДокумента);
			Возврат Ложь;
...
Функция ЗагрузитьДокументыВ1С(Кэш, СписокОтмеченныхДокументов, ДополнительныеПараметры=Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры)
		И ДополнительныеПараметры.Свойство("ИзменениеВДокумент") Тогда
		
		ПараметрыЗагрузки.Вставить("ИзменениеВДокумент", ДополнительныеПараметры.ИзменениеВДокумент); 
...
Функция СоздатьДокумент(Кэш, Вложение, Ини, СоставПакета, МассивОснований, Документ1С = Неопределено) Экспорт
	...
	Если СохранитьСопоставление Тогда
		РежимЗаписиСвойств = "Сопоставление";
	Иначе
		РежимЗаписиСвойств = "ЗаписьСвойств";
	КонецЕсли;
	...
	ПараметрыЗаписать = Новый Структура("Режим, СоставПакета, Документ1С", РежимЗаписиСвойств, СоставПакета, Документ1С);
	...
	ПараметрыМаппинга = Новый Структура("СоставПакета, Документ1С", СоставПакета, Документ1С);
	МодульОбъектаКлиент().ПодготовитьЗаписиМаппингПоДокументу(ПараметрыМаппинга);
...
Функция СбисЗаполнитьСтруктуруОбъекта(ОписаниеОбъекта, ПараметрыЗаполнения) Экспорт
	...
	КонтекстДляКлиента = Новый Структура("Переменные, Ини, ДанныеИсточник, ДанныеРезультат, Вложение, СоставПакета", Новый Структура);
	ЗаполнитьЗначенияСвойств(КонтекстДляКлиента, СбисКонтекст);
	
	Если КонтекстДляКлиента.ДанныеРезультат = Неопределено Тогда 
		
		КонтекстДляКлиента.Вставить("ДанныеРезультат",	Новый Структура);
	...
	Если СбисКонтекст.Свойство("РучныеПравки") Тогда
		
		КонтекстДляКлиента.Вставить("РучныеПравки",	СбисКонтекст.РучныеПравки);
		
	КонецЕсли;
	
	КонтекстДляКлиента.Вставить("ИмяОбъекта",		ОписаниеОбъекта.Ключ);
	КонтекстДляКлиента.ДанныеРезультат.Вставить(ОписаниеОбъекта.Ключ, Новый Структура);
...
Функция ЗаполнитьДаннымиСтруктурыФайлаСтруктуруДокумента(Кэш, СтруктураФайла, Ини, ДокОбъект, МассивОснований, СоставПакета, Вложение, ДопПараметры=Неопределено) Экспорт
	...
	Если Вложение.Свойство("КлассыСопоставленияНоменклатур") Тогда
		 ...
		 МодульОбъектаКлиент().ВложениеСБИС_Обновить(СтруктураФайла, Новый Структура("ТабличныеДанные", Вложение.ТаблДок));	
	...
	Если МодульОбъектаКлиент().ПараметрSaby("РежимОтладки") Тогда
		...
		ИдЗаписи	= МодульОбъектаКлиент().ВложениеСБИС_Получить(Вложение, "Название");

		НаборЗаписей = Новый Массив;
		НаборЗаписей.Добавить(МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("CALL",		"ЗаполнитьДаннымиСтруктурыФайлаСтруктуруДокумента",,, ИдЗаписи, "Документ_Шаблон"));
		НаборЗаписей.Добавить(МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("CLIENT",	"ЗаполнитьДаннымиСтруктурыФайлаСтруктуруДокумента",,КонтекстДляКлиента, ИдЗаписи, "Документ_Шаблон"));
	...
	Если МодульОбъектаКлиент().ПараметрSaby("РежимОтладки") Тогда
		...
		ЗаписьВЛог = МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("SERVER", "ЗаполнитьДаннымиСтруктурыФайлаСтруктуруДокумента",,КонтекстДляСервера, ИдЗаписи, "Документ_Шаблон");
	...
	Если МодульОбъектаКлиент().ПараметрSaby("РежимОтладки") Тогда
		...
		ЗаписьВЛог = МодульОбъектаКлиент().НовыйЗаписьЛогаSABY("RESULT", "ЗаполнитьДаннымиСтруктурыФайлаСтруктуруДокумента",, ДокОбъект, ИдЗаписи, "Документ_Шаблон");
		МодульОбъектаКлиент().СохранитьОтладочныеДанныеСБИС(Новый Структура("Log", ЗаписьВЛог));
