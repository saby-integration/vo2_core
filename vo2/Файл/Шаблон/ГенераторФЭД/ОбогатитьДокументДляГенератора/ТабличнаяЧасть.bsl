...
&НаКлиенте
Процедура ЗаполнитьУвеличениеУменьшениеИтоги(ТекСтрока, ИтогиСоответствие, ТекДокументБезНДС)
	
	Значение = ИтогиСоответствие.Получить("СтоимУвел");
	Если Не Значение = Неопределено Тогда
		ТекСтрока.Вставить("СуммаБезНДСУвел", Формат(Значение, "ЧРД=.; ЧН=0.00; ЧГ="));  
	Иначе
		ТекСтрока.Вставить("СуммаБезНДСУвел", Формат(0, "ЧРД=.; ЧН=0.00; ЧГ="));
	КонецЕсли;
	...
	Иначе
		ТекСтрока.Вставить("СуммаБезНДСУм", Формат(0, "ЧРД=.; ЧН=0.00; ЧГ="));	
	КонецЕсли;
	...
	Иначе
		ТекСтрока.Вставить("СуммаУвел", Формат(0, "ЧРД=.; ЧН=0.00; ЧГ="));
	КонецЕсли;
	...
	Иначе
		ТекСтрока.Вставить("СуммаУм", Формат(0, "ЧРД=.; ЧН=0.00; ЧГ=")); 	
	КонецЕсли;
	...
	ИначеЕсли ТекСтрока.Свойство("СуммаУвел")
		И ТекДокументБезНДС Тогда
		ТекСтрока.Вставить("БезНДСУвел", "без НДС"); 
	Иначе
		ТекСтрока.Вставить("СуммаНДСУвел", Формат(0, "ЧРД=.; ЧН=0.00; ЧГ="));
		ТекСтрока.Вставить("БезНДСУвел", "");
	КонецЕсли;
	...
	ИначеЕсли ТекСтрока.Свойство("СуммаУм")
		И ТекДокументБезНДС Тогда
		ТекСтрока.Вставить("БезНДСУм", "без НДС"); 
	Иначе
		ТекСтрока.Вставить("СуммаНДСУм", Формат(0, "ЧРД=.; ЧН=0.00; ЧГ="));
		ТекСтрока.Вставить("БезНДСУм", "");
	КонецЕсли;
...
&НаКлиенте
Процедура ПрослежТоварРасхождение(СведПрослеж, ПредСведПрослеж, СтрокаТаблДок)
	...
	Для Каждого ТекСтрока ИЗ СведПрослеж Цикл
		...
		СтоимостьБезНДС = ОписаниеТипаЧисло.ПривестиЗначение(ТекСтрока.КолВЕдПрослеж)
						* ОписаниеТипаЧисло.ПривестиЗначение(СтрокаТаблДок.Цена);
		
		Марка = Новый Структура;
		Марка.Вставить("ЕдИзмПрослеж",			ТекСтрока.ЕдИзмПрослеж);
		Марка.Вставить("НаимЕдИзмПрослеж",		ТекСтрока.НаимЕдИзмПрослеж);
		Марка.Вставить("НомТовПрослеж",			ТекСтрока.НомТовПрослеж);
		Марка.Вставить("КолВЕдПрослеж",			ТекСтрока.КолВЕдПрослеж);
		Марка.Вставить("СтТовБезНДСПрослеж",	Формат(СтоимостьБезНДС, "ЧЦ=17; ЧДЦ=2; ЧН=0,00; ЧГ="));
	...
	Для Каждого ТекСтрока Из ПредСведПрослеж Цикл
		...
		Если ИндексСтроки <> Неопределено Тогда
			...
			СтоимостьБезНДС = ОписаниеТипаЧисло.ПривестиЗначение(ТекСтрока.КолВЕдПрослеж)
							* ОписаниеТипаЧисло.ПривестиЗначение(СтрокаТаблДок.ПредСтрТабл.Цена);
			
			Дельта = ОписаниеТипаЧисло.ПривестиЗначение(ТаблПрослеж[ИндексСтроки].КолВЕдПрослеж)
					- ОписаниеТипаЧисло.ПривестиЗначение(ТекСтрока.КолВЕдПрослеж);
			
			ДельтаСумм = ОписаниеТипаЧисло.ПривестиЗначение(ТаблПрослеж[ИндексСтроки].СтТовБезНДСПрослеж)
						- СтоимостьБезНДС;
			
			Если Дельта > 0 Тогда
				ТаблПрослеж[ИндексСтроки].Вставить("УвелКолВЕдПрослеж", Формат(Дельта, "ЧЦ=26; ЧДЦ=11; ЧН=0,00000000000; ЧГ="));
			Иначе
				ТаблПрослеж[ИндексСтроки].Вставить("УмКолВЕдПрослеж", Формат(-Дельта, "ЧЦ=26; ЧДЦ=11; ЧН=0,00000000000; ЧГ="));
			КонецЕсли;
			
			Если ДельтаСумм > 0 Тогда
				ТаблПрослеж[ИндексСтроки].Вставить("УвелСтТовБезНДСПрослеж", Формат(ДельтаСумм, "ЧЦ=17; ЧДЦ=2; ЧН=0,00; ЧГ="));
			Иначе
				ТаблПрослеж[ИндексСтроки].Вставить("УмСтТовБезНДСПрослеж", Формат(-ДельтаСумм, "ЧЦ=17; ЧДЦ=2; ЧН=0,00; ЧГ="));
			КонецЕсли;
			
			ТаблПрослеж[ИндексСтроки].Вставить("ПредКолВЕдПрослеж", ТекСтрока.КолВЕдПрослеж);
			ТаблПрослеж[ИндексСтроки].Вставить("ПредСтТовБезНДСПрослеж", Формат(СтоимостьБезНДС, "ЧЦ=17; ЧДЦ=2; ЧН=0,00; ЧГ="));
		Иначе
			СтоимостьБезНДС = ОписаниеТипаЧисло.ПривестиЗначение(ТекСтрока.КолВЕдПрослеж)
							* ОписаниеТипаЧисло.ПривестиЗначение(СтрокаТаблДок.ПредСтрТабл.Цена);
			...
			Марка.Вставить("КолВЕдПрослеж",		0);
			Марка.Вставить("УмКолВЕдПрослеж",	ТекСтрока.КолВЕдПрослеж);
			Марка.Вставить("ПредКолВЕдПрослеж",	ТекСтрока.КолВЕдПрослеж);
			
			Марка.Вставить("СтТовБезНДСПрослеж",		0);
			Марка.Вставить("УмСтТовБезНДСПрослеж",		Формат(СтоимостьБезНДС, "ЧЦ=17; ЧДЦ=2; ЧН=0,00; ЧГ="));
			Марка.Вставить("ПредСтТовБезНДСПрослеж",	Формат(СтоимостьБезНДС, "ЧЦ=17; ЧДЦ=2; ЧН=0,00; ЧГ="));
