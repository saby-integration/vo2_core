...
Процедура ЗаполнитьПоДокументОснование(Док, Кэш) Экспорт
	...
	МассивСторонДляПатча = Новый Массив;
	МассивСторонДляПатча.Добавить("Получатель");
	МассивСторонДляПатча.Добавить("ГоловнойКонтрагент");
	МассивСторонДляПатча.Добавить("Грузополучатель");
	
	Для каждого ИмяСтороны Из МассивСторонДляПатча Цикл
	
		ИностраннаяОрганизация = ТекДокумент.Свойство(ИмяСтороны)
			И ТекДокумент[ИмяСтороны].Свойство("СвЮЛ")
			И ТекДокумент[ИмяСтороны].СвЮЛ.Свойство("КодСтраны")
			И ТекДокумент[ИмяСтороны].СвЮЛ.КодСтраны <> "643"
			И НЕ ТекДокумент[ИмяСтороны].СвЮЛ.Свойство("ИдСтат");
		
		Если ИностраннаяОрганизация Тогда
			ТекДокумент[ИмяСтороны].СвЮЛ.Вставить("ИдСтат", "ИО");
		КонецЕсли;
