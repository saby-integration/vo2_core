...
Процедура ЗаполнитьГенераторДаннымиВложения(Кэш, Генератор, Вложение, ДопПараметры)
	...
	ЗаполнитьПоИмениСтороны(Кэш, Генератор, Вложение, "ОтпускПроизвёл",		ДопПараметры);
...
Процедура ЗаполнитьОбщиеДанные(Кэш, Генератор, Вложение, ДопПараметры)
	...
	УстановитьЗначениеПоКлючу(Генератор, "СопрДокФХЖ",				ТекДокумент);
	...
	УстановитьЗначениеПоКлючу(Генератор, "НаимЭконСубСост",			ТекДокумент);
	...
	УстановитьЗначениеПоКлючу(Генератор, "СвСчФакт.СпОбстФДОП",		ТекДокумент, "СпОбстФДОП");
	УстановитьЗначениеПоКлючу(Генератор, "СвСчФакт.СпОбстФСЧФДОП",	ТекДокумент, "СпОбстФСЧФДОП");
	УстановитьЗначениеПоКлючу(Генератор, "СвСчФакт.СпОбстФСЧФ",		ТекДокумент, "СпОбстФСЧФ");  
	...
	Если ТекДокумент.Свойство("ДокПодтвОтгр") Тогда
		Если ТипЗнч(ТекДокумент.ДокПодтвОтгр) = Тип("Массив") Тогда
			УстановитьЗначениеПоКлючу(Генератор, "ДокПодтвОтгр", ТекДокумент);
		Иначе
	...
	Если ТекДокумент.Свойство("СодОпер") Тогда
		УстановитьЗначениеПоКлючу(Генератор, "СодОпер", ТекДокумент);
	ИначеЕсли Вложение.Свойство("Регламент") Тогда
	...
	УстановитьЗначениеПоКлючу(Генератор, "Основание", ТекДокумент);  
	УстановитьЗначениеПоКлючу(Генератор, "БезДокОснПер", ТекДокумент);  
	
	Если ТекДокумент.Свойство("РабОргПрод") Тогда	
		УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Должность",	ТекДокумент.РабОргПрод, "Должность");    
		УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Фамилия",		ТекДокумент.РабОргПрод, "Фамилия");
		УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Имя",			ТекДокумент.РабОргПрод, "Имя");
		УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Отчество",	ТекДокумент.РабОргПрод, "Отчество");
	КонецЕсли;
...
Процедура ЗаполнитьПоИмениСтороны(Кэш, Генератор, Вложение, ИмяСтороны, ДопПараметры) Экспорт
	...
	Если Данные.Свойство("СвФЛ")
		ИЛИ Данные.Свойство("СвИП") Тогда 
		УстановитьЗначениеПоКлючу(Генератор, ИмяСтороны + ".СокрНаим",	Данные, "Название");
	КонецЕсли;
	
	Если Данные.Свойство("ОтпустилГруз") Тогда
		Если НЕ Данные.РаботникИнойОрганизации Тогда
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Должность",	Данные.ОтпустилГруз, "Должность");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Фамилия", 	Данные.ОтпустилГруз, "Фамилия");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Имя", 		Данные.ОтпустилГруз, "Имя");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.РабОргПрод.Отчество", 	Данные.ОтпустилГруз, "Отчество");
		ИначеЕсли Данные.ОтпустилГруз.Свойство("Должность") И ЗначениеЗаполнено(Данные.ОтпустилГруз.Должность) Тогда
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.ИнЛицо.ПредОргПер.Должность",	Данные.ОтпустилГруз, "Должность");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.ИнЛицо.ПредОргПер.Фамилия", 	Данные.ОтпустилГруз, "Фамилия");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.ИнЛицо.ПредОргПер.Имя", 		Данные.ОтпустилГруз, "Имя");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.ИнЛицо.ПредОргПер.Отчество", 	Данные.ОтпустилГруз, "Отчество")
		Иначе
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.ИнЛицо.ФЛ.Фамилия", 	Данные.ОтпустилГруз, "Фамилия");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.ИнЛицо.ФЛ.Имя", 		Данные.ОтпустилГруз, "Имя");
			УстановитьЗначениеПоКлючу(Генератор, "ПередалГруз.СвЛицПерГруз.ИнЛицо.ФЛ.Отчество", 	Данные.ОтпустилГруз, "Отчество")
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьРегистрационныеДанныеЛица(Кэш, Генератор, Данные, ИмяСтороны)
	...
	Если Данные.Свойство("СвЮЛ", ЗначениеДанные)
		И ЗначениеДанные.Свойство("КодСтраны")
		И ЗначениеДанные.КодСтраны <> "643" Тогда
		
		КлючДанные = "СвИн";
		Генератор.Вставить(ИмяСтороны + ".СвЮл.Название",	"");
		Генератор.Вставить(ИмяСтороны + ".СвЮл.ИНН",		ИмяСтороны + "." + КлючДанные + ".Идентиф");
		Генератор.Вставить(ИмяСтороны + ".СвЮл.КодСтраны",	ИмяСтороны + "." + КлючДанные + ".КодСтр");
		Генератор.Вставить(ИмяСтороны + ".СвЮл.НаимСтраны",	ИмяСтороны + "." + КлючДанные + ".НаимСтран");
		Генератор.Вставить(ИмяСтороны + ".СвЮл.ИныеСвед",	"");
		Генератор.Вставить(ИмяСтороны + ".СвЮл.ИдСтат",		"");
	ИначеЕсли Данные.Свойство("СвЮЛ", ЗначениеДанные) Тогда
		КлючДанные = "СвЮл";
		Генератор.Вставить(ИмяСтороны + ".СвЮл.Название",	"");
	...
	ИначеЕсли Данные.Свойство("СвФЛ", ЗначениеДанные) Тогда
		...
		Генератор.Вставить(ИмяСтороны + ".СвФл.ДатаОГРН",	ИмяСтороны + "." + КлючДанные + ".ДатаОГРНИП");
		Генератор.Вставить(ИмяСтороны + ".СвФл.ОГРНИП",		"");
	...
	ИначеЕсли Данные.Свойство("СвИн", ЗначениеДанные) Тогда
		...
		Генератор.Вставить(ИмяСтороны + ".СвИн.ИНН",		ИмяСтороны + "." + КлючДанные + ".Идентиф");
		Генератор.Вставить(ИмяСтороны + ".СвИн.КодСтраны",	ИмяСтороны + "." + КлючДанные + ".КодСтр");
		Генератор.Вставить(ИмяСтороны + ".СвИн.НаимСтраны",	ИмяСтороны + "." + КлючДанные + ".НаимСтран");
		Генератор.Вставить(ИмяСтороны + ".СвИн.ИныеСвед",	"");
		Генератор.Вставить(ИмяСтороны + ".СвИн.ИдСтат",		"");
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьБанковскиеРеквизиты(Кэш, Генератор, ИмяРеквизита, Данные)
	...
	Если НЕ Данные.Свойство("БанкРекв") Тогда  
	...
	Если СтруктураБанкРекв.Свойство("РСчет")
		И ЗначениеЗаполнено(СтруктураБанкРекв.РСчет) Тогда  
		
		УстановитьЗначениеПоКлючу(Генератор, ИмяРеквизита + "НомерСчета",	СтруктураБанкРекв, "РСчет");
		
	КонецЕсли;  
	
	Если СтруктураБанкРекв.Свойство("БИК")
		И ЗначениеЗаполнено(СтруктураБанкРекв.БИК) Тогда 
	...
	Если СтруктураБанкРекв.Свойство("НаимБанк")
		И ЗначениеЗаполнено(СтруктураБанкРекв.НаимБанк) Тогда 
		
		УстановитьЗначениеПоКлючу(Генератор, ИмяРеквизита + "НаимБанк",		СтруктураБанкРекв, "НаимБанк");
		
	КонецЕсли; 
	
	Если СтруктураБанкРекв.Свойство("КСчет")
		И ЗначениеЗаполнено(СтруктураБанкРекв.КСчет) Тогда     
...
Процедура ЗаполнитьАдресныеДанные(Кэш, Генератор, ИмяСтороны, Данные)
	...
	КонецЕсли;
	ПараметрыФормат = Новый Структура;
	ПараметрыФормат.Вставить("ПрефиксСторона",	ИмяСтороны + ".Адрес.Адр");
	ПараметрыФормат.Вставить("ПрефиксПолей",	"Адр");
	ПараметрыФормат.Вставить("КодСтраны",		"КодСтр"); 
	ПараметрыФормат.Вставить("АдрИноТекст",		"АдрТекст");  
	ПараметрыФормат.Вставить("НаимСтран",		"НаимСтран"); 
	
	Если Данные.Свойство("GLN") Тогда
		Генератор.Вставить(ИмяСтороны + ".Адрес.ГЛНМеста", Данные.GLN);  
	КонецЕсли;
...
Процедура ЗаполнитьПоАдреснымДанным(Кэш, Генератор, ИсточникАдрес, ПараметрыФормат)   
	...
	// АдрИно формируется всегда, когда адрес в свободной форме, поэтому проверяем дополнительно по коду страны
	ИностранныйАдрес 	=	СтруктураАдреса.Свойство("АдрИно")
							И СтруктураАдреса.АдрИно.Свойство("КодСтр") И СтруктураАдреса.АдрИно.КодСтр <> "643";   
						
	УказанАдресСтрокой	= СтруктураАдреса.Свойство("АдрТекст", лАдресСтрокой)
							Или (НЕ ИностранныйАдрес 
								И СтруктураАдреса.Свойство("АдрИно") 
								И СтруктураАдреса.АдрИно.Свойство("АдрТекст", лАдресСтрокой));
	...
	Если СтруктураАдреса.Свойство("АдрРФ") Тогда
		...
		Генератор.Вставить(ПараметрыФормат.ПрефиксСторона + "РФ." + ПараметрыФормат.ПрефиксПолей + "НаимРегион",	"");
	...
	ИначеЕсли ИностранныйАдрес Тогда
		...
		Генератор.Вставить(ПараметрыФормат.ПрефиксСторона + "Ино." + ПараметрыФормат.НаимСтран,		"");
		
		ЗаполнитьГенераторПоСтруктуре(Кэш, Генератор, ПараметрыФормат.ПрефиксСторона + "Ино.", СтруктураАдреса.АдрИно);	
		
	ИначеЕсли УказанАдресСтрокой Тогда
		
		//Российский адрес в произвольной форме
		Генератор.Вставить(ПараметрыФормат.ПрефиксСторона + "Ино." + ПараметрыФормат.АдрИноТекст,	лАдресСтрокой);
		Генератор.Вставить(ПараметрыФормат.ПрефиксСторона + "Ино." + ПараметрыФормат.КодСтраны,		"643");
		Генератор.Вставить(ПараметрыФормат.ПрефиксСторона + "Ино." + ПараметрыФормат.НаимСтран,		"РОССИЯ");
...
Процедура ЗаполнитьТабличныеДанныеНомСредИдентТов(Аргумент, ДопПараметры)
	Перем ИсточникНомСредИдентТов, мИсточникИдентТов;
	
	Суффикс		= Аргумент.Суффикс;
	Источник	= Аргумент.ИсточникДанных;
	Приемник	= Аргумент.ДанныеРезультат;
	
	Если	Не Источник.Свойство("НомСредИдентТов", ИсточникНомСредИдентТов)
		Или	Не ТипЗнч(ИсточникНомСредИдентТов) = Тип("Массив") Тогда
	...
	Для Каждого ТекСтрока ИЗ ИсточникНомСредИдентТов Цикл
		...
		Если	ТекСтрока.Свойство("КолВедМарк", мИсточникИдентТов)
			И	ТипЗнч(мИсточникИдентТов) = Тип("Строка") Тогда
			НомСредИдентТов.Добавить(Новый Структура("КолВедМарк", мИсточникИдентТов));
		КонецЕсли; 
		
		Если	ТекСтрока.Свойство("ПрПартМарк", мИсточникИдентТов)
			И	ТипЗнч(мИсточникИдентТов) = Тип("Строка") Тогда
			НомСредИдентТов.Добавить(Новый Структура("ПрПартМарк", мИсточникИдентТов));
		КонецЕсли;
...
Процедура ЗаполнитьТабличныеДанныеRnptData(Аргумент, ДопПараметры)
	...
	Для Каждого ТекСтрока ИЗ иСведПРослеж Цикл
		...
		Если ТекСтрока.Свойство("СтТовБезНДСПрослеж") Тогда
			Марка.Вставить("SumCostWoVat", Окр(ТекСтрока.СтТовБезНДСПрослеж,2));
		КонецЕсли;
		...
		Если Корректировка Тогда
			...
			Если ТекСтрока.Свойство("УвелСтТовБезНДСПрослеж") Тогда
				Марка.Вставить("SumCostWoVatUp", ТекСтрока.УвелСтТовБезНДСПрослеж);
			КонецЕсли;
			
			Если ТекСтрока.Свойство("УмСтТовБезНДСПрослеж") Тогда
				Марка.Вставить("SumCostWoVatDown", ТекСтрока.УмСтТовБезНДСПрослеж);
			КонецЕсли;
			
			Если ТекСтрока.Свойство("ПредСтТовБезНДСПрослеж") Тогда
				Марка.Вставить("SumCostWoVatBefore", ТекСтрока.ПредСтТовБезНДСПрослеж);
			КонецЕсли;
...
Процедура ЗаполнитьТабличныеДанныеОбщие(Аргумент, ДопПараметры) Экспорт
	...
	УстановитьЗначениеПоКлючу(ТекДанные, "КодВидТов", 					ТекСтрока); 
	
	ЕстьХарактеристикаСтруктура = ТекСтрока.Свойство("Характеристика") 
								И ТипЗнч(ТекСтрока.Характеристика) = Тип("Структура")
								И ТекСтрока.Характеристика.Свойство("Значение")
		 						И ЗначениеЗаполнено(ТекСтрока.Характеристика.Значение);
	ЕстьХарактеристикаЗначение = ТекСтрока.Свойство("Характеристика")
								И ТипЗнч(ТекСтрока.Характеристика) <> Тип("Структура")
								И ЗначениеЗаполнено(ТекСтрока.Характеристика);
	
	Если ЕстьХарактеристикаСтруктура Тогда
		УстановитьЗначениеПоКлючу(ТекДанные, "КодПартии",	ТекСтрока, "Характеристика.Значение");
	ИначеЕсли ЕстьХарактеристикаЗначение Тогда
		УстановитьЗначениеПоКлючу(ТекДанные, "КодПартии",	ТекСтрока, "Характеристика");
	Иначе
		// Заглушка
	КонецЕсли;
...
Процедура ЗаполнитьТабличныеДанныеКорректировочные(Аргумент, ДопПараметры) Экспорт
	...
	Если МодульОбъектаКлиент().ПолучитьДанныеПоПути(Новый Структура("Путь, Данные", "ПредСтрТабл.НДС.Ставка", ТекСтрока)) = "НДС исчисляется налоговым агентом" Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Если Не (	ТекСтрока.Свойство("СуммаНДСУвел")
			И	ТекСтрока.СуммаНДСУвел = "0.00") Тогда

		УстановитьЗначениеПоКлючу(ТекДанные, "СуммаНДСУвел", ТекСтрока);  
	
	КонецЕсли;  
	
	Если Не (	ТекСтрока.Свойство("СуммаНДСУм")
			И	ТекСтрока.СуммаНДСУм = "0.00") Тогда
	...
	Если Не ТекСтрока.Свойство("СуммаНДСУм")
			И ТекСтрока.Свойство("СуммаНДСУвел")
			И ТекСтрока.СуммаНДСУвел = "0.00" Тогда
...
Процедура ЗаполнитьТабличныеДанныеПредСтрТабл(Аргумент, ДопПараметры) Экспорт
	...
	УстановитьЗначениеПоКлючу(Приемник, "ГТД_До",			Источник);
