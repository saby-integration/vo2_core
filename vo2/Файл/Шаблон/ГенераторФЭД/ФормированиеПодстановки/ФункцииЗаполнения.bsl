...
Процедура ЗаполнитьОбщиеДанные(Кэш, Генератор, Вложение, ДопПараметры)
	...
	КонецЕсли;
	ЕстьОснование = ТекДокумент.Свойство("Основание")
		И ТекДокумент.Основание.Количество()
		И ТекДокумент.Основание[0].Свойство("Дата")
		И ЗначениеЗаполнено(ТекДокумент.Основание[0].Дата)
		И ТекДокумент.Основание[0].Свойство("Номер")
		И ЗначениеЗаполнено(ТекДокумент.Основание[0].Номер);
	
	УПД24 = Вложение.ВерсияФормата = "5.02"
			И  Найти("упддоп,упдсчфдоп", НРег(Вложение.Тип));
		
	Если НЕ ЕстьОснование
		И УПД24 Тогда 
		Генератор.Вставить("БезДокОснПер", "1"); 
	Иначе
		УстановитьЗначениеПоКлючу(Генератор, "Основание", ТекДокумент);
	КонецЕсли;
...
Процедура ЗаполнитьТабличныеДанныеRnptData(Аргумент, ДопПараметры)
	...
	Для Каждого ТекСтрока ИЗ иСведПРослеж Цикл
		...
		Если ТекСтрока.Свойство("СтТовБезНДСПрослеж") Тогда
			Марка.Вставить("SumCostWoVat", ТекСтрока.СтТовБезНДСПрослеж);
		КонецЕсли;
