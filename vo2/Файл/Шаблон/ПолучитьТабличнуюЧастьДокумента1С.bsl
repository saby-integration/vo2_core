...
Функция ПолучитьТабличнуюЧастьДокумента1С(Кэш,Контекст) Экспорт
	...
	Если ПересчитатьВВалютеУчета = Истина Тогда 
		...
		ДопПараметры.Вставить("ИтогСумма", ИтогСумма);  
		ДопПараметры.Вставить("ВалютныйФормат", ВалютныйФормат);
		ДопПараметры.Вставить("ЦенаВключаетНДС", ЦенаВключаетНДС);
		ДопПараметры.Вставить("КурсВзаиморасчетов", КурсВзаиморасчетов); 
		ДопПараметры.Вставить("КратностьВзаиморасчетов", КратностьВзаиморасчетов); 
		ДопПараметры.Вставить("СбисНДСИсчисляетсяАгентом", СбисНДСИсчисляетсяАгентом);
		ДопПараметры.Вставить("СуммаДляПересчетаВключаетНДС", СуммаДляПересчетаВключаетНДС);
	...
	Для Каждого Параметр Из Контекст.ФайлДанные.мТаблДок Цикл
		...
		Если ТипЗнч(ТабЧастьДокумента) = Тип("Массив") Тогда
			Для Каждого Стр Из ТабЧастьДокумента Цикл
				...
				СуммаНДС = ОписаниеЧисла.ПривестиЗначение(СуммаНДС);
				...
				Если Стр.Свойство("ПредСтрТабл") Тогда
					...
					СуммаНДСДо = ОписаниеЧисла.ПривестиЗначение(СуммаНДСДо);
					ПредыдущиеДанные.СуммаБезНал = формат(ОписаниеЧисла.ПривестиЗначение(ПредыдущиеДанные.Сумма) - ?(СуммаВключаетНДС, СуммаНДСДо, 0), Валютныйформат);
                    Если СбисНДСИсчисляетсяАгентом Тогда
                        ПредыдущиеДанные.Сумма = "0.00";
                    Иначе 
						ПредыдущиеДанные.Сумма = формат(ОписаниеЧисла.ПривестиЗначение(ПредыдущиеДанные.Сумма) + ?(СуммаВключаетНДС, 0, СуммаНДСДо), Валютныйформат);
					КонецЕсли;
				...
				Если ПересчитатьВВалютеУчета = Истина Тогда 
					...
					Попытка
						 ...
						 ПолучитьТабличнуюЧастьДокумента1С_ПересчитатьВВалютеУчета(Стр, НоваяСтрока, ДопПараметры); 
...
&НаКлиенте
Процедура ПолучитьТабличнуюЧастьДокумента1С_ПересчитатьВВалютеУчета(Стр, НоваяСтрока, ДопПараметры) 
	
	ИтогСумма = ДопПараметры.ИтогСумма; 
		
	Кэш = МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш();
	
	СуммаДляПересчета = Кэш.ОбщиеФункции.РассчитатьЗначение("CуммаДляПересчета", Стр, Кэш); 
	
	ДопПараметры.Вставить("СуммаДляПересчета", СуммаДляПересчета);
	
	ПересчитатьВВалютеУчета_СведенияОПрослеживаемости(Стр, НоваяСтрока, ДопПараметры);  
	
	Если ЗначениеЗаполнено(ИтогСумма) Тогда   // есть готовые суммы по каждой строке и не надо распределять общую сумму по строкам
		
		Возврат;
		
	КонецЕсли;  
	
	ПересчитатьВВалютеУчета_ОбщиеДанные(Стр, НоваяСтрока, ДопПараметры);
		
КонецПроцедуры 

&НаКлиенте
Процедура ПересчитатьВВалютеУчета_ОбщиеДанные(Стр, НоваяСтрока, ДопПараметры)
	
	ВалютныйФормат					= ДопПараметры.ВалютныйФормат;
	КурсВзаиморасчетов				= ДопПараметры.КурсВзаиморасчетов; 
	КратностьВзаиморасчетов			= ДопПараметры.КратностьВзаиморасчетов; 
	СбисНДСИсчисляетсяАгентом 		= ДопПараметры.СбисНДСИсчисляетсяАгентом;
	СуммаДляПересчетаВключаетНДС	= ДопПараметры.СуммаДляПересчетаВключаетНДС; 
	
	СуммаДляПересчета				= ДопПараметры.СуммаДляПересчета;
	
	Кэш = МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш();
	 
	СуммаНДСДляПересчета = Кэш.ОбщиеФункции.РассчитатьЗначение("CуммаНДСДляПересчета", Стр, Кэш); 

	СуммаВВалютеУчета = СуммаДляПересчета * КурсВзаиморасчетов / ?(КратностьВзаиморасчетов = 0, 1, КратностьВзаиморасчетов);
	СуммаНДСВВалютеУчета = СуммаНДСДляПересчета * КурсВзаиморасчетов / ?(КратностьВзаиморасчетов = 0, 1, КратностьВзаиморасчетов); 
	...
	Если НЕ (НоваяСтрока.Свойство("Цена") И Число(НоваяСтрока.Цена) = 0) Тогда
		НоваяСтрока.Вставить("Цена", Формат(Число(НоваяСтрока.СуммаБезНал) 
			/ ?(Число(НоваяСтрока.Кол_во) = 0, 1, Число(НоваяСтрока.Кол_во)), ВалютныйФормат));
	КонецЕсли; 
	
	ПересчитатьВВалютеУчета_ОбщиеДанныеПараметрыСтроки(Стр, НоваяСтрока, ДопПараметры);
	
КонецПроцедуры 

&НаКлиенте
Процедура ПересчитатьВВалютеУчета_ОбщиеДанныеПараметрыСтроки(Стр, НоваяСтрока, ДопПараметры)  
	
	ВалютныйФормат	= ДопПараметры.ВалютныйФормат;
	ЦенаВключаетНДС	= ДопПараметры.ЦенаВключаетНДС;
	
	Если Не НоваяСтрока.Свойство("Параметр") Тогда
	...
	Для Каждого мПараметр Из НоваяСтрока.Параметр Цикл
		...
		Если мПараметр.Имя = "Цена1С" Тогда
			...
			мПараметр.Значение = Формат(Число(?(ЦенаВключаетНДС, НоваяСтрока.Сумма, НоваяСтрока.СуммаБезНал)) 
									/ ?(Число(НоваяСтрока.Кол_во) = 0, 1, Число(НоваяСтрока.Кол_во)), ВалютныйФормат);	
...
&НаКлиенте
Процедура ПересчитатьВВалютеУчета_СведенияОПрослеживаемости(Стр, НоваяСтрока, ДопПараметры)
	
	Перем СведенияОПрослеживаемости;
	
	Если Не НоваяСтрока.Свойство("СведПрослеж", СведенияОПрослеживаемости)
		Или Не СведенияОПрослеживаемости.Количество() Тогда
	...
	Если МодульОбъектаКлиент().ПривестиТипСбис(КурсВзаиморасчетов, "число") = 1 Тогда
		
		КурсПоСтроке = СуммаДляПересчета / ?(НоваяСтрока.Сумма = 0, 1, НоваяСтрока.Сумма);  
		
	Иначе

		КурсПоСтроке = КурсВзаиморасчетов / ?(КратностьВзаиморасчетов = 0, 1, КратностьВзаиморасчетов);
	
	КонецЕсли;	
		
	Для Каждого СтрокаПрослеж Из СведенияОПрослеживаемости Цикл
		
		Если СтрокаПрослеж.Свойство("СтТовБезНДСПрослеж") Тогда 
		
			СтрокаПрослеж.СтТовБезНДСПрослеж = Окр(СтрокаПрослеж.СтТовБезНДСПрослеж * КурсПоСтроке, 0);  
