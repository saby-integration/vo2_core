...
Функция СформироватьНаборСопоставленийНоменклатурыПоТабличнойЧасти(Вложение, Кэш, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаНоменклатуры Из ТаблДок Цикл
		...
		ДопПараметры = Новый Структура("ЗаполнениеНоменклатуры1С", Истина);
		ГолыйКлассНоменклатурыДляСопоставления = МодульОбъектаКлиент().НовыйСтрокаСопоставленияСБИСКлиент(ДанныеЗаполнения, ДопПараметры);
	...
	Попытка
		ОбогащенныеДанныеНоменклатурыДляСопоставления =	МодульСопоставления.НоменклатураПоставщика_МассовыйПоиск(ПараметрыДляОбогащения, ДопПараметры); 
	Исключение          
