...
Функция СформироватьТабличнуюЧастьПоНаборуСопоставленийНоменклатуры(ВходныеДанные, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого Сопоставление Из НаборСопоставлений Цикл   
		...
		Для Каждого СтрокаНоменклатуры1С Из Сопоставление.Номенклатура1С Цикл
			...
			Если НЕ ЗначениеЗаполнено(Число(Номенклатура1С.Кол_во)) Тогда
				Продолжить;
			КонецЕсли;
			...
			ИначеЕсли Сопоставление.НоменклатураСБИС.Свойство("GTIN") Тогда
			...
			Если НЕ ДопПараметры.СвернутьСтроки Тогда
				Если ЗначениеЗаполнено(РассчитаннаяСтрокаТЧ.Номенклатура) Тогда
			...
			Если НЕ ФайлПерегенерируется = Неопределено
				...
				СписокАтрибутов.Добавить("Характеристика");
