...
Функция ПолучитьТабличнуюЧастьДокумента1С(Кэш,Контекст) Экспорт
	...
	НовыйФорматУПД = Кэш.ОбщиеФункции.РассчитатьЗначение("_УПД24", Контекст.ФайлДанные, Кэш) = Истина;
	...
	Для Каждого Параметр Из Контекст.ФайлДанные.мТаблДок Цикл
		...
		Если ТипЗнч(ТабЧастьДокумента) = Тип("Массив") Тогда
			Для Каждого Стр Из ТабЧастьДокумента Цикл
				...
				ИспользоватьОбратноеСопоставление = Кэш.ОбщиеФункции.РассчитатьЗначение("ИспользоватьСопоставлениеНоменклатуры", Контекст.ФайлДанные);
				...
				Если (МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры")
					...
					И НЕ (Контекст.ДокументДанные.Реестр1С_Тип = "ЗаказПоставщику" 
					ИЛИ Контекст.ДокументДанные.Реестр1С_Тип = "Счет"
					ИЛИ Контекст.ДокументДанные.Реестр1С_Тип = "СчетНаОплату"
					ИЛИ Контекст.ДокументДанные.Реестр1С_Тип = "СчетНаОплатуПокупателю"
					ИЛИ Контекст.ДокументДанные.Реестр1С_Тип = "ЗаказПокупателя"
					ИЛИ Контекст.ДокументДанные.Реестр1С_Тип = "ЗаказКлиента"
					ИЛИ Контекст.ДокументДанные.Реестр1С_Тип = "СчетНаОплатуКлиенту") Тогда
...
Функция СформироватьТабличнуюЧастьПоНаборуСопоставленийНоменклатуры(ВходныеДанные, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого Сопоставление Из НаборСопоставлений Цикл   
		...
		Для Каждого СтрокаНоменклатуры1С Из Сопоставление.Номенклатура1С Цикл
			...
			Если ДопПараметры.Свойство("ПересчетПоДокументуОснованию") 
				И ДопПараметры.Свойство("ПересчетПоДокументуОснованию") = Истина Тогда
			...
			Если НЕ ДанныеЕдиницы = Неопределено 
				И НЕ ДопПараметры.Свойство("ПересчетПоДокументуОснованию") 
				И ДопПараметры.Свойство("ПересчетПоДокументуОснованию") = Истина  Тогда
