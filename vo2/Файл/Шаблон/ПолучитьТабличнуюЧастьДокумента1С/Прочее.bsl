...
Функция ПолучитьДанныеСделкиEDI(Кэш, Контекст, Отказ=Ложь)
	...
	СоставПакета = Кэш.Интеграция.ПрочитатьДокумент(Кэш, ИДПакетаСделки, Новый Структура, Отказ);  
...
&НаКлиенте
Процедура ОбновитьПараметрыВложения(Кэш, Контекст) Экспорт
	...
	Для каждого Изменение Из Изменения Цикл
		// Могут быть имена параметров на СБИС со спец. символами, а могут состоять из цифр
		ИмяПараметра = "_" + Кэш.ОбщиеФункции.сбисЗаменитьНедопустимыеСимволы(Изменение.Имя);
		ЛокальныйКонтекст.мПараметр.Вставить(ИмяПараметра, Новый Структура("Параметр_Имя, Параметр_Значение", Изменение.Имя, Изменение.Значение));
	КонецЦикла;
	
	ФормаФайл.ДобавитьМПараметр(Кэш, ЛокальныйКонтекст, Файл.Документ);
