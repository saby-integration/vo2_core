...
Процедура ОбработатьДокумент(СтрокаТаблДок, ДопПараметры)
	...
	// Для нового формата в приоритете ТаблДок_Название, иначе берется синоним
	// Для старого формата всегда ТаблДок_Примечание
	Если ЗначениеЗаполнено(СтрокаТаблДок.ДокументТабл)
		И (МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ИспользоватьНовыйФорматАктаСверки")
		И СтрокаТаблДок.Свойство("ТаблДок_Название")) Тогда

		НоваяСтрока.Вставить("Название", СтрокаТаблДок.ТаблДок_Название);

	ИначеЕсли ЗначениеЗаполнено(СтрокаТаблДок.ДокументТабл)
		И (МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ИспользоватьНовыйФорматАктаСверки")
