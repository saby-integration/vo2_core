...
Функция ПрименятьФункционалНовыеКонтрагенты(Данные, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ПолучитьЗначениеФичи(Новый Структура("НазваниеФичи", "НовыеКонтрагенты")) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого Вложение Из МассивВложений Цикл
		...
		Для Каждого ДостуныйФормат Из ФорматыДокументов Цикл 
			...
			ФорматВложения = ГлобальныйКэш.ТекущийСеанс.Модули.ФункцииДокументов.СбисЗаменитьНедопустимыеСимволы(Вложение.Тип + "_" + Вложение.ВерсияФормата);
