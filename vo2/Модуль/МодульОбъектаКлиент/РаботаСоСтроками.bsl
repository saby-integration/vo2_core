...
//  КлючФормата				 - Строка	 - как отформатировать строку
// 
...
Функция ПрименитьФорматКСтроке(ФорматнаяСтрока, ПараметрыФорматнойСтроки = Неопределено, КлючФормата = "СтрШаблон") Экспорт
	...
	Если КлючПроверка = "стршаблон" Тогда
		...
		// Применяет шаблон, в зависимости от типа Параметров строки
		
		// Структура/Соответствие
		// Пример "Hello {МойКлюч1} world {МойКлюч2}", 
		// для применения формата требуется Структура, Фиксированная структура, либо Соответствие с ключом "МойКлюч1" и МойКлюч2 и значением которое надо подставить в шаблон.
		
		// Массив
		// Пример "Hello {0} world {1}"
		// Для применения требуется Массив/Фиксированный массив, где по порядку будут расставлены значения для подстановки в шаблон
		
		// Строка - удалит ключ из шаблона
