...
//		Размер			- Число, ограничивающее выборку несколькими первыми записями
//		ТекстЗапроса    - Строка, текст запроса для поиска стороны
// 
// Возвращаемое значение:
...
Функция Сторона_НайтиКарточки1СЗапросом(оУчастник, ДопПараметры) Экспорт
	...
	Если ДопПараметры.Свойство("ТекстЗапроса") Тогда
		ТекстЗапроса = ДопПараметры.ТекстЗапроса;
	ИначеЕсли  ДопПараметры.Свойство("УзелИниПоиска", УзелИниПоиска)
	...
	РезультатПоиска = Сторона_НайтиКарточки1СЗапросомСервер(ТекстЗапроса, ПараметрыПостроенияЗапроса, ДопПараметры);
	РезультатИтог	= Новый Массив;
	
	// Поиск стороны в Контуре (с) Сыч
	КонтурEDI = МодульОбъектаКлиент().СведенияОбУстановленныхРасширениях("КонтурEDI");
	Если НЕ РезультатПоиска.Количество() 
		И КонтурEDI
		И ОУчастник.Свойство("GLN") Тогда 
		
		СсылкаНаСторону = БыстроеВнедрениеEDI_ПоискСтороныВКонтуре(оУчастник.GLN);
		Если ЗначениеЗаполнено(СсылкаНаСторону) Тогда
			РезультатИтог.Добавить(СсылкаНаСторону);
		КонецЕсли;
...
Функция Сторона_НайтиКарточки1СЗапросомСервер(ТекстЗапроса, ПараметрыЗапроса, ДопПараметры)
	...
	Возврат МодульОбъектаСервер().ВыполнитьЗапросСПараметрами(ТекстЗапроса, ПараметрыЗапроса, ДопПараметры);
