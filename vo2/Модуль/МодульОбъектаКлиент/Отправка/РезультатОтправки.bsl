...
Функция НовыйРезультатОтправки(ПараметрыКонструктора) Экспорт
	...
	ПараметрыОтправки = РезультатОтправки_ПараметрыОтправки(РезультатОтправки, ПараметрыКонструктора);
	...
	ЗначенияОжиданияЦепочек.Вставить("РезультатОтправки_Асинх_Отправка",	ПараметрыОтправки.ОжиданиеОтвета * 2 + 10);
	
	// Для обработки пользовательких действий + кэширование результатов
	ПользовательскиеДействия = Новый Структура("КэшЗапросов, ТекущийЗапрос, РезультатЗапроса", Новый Соответствие);
	...
	ЦепочкаКомандОбработки_УстановитьДанные(РезультатОтправки, "КэшПользовательскихДействий",	ПользовательскиеДействия);
...
&НаКлиенте
Функция РезультатОтправки_ПараметрыОтправки(РезультатОтправки, ПараметрыКонструктора)
	...
	КонецЕсли;
	ПараметрыОтправки.ИмяОбработчикаФатальныхОшибок = "РезультатОтправки_Асинх_ФатальнаяОшибка";
...
Функция РезультатОтправки_Выгрузить(РезультатОтправки, ВерсияФорматаОтвета=1) Экспорт
	...
	РезультатНовый.Вставить("ОбщееВремя", (ПолучитьДатуВМиллисекундах() - РезультатОтправки.ВремяНачала) / 1000);
...
Процедура РезультатОтправки_ПодготовитьСледующееДействие(РезультатОтправки, ПараметрыПодготовкиВходящие = Неопределено) Экспорт
	...
	Если		ОчередьДействий.Количество() < ЧислоПотоков * 2
		...
		Попытка
			...
			Если ВыбранаСтрока.Свойство("МассивПодготовленныхПакетов", МассивПодготовленныхПакетов) Тогда
				...
				Для Каждого ПодготовленныйПакет Из МассивПодготовленныхПакетов Цикл
					...
					РезультатОтправки_ПроверитьИПодготовитьКоманду(РезультатОтправки, ЦепочкаКомандОбработки, ПодготовленныйПакет);
			...
			Иначе
				...
				ЦепочкаКомандОбработки	= НовыйЦепочкаКомандОбработки();
				РезультатОтправки_ПроверитьИПодготовитьКоманду(РезультатОтправки, ЦепочкаКомандОбработки, ВыбранаСтрока);
		...
		Исключение
			...
			ФатальнаяОшибка = НовыйСбисИсключение(ИнформацияОбОшибке(), "МодульОбъектаКлиент.РезультатОтправки_МассоваяОтправка_Завершение");
			РезультатОтправки_ОбработатьФатальнуюОшибка(РезультатОтправки, ФатальнаяОшибка);
...
//
&НаКлиенте
Процедура РезультатОтправки_ПроверитьНакопленныеДействия(РезультатОтправки) Экспорт
	...
	Попытка
		...
		Для Каждого КлючИЗначениеДействие Из ОчередьДействий Цикл
			...
			ИначеЕсли	СтатусДействия = "ГотовКЗапуску" Тогда
				...
				РезультатОтправки_ЗапуститьЦепочкуКоманд(РезультатОтправки, ЦепочкаКомандОбработки);
				Продолжить;
...
&НаКлиенте
Процедура РезультатОтправки_ЗапуститьЦепочкуКоманд(РезультатОтправки, ЦепочкаКомандОбработки) Экспорт
...
Процедура РезультатОтправки_МассоваяОтправка_Завершение(АргументЗавершения="Завершено", РезультатОтправки) Экспорт 
	...
	ИначеЕсли	АргументЗавершения = "ОжиданиеОтветов" Тогда
		...
		ОтложенноОбновитьПрогрессБар = НовыйСбисОписаниеОповещения("РезультатОтправки_Асинх_СпрятатьСтатус", МодульОбъектаКлиент());
	...
	Если Отказ Тогда
		...
		РезультатОтправки_СпрятатьСтатус(РезультатОтправки);
		РезультатОтправки_ПоказатьСтатус(РезультатОтправки, "Завершение отправки", Неопределено,,Новый Структура);
