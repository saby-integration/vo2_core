...
Функция СоставПакета_СтруктураКонтрагентаДляОтправки(СоставПакета) Экспорт
	Перем ДанныеКонтрагентаСвФлЮлИпИн, лИдПодразделения, лПараметрПроверить;
	...
	//Контакты
	Если	СоставПакета.Контрагент.Свойство("Контакт", лПараметрПроверить)
		И	ЗначениеЗаполнено(лПараметрПроверить) Тогда
		
		Если лПараметрПроверить.Свойство("Телефон")  Тогда
			
			kontr.Вставить( "Телефон", лПараметрПроверить.Телефон );
			
		КонецЕсли;
		
		Если лПараметрПроверить.Свойство("EMAIL")  Тогда
			
			kontr.Вставить( "Email", лПараметрПроверить.EMAIL );
	...
	Если СоставПакета.Контрагент.Свойство("СвФЛ") Тогда
		...
		Если	СоставПакета.Контрагент.Свойство("Параметр", лПараметрПроверить)
			И	ЗначениеЗаполнено(лПараметрПроверить) Тогда
			
			Для Каждого Параметр из лПараметрПроверить Цикл
	...
	//Подразделение
	лПодразделение = СторонаSABY_Получить(СоставПакета.Контрагент, "Подразделение");
	Если ЗначениеЗаполнено(лПодразделение) Тогда
		
		Подразделение = Новый Структура;
		Подразделение.Вставить( "Идентификатор", лПодразделение.Идентификатор); 
		...
		Если ОбъектSABY_ЭтоТип(СоставПакета, "КонвертСБИС") Тогда
			...
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить("КодФилиала", лПодразделение.Идентификатор);
...
Функция СоставПакета_СтруктураОрганизацииДляОтправки(СоставПакета) Экспорт
	...
	КлючИдентификатора = СторонаSABY_Получить(СоставПакета.НашаОрганизация, "КлючИдентификатора");
	...
	Если	Не	ЕстьКодФилиалаОрганизации
		И		ОбъектSABY_ЭтоТип(СоставПакета, "КонвертСБИС")Тогда								
		
		// Сохранение старой логики - если основание для документа - конверт, то ИД подразделения перекладывался на КодФилиала
		КодФилиалаОрганизации		= СторонаSABY_Получить(СоставПакета.НашаОрганизация, "КодФилиала");
		ЕстьКодФилиалаОрганизации	= ЗначениеЗаполнено(КодФилиалаОрганизации);
...
Процедура СоставПакета_Записать(СоставПакета, ДопПараметры) Экспорт 
	Перем ОбработчикРезультата, ФорматЗаписиДокумент;
	
	Если ТипЗнч(ДопПараметры) = Тип("Строка") Тогда
		
		ФорматЗаписиДокумент = ДопПараметры;
		
	Иначе
		
		ДопПараметры.Свойство("ФорматЗаписи",			ФорматЗаписиДокумент);
		ДопПараметры.Свойство("ОбработчикРезультата",	ОбработчикРезультата);
		
	КонецЕсли;
	
	Если ФорматЗаписиДокумент = Неопределено Тогда
		
		ФорматЗаписиДокумент = "отправкабездо";
		
	Иначе
		
		ФорматЗаписиДокумент = НРег(ФорматЗаписиДокумент);
		
	КонецЕсли;
	
	Если ФорматЗаписиДокумент = "черновик" Тогда
		
		// Пока подготвка для записи черновика не через отправку, а через ЗаписатьДокумент.
		ОшибкаЗаписиНаБЛ	= Ложь;
		ДокументЗаписать	= СоставПакета_Выгрузить(СоставПакета, "ЗаписатьДокумент");
		// TODO чтобы работала запись черновика нужно изменить выгрузку вложения для ЗаписатьДокумент на extsdk
		// Сейчас Отправка_ОбработатьXMLФайл формирует ссылку на файл и плагин в отправке его грузит сам, то тут нужно:
		// либо сделать нормальную обертку в extSDK, которая зальет ссылку на диск и передаст в ЗаписатьДокумент результат
		// либо передавать двоичные данные, как API
		РезультатЗаписи		= ПараметрSABY("Интеграция", "Сеанс").СБИС_ЗаписатьДокумент(ПолучитьТекущийЛокальныйКэш(), ДокументЗаписать,,ОшибкаЗаписиНаБЛ);
		
		Если Не ОбработчикРезультата = Неопределено Тогда
			
			ВыполнитьСбисОписаниеОповещения(РезультатЗаписи, ОбработчикРезультата);
			Возврат;
			
		КонецЕсли;
		
		Если ОшибкаЗаписиНаБЛ Тогда
			
			ВызватьИсключение СбисИсключение_Выгрузить(РезультатЗаписи);
			
		КонецЕсли;
		
	ИначеЕсли ФорматЗаписиДокумент = "отправкабездо" Тогда
		...
		ИначеЕсли СоставПакета.Свойство("Вложение")	Тогда
			...
			Для Каждого Вложение Из СоставПакета.Вложение Цикл 
				...
				ВложениеТребуетЗаписи = ВложениеСБИС_Получить(Вложение, "ТребуетЗаписиНаБЛ");
...
Процедура СоставПакета_Обновить(СоставПакета, Изменения, ДопПараметры = Неопределено) Экспорт
	...
	Если Изменения = "ПерегенерироватьВложения" Тогда
		
		яСоставПакета_ПерегенерироватьВложения(СоставПакета, ДопПараметры);
		Возврат;
