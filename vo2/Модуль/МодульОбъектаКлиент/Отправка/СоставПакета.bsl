...
Функция СоставПакета_ПодготовитьКОтправке(СоставПакета, ДопПараметры=Неопределено) Экспорт
	Перем СбисШифрование, Статус, Этап, МодульИнтеграции, ПараметрыПодготовкиВложений;
	...
	ЭтоЧерновик			= НЕ	ДопПараметры = Неопределено
						И		ДопПараметры.Свойство("ЭтоЧерновик")
						И		ДопПараметры.ЭтоЧерновик;
	...
	ИначеЕсли ЭтоЧерновик Тогда	
		// Ничего не делать для черновика это только ВыполнитьДействие
...
Функция СоставПакета_Идентификатор(СоставПакета, ДопПараметры = Неопределено) Экспорт 
	...
	Если Не ДопПараметры = Неопределено Тогда
		...
		ИначеЕсли ДопПараметры.Свойство("Направление") Тогда
