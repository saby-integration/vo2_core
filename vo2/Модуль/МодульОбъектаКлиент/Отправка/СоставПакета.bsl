...
Функция СоставПакета_СтруктураКонтрагентаДляОтправки(СоставПакета) Экспорт
	Перем ДанныеКонтрагентаСвФлЮлИпИн, лИдПодразделения;
	...
	Иначе		
		...
		Иначе
			...
			Если ЗначениеЗаполнено(СбисИдентификаторКонтрагента) Тогда
				kontr.Вставить("Идентификатор", СбисИдентификаторКонтрагента);
			КонецЕсли;
		КонецЕсли;
	...
	Если СоставПакета.Контрагент.Свойство("СвФЛ") Тогда
		ДанныеКонтрагентаСвФлЮлИпИн = Новый Структура;
		Если СоставПакета.Контрагент.Свойство("Параметр") Тогда
			Для Каждого Параметр из СоставПакета.Контрагент.Параметр Цикл
				Если Параметр.Имя = "ЧастноеЛицо" Тогда
					ДанныеКонтрагентаСвФлЮлИпИн.Вставить("ЧастноеЛицо", Параметр.Значение);
				КонецЕсли;
				Если Параметр.Имя = "СНИЛС" Тогда
					ДанныеКонтрагентаСвФлЮлИпИн.Вставить("СНИЛС", Параметр.Значение);						
				КонецЕсли;     					
			КонецЦикла;
		КонецЕсли;  
		Если СоставПакета.Контрагент.СвФЛ.Свойство("ИНН") Тогда  
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "ИНН", СоставПакета.Контрагент.СвФЛ[КлючИдентификатора] );
		КонецЕсли;
		Если СоставПакета.Контрагент.СвФЛ.Свойство("Фамилия") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "Фамилия", СоставПакета.Контрагент.СвФЛ.Фамилия );	
		КонецЕсли;
		Если СоставПакета.Контрагент.СвФЛ.Свойство("Имя") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "Имя", СоставПакета.Контрагент.СвФЛ.Имя );	
		КонецЕсли;
		Если СоставПакета.Контрагент.СвФЛ.Свойство("Отчество") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "Отчество", СоставПакета.Контрагент.СвФЛ.Отчество );	
		КонецЕсли;
		Если СоставПакета.Контрагент.СвФЛ.Свойство("КодФилиала") и ЗначениеЗаполнено(СоставПакета.Контрагент.СвФЛ.КодФилиала) Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "КодФилиала", СоставПакета.Контрагент.СвФЛ.КодФилиала );	
		КонецЕсли;
		kontr.Вставить( "СвФЛ", ДанныеКонтрагентаСвФлЮлИпИн );
		
	ИначеЕсли СоставПакета.Контрагент.Свойство("СвИн") Тогда
		ДанныеКонтрагентаСвФлЮлИпИн = Новый Структура;
		ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "ИНН", СоставПакета.Контрагент.СвИн[КлючИдентификатора] );  
		Если СоставПакета.Контрагент.СвИн.Свойство("КПП") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "КПП", СоставПакета.Контрагент.СвИн.КПП );
		КонецЕсли;
		Если СоставПакета.Контрагент.СвИн.Свойство("КодСтраны") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "КодСтраны", СоставПакета.Контрагент.СвИн.КодСтраны );	
		КонецЕсли;
		Если СоставПакета.Контрагент.СвИн.Свойство("КодФилиала") и ЗначениеЗаполнено(СоставПакета.Контрагент.СвИн.КодФилиала) Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "КодФилиала", СоставПакета.Контрагент.СвИн.КодФилиала );	
		КонецЕсли;
		Если СоставПакета.Контрагент.СвИн.Свойство("Название") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "Название", СоставПакета.Контрагент.СвИн.Название );	
		КонецЕсли;
		kontr.Вставить( "СвЮЛ", ДанныеКонтрагентаСвФлЮлИпИн );	
		
	Иначе
		ДанныеКонтрагентаСвФлЮлИпИн = Новый Структура;
		ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "ИНН", СоставПакета.Контрагент.СвЮЛ[КлючИдентификатора] );  
		Если СоставПакета.Контрагент.СвЮЛ.Свойство("КПП") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "КПП", СоставПакета.Контрагент.СвЮЛ.КПП );
		КонецЕсли;
		Если СоставПакета.Контрагент.СвЮЛ.Свойство("КодСтраны") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "КодСтраны", СоставПакета.Контрагент.СвЮЛ.КодСтраны );	
		КонецЕсли;
		Если СоставПакета.Контрагент.СвЮЛ.Свойство("КодФилиала") и ЗначениеЗаполнено(СоставПакета.Контрагент.СвЮЛ.КодФилиала) Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "КодФилиала", СоставПакета.Контрагент.СвЮЛ.КодФилиала );	
		КонецЕсли;
		Если СоставПакета.Контрагент.СвЮЛ.Свойство("Название") Тогда
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить( "Название", СоставПакета.Контрагент.СвЮЛ.Название );	
		КонецЕсли;
		kontr.Вставить( "СвЮЛ", ДанныеКонтрагентаСвФлЮлИпИн );
	КонецЕсли;
	
	//Подразделение
	Если	СоставПакета.Контрагент.Свойство("Подразделение", лИдПодразделения)
		И	лИдПодразделения.Свойство("Идентификатор", лИдПодразделения) Тогда
		
		Подразделение = Новый Структура;
		Подразделение.Вставить( "Идентификатор", лИдПодразделения); 
		kontr.Вставить( "Подразделение", Подразделение );
		
		Если ОбъектSABY_ЭтоТип(СоставПакета, "КонвертСБИС") Тогда
			
			// Сохранение старой логики - если основание для документа - конверт, то ИД подразделения перекладывался на КодФилиала
			ДанныеКонтрагентаСвФлЮлИпИн.Вставить("КодФилиала", лИдПодразделения);
...
Функция СоставПакета_СтруктураОрганизацииДляОтправки(СоставПакета) Экспорт
	...
	ЕстьКодФилиалаОрганизации	=		ЗначениеЗаполнено(КодФилиалаОрганизации)
								Или	(	СвФлЮлИсточник.Свойство("КодФилиала", КодФилиалаОрганизации)
									И	ЗначениеЗаполнено(КодФилиалаОрганизации))
								// Сохранение старой логики - если основание для документа - конверт, то ИД подразделения перекладывался на КодФилиала
								Или	(	СоставПакета.НашаОрганизация.Свойство("Подразделение",	КодФилиалаОрганизации)
									И	КодФилиалаОрганизации.Свойство("Идентификатор",			КодФилиалаОрганизации)
									И	ОбъектSABY_ЭтоТип(СоставПакета, "КонвертСБИС"));
	
	Если ЕстьКодФилиалаОрганизации Тогда
...
Функция СоставПакета_ПодготовитьКОтправке(СоставПакета, ДопПараметры=Неопределено) Экспорт
	...
	document.Вставить("НаборДопПолей", Новый Структура);
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(DopPolya) Тогда
		document.Вставить("ДопПоля", DopPolya);
		document.НаборДопПолей.Вставить("ЗаписатьДокумент", DopPolya); // TODO Оставить доп поля только в НаборДопПолей
	КонецЕсли;
	...
	Если	СоставПакета.Параметры.Свойство("НужнаПроверкаМаркировки")
		...
		document.НаборДопПолей.Вставить("ВыполнитьДействие", DopPolya);
		
	КонецЕсли;
	
	Если Не СоставПакета_Получить(СоставПакета, "НеЗапускатьВДокументооборот") = Неопределено
		И СоставПакета_Получить(СоставПакета, "НеЗапускатьВДокументооборот") Тогда
...
Процедура СоставПакета_РазобратьДляЗагрузки(СоставПакета, ДопПараметры) Экспорт
	...
	Если Не СоставПакета.Свойство(ИмяРеквизитаВложений, ВложенияРазобрать) Тогда
		...
		ВызватьИсключение СбисИсключение_Выгрузить(ОшибкаРазбораПакета);
	...
	Для Каждого ВложениеСБИС Из ВложенияРазобрать Цикл
		...
		Исключение
			...
			ВложениеСБИС_Обновить(ВложениеСБИС, ОшибкаРазбора, "Статус");
...
Процедура СоставПакета_Записать(СоставПакета, ДопПараметры) Экспорт 
	...
	Если Не	ДокументСохраненНаБЛ Тогда
		...
		СоставПакета_Вставить(СоставПакета, "НеЗапускатьВДокументооборотБыло",	НеЗапускатьВДокументооборотБыло);
		СоставПакета_Вставить(СоставПакета, "НеЗапускатьВДокументооборот",		НеЗапускатьВДокументооборот);
...
Процедура СоставПакета_Записать_Завершить(РезультатЗаписи, ДопПараметры = Неопределено) Экспорт 
	...
	Для Каждого Пакет Из ДопПараметры.МассивГотовыхПакетов Цикл
		...
		СоставПакета_Очистить(Пакет, "НеЗапускатьВДокументооборотБыло");
		СоставПакета_Вставить(Пакет, "НеЗапускатьВДокументооборот", Ложь);
...
Процедура СоставПакета_Обновить(СоставПакета, Изменения, ДопПараметры = Неопределено) Экспорт
	...
	Если Изменения.Свойство("НеЗапускатьВДокументооборот") Тогда
		...
		СоставПакета_Вставить(СоставПакета, "НеЗапускатьВДокументооборот", Изменения.НеЗапускатьВДокументооборот = Истина);

	КонецЕсли;	

	Если Изменения.Свойство("Провести") Тогда
		
		СоставПакета_Вставить(СоставПакета, "Провести", Изменения.Провести = Истина);
