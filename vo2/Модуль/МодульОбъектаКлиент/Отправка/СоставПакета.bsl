...
Процедура СоставПакета_РазобратьДляЗагрузки(СоставПакета, ДопПараметры) Экспорт
	...
	Для Каждого ВложениеСБИС Из ВложенияРазобрать Цикл
		...
		Если	ПараметрыВложения.Направление = "Исходящий" Тогда
			...
			Для Каждого ЭлементСпискаДокумент Из ВложениеСБИС_Получить(ВложениеСБИС, "Документы1С") Цикл
	...
	Для Каждого ВложениеСБИС Из ВложенияРазобрать Цикл
		...
		Если		СтатусыВложения = Неопределено
			Или		СтатусыВложения.Общий = "Ошибка"
			Или	Не	СтатусыВложения.Ини Тогда
...
Процедура СоставПакета_Записать(СоставПакета, ДопПараметры) Экспорт 
	...
	СоставПакетаДляОтправки = ПолучитьЗначениеПараметраТекущегоСеанса("ФункцииДокументов").СбисСкопироватьОбъектНаКлиенте(СоставПакета);
	
	СоставПакетаДляОтправки.Вставить("НеЗапускатьВДокументооборот", НеЗапускатьВДокументооборот);
	
	МассивГотовыхПакетов = Новый Массив;
	МассивГотовыхПакетов.Добавить(СоставПакетаДляОтправки);
	...
	Если Не ДопПараметры.Свойство("ОбработчикРезультата", ОбработчикРезультата) Тогда
		ОбработчикРезультата = НовыйСбисОписаниеОповещения("СоставПакета_Записать_Завершить", МодульОбъектаКлиент(), Новый Структура);
	КонецЕсли;
