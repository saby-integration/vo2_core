...
Процедура РезультатОтправки_ОтправитьНакопленные(РезультатОтправки, ДопПараметры=Неопределено)
	...
	Для Каждого КлючИЗначениеПодготовлен Из ПодготовленныеКОтправкеПакеты Цикл		
		...
		СтатусОтправки			= ОтправкаДокументаСБИС_Получить(ОтправкаДокументаСбис, "СтатусПакета");
		
		Если СтатусОтправки = "ОжидаетАктивации" Тогда
		...
		ИначеЕсли 	СтатусОтправки = "Переотправка" Тогда
			
			// При повторной отправке, выставим статус готовности
			ОтправкаДокументаСБИС_Установить(КлючИЗначениеПодготовлен.Значение, "СтатусПакета", "Готов");
			
		ИначеЕсли Не	(	СтатусОтправки = "Готов"
						Или	СтатусОтправки = "Подготовка" 
						Или	СтатусОтправки = "Черновик") Тогда
					
			// Не известный статус пакета. Ошибка
			СообщениеДетализации		= "Отправка не выполнена. Неизвестный статус " + СтатусОтправки + ".";
			ОшибкаОтправки				= НовыйСбисИсключение(773, "МодульОбъектаКлиент.РезультатОтправки_ОтправитьНакопленные",,,СообщениеДетализации);
			ПараметрыОбработатьОшибку	= Новый Структура("Исключение", ОшибкаОтправки);
			РезультатОтправки_ОбработатьОшибку(РезультатОтправки, ОтправкаДокументаСбис, ПараметрыОбработатьОшибку);
			Продолжить;
...
Процедура РезультатОтправки_ОтправитьЗаписиПрикладнойСтатистики(РезультатОтправки)
	...
	ПрикладнаяСтатистика_Отправить(РезультатОтправки.ПрикладнаяСтатистика);
