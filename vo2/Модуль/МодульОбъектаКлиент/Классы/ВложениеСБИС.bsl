...
// Функция - создаёт новые вложение СБИС
//
// Параметры:
...
Функция ВложениеСБИС_Пустой()
	...
	ВложениеСБИС.Параметры		= Новый Структура("ИспользоватьГенератор, ИспользоватьСопоставлениеНоменклатуры, ФайлПерегенерируется, НаборМодулейДляФормата", Ложь, Ложь, Ложь, "");
...
Функция ВложениеСБИС_РассчитанныйОбъект(ВложениеСБИС, ДопПараметры)
	...
	ИначеЕсли  	ДопПараметры.Свойство("Режим", РежимОпределенияОбъекта)		
		...
		// Попали в режим поиска основания, или документа, но при этом расчитанного объекта ещё нет
		ЭтоПолныйРасчёт = Ложь;
		
	Иначе
		
		ЭтоПолныйРасчёт = Истина;
...
Процедура ВложениеСБИС_УстановитьИниЗагрузки(ВложениеСБИС, ДопПараметры)
	...
	ВозможностьСоздаватьНоменклатуру	= Документ_Модуль.РассчитатьПараметрИни(КэшДляРасчетов,
		"СоздаватьНоменклатуру",,
		ПараметрыРассчитать,
		ДопПараметрыРасчета);
	СопоставлятьПередЗагрузкой			= Документ_Модуль.РассчитатьЗначение("СопоставлятьПередЗагрузкой", ПараметрыРассчитать);
	...
	ВложениеСБИС.Параметры.Удалить("РедактируемыеПоля");	// Кэш функции Поля1С вложения при полном расчёте, включая изменения пользователя
	ВложениеСБИС.Параметры.Удалить("РассчитанныйОбъект");	// Кэш функции РассчитанныйОбъект вложения при полном расчёте

	ВложениеСБИС.Параметры.Удалить("ПоляДляПоиска");		// Кэш функции Поля1С вложения при неполном расчёте (поиск документа/основания)
	ВложениеСБИС.Параметры.Удалить("БазовыеПоляОбъекта");	// Кэш функции РассчитанныйОбъект вложения при при неполном расчёте (поиск документа/основания
	...
	ВложениеСБИС_Заполнить(ВложениеСБИС, ВозможностьСоздаватьНоменклатуру <> Ложь, "СоздаватьНоменклатуру");
...
Процедура ВложениеСБИС_УстановитьПараметрыСторон(ВложениеСБИС, ДопПараметры)
	Перем мСторонаДокумента, ПутьКонтрагента, ПутьОрганизации, ПутьСтороныСопоставления, УзелМСторона;
	...
	РолиПоИни.Вставить(РолиПоИни.Получить("Контрагент"),	"Контрагент");
	РолиПоИни.Вставить(РолиПоИни.Получить("Организация"),	"Организация");
	...
	Если СтруктураИниФайла.Свойство("мСторона", мСторонаДокумента) Тогда
		...
		// Заполнить синонимы и данные сторон рассчёта
		Для Каждого КлючИЗначениеУзелМСторона Из мСторонаДокумента Цикл
			...
			Если	КлючИЗначениеУзелМСторона.Значение.Свойство("Сторона", УзелМСторона) Тогда
				// Костыли для определения данных сторон
				Если	УзелМСторона.Свойство("Данные") Тогда
		...
		Если Не РольДляСоответствия = Неопределено Тогда
			...
			Если КонтрагентСопоставления = Неопределено Тогда
				...
				ОшибкаРолиСопоставления	= НовыйСбисИсключение(608, "МодульОбъектаКлиент.ВложениеСБИС_УстановитьПараметрыСторон",,,ТекстСообщения);
...
Процедура НовыйВложениеСБИС_ЗаполнитьПоДокументФайл(ВложениеСБИС, ДанныеЗаполнить)
	...
	СуммаВложения			= ?(ДанныеЗаполнить.Свойство("СуммаВложения"),	ДанныеЗаполнить.СуммаВложения,		0);
	ДатаВложения			= ?(Док.Файл.Документ.Свойство("Дата"),			Док.Файл.Документ.Дата,				"");
	НомерВложения			= ?(Док.Файл.Документ.Свойство("Номер"),		СокрЛП(Док.Файл.Документ.Номер),	"");
	
	СуммаВложения			= Формат(СуммаВложения, "ЧЦ=17; ЧДЦ=2; ЧРД=.; ЧГ=0; ЧН=0.00");
	НазваниеВложения		= "";
	Если Док.Файл.Документ.Свойство("Название") Тогда
		
		НазваниеВложения = Док.Файл.Документ.Название
							+ " № " +			НомерВложения
							+ " от " +			ДатаВложения
...
Процедура ВложениеСБИС_ЗаполнитьПоДокументФайл_РасчетныеПоля(ВложениеСБИС, ДанныеЗаполнить)
	Перем лДанныеСторон, ОтправительДанные, ПолучательДанные, РольСтороны, СертификатОтправителя, ЗапретРедакцииПолучателя,
			ВерсияФайлаСформированаОбработкой, ФорматФайлаСформированаОбработкой;
	...
	Док					= ДанныеЗаполнить.ДокументФайл;
	ДокФайлОписание		= Док.Файл; // Возможно, имеет смысл вынести все обращения к Док в Файл_Шаблон, т.к. это привязка к формату.
	...
	Если Не лДанныеСторон = Неопределено Тогда
		...
		Если ДанныеЗаполнить.Свойство("ОтправительРоль", РольСтороны) Тогда
			...
			СертификатОтправителя	= ПрочитатьДополнительныйПараметрСтороны(ОтправительДанные, "Сертификат");
			
		КонецЕсли;
		
		Если ДанныеЗаполнить.Свойство("ПолучательРоль", РольСтороны) Тогда
	...
	Тип							= ФункцииРаботыСДокументами.РассчитатьЗначение("Вложение_Тип", 					КонтекстДляРасчета.ФайлДанные);
	ПодТип						= ФункцииРаботыСДокументами.РассчитатьЗначение("Вложение_ПодТип",				КонтекстДляРасчета.ФайлДанные);
	ВерсияФормата				= ФункцииРаботыСДокументами.РассчитатьЗначение("Вложение_ВерсияФормата",		КонтекстДляРасчета.ФайлДанные);
	ПодВерсияФормата			= ФункцииРаботыСДокументами.РассчитатьЗначение("Вложение_ПодВерсияФормата",		КонтекстДляРасчета.ФайлДанные);
	...
	ФункцииРаботыСДокументами.ЗаполнитьАтрибуты(Кэш, "ДопПоля", КонтекстДляРасчета.ФайлДанные, ДопПоля);
	...
	Если ЗначениеЗаполнено(ИдентификаторГосконтракта) Тогда
		...
		ВложениеСБИС.Вставить("ИдентификаторГосконтракта", ИдентификаторГосконтракта);
...
&НаКлиенте
Процедура ВложениеСБИС_ЗаполнитьПоДокументФайл_СопоставлениеНоменклатуры(ВложениеСБИС, ДанныеЗаполнить)
...
Функция ВложениеСБИС_Получить_ТипОбъекта(ВложениеСБИС, ДопПараметры)
	Перем СтруктураИниФайла, ТипПолный, РежимПоиска, ЗначениеУзлаРасчёта;
	...
	Если	Не ВложениеСБИС.Параметры.Свойство("ПолныйТипОбъекта", ТипПолный) Тогда
		...
		ДопПараметры.Свойство("Режим", РежимПоиска);
		ИмяУзлаРасчётаТипа = "Документ";

		Если РежимПоиска = ИмяУзлаРасчётаТипа Тогда
			
			// Это расчёт Документ, не отправляем на вычислить а пробуем опредилить по типу на узле
			ЕстьТип	=	СтруктураИниФайла.Свойство(ИмяУзлаРасчётаТипа, ТипПолный)
		...
		Иначе
			...
			ЗначениеДокумент = Документ_Модуль.РассчитатьПараметрИни(ЛокальныйКэш, ИмяУзлаРасчётаТипа, ЗначениеУзлаРасчёта, КонтекстДляРассчитать, Новый Структура("ВызовСервера", Истина));
			Если	ЗначениеДокумент = Неопределено
				И	СтруктураИниФайла.Свойство(ИмяУзлаРасчётаТипа, ЗначениеУзлаРасчёта) Тогда
...
Функция ВложениеСБИС_Выгрузить(Вложение, КлючВыгрузки = Неопределено, ДопПараметры = Неопределено) Экспорт
	...
	Если Вложение.Параметры.Свойство("ФайлПерегенерируется") Тогда
		ФайлПерегенерируется = Вложение.Параметры.ФайлПерегенерируется;
	Иначе
		ФайлПерегенерируется = Истина;
	КонецЕсли;
	
	ДопПараметры = Новый Структура("СвернутьСтроки, НаправлениеСворачивания, ФайлПерегенерируется", СвернутьСтроки, НаправлениеСворачивания, ФайлПерегенерируется);
...
Функция ВложениеСБИС_НайтиПодходящиеДокументы(ВложениеСБИС, ДопПараметры)
	Перем	лРасчитанныйПараметр, УзелИниПоиска, ИмяИниДляПоиска, ТипОбъектаПоиск, СоставПакета, ИмяРеквизитаРасчет, ИмяРеквизитаРасчетВспом,
	...
	ПараметрыФильтра  = Новый Структура("УзелИни, ОписаниеЗапроса, СоставПакета, Режим", УзелИниПоиска, ОписаниеЗапроса, ДопПараметры.СоставПакета);
	...
	Если Не ПолучитьЗначениеПараметраСбис("СопоставлениеПоКонтрагенту") = "Не использовать" Тогда
		...
		ИначеЕсли	ДопПараметры.Свойство("СоставПакета", СоставПакета) Тогда
			ПараметрыФильтра.Вставить("КонтрагентСБИС",	СоставПакета_Получить(СоставПакета, "Контрагент"));
		Иначе
...
Процедура ВложениеСБИС_Обновить(ВложениеСБИС, Изменения, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Изменения.Свойство("ПараметрыДокумента", ТекущиеИзменения) Тогда	
		...
		ВложениеСБИС_ОбновитьПараметрыДокумента(ВложениеСБИС, ТекущиеИзменения);
		ВложениеСБИС.Вставить("Перегенерировать", Истина);
		
	ИначеЕсли Изменения.Свойство("Номер") Тогда	
		
		ВложениеСБИС_ОбновитьДанныеДокумента(ВложениеСБИС, Изменения);
		ВложениеСБИС.Вставить("Перегенерировать", Истина);
...
&НаКлиенте
Процедура ВложениеСБИС_ОбновитьПараметрыДокумента(ВложениеСБИС, Изменения)
...
Процедура ВложениеСБИС_ПропатчитьНовымСопоставлением(Вложение, ДопПараметры = Неопределено) Экспорт		
	...
	// 3. Обновлённые данные выгрузим в ТаблДок и запишем его на вложение.
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
