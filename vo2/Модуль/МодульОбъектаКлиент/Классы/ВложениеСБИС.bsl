...
Процедура ВложениеСБИС_РазобратьДляЗагрузки(ВложениеСБИС, ДопПараметры) Экспорт
	...
	ВложениеСБИС.Вставить("Стороны", Новый Структура("Синонимы", Новый Соответствие));
...
Функция ВложениеСБИС_Поля1С(ВложениеСБИС, ДопПараметры)
	...
	СинонимСтороны	= ВложениеСБИС.Стороны.Синонимы.Получить("Контрагент");
	...
	СинонимСтороны	= ВложениеСБИС.Стороны.Синонимы.Получить("Организация");
	Если СинонимСтороны = Неопределено Тогда
...
Процедура ВложениеСБИС_УстановитьИниЗагрузки(ВложениеСБИС, ДопПараметры)
	...
	Если СтруктураИниФайла.Свойство("мСторона", мСторонаДокумента) Тогда
		...
		Если Не Контрагент_Роль = Неопределено	
			И	мСторонаДокумента.Свойство(Контрагент_Роль) Тогда
			
			// Заполнить синонимы в обе стороны
			ВложениеСБИС.Стороны.Синонимы.Вставить("Контрагент",	Контрагент_Роль);
			ВложениеСБИС.Стороны.Синонимы.Вставить(Контрагент_Роль, "Контрагент");
			
		КонецЕсли;
		
		Если Не Организация_Роль = Неопределено	
			И	мСторонаДокумента.Свойство(Организация_Роль) Тогда
			
			// Заполнить синонимы в обе стороны
			ВложениеСБИС.Стороны.Синонимы.Вставить("Организация",		Организация_Роль);
			ВложениеСБИС.Стороны.Синонимы.Вставить(Организация_Роль,	"Организация");
