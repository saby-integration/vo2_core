...
Процедура ВложениеСБИС_НайтиПодходящиеДокументы_УстановитьФильтрПоДате(ВложениеСБИС, ДопПараметры)
	...
	Если ЕстьДата Тогда
		...
		Если	УзелИниПоиска.Свойство("Поиск_Дата") Тогда
			...
			ИмяРеквизитаРасчет = СтроковоеЗначениеУзлаИни(УзелИниПоиска.Поиск_Дата, Новый Структура("ИмяРеквизита", Истина))
	...
	ЗначениеСопоставлениеПоДате	= ПолучитьЗначениеПараметраСбис("СопоставлениеПоДате");
	Если	ДопПараметры.Свойство("ПоляПоиска", ПоляПоиска) Тогда
		...
		Если ПоляПоиска.Свойство("Дата", ПоляПоиска) Тогда
			...
			Для Каждого КлючИЗначениеПараметр Из ПоляПоиска.Значение Цикл
				...
				ВложениеСБИС_НайтиПодходящиеДокументы_ФильтрСопоставлениеПоДате(ОписаниеЗапроса, КлючИЗначениеПараметр.Значение, ИмяРеквизитаРасчет, ЗначениеСопоставлениеПоДате);
	...
	Если Не	ЕстьПоискПоДатеВХ	
		...
		ВложениеСБИС_НайтиПодходящиеДокументы_ФильтрСопоставлениеПоДате(ОписаниеЗапроса, ЗначениеДатаВх, ИмяРеквизитаРасчет, ЗначениеСопоставлениеПоДате);
...
&НаКлиенте
Процедура ВложениеСБИС_НайтиПодходящиеДокументы_ФильтрСопоставлениеПоДате(ОписаниеЗапроса, ЗначениеДаты, УзелРеквизита, ЗначениеСопоставлениеПоДате = Неопределено)
	
	ИмяРеквизитаРасчет = СтроковоеЗначениеУзлаИни(УзелРеквизита, Новый Структура("ИмяРеквизита", Истина));

	Если		ЗначениеСопоставлениеПоДате = "Точное совпадение" Тогда
		
		УсловиеДобавить = " И Объект.{ИмяРеквизитаДатаВх} = &ДатаВхНач";
		
	ИначеЕсли	ЗначениеСопоставлениеПоДате = "Отклонение 1 день" Тогда
	...
	ОписаниеЗапроса.Параметры.Вставить("ДатаВхНач", ЗначениеДаты);
