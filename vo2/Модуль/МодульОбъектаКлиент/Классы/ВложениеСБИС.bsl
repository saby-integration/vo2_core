...
Процедура ВложениеСБИС_УстановитьПараметрыСторон(ВложениеСБИС, ДопПараметры)
	...
	Если СтруктураИниФайла.Свойство("мСторона", мСторонаДокумента) Тогда
		...
		ПараметрыИни			= ВложениеСБИС_Получить(ВложениеСБИС, "ПараметрыДанныхВложения");
		КонтрагентСопоставления = РолиПоИни.Получить("Сопоставление");
		Если Не КонтрагентСопоставления = Неопределено Тогда
			
			РольДляСоответствия = ДанныеСтруктурыФайла.Синонимы.Получить(КонтрагентСопоставления);
			Если РольДляСоответствия = Неопределено Тогда
