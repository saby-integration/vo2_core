...
Процедура ВложениеСБИС_РазобратьДляЗагрузки(ВложениеСБИС, ДопПараметры) Экспорт
	...
	Если Не ВложениеСБИС.Свойство("Параметры") Тогда
		...
		ПараметрыВложения.Вставить("РедактироватьСуществующий",		Ложь); // Установлен документ в редактируемых полях и есть возможность редактировать
		ПараметрыВложения.Вставить("ВозможноРедактироватьДокумент", Ложь);
...
Процедура ВложениеСБИС_УстановитьИниЗагрузки(ВложениеСБИС, ДопПараметры)
	...
	ВложениеСБИС.Параметры.РедактироватьСуществующий =	ВозможноРедактироватьДокумент
													И	ЗначениеЗаполнено(Поля1С.Документ.Значение);
...
Процедура ВложениеСБИС_Обновить(ВложениеСБИС, Изменения, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли Изменения.Свойство("ПодразделИни", ТекущиеИзменения) Тогда
		...
		ТекущийТипВложение = ВложениеСБИС_Получить(ВложениеСБИС, "ТипОбъекта", Новый ФиксированнаяСтруктура("ТипКратко", Истина));
	...
	ИначеЕсли Изменения.Свойство("Поля1С") Тогда
		...
		Если		Изменения.Поля1С.Свойство("Документ")
			...
			ВложениеСБИС.Параметры.РедактироватьСуществующий = ЗначениеЗаполнено(Изменения.Поля1С.Документ);
