...
Процедура ЛокальныйКэш_ОчиститьОбъект(ЛокальныйКэш) Экспорт
	...
	ТекущийСеанс_ОчиститьОбъект(ЛокальныйКэш.ТекущийСеанс);
...
&НаКлиенте
Процедура ТекущийСеанс_ОчиститьОбъект(ТекущийСеанс)
...
Функция НовыйТекущийСеанс(ПараметрыКонструктора = Неопределено)
	...
	Если ПараметрыКонструктора.Свойство("ПараметрыСистемы", ПараметрыСистемыТекущие) Тогда
		...
		ТекущийСеансПустой.Интерфейс.КэшироватьИнтерфейсныеФормы = Не ПараметрыСистемыТекущие.Клиент.ЭтоLinux И ОпцияПоСовестимости;
...
Функция СписокДоступныхСпособовСопоставленияНоменклатуры(Режим = "Интерфейс", ДопПараметры = Неопределено) Экспорт
	...
	РасширенныйФункционалСопоставленияНоменклатуры	= (ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры") ИЛИ ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры4100"));
