...
// Функция - создает описание сопоставления номенклатуры СБИС
//
// Параметры:
...
Функция НовыйСтрокаСопоставленияСБИСКлиент(ДанныеЗаполнитьВходящие = Неопределено, ДопПараметры = Неопределено) Экспорт // TO DO Ду-ду нужно разнести строку класса и строку СБИС в разные функции. Путаница происходит при заполнении
	...
	Если Не ДопПараметры.Свойство("ПорядокАвтоматическогоСопоставления") Тогда
		...
		Если ЗначениеЗаполнено(ПорядокАвтоматическогоСопоставления) Тогда
			ПорядокАвтоматическогоСопоставления = МодульОбъектаКлиент().СбисРазложитьСтрокуВМассивПодстрок(ПорядокАвтоматическогоСопоставления,",");
		Иначе
...
Процедура СтрокаСопоставленияСБИСКлиент_Заполнить(СтрокаСопоставленияСБИС, СопоставлениеЗаполнить, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли СопоставлениеЗаполнить.Свойство("Кол_во")
		И СопоставлениеЗаполнить.Кол_во > 0 // Когда у нас есть количество по строке, но нет учёта по финансам, то заполняем при формировании ТаблДока на загрузку 
		И СопоставлениеЗаполнить.Свойство("Цена") Тогда
		СписокСвойствЗаполнения = "Цена, Кол_во, Сумма, СуммаБезНал"; 
		ЗаполнитьЗначенияСвойств(СтрокаСопоставленияСБИС.НоменклатураСБИС, СопоставлениеЗаполнить, СписокСвойствЗаполнения);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(СтрокаСопоставленияСБИС.НоменклатураСБИС.Код) Тогда
		...
		Если	СопоставлениеЗаполнить.Свойство("ТипКода", ТипКода)
			И	ЗначениеЗаполнено(ТипКода) Тогда
		...
		Если НЕ ЗначениеЗаполнено(СЛокальнаяПеременная) Тогда
			Если 	(СопоставлениеЗаполнить.Свойство("Наименование", СЛокальнаяПеременная)
						ИЛИ СопоставлениеЗаполнить.Свойство("Название", СЛокальнаяПеременная))
					И ЗначениеЗаполнено(СЛокальнаяПеременная) Тогда
				ТипКода = "Наименование";
			Иначе
				СЛокальнаяПеременная = "";
				ТипКода = "";
			КонецЕсли;
...
&НаКлиенте
Функция СтрокаСопоставленияСБИСКлиент_ВыгрузитьДанныеДляСозданияНоменклатуры1С(ДанныеДляЗаполнения, ДопПараметры)
	Перем ИниКонфигурацияЛок;
	
	ДанныеДляСозданияНоменклатуры1С = Новый Структура;

	ИниКонфигурацияЛок = ДопПараметры.ИниКонфигурация;
	ОсновнаяЕдиница = Неопределено;
	// На СБИС очень прикольно записывается ключ единицы, в 1С мы такое не храним, потому договорились что берем первую единицу из списка как основную
	Для Каждого Единица Из ДанныеДляЗаполнения.Единицы Цикл 
		ОсновнаяЕдиница = Единица.Значение;
		Прервать;
	КонецЦикла;
	
	ОКЕИ = СопоставлениеДляЕдиницыКлиент_Получить(ОсновнаяЕдиница, "ОКЕИ");
	
	ФрмРаботаСДокументами = НайтиФункциюСеансаОбработки("сбисНоменклатураВид", "РаботаСДокументами1С");
	
	Если ИниКонфигурацияЛок.Свойство("Номенклатура_ВидНоменклатуры") Тогда
		ВидНоменклатуры = ФрмРаботаСДокументами.сбисНоменклатураВид(ДопПараметры.ВложениеСБИС); 
		ПозицияРазделителя = СбисСтрНайти(ИниКонфигурацияЛок.Номенклатура_ВидНоменклатуры.Значение, ".");
		РеквизитЗаполнения = Сред(ИниКонфигурацияЛок.Номенклатура_ВидНоменклатуры.Значение, ПозицияРазделителя + 1); 
		ДанныеДляСозданияНоменклатуры1С.Вставить(РеквизитЗаполнения, 	ВидНоменклатуры);
	КонецЕсли;
	
	ЭтоУслуги = ФрмРаботаСДокументами.сбисНоменклатураУслуга(ДопПараметры.ВложениеСБИС);
	
	ДанныеДляСозданияНоменклатуры1С.Вставить("Наименование", 		ДанныеДляЗаполнения.Наименование);
	ДанныеДляСозданияНоменклатуры1С.Вставить("НаименованиеПолное",  ДанныеДляЗаполнения.Наименование);
	ДанныеДляСозданияНоменклатуры1С.Вставить("ОКЕИ", 				ОКЕИ);  
	ДанныеДляСозданияНоменклатуры1С.Вставить("Комментарий", 		"Создано обработкой online.sbis");
	ДанныеДляСозданияНоменклатуры1С.Вставить("Услуги", 				ЭтоУслуги);	
	
	Возврат ДанныеДляСозданияНоменклатуры1С;
