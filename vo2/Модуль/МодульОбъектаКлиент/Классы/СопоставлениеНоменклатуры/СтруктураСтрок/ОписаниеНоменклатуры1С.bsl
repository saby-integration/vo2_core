...
Функция ОписаниеНоменклатуры1СКлиент_Получить(ОписаниеНоменклатуры1С, КлючПоиска) Экспорт
	...
	Если ТипЗнч(КлючПоиска) = Тип("Массив") Тогда
		...
		Для Каждого Ключ Из КлючПоиска Цикл   
			...
			ИначеЕсли Ключ = "НДС" Тогда
				...
				НДС.Сумма = ОписаниеНоменклатуры1С.Сумма;
	...
	ИначеЕсли		КлючПоиска = "Характеристика" Тогда
	...
	Иначе // To do Ду-ду Вот это "Иначе" оформлено некорректно. Просмотреть вызовы и исправить на присвоение ОписаниеНоменклатуры1С[Ключ] с вызовом исключения и возвратом Неопределено  (с) Сычев 
