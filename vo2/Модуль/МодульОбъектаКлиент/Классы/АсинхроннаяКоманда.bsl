...
Функция АсинхроннаяСбисКоманда_Получить(АсинхроннаяСбисКоманда, КлючПолучить, ТипПолучить=Неопределено) Экспорт
	...
	ИначеЕсли КлючПолучить = "АргументВызова" Тогда
		
		ВывестиДанные = Не АсинхроннаяСбисКоманда_Получить(АсинхроннаяСбисКоманда, "СкрытьДанные");
		Если Не ВывестиДанные Тогда
			
			Возврат Неопределено;
			
		КонецЕсли;
		
		Результат = АсинхроннаяСбисКоманда.АргументВызова;
		
		Если	ТипЗнч(Результат) = Тип("Структура")
			И	Результат.Свойство("Метод")
			И	Результат.Свойство("ПараметрыМетода") Тогда
			
			Результат = Результат.ПараметрыМетода;
...
Процедура АсинхроннаяСбисКоманда_Выполнить(АсинхроннаяСбисКоманда) Экспорт
	...
	Исключение
		...
		СобытиеПослеОтправки	= НовыйАсинхронноеСбисСобытие(АсинхроннаяСбисКоманда, РезультатОтправка, "Error");
	КонецПопытки;
...
Функция АсинхроннаяСбисКоманда_ПолучитьОтветСинхронно(АсинхроннаяСбисКоманда, ДопПараметры = Неопределено) Экспорт
	...
	Пока Истина Цикл
		...
		ИначеЕсли ТекущаяДата() - АсинхроннаяСбисКоманда.ВремяВызова > АсинхроннаяСбисКоманда.ВремяОжиданияОтвета Тогда
			...
			Результат = НовыйСбисИсключение(775101,"МодульОбъектаКлиент.СБИСПлагин_ПолучитьОтвет_Синхронно",,,
			"Нам не удалось получить ответ от плагина в течении " + Строка(АсинхроннаяСбисКоманда.ВремяОжиданияОтвета) + " секунд");
			
			ВызватьСбисИсключение(Результат, ПолучитьЗначениеПараметраСбис("ИнтеграцияИмя") + ".СБИСПлагин_ПолучитьОтвет_Синхронно");
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
КонецФункции
