...
//			ИдентификаторДействия		- Необязательный, Строка(ИмяПроцедуры) - ключ для поиска действия. Если не указан, то берется имя процедуры из описания оповещения.
...
Функция НовыйОтложенноеДействие(ПараметрыКонструктора) Экспорт
	...
	Попытка
		...
		ОбъектОтложенногоДействия = НовыйПрикладнойОбъектSABY("ОтложенноеДействие");
		ОбъектОтложенногоДействия.Вставить("Аргумент");
		ОбъектОтложенногоДействия.Вставить("ОписаниеОповещения");
		ОбъектОтложенногоДействия.Вставить("Запущен",			Ложь);
		ОбъектОтложенногоДействия.Вставить("Завершен",			Ложь);
		
		СледующийВызовМС = ПолучитьДатуВМиллисекундах();
		ОтложенноеДействие_Установить(ОбъектОтложенногоДействия, "СледующийВызовМС", СледующийВызовМС);
		ОтложенноеДействие_Установить(ОбъектОтложенногоДействия, "ПериодичностьМС",	100);
		ОтложенноеДействие_Установить(ОбъектОтложенногоДействия, "ЧислоВызовов",		1);
		ОтложенноеДействие_Установить(ОбъектОтложенногоДействия, "ПоследнийВызовМС", );
		...
		Иначе
			...
			Если ПараметрыКонструктора.Свойство("ИдентификаторДействия") Тогда
				...
				ПрикладнойОбъектSABY_Установить(ОбъектОтложенногоДействия, "Идентификатор", ПараметрыКонструктора.ИдентификаторДействия);
				
			КонецЕсли;
			
			Если ПараметрыКонструктора.Свойство("Периодичность") Тогда
				
				ПериодичностьМС = ПараметрыКонструктора.Периодичность * 1000;
				ОтложенноеДействие_Установить(ОбъектОтложенногоДействия, "ПериодичностьМС", ПериодичностьМС);
				...
				Если ПараметрыКонструктора.Свойство("ВызватьСразу") И Не ПараметрыКонструктора.ВызватьСразу Тогда
					...
					ОтложенноеДействие_Установить(ОбъектОтложенногоДействия, "СледующийВызовМС", СледующийВызовМС + ПериодичностьМС);
			...
			Если ПараметрыКонструктора.Свойство("ЧислоВызовов") Тогда
				...
				ОтложенноеДействие_Установить(ОбъектОтложенногоДействия, "ЧислоВызовов",		ПараметрыКонструктора.ЧислоВызовов);
	...
	Исключение
		...
		ВызватьИсключение СбисИсключение_Выгрузить(НовыйСбисИсключение(ИнформацияОбОшибке(), "МодульОбъектаКлиент.НовыйОтложенноеДействие"));
...
Функция ОтложенноеДействие_Скопировать(ОтложенноеДействие) Экспорт
	...
	Возврат НовыйПрикладнойОбъектSABY(ОтложенноеДействие);
	
КонецФункции

&НаКлиенте
Функция ОтложенноеДействие_Получить(ОтложенноеДействие, КлючПолучить) Экспорт
	Перем РезультатПолучить;
	
	Если Не ОбъектSABY_ЭтоТип(ОтложенноеДействие, "ОтложенноеДействие") Тогда
		
		ВызватьИсключение "Указанный объект не является типом ""Отложенное действие""";
		
	КонецЕсли;
	
	Если КлючПолучить = "ЦепочкаКомандОбработки" Тогда
		
		Если	ТипЗнч(ОтложенноеДействие.ОписаниеОповещения.ДополнительныеПараметры) = Тип("ФиксированнаяСтруктура")
			И	ОтложенноеДействие.ОписаниеОповещения.ДополнительныеПараметры.Свойство("ЦепочкаКомандОбработки", РезультатПолучить) Тогда
	...
	Возврат ПрикладнойОбъектSABY_Получить(ОтложенноеДействие, КлючПолучить, "Параметр");
	
КонецФункции

// Процедура - set-метод для отложенного действия
//
// Параметры:
...
//
&НаКлиенте
Процедура ОтложенноеДействие_Установить(ПрикладнойОбъектSABY, КлючДобавить, ЗначениеДобавить = Неопределено) Экспорт 

	Если Не ОбъектSABY_ЭтоТип(ПрикладнойОбъектSABY, "ОтложенноеДействие") Тогда
		
		ВызватьИсключение "Указанный объект не является типом ""Отложенное действие""";
		
	КонецЕсли;
	
	ПрикладнойОбъектSABY_Установить(ПрикладнойОбъектSABY, КлючДобавить, ЗначениеДобавить, "Параметр");
...
//  ДопПараметры		 - Структура, ФиксированнаяСтруктура		  - расширение. Аргумент для подключения отложенно описания оповещения с аргументом
//
&НаКлиенте
Процедура ПодключитьОтложенноеДействие(ОтложенноеДействие, ДопПараметры = Неопределено) Экспорт
	...
	Если		ЭтоТипСбис(ОтложенноеДействие, "СбисОписаниеОповещения") Тогда
		...
		// Для описания оповещения, ДопПараметр это аргумент. Для необходимости настройки подключения, делать ФиксированнуюСтруктуру
		Если	Не	ТипЗнч(ДопПараметры) = Тип("ФиксированнаяСтруктура")
			Или	Не	ДопПараметры.Свойство("Аргумент", ОтложеннаяОперацияПодключить.Аргумент) Тогда
			
			ОтложеннаяОперацияПодключить.Аргумент = ДопПараметры;
	...
	Иначе
		...
		ВызватьИсключение "Ошибка подключения отложенного действия: Неизвестный тип (" + ТипЗнч(ОтложенноеДействие) + ")";
		
	КонецЕсли;
	
	ОтложенныйИД			= ПрикладнойОбъектSABY_Получить(ОтложеннаяОперацияПодключить,,"Ид");
	...
	ОперацияЗапущена		= ОтложенныеПроцедуры.Получить(ОтложенныйИД);
	Если Не ОперацияЗапущена = Неопределено Тогда
		
		СбисИсключение = НовыйСбисИсключение(,"МодульОбъектаКлиент.ПодключитьОтложенноеДействие", 735,, "Действие с идентификатором " + ОтложенныйИД + "уже запущено"); 
		ВызватьИсключение СбисИсключение_Выгрузить(СбисИсключение);
		
	Иначе
		
		ОтложенныеПроцедуры.Вставить(ОтложенныйИД, ОтложеннаяОперацияПодключить); 
		
	КонецЕсли;
	
	Если СбисОтложенныеОперации.Запущено Тогда
		
		//Переключаем чтобы вызов произошел сразу.
...
Функция ПолучитьОтложенноеДействие(ОтложенноеДействие) Экспорт
	...
	Иначе
		КлючОперации = ПрикладнойОбъектSABY_Получить(ОтложенноеДействие,, "Ид");
	КонецЕсли;
...
Процедура ОтключитьОтложенноеДействие(ОтложенноеДействие) Экспорт
	...
	Если Не ОтложеннаяОперация = Неопределено Тогда
		...
		ГлобальныйКэш.ТекущийСеанс.сбисОтложенныеОперации.Процедуры.Удалить(ПрикладнойОбъектSABY_Получить(ОтложеннаяОперация,, "Ид"));
