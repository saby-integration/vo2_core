...
Процедура СоздатьЧерновик(ДанныеПакета, ДопПараметры) Экспорт
	...
	Попытка
		...
		ИниТекущегоРеестра = ДанныеПакета.ИмяИни;
		ЗначениеИниТекущегоРеестра = МодульНастройки.Ини(ПолучитьТекущийЛокальныйКэш(), ИниТекущегоРеестра);
		...
		КонецЕсли;
		ЗначениеУзлаИни = Новый Структура("Значение,РассчитанноеЗначение", Истина, Истина);
		ЗначениеИниТекущегоРеестра.Вставить("НеЗапускатьВДокументооборот", ЗначениеУзлаИни);
		...
		Иначе
			// old
			СоставПакетаОтправить = Новый Структура("СоставПакета", ДанныеПакета.СоставПакета);
