...
Функция СписокСкладскихПараметров()  
	...
	СписокПараметров.Добавить("ReceiptDocType");
	СписокПараметров.Добавить("primary_document_custom_name");
	СписокПараметров.Добавить("document_date");
	СписокПараметров.Добавить("document_number");
...
Функция ПолучитьСкладскиеПараметрыДокумента(СоставПакета, ДопПараметры = Неопределено) Экспорт
	...
	ПараметрыДокументаВыбытия	= ПолучитьПараметрыДокументаВыбытия(ПараметрыДокументаСбис);
	...
	СкладскиеПараметрыДокумента.Вставить("ПараметрыДокументаВыбытия",	ПараметрыДокументаВыбытия);
...
// Функция получает структуру параметров документа выбытия для маркировки
//
// Параметры:
//  ПараметрыДокумента	 - структура - параметры, полученные с онлайна:
//										- marking_shipment_reason - причина вывода кодов (тип договора)
//										- to_not_participant - участник НЕ зарегистрирован в ГИС МТ
//										- ReceiptDocType / ТипДокументаВыбытия - текущий тип документа выбытия
//										- primary_document_custom_name - имя документа выбытия
//										- document_date - дата документа выбытия
//										- document_number - номер документа выбытия
//  ДопПараметры		 - структура - дополнительные параметры документа 
// 
// Возвращаемое значение:
//   - Структура:
//		- СписокТиповДокументаВыбытия - СписокЗначений
//		- ТипДокументаВыбытия - Строка - тот тип документа, который сейчас установлен в документе
//		- ПоказыватьПолеИмяДокументаВыбытия - Булево
//		- ПоказыватьПолеДатаДокументаВыбытия - Булево
//		- ПоказыватьПолеНомерДокументаВыбытия - Булево
//
&НаКлиенте
Функция ПолучитьПараметрыДокументаВыбытия(ПараметрыДокументаСбис, ДопПараметры = Неопределено) Экспорт
	
	ПараметрыДокументаВыбытия = Новый Структура();
	ПараметрыДокументаВыбытия.Вставить("СписокТиповДокументаВыбытия",				Новый СписокЗначений);
	ПараметрыДокументаВыбытия.Вставить("ТипДокументаВыбытия",						"");
	ПараметрыДокументаВыбытия.Вставить("ИмяДокументаВыбытия",						"");
	ПараметрыДокументаВыбытия.Вставить("ДатаДокументаВыбытия",						Неопределено);
	ПараметрыДокументаВыбытия.Вставить("НомерДокументаВыбытия",						"");
	
	ПараметрыДокументаВыбытия.СписокТиповДокументаВыбытия = СписокТиповДокументаВыбытия(ПараметрыДокументаСбис, ДопПараметры);
	
	Если	ПараметрыДокументаСбис.Свойство("ReceiptDocType") 
			И ЗначениеЗаполнено(ПараметрыДокументаСбис.ReceiptDocType)
			И ЗначениеЗаполнено(ПараметрыДокументаВыбытия.СписокТиповДокументаВыбытия) Тогда
		ПараметрыДокументаВыбытия.ТипДокументаВыбытия	= ПараметрыДокументаСбис.ReceiptDocType;
	КонецЕсли;
	
	Если ПараметрыДокументаСбис.Свойство("primary_document_custom_name") Тогда
		ПараметрыДокументаВыбытия.ИмяДокументаВыбытия	= ПараметрыДокументаСбис.primary_document_custom_name;
	КонецЕсли;
	
	Если ПараметрыДокументаСбис.Свойство("document_date") И ЗначениеЗаполнено(ПараметрыДокументаСбис.document_date) Тогда
		ПараметрыДокументаВыбытия.ДатаДокументаВыбытия	= ПривестиТипСбис(ПараметрыДокументаСбис.document_date, Тип("Дата"));
	КонецЕсли;
	
	Если ПараметрыДокументаСбис.Свойство("document_number") Тогда
		ПараметрыДокументаВыбытия.НомерДокументаВыбытия	= ПараметрыДокументаСбис.document_number;
	КонецЕсли;

	Возврат ПараметрыДокументаВыбытия;																	
																		
КонецФункции

// Функция возвращает список для выбора типа документа выбытия в зависимости от регистрации контрагента в ГИС МТ и кода причины вывода (типа договора)
//
// Параметры:
//  ПараметрыДокумента	 - структура - параметры, полученные с онлайна:
//										- marking_shipment_reason - причина вывода кодов (тип договора)
...
//	("4",	"Прочий документ");
//	("5",	"Не выбрано");
//	("6",	"Указано на позициях");
//	("7",	"Акт протезно-ортопедического изделия");
//	("9",	"Электронный сертификат");
//	("10", "Акт уничтожения");
//	("11", "Декларация");
&НаКлиенте
Функция СписокТиповДокументаВыбытия(ПараметрыДокумента, ДопПараметры = Неопределено) Экспорт
	
	ТипыДокументаВыбытия = Новый СписокЗначений;
	
	КодПричиныВывода = "";
	НеЗарегистрированВГИСМТ = Истина;
	Если Не ПараметрыДокумента = Неопределено Тогда
		
		Если ПараметрыДокумента.Свойство("marking_shipment_reason") Тогда
			КодПричиныВывода = Строка(ПараметрыДокумента.marking_shipment_reason);
		КонецЕсли;
		
		Если ПараметрыДокумента.Свойство("to_not_participant") Тогда
			НеЗарегистрированВГИСМТ = НРег(ПараметрыДокумента.to_not_participant) = "true";
		КонецЕсли;
		
	КонецЕсли;
	
	Если НеЗарегистрированВГИСМТ Тогда
		
		Если КодПричиныВывода = "3" Тогда // Безвозмездная передача
			
			ТипыДокументаВыбытия.Добавить("1",	"Товарная накладная");
			ТипыДокументаВыбытия.Добавить("4",	"Прочий документ");
			
		КонецЕсли;
		
	КонецЕсли;
	
	Если	КодПричиныВывода = "4"				// Дистанционная торговля
			ИЛИ КодПричиныВывода = "11" Тогда	// Продажа по образцам
			
		ТипыДокументаВыбытия.Добавить("5",	"Не выбрано");
		ТипыДокументаВыбытия.Добавить("0",	"УПД");
		ТипыДокументаВыбытия.Добавить("1",	"Товарная накладная");
		ТипыДокументаВыбытия.Добавить("2",	"Кассовый чек");
		ТипыДокументаВыбытия.Добавить("3",	"Товарный чек");
		ТипыДокументаВыбытия.Добавить("4",	"Прочий документ");
		
	КонецЕсли;
		
	Возврат ТипыДокументаВыбытия;
...
Функция ИзменитьСкладскойПараметрДокумента(СоставПакета, УстанавливаемыеПараметры) Экспорт 
	...
	ИначеЕсли   УстанавливаемыеПараметры.Свойство("ИдентификаторГосконтракта", ЛНовоеЗначениеУстановить) Тогда
		...
		ПараметрыВызова.Вставить("value",	ЛНовоеЗначениеУстановить);
	ИначеЕсли   УстанавливаемыеПараметры.Свойство("ТипДокументаВыбытия", ЛНовоеЗначениеУстановить) Тогда
		ПараметрыВызова.Вставить("name",	"ReceiptDocType");
		ПараметрыВызова.Вставить("value",	ЛНовоеЗначениеУстановить);
	ИначеЕсли   УстанавливаемыеПараметры.Свойство("ИмяДокументаВыбытия", ЛНовоеЗначениеУстановить) Тогда
		ПараметрыВызова.Вставить("name",	"primary_document_custom_name");
		ПараметрыВызова.Вставить("value",	ЛНовоеЗначениеУстановить);
	ИначеЕсли   УстанавливаемыеПараметры.Свойство("ДатаДокументаВыбытия", ЛНовоеЗначениеУстановить) Тогда
		ПараметрыВызова.Вставить("name",	"document_date");
		ПараметрыВызова.Вставить("value",	ЛНовоеЗначениеУстановить);
	ИначеЕсли   УстанавливаемыеПараметры.Свойство("НомерДокументаВыбытия", ЛНовоеЗначениеУстановить) Тогда
		ПараметрыВызова.Вставить("name",	"document_number");
		ПараметрыВызова.Вставить("value",	ЛНовоеЗначениеУстановить);
	Иначе
...
Процедура ЗаполнитьНомСредИдентТов_ИдентТрансУпак(НомСредИдентТов, ДанныеПоМаркировке) Экспорт
	...
	Для Каждого Элемент Из ДанныеПоМаркировке.ИдентТрансУпак Цикл
		...
		НомСредИдентТов.Добавить(Новый Структура("ИдентТрансУпак", Элемент.Значение.ЗначениеШтрихкода));
...
Процедура ЗаполнитьНомСредИдентТов_НомУпак(НомСредИдентТов, ДанныеПоМаркировке) Экспорт
	...
	Для Каждого Элемент Из ДанныеПоМаркировке.НомУпак Цикл 
		...
		ДанныеДобавить.Добавить(Новый Структура("Значение", Элемент.Значение.ЗначениеШтрихкода));   				
		ДанныеОчистить.Добавить(Элемент);
		
		Если СбисЭтоКодОСУ(Элемент.Значение.ЗначениеШтрихкода) Тогда   // коды объемно-сортового учета раскладываются по одному на строку 
...
Процедура ЗаполнитьНомСредИдентТов_КИЗ(НомСредИдентТов, ДанныеПоМаркировке, Количество) Экспорт
	...
	СчКоличестваПоМаркировке = 0;
	Для Каждого Элемент Из ДанныеПоМаркировке.КИЗ Цикл
		
		ДанныеДобавить.Добавить(Новый Структура("Значение", Элемент.Значение.ЗначениеШтрихкода));
		ДанныеОчистить.Добавить(Элемент);
		
		СчКоличестваПоМаркировке = СчКоличестваПоМаркировке + Элемент.Значение.Количество;
		Если СчКоличестваПоМаркировке >= Количество Тогда
			Прервать;
		КонецЕсли;
...
//
&НаКлиенте
Процедура ЗаполнитьНомСредИдентТов_КолВедМарк(НомСредИдентТов, ДанныеПоМаркировке, Количество) Экспорт
	...
	Для Каждого Элемент Из ДанныеПоМаркировке.КолВедМарк Цикл
		...
		КолВоДобавить = Мин(Количество, Элемент.Значение.Количество);
		НомСредИдентТов.Добавить(Новый Структура("КолВедМарк", Строка(КолВоДобавить)));
		
		Элемент.Значение.Количество = Элемент.Значение.Количество - КолВоДобавить;
		Если Элемент.Значение.Количество = 0 Тогда
			ДанныеОчистить.Добавить(Элемент);
		КонецЕсли;
		
		СчКоличестваПоМаркировке = СчКоличестваПоМаркировке + КолВоДобавить;
		Если СчКоличестваПоМаркировке >= Количество Тогда
			Прервать;
		КонецЕсли;
...
Процедура ЗаполнитьНомСредИдентТов_ПрПартМарк(НомСредИдентТов, ДанныеПоМаркировке) Экспорт
	...
	Для Каждого Элемент Из ДанныеПоМаркировке.ПрПартМарк Цикл
		...
		НомСредИдентТов.Добавить(Новый Структура("ПрПартМарк", Элемент.Значение.ЗначениеШтрихкода));
...
Функция СписокТоварныхГрупп_Наборы()
	...
	СписокТоварныхГрупп.Добавить("КреслаКоляски");
...
Процедура ОбработатьРезультатПроверкиТокена(СоставПакета, РезультатПроверкиТокена, ДопПараметры = Неопределено) Экспорт 
	...
	ИначеЕсли НРег(РезультатНаличияТокена) = "error" Тогда  
		...
		КодОшибки		= 310000 + РезультатПроверкиТокена.Code;
		ОшибкаОбмена 	= НовыйСбисИсключение(, "МодульОбъектаКлиент.ОбработатьРезультатПроверкиТокена",КодОшибки, РезультатПроверкиТокена.Description);
...
Процедура СоздатьТокенГИСМТ(РезультатДиалога, ДопПараметры) Экспорт  
	...
	Если Не ПолучитьЗначениеПараметраСбис("ОбменВключен") Тогда
		...
		ОшибкаОбмена = НовыйСбисИсключение(775102,"МодульОбъектаКлиент.СоздатьТокенГИСМТ",,,"Сертификаты не найдены. Выберите способ обмена в настройках соединения."); 
