...
//
&НаКлиенте
Процедура ЗаполнитьНомСредИдентТов_КИЗ(НомСредИдентТов, ДанныеПоМаркировке) Экспорт
...
Процедура ОбработатьРезультатПроверкиТокена(СоставПакета, РезультатПроверкиТокена, ДопПараметры = Неопределено) Экспорт 
	...
	Если НРег(РезультатНаличияТокена) = "ok" Тогда   
		...
		Попытка
			...
			Если ПолучитьЗначениеПараметраСБИС("ИспользоватьНовоеСопоставлениеНоменклатуры") Тогда
		...
		Исключение  
			...
			ВызватьИсключение СбисИсключение_Представление(ОшибкаПроверки);
	...
	ИначеЕсли НРег(РезультатНаличияТокена) = "error" Тогда  
		...
		ОшибкаОбмена 	=  НовыйСбисИсключение(СтруктураОшибки,"МодульОбъектаКлиент.ОбработатьРезультатПроверкиТокена");
