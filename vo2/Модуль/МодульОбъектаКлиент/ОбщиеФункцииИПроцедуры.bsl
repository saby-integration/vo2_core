...
Функция СкачатьВФайлПоПараметрам(ПараметрыВходящие, ДопПараметры = Неопределено) Экспорт
	Перем UrlДляСкачивания, ИмяФайла, ПолноеИмяФайла, РасширениеФайла, СоздаватьНовый, Сервер;
	...
	Если UrlДляСкачивания = Неопределено Тогда
		...
		ОшибкаСкачивания = НовыйСбисИсключение(785101, "МодульОбъектаКлиент.СкачатьВФайлПоПараметрам",,"Отсутствует URL");
		ВызватьИсключение СбисИсключение_Выгрузить(ОшибкаСкачивания);
		
	КонецЕсли;

	Сервер = ПолучитьАдресСервераИзURL(UrlДляСкачивания);
	...
	Если ОшибкаСкачивания Тогда
		...
		СообщениеОбОшибке = "Не удалось скачать файл по ссылке: " + UrlДляСкачивания + ". Рекомендуем проверить доступ к ресурсу " + Сервер;
		ОшибкаСкачивания = НовыйСбисИсключение(401102, "МодульОбъектаКлиент.СкачатьВФайлПоПараметрам", ,,СообщениеОбОшибке);
		ВызватьИсключение СбисИсключение_Выгрузить(ОшибкаСкачивания);
...
Функция СоздатьОписаниеТипаПоОбъекту(ОбъектТипов) Экспорт
	...
	Если ТипЗнч(ОбъектТипов) = Тип("Строка") Тогда
		...
		Исключение
			...
			ВызватьИсключение(СбисИсключение_Выгрузить(ОшибкаУстановкиФильтра));
...
Функция ВыполнитьДействиеСДокументомСБИС_НаборВложенийДляВыполнитьДействие(ПараметрыДействия, ДопПараметры)
	...
	Для Каждого Вложение Из ПакетВложения Цикл
		...
		ИмяФайла = ВложениеСБИС_Получить(Вложение, "ИмяФайла");
...
Функция ПолучитьФормуОбработки(СбисИмяФормы, ДопПараметры=Неопределено) Экспорт 
	...
	КлючФормыДляКонтроля	= НРег(СбисИмяФормы);
	ЭтоГлавноеОкно			= КлючФормыДляКонтроля = "формаглавноеокно" И Стандартная;
	...
	Иначе
		...
		ЭтоИнтерфейсныйМодуль = Не ГлобальныйКэш.ТекущийСеанс.Формы.Интерфейсные.НайтиПоЗначению(КлючФормыДляКонтроля) = Неопределено;
	...
	Если Не ГлобальныйКэш = Неопределено Тогда
		...
		Если		КэшироватьПолученную = Неопределено
			...
			И	Не	ПараметрSABY("КэшироватьИнтерфейсныеФормы", "Сеанс") Тогда
	...
	Если КэшироватьПолученную = Неопределено Тогда
		...
		КэшироватьПолученную = Не ЭтоГлавноеОкно;
	...
	Если		ПолучитьНовую
		...
		ЕстьТакаяФормаВОбработках =		ЭтоГлавноеОкно		
								Или		Стандартная И ГлобальныйКэш.ТекущийСеанс.Формы.Стандартные.НайтиПоЗначению(КлючФормыДляКонтроля)	<> Неопределено
								Или	Не	Стандартная И ГлобальныйКэш.ТекущийСеанс.Формы.Клиентские.НайтиПоЗначению(КлючФормыДляКонтроля)		<> Неопределено;
		...
		Если ЕстьТакаяФормаВОбработках Тогда
			...
			#Если ТолстыйКлиентОбычноеПриложение Тогда
				...
				Исключение
					...
					ВызватьИсключение СбисИсключение_Выгрузить(ОшибкаПолучения);
					
				КонецПопытки;
				
				Если ЭтоГлавноеОкно Тогда
					
					Результат.ПриСозданииНаСервере();
			...
			// Проверка интерфейсных модулей и установка параметров связи кэша
			Если	Не	ЭтоИнтерфейсныйМодуль
				И		ГлобальныйКэш.ТекущийСеанс.Формы.СлужебныеМодули.НайтиПоЗначению(КлючФормыДляКонтроля) = Неопределено Тогда
				
				Если ЭтоИнтерфейсныйМодуль(Результат) Тогда
					
					ЭтоИнтерфейсныйМодуль	= Истина;
					КэшироватьПолученную	= ПараметрSABY("КэшироватьИнтерфейсныеФормы", "Сеанс");
					ГлобальныйКэш.ТекущийСеанс.Формы.Интерфейсные.Добавить(КлючФормыДляКонтроля);
					
				Иначе
					
					ГлобальныйКэш.ТекущийСеанс.Формы.СлужебныеМодули.Добавить(КлючФормыДляКонтроля);
					
				КонецЕсли;
				
			КонецЕсли;
				
			Если		ЭтоИнтерфейсныйМодуль
				И НЕ	ЭтоГлавноеОкно Тогда
				
				Результат.Кэш = ПолучитьТекущийЛокальныйКэш();
				Результат.ПараметрыФормы = ПараметрыИнтерфейсныйМодулей();
...
// Функция - проверяет, является ли модуль интерфейсным по наличию признака ПараметрыФормы
//
// Параметры:
//  ФормаОбработки	 - Форма	 - проверяемая форма
// 
// Возвращаемое значение:
//  Булево - является ли форма интерфейсным модулем
//
&НаКлиенте
Функция ЭтоИнтерфейсныйМодуль(ФормаОбработки)
	
	ПараметрыПроверитьДо = Новый Структура("Кэш, ПараметрыФормы");
	ЗаполнитьЗначенияСвойств(ПараметрыПроверитьДо, ФормаОбработки);
	Если	Не ПараметрыПроверитьДо.ПараметрыФормы = Неопределено
		И	Не ПараметрыПроверитьДо.Кэш = Неопределено Тогда
		
		// Есть заполненные параметры формы.
		Возврат Истина;
		
	КонецЕсли;
	ПараметрыПроверитьПосле = Новый Структура("Кэш, ПараметрыФормы", "Тест", "Тест");
	ЗаполнитьЗначенияСвойств(ФормаОбработки, ПараметрыПроверитьПосле);
	ПараметрыПроверитьПосле.ПараметрыФормы = Неопределено;
	ПараметрыПроверитьПосле.Кэш = Неопределено;
	ЗаполнитьЗначенияСвойств(ПараметрыПроверитьПосле, ФормаОбработки);
	
	Результат = Ложь;
	Если	ПараметрыПроверитьПосле.ПараметрыФормы = "Тест"
		И	ПараметрыПроверитьПосле.Кэш = "Тест" Тогда
	...
	// Восстановить значения, как было до проверки
	ЗаполнитьЗначенияСвойств(ФормаОбработки, ПараметрыПроверитьДо);
...
Функция ПолучитьBASE64ПоИмениФайлаКлиент(ПутьКФайлуКлиент) Экспорт
	...
	# Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Исключение
			...
			ВызватьИсключение (СбисИсключение_Выгрузить(ОшибкаПреобразования));
...
Функция ПолучитьИдКоннекшена(Отказ = Ложь) Экспорт
    Перем ТекущееПодключение;
	
	Если Не ПолучитьЗначениеПараметраСбис("ДоступныСерверныеНастройки") Тогда
		// Если интеграция каталог
		Возврат Неопределено;
	КонецЕсли;
	Если	ГлобальныйКэш.КэшНастроек.Свойство("ВыбранныеНастройки", ТекущееПодключение)
		И	ЭтоТипСбис(ТекущееПодключение, "КоннекшенSaby") Тогда
		
		ПодключениеНаБЛ = КоннекшенSABY_Получить(ТекущееПодключение, "ПодключениеНаБЛ");
		
	Иначе
		
		ТекущееПодключение = ГлобальныйКэш.ТекущийСеанс.Модули.Настройки.СбисПолучитьНастройкиСБИС(ГлавноеОкно.Кэш, Отказ);
		
		Если Отказ Тогда
			
			Возврат НовыйСбисИсключение(ТекущееПодключение, МодульОбъектаКлиентСервер_Представление() + "ПолучитьИдКоннекшена");
	...
	//Нет подключения на БЛ
	Если ПодключениеНаБЛ = Неопределено Тогда
		
		Возврат Неопределено;
		
	КонецЕсли;
	
	Возврат КоннекшенSABY_Получить(ПодключениеНаБЛ, "Идентификатор");
...
Функция ПолучитьИдИнтеграции(Отказ = Ложь) Экспорт
	...
	Если	ГлобальныйКэш.КэшНастроек.Свойство("ВыбранныеНастройки", ТекущееПодключение)
		...
		Возврат КоннекшенSABY_Получить(ТекущееПодключение, "ИдентификаторИнтеграции");
...
&НаКлиенте
Функция ПолучитьАдресСервераИзURL(СтрокаURL)

	Результат = "";
	Если ПустаяСтрока(СтрокаURL) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ПозицияПротокола = Найти(СтрокаURL, "://");
	
	Если ПозицияПротокола > 0 Тогда
		ОстальнаяЧасть = Сред(СтрокаURL, ПозицияПротокола + 3);
	Иначе
		ОстальнаяЧасть = СтрокаURL;
	КонецЕсли;
	
	ПозицияСлеша = Найти(ОстальнаяЧасть, "/");
	
	Результат = Лев(ОстальнаяЧасть, ПозицияСлеша-1);	
...
#Область include_core2_vo2_Модуль_МодульОбъектаКлиент_ОбщиеФункцииИПроцедуры_Разное
#КонецОбласти
