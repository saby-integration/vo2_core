...
Процедура СформироватьРасхождениеСДокументомСБИС(ПараметрыВходящие, ДопПараметры) Экспорт 
	Перем ВложениеРасхождение, ИниВычиткиРасхождения, ОшибкаФормирования;
	...
	МФайлОтлонение		= СбисРазложитьСтрокуВМассивПодстрок(ДокументОтклонения, "_")[0];
	
	Если	Не ИниДокумента.Свойство("мФайл", ИниВычиткиРасхождения)
		Или	Не ИниВычиткиРасхождения.Свойство(МФайлОтлонение, ИниВычиткиРасхождения) Тогда
		
		ИсключениеОшибки = НовыйСбисИсключение(779, "МодульОбъектаКлиент.СформироватьРасхождениеСДокументомСБИС",,, "Отсутствует узел " + МФайлОтлонение + " в файле настроек " + ИмяИниДокумента);
	...
	ЗначениеИни = Новый Структура("мФайл", Новый Структура(МФайлОтлонение, ИниВычиткиРасхождения));
	...
	Если Не КонтекстРасхождение.ДокументДанные.Свойство("мФайл") Тогда
		...
		ИсключениеОшибки = НовыйСбисИсключение(726, "МодульОбъектаКлиент.СформироватьРасхождениеСДокументомСБИС",,, "Документ по настройке " + ИмяИниДокумента + ".мФайл." + МФайлОтлонение + " не рассчитан!");
		ВыполнитьСбисОписаниеОповещения(ИсключениеОшибки, ДопПараметры.ОбработчикРезультата);
		Возврат;
	...
	Если	Не МодульКонтекст_Расхождение.ПрочитатьДокумент(ГлавноеОкно.Кэш, КонтекстРасхождение)
		...
		ИсключениеОшибки = НовыйСбисИсключение(726, "МодульОбъектаКлиент.СформироватьРасхождениеСДокументомСБИС",,, "Документ по настройке " + ИмяИниДокумента + ".мФайл." + МФайлОтлонение + " не сформирован!");
		ВыполнитьСбисОписаниеОповещения(ИсключениеОшибки, ДопПараметры.ОбработчикРезультата);
		Возврат;
...
Функция СформироватьСписокДляВызоваРасхождений(СтрокиОтмеченные, ДопПараметры) Экспорт
	...
	Для Каждого СтрокаТаблДок Из СтрокиОтмеченные Цикл
		...
		Если СтруктураИниВложения.Свойство("ФайлНастроекРасхождения") Тогда
			...
			ИмяДокумента = ПолучитьФормуОбработки("Документ_Шаблон").РассчитатьЗначение("ФайлНастроекРасхождения", Контекст);
