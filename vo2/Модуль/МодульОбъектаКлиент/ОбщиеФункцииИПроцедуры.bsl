...
Процедура СбисЗавершитьРаботу() Экспорт
	...
	Попытка
		...
		ГлавноеОкно.Кэш.Интеграция.Завершить(ГлавноеОкно.Кэш, Новый Структура("ЗавершитьРаботу", Истина), Ложь);
...
Функция ПолучитьФормуОбработки(СбисИмяФормы, ДопПараметры=Неопределено) Экспорт 
	...
	Если Не ГлобальныйКэш = Неопределено Тогда
		...
		Если		КэшироватьПолученную = Неопределено
			И	Не	ГлобальныйКэш.ТекущийСеанс.Формы.Интерфейсные.НайтиПоЗначению(СбисИмяФормы) = Неопределено Тогда
			
			КэшироватьПолученную = ПолучитьЗначениеПараметраТекущегоСеанса("ОпцияКэшироватьФормуИнтерфейса");
