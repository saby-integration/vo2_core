...
Процедура ЗапуститьМассовуюОтправкуДокументов_Завершение(РезультатОтправки, ДопПараметры=Неопределено) Экспорт
	...
	// Обраюотать узел ТребуютсяПользовательскиеДействия
	...
	КонецЕсли;
	Если МодульОбъектаКлиент().ПолучитьЗначениеФичи(Новый Структура("НазваниеФичи", "РеестрЗаказов"))
		И НРег(Кэш.Текущий.ТипДок) = "заказвх" Тогда
		ОбновитьДополнительныйАккордеон();
	КонецЕсли;
...
Процедура СбисЗавершитьРаботу() Экспорт
	...
	ОбработчикПослеЗавершения = НовыйСбисОписаниеОповещения("СбисЗавершитьРаботу_ПослеЗавершения", МодульОбъектаКлиент());
	ПодключитьОтложенноеДействие(ОбработчикПослеЗавершения, Истина);
...
//
&НаКлиенте
Процедура СбисЗавершитьРаботу_ПослеЗавершения(АргументЗакрытия, ДопПараметры = Неопределено) Экспорт
...
Функция ВыполнитьДействиеСДокументомСБИС(ПараметрыДействия, ДопПараметры = Неопределено) Экспорт 
	...
	ПараметрыДействия.Свойство("Сертификат", Сертификат);
	...
	Если СоставПакета.Свойство("Вложение", ПакетВложения) Тогда
		Для Каждого Вложение Из ПакетВложения Цикл 
			...
			ВложениеТребуетЗаписи =		Вложение.Свойство("ТребуетПодписания")
									И	Вложение.ТребуетПодписания
									Или	Вложение.Свойство("Модифицирован")
									И	Вложение.Модифицирован;

			Если Не ВложениеТребуетЗаписи Тогда
...
Функция ПолучитьФормуОбработки(СбисИмяФормы, ДопПараметры=Неопределено) Экспорт 
	...
	Если Не ГлобальныйКэш = Неопределено Тогда
		...
		Если		КэшироватьПолученную = Неопределено
			...
			И	Не	ПолучитьЗначениеПараметраТекущегоСеанса("ОпцияКэшироватьФормуИнтерфейса") Тогда
