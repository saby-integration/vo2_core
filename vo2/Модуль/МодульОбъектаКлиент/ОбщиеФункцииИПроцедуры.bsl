...
Процедура ВыполнитьДействиеСДокументомСБИС(ПараметрыДействия, ДопПараметры=Неопределено) Экспорт 
	...
	фрм = ГлавноеОкно.СбисНайтиФормуФункции(ИмяФункции, "Файл_"+СоставПакета.Тип+"_"+СтрЗаменить(СоставПакета.Вложение[0].ВерсияФормата,".","_"), "РаботаСДокументами1С");
...
&НаКлиенте
Функция СсылочнйТипСтрокойВзовКлиента(Ссылка, ДопПараметры = Неопределено) 
...
&НаКлиенте
Функция СсылочнйТипСтрокой(Ссылка, ДопПараметры = Неопределено) Экспорт 

	#Если Не ВебКлиент Тогда
	
	Возврат СсылочнйТипСтрокойВзовКлиента(Ссылка, ДопПараметры = Неопределено);
		
	#Иначе
	
	Возврат МодульОбъектаСервер().СсылочнйТипСтрокойВзовСервера(Ссылка, ДопПараметры = Неопределено);
...
#Область include_core_vo2_Модуль_МодульОбъектаКлиент_ОбщиеФункцииИПроцедуры_РаботаСИниИXSLT
#КонецОбласти
