...
Функция ОбновитьИзмененияПоДокументамСбис(ПараметрыОбновленияВходящие, ДопПараметры) Экспорт
	...
	Если МодульРаботыСоСтатусамиСБИС = Ложь Тогда
		СбисИсключение = НовыйСбисИсключение(, "МодульОбъектаКлиент.СбисВыполнитьОбновлениеСтатусов",779,,"Не удалось определить модуль для обработки статусов Saby");
...
Процедура СбисВыполнитьОбновлениеПачкиСтатусов(ДействиеОбновления, ДопПараметры) Экспорт
	Перем 	ПоследнееСобытие, ИдСобытияДляНавигации, ДатаСобытияДляНавигации; 
...
#КонецОбласти
	...
	Если СписокИзменений.Документ.Количество() Тогда
		
		ДействиеОбновления.Параметры.ЕстьИзменения = Истина;
		
		ПараметрыЗаписатьИзменения = Новый Структура("МассовоеДействие, ИдАккаунта", ДействиеОбновления);
		ПараметрыЗаписатьИзменения.ИдАккаунта = Кэш.Интеграция.ПолучитьИдТекущегоАккаунта(Кэш);
		
		ДопПараметры.МодульСтатусовСбис.СбисЗаписатьИзмененияПоДокументам1С(Кэш, МассивДокументов, ПараметрыЗаписатьИзменения);
	...
	// Обновить метки по событиям, которые мы обрабатывали
	Если	ИдСобытияДляНавигации	= ""
		И	ДатаСобытияДляНавигации = "" Тогда
		
		// События есть, но в них нет ни одного с датой, или идентификатором. Почему так может быть?
		// В любом случае, прерываем обновление статусов.
		// Скидываем ИД последнего события, чтобы упростить корректировку - достаточно поправить период.
		СтруктураНастроек.ИдентификаторПоследнегоСобытия	= "";
		СообщениеДеталиазции		= "Неверный формат списка: отсутствует описание событий документов. Требуется ручная корректировка периода для дальнейшего обновления!";
		ОшибкаОбновленияСтатусов	= НовыйСбисИсключение("МодульОбъектаКлиент.СбисВыполнитьОбновлениеПачкиСтатусов", 726,, СообщениеДеталиазции);
		ВызватьИсключение(СбисИсключение_Выгрузить(ОшибкаОбновленияСтатусов));
		
	ИначеЕсли	СписокИзменений.Документ.Количество() Тогда

		// У нас есть дата или ид события навигации, чтобы сдвинуть чтение пачки дальше.
		// Даже при самом негативном сценарии, если событие с данными было 2 в списке и больше ничего нет, то может последнее событие в следующей пачке будет с данными для перехода дальше.
		СтруктураНастроек.ИдентификаторПоследнегоСобытия	= ИдСобытияДляНавигации;
		СтруктураНастроек.ДатаПоследнегоЗапросаСтатусов		= ПривестиТипСбис(ДатаСобытияДляНавигации, Тип("Дата"));
		
	Иначе
		
		// Изменений не было, метки не трогаем.
