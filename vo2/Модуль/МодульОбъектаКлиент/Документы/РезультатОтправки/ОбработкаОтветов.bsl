...
Процедура РезультатОтправки_ОбработатьОшибку(РезультатОтправки, ОтправкаДокументаСБИС, ПараметрыОшибки) Экспорт
	...
	Для Каждого Вложение Из СоставПакета.Вложение Цикл
		...
		Если Не Вложение.Свойство("Документы1С") 
			ИЛИ НЕ ЗначениеЗаполнено(Вложение.Документы1С) Тогда
		...
		ИначеЕсли Не Вложение.Свойство("Документы1С") Тогда
			Продолжить;
		КонецЕсли;
		Для Каждого Документ1С Из Вложение.Документы1С Цикл
...
Процедура РезультатОтправки_Асинх_ПослеОтправки(АсинхронноеСбисСобытие, ПараметрыВызова) Экспорт
	...
	РезультатОтправки_Асинх_ДобавитьПоток(АсинхронноеСбисСобытие, ПараметрыВызова);
...
Процедура РезультатОтправки_Асинх_Ошибка(АсинхронноеСбисСобытие, ПараметрыВызова) Экспорт
	...
	Если ВозможнаПереотправка Тогда
		...
		ПараметрыОтправки = ОтправкаДокументаСБИС_Получить(ОтправкаДокументаСБИС, "Параметры");
...
&НаКлиенте
Процедура РезультатОтправки_Асинх_ДобавитьПоток(АсинхронноеСбисСобытие, ПараметрыВызова) Экспорт 

	РезультатОтправки_ДобавитьПоток(ПараметрыВызова.РезультатОтправки, ПараметрыВызова.Команда);
...
Процедура РезультатОтправки_ДобавитьПоток(РезультатОтправки, АсинхронноеСбисКоманда) Экспорт
	...
	Если АсинхронноеСбисКоманда.ПараметрыКоманды.МногопоточныйРежим Тогда
