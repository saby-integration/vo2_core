...
Процедура ПропатчитьФайлВложенияСБИС(ВложениеСБИС, ДопПараметры) Экспорт
	...
	Если МодульИмениФайла = Ложь Тогда
	...
	Если ФайлПропатчить.Свойство("Имя") Тогда
		
		ВложениеСБИС_Установить(ВложениеСБИС, "ИмяФайла", ФайлПропатчить.Имя);
...
Процедура ПропатчитьИмяФайлаВложения(ВложениеСБИС, ДопПараметры) 
	...
	ФайлПропатчить		= СтруктураФайла.Файл;
...
Функция ЗапуститьПодготовкуДокументаСбисПоДанным1С(СтрокаСпискаДокументов, ДопПараметры) Экспорт 
	...
	Попытка
		...
		ПараметрыГенерации	= Новый Структура("БолванкаДляПакетов, ИмяИни, НеЗапускатьВДокументооборот", СтрокаСпискаДокументов.СоставПакета, ИниТекущегоРеестра, НеЗапускатьВДокументооборот);
		НаборПакетов		= НовыйНаборПакетов(ПараметрыГенерации, ДопПараметры);
		
		НаборПакетов_Вставить(НаборПакетов, "СоставПакета1С", СтрокаСпискаДокументов.СоставПакета);
		НаборПакетов_Прочитать(НаборПакетов);
		
		ОшибкиЧтения = НаборПакетов_Получить(НаборПакетов, "Ошибки");
		
		Для Каждого ОписаниеОшибки Из ОшибкиЧтения Цикл
		
			ОшибкаРазбора = Истина;
			ПараметрыДляПакет = Новый Структура("Ошибка, ОснованиеПакета", ОписаниеОшибки.Ошибка, ОписаниеОшибки.Основание);
	...
	НаборПакетов_Вставить(НаборПакетов, "Тип", ТипФормируемогоДокумента.ТипПакета);
	Если ЗначениеЗаполнено(ТипФормируемогоДокумента.ПодтипПакета) Тогда
		
		НаборПакетов_Вставить(НаборПакетов, "ПодТип",	ТипФормируемогоДокумента.ПодтипПакета);

	КонецЕсли;
	
	МассивПакетов = НаборПакетов_Выгрузить(НаборПакетов);
	
	Если Не Кэш.Свойство("РезультатОтправки") Тогда

		ОшибкаСообщить = Неопределено;
		// Это не отправка. Сообщить получившиеся ошибки
		Для Счетчик = 0 По МассивПакетов.Вграница() Цикл
			
			Если Не МассивПакетов[Счетчик].Свойство("Ошибка", ОшибкаСообщить) Тогда
				
				Продолжить;
				
			КонецЕсли;
			
			СбисСообщить(ОшибкаСообщить, Новый Структура("Отправлять", Ложь));
			МассивПакетов.Удалить(Счетчик);
			Счетчик = Счетчик - 1;
