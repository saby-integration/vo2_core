...
Процедура ПечатьПослеАвторизацииСБИС(Результат, Параметры) Экспорт
	...
	ИначеЕсли	СписокИни.Количество() = 1 Тогда
		ВыполнитьСбисОписаниеОповещения(СписокИни[0].Значение, ОбработчикВыбора);
	Иначе
...
Процедура ПечатьПослеВыбораИни(Результат, Параметры) Экспорт
	...
	Иначе
		...
		Для Каждого мФайл Из ЗначениеИни.мФайл Цикл
	...
	Для Каждого ТекущаяСтрока Из МассивДокументов1С Цикл
		...
		ТекущийРаздел	= Кэш.ОбщиеФункции.РассчитатьЗначение("ДокументРаздел", ЗначениеИни);
		фрм				= МодульОбъектаКлиент().НайтиФункциюСеансаОбработки("ПоказатьДокумент", "Раздел_" + ТекущийРаздел + "_Шаблон");
