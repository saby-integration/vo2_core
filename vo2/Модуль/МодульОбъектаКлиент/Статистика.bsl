...
Функция СбисСтатистика_СформироватьДанныеСообщений(ПараметрыСтатистики, ДопПараметры = Неопределено)
    ...
    Если Действие = "Отправка" Тогда
        Статистика_ОбработатьОтправку(ПараметрыСтатистики, Результат);
        
    ИначеЕсли Действие = "Загрузка" Тогда
       Статистика_ОбработатьЗагрузку(ПараметрыСтатистики, Результат);
	   
    ИначеЕсли Действие = "Ошибка" Тогда
        Статистика_ОбработатьОшибку(ПараметрыСтатистики, Результат);
        
    ИначеЕсли Действие = "Обновление" Тогда
        Статистика_ОбработатьОбновление(ПараметрыСтатистики, Результат);
        
    ИначеЕсли Действие = "Аутентификация" Тогда
        Статистика_ОбработатьАутентификацию(ПараметрыСтатистики, Результат);
        
    ИначеЕсли Действие = "ПрикладнаяСтатистика" Тогда
        Статистика_ОбработатьПрикладнуюСтатистику(ПараметрыСтатистики, Результат);
		
	ИначеЕсли Действие = "СофрмированыВложения" Тогда
		Статистика_ОбработатьСоставВложений(ПараметрыСтатистики, Результат);	
    КонецЕсли;
...
&НаКлиенте
Процедура Статистика_ОбработатьОшибку(ПараметрыСтатистики, Результат)
...
&НаКлиенте
Процедура Статистика_ОбработатьОтправку(ПараметрыСтатистики, Результат)
    ...
    Если РезультатОтправки.Ошибок Тогда
        Статистика_ОбработатьОшибкиОтправки(РезультатОтправки, ПараметрыСтатистики, Результат);
	КонецЕсли;
	
КонецПроцедуры       

&НаКлиенте
Процедура Статистика_ОбработатьОшибкиОтправки(РезультатОтправки, ПараметрыСтатистики, Результат)
...
&НаКлиенте
Процедура Статистика_ОбработатьЗагрузку(ПараметрыСтатистики, Результат)
...
&НаКлиенте
Процедура Статистика_ОбработатьАутентификацию(ПараметрыСтатистики, Результат)
...
&НаКлиенте
Процедура Статистика_ОбработатьПрикладнуюСтатистику(ПараметрыСтатистики, Результат)
...
&НаКлиенте
Процедура Статистика_ОбработатьОбновление(ПараметрыСтатистики, Результат)
...
Функция СбисСтатистика_СформироватьСтатистикуДляIntegration(ПараметрыСтатистики)
	...
	SystemInfo.Вставить("ExtSysSubtype",		ИзвлечьПрефикс(КэшНастроек.ПараметрыНастроек.ПрефиксУстановленныхНастроек));//из Кэша
...
&НаКлиенте
Функция ИзвлечьПрефикс(Строка) Экспорт
...
Функция СбисСтатистика_СформироватьСтатистикуОшибокДляIntegration(ПараметрыСтатистики, НовоеСообщение)
	...
	SystemInfo = Новый Структура;
	SystemInfo.Вставить("ExtSysType",			"1C");
	SystemInfo.Вставить("ExtSysSubtype",		ИзвлечьПрефикс(КэшНастроек.ПараметрыНастроек.ПрефиксУстановленныхНастроек));
...
&НаКлиенте
Процедура Статистика_ОбработатьСоставВложений(ПараметрыСтатистики, Результат)
	Перем СпособФормированияВложения;
	
	Для Каждого Вложение Из ПараметрыСтатистики.Вложения Цикл
		
		ПараметрыВложения = ВложениеСБИС_Получить(Вложение, "Параметры");
		
		Если		ПараметрыВложения = Неопределено
			Или Не	ПараметрыВложения.Свойство("СпособФормирования", СпособФормированияВложения) Тогда
			
			ПараметрыСообщения = Новый Структура("Функционал, Количество, Действие, Контекст", "Обработка 1С", 1,
