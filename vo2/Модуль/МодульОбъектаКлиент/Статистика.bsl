...
Процедура СбисСтатистика_СформироватьИЗаписать(ПараметрыСтатистики, ДопПараметры = Неопределено) Экспорт
	Если Не ПолучитьЗначениеПараметраСбис("ОбменВключен") Или Не ПолучитьЗначениеПараметраСбис("Авторизован") Тогда
	...
	Попытка
		...
		Для Каждого СообщениеСтатистики Из ДанныеСтатистики.Прикладная Цикл
			РезультатОтправки = ГлобальныйКэш.ТекущийСеанс.Модули.Интеграция.ExtSysMarking_SendStatisticData(СообщениеСтатистики, Новый Структура("Кэш", ГлавноеОкно.Кэш), Отказ);
			Если Отказ Тогда
...
Функция	СбисСтатистика_СформироватьДанныеСообщений(ПараметрыСтатистики, ДопПараметры = Неопределено) 
	...
	ИначеЕсли Действие = "ПрикладнаяСтатистика" Тогда
		...
		Если		ТипЗнч(ПараметрыСтатистики.Сообщения) = Тип("Структура") Тогда
			...
			ПараметрыСообщения = Новый Структура("Функционал, Действие, Контекст", ФункционалПрикладнойИмя); 	
