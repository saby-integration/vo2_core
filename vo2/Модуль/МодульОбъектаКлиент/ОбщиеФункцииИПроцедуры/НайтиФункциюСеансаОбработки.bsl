...
//
&НаКлиенте
Функция ПользовательскаяТочкаВхода(СбисИмяФункции, СбисОсновныеФормы, ДопПараметры = Неопределено) Экспорт
	...
	Иначе
		Попытка
			Возврат НовыйСбисОписаниеОповещения(СбисИмяФункции, фрм, ДопПараметры);
		Исключение
			// Сюда попадаем, если больше 2х аргументов, т.е. описание оповещения не сформируется
			Возврат Новый ФиксированнаяСтруктура("Модуль, ИмяПроцедуры", фрм, СбисИмяФункции);
		КонецПопытки;
	КонецЕсли;
...
&НаКлиенте
Процедура ВызватьПользовательскуюТочкуВхода(ОписаниеТочкиВходаВФК, Аргумент, ДопПараметры = Неопределено) Экспорт
	...
	Если ПолучитьЗначениеПараметраСбис("РежимОтладки") Тогда
		...
		лОтладочныеДанные.Тип		= "ВФК";
		лОтладочныеДанные.Вызов		= ОписаниеТочкиВходаВФК.ИмяПроцедуры;
		лОтладочныеДанные.Время		= ТекущаяДата();
		лОтладочныеДанные.Сообщение		= Аргумент;
		лОтладочныеДанные.Идентификатор = "";
		лОтладочныеДанные.Модуль		= "ВФК";
	...
	Если ЭтоТипСбис(ОписаниеТочкиВходаВФК, "СбисОписаниеОповещения") Тогда
		ВыполнитьСбисОписаниеОповещения(Аргумент, ОписаниеТочкиВходаВФК);
	Иначе
		
		фрм = ОписаниеТочкиВходаВФК.Модуль;
		СбисИмяФункции = ОписаниеТочкиВходаВФК.ИмяПроцедуры;
		
		СтрокаАргументы = "";
		Для СчетчикСТрок = 0 По Аргумент.Количество() - 1 Цикл 
			СтрокаАргументы = СтрокаАргументы + ", Аргумент[" + СчетчикСТрок + "]";			
		КонецЦикла;
