...
Процедура ИзменитьПараметрСбис(ИмяПараметраСбис, ЗначениеПараметраСбис, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ИмяПараметраСбис = "НастройкиКриптографии" Тогда
		...
		Если ЗначениеЗаполнено(ЗначениеПараметраСбис) Тогда
			...
			ИзменитьПараметрСбис_ВКэш(ЗначениеКэшПрописать, "ТекущийСеанс.Модули.Криптография1С", ЗначениеПараметраСбис, Истина);
...
Процедура ИзменитьПараметрСбис_СпособСопоставленияНоменклатуры(ЗначениеКэшПрописать, ЗначениеПараметраСбис, ЭтоГлобальныйПараметр, ДопПараметрыИзменения = Неопределено)
	...
	Если РасширенныйФункционалСопоставленияНоменклатуры Тогда
		...
		КонецЕсли;
	Иначе
		// Имя модуля в зависимости от способа хранения прописывается ранее в ФайлыНастроекОбщее.УстановитьИниКонфигурации
		//TODO #3 - спилить, когда все перейдут на новое сопоставление
		ИмяФормыРаботыСНоменклатурой = ПолучитьЗначениеПараметраСбис("ИмяМодуляРаботыСНоменклатурой");
	КонецЕсли;
	
	ИзменитьПараметрСбис_ВКэш(ЗначениеКэшПрописать, "ФормаРаботыСНоменклатурой", ИмяФормыРаботыСНоменклатурой, ЭтоГлобальныйПараметр);
	
	Отказ = Ложь;
	ФормаРаботыСоНоменклатурой = ЗначениеКэшПрописать.ГлавноеОкно.СбисПолучитьФормуОбработки(ЗначениеКэшПрописать, ИмяФормыРаботыСНоменклатурой, Новый Структура, Отказ);
	...
	Попытка
		ФормаРаботыСоНоменклатурой.УстановитьПараметрыМодуля(,ЗначениеКэшПрописать);
	Исключение
