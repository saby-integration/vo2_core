...
Функция ОбъединитьОбъектыУниверсальныхКоллекций(ОбъектОсновной = Неопределено, ОбъектОбъединения, ДопПараметры)  Экспорт
	...
	Копия			=	ГлубокаяКопия
					Или	(	ДопПараметры.Свойство("Копия")
						И	ДопПараметры.Копия);
						
	Если ДопПараметры.Свойство("КлючиПеренестиКакЕсть") Тогда
		КлючиПеренестиКакЕсть = ДопПараметры.КлючиПеренестиКакЕсть;
	Иначе
		КлючиПеренестиКакЕсть = Новый Массив;
	КонецЕсли;
						
	КонтекстОбъединенияРек = Новый ФиксированнаяСтруктура("ГлубокаяКопия, Копия, КлючиПеренестиКакЕсть", ГлубокаяКопия, Копия, КлючиПеренестиКакЕсть);
	
	Возврат ОбъединитьОбъектыУниверсальныхКоллекций_Объект(ОбъектОсновной, ОбъектОбъединения, КонтекстОбъединенияРек) 
...
//  ОбъектОсновной с значениями объектов из ОбъектыОбъединения
//
Функция ОбъединитьОбъектыУниверсальныхКоллекций_Объект(ОбъектОсновной = Неопределено, ОбъектОбъединения, КонтекстОбъединить)  Экспорт
	...
	Иначе
		...
		// Отнесемся к остальному тоже как к простому типу, но в котором мы не уверены
		//Попытка
		//	Результат = ОбъектОбъединения;			
		//Исключение
...
Функция ОбъединитьОбъектыУниверсальныхКоллекций_Структура(ОбъектОсновной = Неопределено, ОбъектОбъединения, КонтекстОбъединения)
	...
	Если	КонтекстОбъединения.Копия
		Или	ОбъектОсновной = Неопределено Тогда
		
		Результат = Новый Структура;
		ОбъединитьОбъектыУниверсальныхКоллекций_ОбъединитьСтруктуры(Результат, ОбъектОсновной, КонтекстОбъединения);
	...
	ОбъединитьОбъектыУниверсальныхКоллекций_ОбъединитьСтруктуры(Результат, ОбъектОбъединения, КонтекстОбъединения);
	
	Возврат Результат;
	
КонецФункции

Процедура ОбъединитьОбъектыУниверсальныхКоллекций_ОбъединитьСтруктуры(ОбъектОсновной, ОбъектОбъединения, КонтекстОбъединения)
	Перем ЗначениеТекущее;
	...
	Для Каждого КлючИЗначение Из ОбъектОбъединения Цикл 
		...
		Если	(ОбъектОсновной.Свойство(КлючИЗначение.Ключ, ЗначениеТекущее)
			Или	КонтекстОбъединения.ГлубокаяКопия)
			И КонтекстОбъединения.КлючиПеренестиКакЕсть.Найти(КлючИЗначение.Ключ) = Неопределено Тогда
			
			ЗначениеВставить = ОбъединитьОбъектыУниверсальныхКоллекций_Объект(ЗначениеТекущее, КлючИЗначение.Значение, КонтекстОбъединения);
		...
		ОбъектОсновной.Вставить(КлючИЗначение.Ключ, ЗначениеВставить)
