...
Функция ВыполнитьЗапросСПараметрами(ТекстЗапросаВх, ПараметрыЗапросаВх, ДопПараметры = Неопределено) Экспорт
	...
	Попытка
		...
		Возврат РезультатЗапросаНаКлиент(Запрос.Выполнить());
...
Функция РезультатЗапросаНаКлиент(РезультатЗапроса) Экспорт 
	...
	Исключение
		...
		ВызватьИсключение(СбисИсключение_ВСтроку(ИнформацияОбОшибке(), "МодульОбъектаСервер.РезультатЗапросаНаКлиент")); 
