...
Функция СериализоватьОбъектДляВозвратаНаКлиент(ОбъектСериализации) Экспорт 
	...
	Если ТипЗнч(ОбъектСериализации) = Тип("РезультатЗапроса") Тогда
		...
		Если Не ОбъектСериализации.Пустой() Тогда
			...
			НаборКлючей = СериализоватьОбъектДляВозвратаНаКлиент_КлючиСтроки(ОбъектСериализации);
	...
	ИначеЕсли ТипЗнч(ОбъектСериализации) = Тип("РезультатЗапроса") Тогда
...
//
// Параметры:
//  ОбъектСериализации	 - Произвольный	 - объект, который содержит свойство "Колонки", у каждой из которых есть "Имя". 
// 
// Возвращаемое значение:
//  Строка - Разделенные через "," ключи строки для создания и заполениния структуры.
//
Функция СериализоватьОбъектДляВозвратаНаКлиент_КлючиСтроки(ОбъектСериализации) Экспорт 
	
	НаборКлючей			= "";
	ТипНеСериализуется	= Тип("РезультатЗапроса");
	
	Для Каждого Колонка Из ОбъектСериализации.Колонки Цикл
		
		Если Колонка.ТипЗначения.СодержитТип(ТипНеСериализуется) Тогда
	...
	Возврат Лев(НаборКлючей, СтрДлина(НаборКлючей)-1);
...
Функция ХешСуммаЗначенияSABY(ДанныеХешПолучить, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли   ТипЗнч(ДанныеХешПолучить) = Тип("ОбъектМетаданных") Тогда
...
#Область include_core2_vo2_МодульОбъекта_Сервер_ОбщиеФункцииИПроцедуры_ХранилищеОбщихНастроек
#КонецОбласти
