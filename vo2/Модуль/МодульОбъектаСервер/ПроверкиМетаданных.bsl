...
Функция СбисСуществуетОбъектКонфигурацииСервер(ПутьОбъекта, ДопПараметры = Неопределено) Экспорт
	...
	КонтекстПолученияПути = Новый Структура("ОбъектМетаданных, ПутьКМетаданным, ПутьОбъекта", ОбъектМетаданных, ПутьКМетаданным, ПутьМассивом);
...
Процедура ОпределитьПутьМетаданных(КонтекстПоискаПути, ДопПараметры)
	...
	ОбъектМетаданных	= КонтекстПоискаПути.ОбъектМетаданных;
	ПутьКМетаданным		= КонтекстПоискаПути.ПутьКМетаданным;
	ПутьОбъекта			= КонтекстПоискаПути.ПутьОбъекта;
	
	Если ДопПараметры.Свойство("Объект")
		И Метаданные.НайтиПоТипу(ТипЗнч(ДопПараметры.Объект)) <> Неопределено Тогда 	
		
		ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗнч(ДопПараметры.Объект));
		
		Если НЕ ДопПараметры.Свойство("НаборОбъектов", НаборОбъектов) Тогда
			ПутьКМетаданным = Неопределено;
		ИначеЕсли ПутьОбъекта.Количество() < 3 Тогда
			ПутьКМетаданным = Новый Структура(НаборОбъектов);
		Иначе

			ОбъектыТабличнойЧасти = Новый Структура("Реквизиты");
			ПутьКМетаданным = Новый Структура(НаборОбъектов, ОбъектыТабличнойЧасти);
		КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли НЕ ДопПараметры.Свойство("НаборОбъектов", НаборОбъектов) Тогда
			ПутьКМетаданным.Вставить(КлассМетаданных, Неопределено);
		ИначеЕсли ПутьОбъекта.Количество() < 3 Тогда
			ПутьКМетаданным.Вставить(КлассМетаданных, Новый Структура(НаборОбъектов));
		Иначе

			ОбъектыТабличнойЧасти = Новый Структура("Реквизиты");
			ПутьКМетаданным.Вставить(КлассМетаданных, Новый Структура(НаборОбъектов, ОбъектыТабличнойЧасти));
