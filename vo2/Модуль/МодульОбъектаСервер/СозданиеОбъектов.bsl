...
Процедура СоздатьПерезаполнитьОбъектПоНашимДанным(НашиДанныеВходящие, ДопПараметры) Экспорт
	Перем Объект1СЗначение, Объект1ССсылка, ОбъектТип, лРежимЗаписи, лРежимПроведения;
	...
	Если ЭтоДокумент1С Тогда
		...
		Если	Не	НашиДанныеВходящие.Свойство("РежимЗаписи", лРежимЗаписи)
			Или	Не	ТипЗнч(лРежимЗаписи) = Тип("РежимЗаписиДокумента") Тогда
			
			лРежимЗаписи = РежимЗаписиДокумента.Запись;
			
		КонецЕсли;
		
		Если	Не НашиДанныеВходящие.Свойство("РежимПроведения", лРежимПроведения)
			Или	Не	ТипЗнч(лРежимПроведения) = Тип("РежимПроведенияДокумента") Тогда
	...
	Исключение
		...
		ВызватьИсключение СбисИсключение_ВСтроку(НовыйСбисИсключение(ИнформацияОбОшибке(), "МодульОбъектаСервер.СоздатьПерезаполнитьОбъектПоНашимДанным"));
	...
	Попытка
		...
		ИначеЕсли Лев(ОбъектКлюч, 22) = "планвидовхарактеристик" Тогда
			...
			Исключение
				...
				ВызватьИсключение СбисИсключение_ВСтроку(НовыйСбисИсключение(ИнформацияОбОшибке(), "МодульОбъектаСервер.НовыйОбъект1С", 709101, 
				"Недостаточно прав для создания свойства, необходимо запустить обработку под правами администратора или настроить права"));
...
Процедура ЗаполнитьОбъект1СНашимиДанными(Объект1СЗначение, Объект1ССтрукутраНаша, ДопПараметры)
	Перем ИниОбъекта, ДополнятьТабЧасть, РедактироватьТабЧасть, ЗаполняемыеТабЧасти, ЗаполнитьДаннымиДругогоОбъекта, ДокументОснование;
