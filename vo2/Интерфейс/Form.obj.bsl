...
Процедура ПриЗакрытии(ЗавершениеРаботы=Ложь)
	...
	Исключение
		МодульОбъектаКлиент().СообщитьСбисИсключение(ИнформацияОбОшибке(), Новый Структура("СтатусСообщения, ИмяКоманды", "ПредупреждениеSaby", "ФормаИндикатора.ПриЗакрытии"));
	КонецПопытки;
