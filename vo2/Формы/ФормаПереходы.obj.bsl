...
Процедура ПодробнееНажатие(Элемент) 
	...
	ФормаПереходовПодробнее = МестныйКэш.ГлавноеОкно.сбисПолучитьФорму("ФормаПереходыПодробнее_ДокОтгрВх",,СтруктураПодробнее,ЭтаФорма);
...
Процедура УстановитьФормуНаСервере(ПараметрыУстановки)
	...
	Если ПараметрыУстановки.СоставПакета.Тип = "ДокОтгрВх" Тогда
		сбисЭлементФормы(ЭтаФорма,"ДатаПолучения").Видимость = Истина;
		сбисЭлементФормы(ЭтаФорма,"Подробнее").Видимость = Истина;
		ДатаПолучения = ТекущаяДата();
	Иначе
		сбисЭлементФормы(ЭтаФорма,"ДатаПолучения").Видимость = Ложь;
		сбисЭлементФормы(ЭтаФорма,"Подробнее").Видимость = Ложь;
	КонецЕсли;				
	сбисЭлементФормы(ЭтаФорма, "ПанельИсполнитель").Видимость = ПараметрыОтображения.ИсполнительВидимость; 
	сбисЭлементФормы(ЭтаФорма, "ПанельДатаПодробнее").Видимость = ПараметрыОтображения.ДатаПодробнееВидимость;
