...
Процедура Отправить(Команда)
	...
	Для Каждого Элемент Из ДанныеСторон Цикл
		...
		Иначе
			...
			Если Элемент.Значение.Свойство("Ошибка") Тогда
				...
				МассивОшибок.Добавить(Новый Структура("ТекстОшибки, Ссылка1С", Элемент.Значение.Ошибка.message, Элемент.Ключ));
	...
	Для Каждого Контрагент Из СписокПолучателей Цикл
		...
		Если КорреспонденцияКОтправке = Неопределено Тогда
			...
			фрм = МестныйКэш.ГлавноеОкно.сбисНайтиФормуФункции("СформироватьПакетКорреспонденции", "Файл_Шаблон",, МестныйКэш); 
			Если фрм = Ложь Тогда
				МассивОшибок.Добавить(Новый Структура("НашаОрганизация, ТекстОшибки, Ссылка1С", НашаОрганизация, "Не найден обработчик подстановки!", Неопределено));
				Продолжить;
			КонецЕсли;
			КорреспонденцияКОтправке = фрм.СформироватьПакетКорреспонденции(МестныйКэш, ПараметрыФормированияКорр, ОшибкаФормирования);
			Если ОшибкаФормирования Тогда
				МассивОшибок.Добавить(Новый Структура("НашаОрганизация, ТекстОшибки, Ссылка1С", НашаОрганизация, КорреспонденцияКОтправке.message, Неопределено));
				Продолжить;
			Иначе
	...
	ДопПараметрыОтправки = Новый Структура("Реестра, ОбработчикРезультата", МестныйКэш.Текущий, ОбработчикРезультатОтправки);
...
Процедура ОтправитьДокументы_Завершение(РезультатОтправки, ДопПараметры) Экспорт
	...
	фрм.ПоказатьРезультатОтправки(МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш());	
