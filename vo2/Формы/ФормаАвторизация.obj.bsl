...
Процедура ПриОткрытии(Отказ)
	...
	Если Кэш.ТекущийСеанс.РасчитанныеЗначения.Свойство("ОшибкаНаФормеАвторизации", ТекстОшибки) Тогда
		Кэш.ТекущийСеанс.РасчитанныеЗначения.Удалить("ОшибкаНаФормеАвторизации");
	КонецЕсли;
	сбисОбновитьОтображение(Новый Структура("ТекстОшибки", ТекстОшибки));
...
Функция Авторизоваться(ГлобальныйКэш=Неопределено) Экспорт
	...
	КонецЕсли;
	Если Не Кэш.СБИС.ОбменВключен Тогда
		Ошибка = Новый Структура("code, message, details", 100, "Не включен обмен с Saby", "Измените способ обмена в настройках соединения.");
	...
	Если Отказ Тогда
		...
		ИначеЕсли Результат.code = 775 Тогда // Не получен ответ от SABY плагина
			Результат.details = "Перезагрузите SABY плагин";
			ПараметрыОбновления.ТекстОшибки = "Не получен ответ от SABY плагина. " + Результат.details;
	...
	Если Результат <> МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ИдентификаторСессии") Тогда
...
Процедура Авторизация_Завершение(Результат, ДополнительныеПараметры=Неопределено) Экспорт
	...
	Если Кэш.ТекущийСеанс.Свойство("ВыполненПереходНаExtSDK2") И Кэш.ТекущийСеанс.ВыполненПереходНаExtSDK2 = Истина Тогда
		//нужно перезапустить главное окно, форма авторизации закрыта
