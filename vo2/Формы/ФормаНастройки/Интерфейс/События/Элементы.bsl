...
Процедура НастройкаЭПНажатие(Элемент)
	...
	ПараметрыЗапросаФормы = Новый Структура("Владелец", ЭтаФорма);
	фрм = МодульОбъектаКлиент().ПолучитьФормуОбработки("ФормаНастройкиПодписания", ПараметрыЗапросаФормы);
	фрм.ОписаниеОповещенияОЗакрытии = МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("НастройкиПодписания_РезультатДиалога", ЭтаФорма);
	фрм.Открыть();
...
Процедура НастройкиПодписания_РезультатДиалога(РезультатВвода, ДопПараметры = Неопределено)  Экспорт
	...
	МодульОбъектаКлиент().ИзменитьПараметрСБИС("НастройкиКритографии", РезультатВвода);
