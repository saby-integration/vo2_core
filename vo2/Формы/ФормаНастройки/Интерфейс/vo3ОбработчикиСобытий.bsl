...
#Область include_core_base_Helpers_OpenURL
#КонецОбласти
...
Процедура ЗаполнитьПараметрыВО3() 
	...
	ЭтаФорма.КаталогХраненияНастроек = get_prop(context_params, "КаталогХраненияНастроек", "");
...
Процедура ПереложитьНастройкиВО3вВО2()
	...
	ЭтаФорма.АдресСервера = api_url;
	Если Найти(api_url, "https://") > 0 Тогда
		ЭтаФорма.АдресСервера = Сред(api_url, 9);
	КонецЕсли;
	...
	ЭтаФорма.context_params.Вставить("EncryptSelectively", ШифроватьВыборочно);
	...
	СпособХраненияНастроек = ?(ИспользоватьНастройкиИзКаталога, 0, 1);
	КаталогНастроек = КаталогХраненияНастроек;
	ЭтаФорма.context_params.Вставить("ИспользоватьНастройкиИзКаталога", ИспользоватьНастройкиИзКаталога);
	ЭтаФорма.context_params.Вставить("КаталогХраненияНастроек", КаталогХраненияНастроек);
	// BSLLS:UnusedLocalVariable-on
КонецПроцедуры
