...
// Процедура устанавливает видимость элементов формы в зависимости от выбранного варианта настроек прокси		
	...
	МестныйКэш.ГлавноеОкно.СбисЭлементФормы(ЭтаФорма, "ПорядокАвтоматическогоСопоставленияФрм").Доступность	= ИспользоватьАвтоматическоеСопоставлениеНоменклатуры;
...
Процедура СбисПрименитьНастройки()
	...
	Если МодульОбъектаКлиент.ПолучитьЗначениеПараметраСбис("ЗаполнениеКонтрагента1С") <> ЗаполнениеКонтрагента1С Тогда
		...
		Если ЭтаФорма["ЗаполнениеКонтрагента1С"] = "ПокупательСБИС" Тогда 
			ЗаписьПрикладнойСтатистики = МодульОбъектаКлиент.НовыйЗаписьПрикладнойСтатистики("Вести учет по контрагентам", "Настройка загрузки заказа");
		Иначе
			ЗаписьПрикладнойСтатистики = МодульОбъектаКлиент.НовыйЗаписьПрикладнойСтатистики("Вести учет по грузополучателям", "Настройка загрузки заказа");
		КонецЕсли;
	...
	СписокСвойствЗаполнения = "ТипГрузополучателя
...
Процедура СбисУстановитьФорму(ПараметрыОткрытия)
	...
	Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры") Тогда 
		...
		ПорядокОбхода = МодульОбъектаКлиент().СбисРазложитьСтрокуВМассивПодстрокНаКлиенте(ПорядокАвтоматическогоСопоставления, Разделитель); 
...
Процедура ЗаполнитьПорядокАвтоматическогоПоискаСопоставлений(ПорядокОбхода = Неопределено)
	...
	МасПорядокАвтоматическогоСопоставленияПоУмолчанию.Добавить("Код");
