...
Процедура ПриОткрытии()
	...
	Если СпособОбмена = 0 И ПустаяСтрока(СпособОбменаСписокВыбора.НайтиПоЗначению(0))	Тогда
		...
		Попытка 
			сбисЭлементФормы(ЭтаФорма, "СпособОбмена").Значение = СпособОбменаСписокВыбора.Получить(0).Значение;  //по индексу - SDK
		Исключение	
		КонецПопытки;		
...
// Процедура устанавливает видимость элементов формы в зависимости от выбранного варианта настроек прокси		
	...
	ВременныйКэш = Новый Структура("СБИС,ТекущийСеанс,ТихийРежим", МестныйКэш.СБИС,МестныйКэш.ТекущийСеанс,Истина);
