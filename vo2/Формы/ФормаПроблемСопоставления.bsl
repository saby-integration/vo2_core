...
Функция Показать(Кэш, СтркаТаблицыЗначений) Экспорт
	...
	Для Каждого КлючИЗначение Из ИтогиСравнения Цикл
		...
		Сообщение = КлючИЗначение.Значение.ОписаниеКратко
			+ " (СБИС: " + КлючИЗначение.Значение.Значение1С + ","
			+ " 1С: " + КлючИЗначение.Значение.ЗначениеСБИС + ")";
...
Процедура СформирвоатьСтруктуруКолонок(СписокКолонок, Группировка)
	...
	Для каждого Колонка Из СписокКолонок Цикл
		...
		Если СписокКолонок.Количество() = 2
			И Не СписокКолонок.Получить(ИмяГруппы + "СБИС") = Неопределено
			И Не СписокКолонок.Получить(ИмяГруппы + "1С") = Неопределено Тогда
			Продолжить;
		ИначеЕсли СписокКолонок.Получить(ИмяГруппы) = Неопределено Тогда
			...
			СписокКолонок["_Порядок"].Удалить(СписокКолонок["_Порядок"].Найти(ИмяГруппы + ?(Постфикс = "СБИС", "1С", "СБИС")));
