...
Функция ЗаполнитьИзФайла(СтрокаТаблДок, СоставПакета, Кэш) Экспорт
	...
	Если Не ЗначениеЗаполнено(СтруктураКонтрагента) Тогда
		МодульОбъектаКлиент().СбисСообщить("В файле отсутствует блок информации, необходимый для создания контрагента.");
		Возврат Неопределено;
	КонецЕсли;
	ИниКонтрагента = СтруктураИниФайла.мСторона[РольКонтрагента];
	
	СвФлЮл = Неопределено;
	Если СтруктураКонтрагента.Свойство("СвИн",СвФлЮл) Тогда
		МодульОбъектаКлиент().СбисСообщить("Создание контрагента недоступно. Укажите контрагента вручную.");
		Возврат Неопределено;
	ИначеЕсли СтруктураКонтрагента.Свойство("СвЮЛ",СвФлЮл) Тогда
