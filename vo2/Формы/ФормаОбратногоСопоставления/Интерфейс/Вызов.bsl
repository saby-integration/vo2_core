...
Процедура СбисУстановитьФорму(ПараметрыОткрытия)
	...
	Вложение 				= ПараметрыОткрытия.Вложение;
	...
	Для Каждого СтрТаблДокСБИС из ТаблДок Цикл
		СписокСтрокДокументаКонтрагента.Добавить(СтрТаблДокСБИС, СтрТаблДокСБИС.Название); 
	КонецЦикла;
	
	ЭлементГруппа = МодульОбъектаКлиент.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТаблицаСопоставлений.ТаблицаСопоставленийНоменклатураСБИС");
	Если ЭлементГруппа = Неопределено Тогда
		ОписаниеПараметраИзменить = Новый ФиксированнаяСтруктура("Форма, Реквизит", ЭтаФорма, "ТаблицаСопоставлений.НоменклатураКонтрагента");
	Иначе
		// Для УФ сначала получаем группу, а потом изменяем уже подчиненный элемент
		ОписаниеПараметраИзменить = Новый ФиксированнаяСтруктура("Форма, Реквизит", ЭлементГруппа, "ТаблицаСопоставленийНоменклатураКонтрагента");
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуСопоставленийПоУмолчанию(Команда = Неопределено)
	...
	ИначеЕсли Вложение.Параметры.Свойство("КлассыСопоставленияНоменклатур", ЛокальнаяПеременная) Тогда
		ОбогащенныеДанныеНоменклатурыДляСопоставления = ЛокальнаяПеременная;
	...
	Для Каждого СтокаСопоставления Из ОбогащенныеДанныеНоменклатурыДляСопоставления Цикл
		...
		Если Вложение.Свойство("КлассыСопоставленияНоменклатур") Тогда
			КоличествоВДокументе1С  = Число(ТаблДок[НомерСтрокиТаблДок].Кол_во);
			ЦенаВДокументе1С 	    = Число(ТаблДок[НомерСтрокиТаблДок].Цена);
