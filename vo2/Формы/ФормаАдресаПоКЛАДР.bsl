...
// Формирует структуру адреса, полученного с kladr.saby.ru	
Функция СбисСформироватьСтруктуруАдресаПоКЛАДР(АдресСтрокой)
	...
	МассивJSON = МодульОбъектаСервер().СбисПрочитатьJSON(СтрокаJSON);

	Возврат МассивJSON; 
КонецФункции	

// Получает адрес с kladr.saby.ru, записывает во временный файл	
Функция АдресПоКладрJSON(Адрес)
	Результат = ПолучитьИмяВременногоФайла();
	Сервис		= "postal-service/service/code?addr="+Адрес;
	сбисСервер	= "online.saby.ru";
	сбисПротокол= "https";
	сбисПрефикс	= СтрПолучитьСтроку(СтрЗаменить(АдресСервера, "online", Символы.ПС), 1);
