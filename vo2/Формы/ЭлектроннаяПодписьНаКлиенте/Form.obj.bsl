...
Функция сбисСоздатьМенеджерКриптографии(Кэш, Алгоритм = Неопределено) Экспорт
	...
	Для Каждого СвойстваПрограммы Из ОписанияПрограмм Цикл
		...
		Если Кэш.СовместимостьМетодов.АлгоритмПодписи.АлгоритмПодписиПолучитьИнформацию Тогда 
	...
	Если Менеджер = Неопределено Тогда
		ОшибкаКриптографии = МодульОбъектаКлиент().НовыйСбисИсключение(731102,"МодульОбъекта.МенеджерКриптографииБазовый",,,"Не удалось создать менеджер криптографии");
		МодульОбъектаКлиент().СбисСообщить(ОшибкаКриптографии);
	КонецЕсли;
