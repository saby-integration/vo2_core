...
&НаКлиенте
Функция ПодготовитьСписокДокументовКСопоставлению(ВходящийКэш, СписокОтмеченныхДокументов) Экспорт
...
Процедура РассчитатьДельтуИСуммуДокумента1С(СтрокаТаблицыДокументов)
	...
	Если ДельтаСтроки > ?( МестныйКэш.Парам.СопоставлениеПоСумме < 0, -МестныйКэш.Парам.СопоставлениеПоСумме, МестныйКэш.Парам.СопоставлениеПоСумме ) Тогда
...
Процедура ТаблицаСопоставленияДокументовДокумент1СОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Тип") Тогда
		Попытка
			...
			ПараметрыФормы 			= Новый Структура("ПараметрыФормы", ПараметрыФормыВыбора);
			...
			ПараметрыВыбораЗначения = Новый Структура("Обработчик, ПараметрыОтбораВыбора", ОбработкаВыбора, ПараметрыФормы); 
...
Процедура ТаблицаСопоставленияДокументовВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	Если		Прав(ИмяКолонки, 12) = "ДокументСБИС" Тогда
		...
		МестныйКэш.ГлавноеОкно.ОткрытьДокументОнлайнПоПакету(СбисТекущиеДанные.СоставПакета[0].Значение, МестныйКэш);
	...
	ИначеЕсли	Лев(ИмяКолонки, 22) = "СостояниеСопоставления" Тогда
		...
		ИначеЕсли	СбисТекущиеДанные.СостояниеСопоставления = 5 Тогда
			...
			фрм.Показать(МестныйКэш, СбисТекущиеДанные);
...
Процедура СохранитьНажатие(Элемент)
	...
	КонецЕсли;
	МестныйКэш.СБИС.ПараметрыИнтеграции.Свойство("ИдАккаунта", СбисИдАккаунта);
	...
	Для Каждого ЭлементСписка из СписокОтмеченныхДокументов Цикл  
		...
		Если Строка.Документ1С<>Неопределено и Не Строка.Документ1С.Пустая() Тогда
			...
			фрм.ЗаписатьПараметрыДокументаСБИС(СтруктураСвойств, Строка.Документ1С, МестныйКэш.Ини.Конфигурация, МестныйКэш.Парам.КаталогНастроек, Новый Структура("ИдАккаунта", СбисИдАккаунта));
			...
			Если ЗначениеЗаполнено(Строка.РанееСопоставленныеДокументы1С) Тогда
				Для Каждого РанееСопоставленныйДокумент1С Из Строка.РанееСопоставленныеДокументы1С Цикл
					...
					Если ПредыдущийДокумент1С<>Строка.Документ1С Тогда // если раньше было сопоставление с другим документом, удаляем его
						...
						фрмУдПар.УдалитьПараметрыДокументаСБИСПарам(СтруктураСвойств, ПредыдущийДокумент1С, МестныйКэш.Ини, МестныйКэш.Парам.КаталогНастроек, Новый Структура("Регламент",Префикс));
						//<-- KES 050751151 Статусы в разрезе регламентов (СОПОСТАВЛЕНИЕ)
						
						если МестныйКэш.парам.СостояниеЭД тогда
							фрмУдПарЭД.УдалитьПараметрыДокументаСБИС(СтруктураСвойств, ПредыдущийДокумент1С, МестныйКэш.Ини, МестныйКэш.Парам.КаталогНастроек);
						конецесли;
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
...
Процедура КонтекстноеМенюУтвердитьОтклонить(Команда)
	СписокОтмеченныхДокументов = ПолучитьВыбранныеСтроки();
	ГлавноеОкно = МестныйКэш.ГлавноеОкно;
	фрм = ГлавноеОкно.сбисНайтиФормуФункции("сбисУтвердитьОтклонить","РаботаСДокументами1С","",МестныйКэш);
	фрм.сбисУтвердитьОтклонить(МестныйКэш, СписокОтмеченныхДокументов, Команда);
	...
	Для Каждого СтрокаТаблицы из СписокОтмеченныхДокументов Цикл 
		...
		МассивНайденныхСтрок = ГлавноеОкно[МестныйКэш.ТаблДок.Имя].НайтиСтроки(ОтборИдСБИС);
