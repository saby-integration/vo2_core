...
Процедура ТаблицаСопоставленияДокументовВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		ИмяКолонки = ИмяТаблицы + ИмяКолонки;
	#КонецЕсли
	СтандартнаяОбработка	= Ложь;
	СбисТекущиеДанные		= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, ИмяТаблицы).ТекущиеДанные;
	
	Если ИмяКолонки = "ТаблицаСопоставленияДокументовДокументСБИС" Тогда
		МестныйКэш.ГлавноеОкно.ОткрытьДокументОнлайнПоПакету(СбисТекущиеДанные.СоставПакета[0].Значение, МестныйКэш);
	ИначеЕсли Лев(ИмяКолонки, 52) = "ТаблицаСопоставленияДокументовСостояниеСопоставления" Тогда
		
		// 0 - Документ сопоставлен
		// 1 - Начать сопоставление
		// 2 - Посмотреть проблемы сопоставления
		Если		СбисТекущиеДанные.СостояниеСопоставления = 1 Тогда
		ИначеЕсли	СбисТекущиеДанные.СостояниеСопоставления = 4
	...
	ИначеЕсли ИмяКолонки<>"ТаблицаСопоставленияДокументовОтмечен" Тогда
