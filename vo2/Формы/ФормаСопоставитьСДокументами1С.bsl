...
Процедура ТаблицаСопоставленияДокументовВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	ИначеЕсли Не ИмяКолонки = "Отмечен" Тогда
		...
		ПараметрыФормыПоказать = Новый Структура("Документ, СохранитьРезультат", оДок, Ложь);
		...
		#Если ТолстыйКлиентОбычноеПриложение Тогда
			РезультатСопоставления = ФормаСопоставленияДокументов.Показать(МестныйКэш, оДок, Ложь);
			МодульОбъектаКлиент().ВыполнитьСбисОписаниеОповещения(РезультатСопоставления, ОбрабочикЗакрытия);
		#Иначе
...
Процедура ФильтрСопоставленияПоУмолчанию()
	...
	Если РежимРаботыФормы = "Сравнение" Тогда
		ФильтрСтатусСопоставления = 2;
	Иначе
		ФильтрСтатусСопоставления = 1;
	КонецЕсли;
	ЭлементОчистить = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "КнопкаОчиститьФильтр").Видимость = Истина;
	СобратьПредставлениеФильтра();
...
Процедура СравнитьДокументы(Команда)
	...
	ПараметрыСтатистики = Новый Структура("Действие, Сообщения", "ПрикладнаяСтатистика", "Сравнение документов.Нажатие на кнопку сравнения");
...
#Область include_core2_vo2_Формы_ОбщиеМеханизмы_ОписаниеОповещенияОЗакрытии
#КонецОбласти
