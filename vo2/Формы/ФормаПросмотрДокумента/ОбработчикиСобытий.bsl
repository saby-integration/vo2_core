...
Процедура ОбновитьПоляФормыПоВложениюСБИС(ВложениеСБИС)
	...
	// Переименовать элементы через точку
	...
	ИначеЕсли НЕ (Вложение.Свойство("КлассыСопоставленияНоменклатур") 
		...
		Если НЕ ИниКонфигурация.Свойство("АвтоматическоеСозданиеНоменклатуры") Тогда
		...
		Иначе 
			МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧастьСоздатьСопоставитьНоменклатуруВ1С").Видимость = Ложь;
		КонецЕсли;
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ОтметитьВсе").Доступность = Ложь;
	ИначеЕсли МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры_ОбратноеСопоставление")
		И Вложение.Параметры.Свойство("КлассыСопоставленияНоменклатур") Тогда
		Вложение.Вставить("КлассыСопоставленияНоменклатур", Вложение.Параметры.КлассыСопоставленияНоменклатур);
	Иначе
...
Процедура СбисСтрокаТабличнаяЧастьДоИзменения(ВходящийКэш, ТабЧасть) 
    ...
    ЗаполнитьЗначенияСвойств(НоменклатураОписание,	ТабЧасть.ТекущиеДанные);
    ЗаполнитьЗначенияСвойств(ПрочееОписание,		ТабЧасть.ТекущиеДанные);
...
Процедура УстановитьВидимостьПоВложениюСБИС(ВложениеСБИС)
	...
	РасширенныйФункционалСопоставленияНоменклатуры =	(МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры") 
														...
														И ВложениеСБИС.Свойство("КлассыСопоставленияНоменклатур");
	
	ВидимостьЕдИзм				= МодульТекущий.ПолучитьЗначениеПараметраСБИС("ЕстьЕдиницыНаФормеПросмотра");
	ВидимостьХарактеристика		= Ложь;
	ВидимостьСоздатьКонтрагент	= Ложь;
	ВидимостьКоэффициент		= ВидимостьЕдИзм И Не РасширенныйФункционалСопоставленияНоменклатуры;
	ВидимостьЕдИзмОрг			= ВидимостьЕдИзм И Не РасширенныйФункционалСопоставленияНоменклатуры;
	...
	Если	ЗначениеЗаполнено(ПараметрыНоменклатуры) Тогда
		...
		ВидимостьХарактеристика		= ПараметрыНоменклатуры.ЕстьХарактеристики И Не РасширенныйФункционалСопоставленияНоменклатуры;
	...
	ЭлементДатаПоле.Доступность = Истина;
	...
	Иначе
		...
		Если	ДанныеВложенияРазобраны
			...
			ЭлементДатаПоле.Доступность			= ПараметрыДокумент.Доступность;
	...
	Если	ДанныеВложенияРазобраны Тогда	
		...
		Если ЗагрузкаПоддерживается Тогда
			...
			МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.Идентификатор").Видимость				= Ложь;
			...
			МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.Характеристика").Видимость			= ВидимостьХарактеристика;	
	 		МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.ЕдИзмОрг").Видимость					= ВидимостьЕдИзмОрг;
	 		МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.ЕдИзм").Видимость						= ВидимостьЕдИзм;
			МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.Коэффициент"	).Видимость				= ВидимостьКоэффициент;
		...
		ИначеЕсли	СопоставлениеПередОтправкой Тогда	
			...
			МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.Характеристика").Видимость			= Ложь;	
	 		МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.ЕдИзмОрг").Видимость					= Ложь;
