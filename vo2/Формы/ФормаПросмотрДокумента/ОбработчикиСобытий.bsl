...
Процедура Документ1СВыборВариантаСписокПослеДиалога(ПараметрыУстановитьВходящие, ДопПараметры = Неопределено) Экспорт
	...
	РедактируемыеПоляВложения.Документ = ПараметрыУстановитьВходящие.Значение;
...
Процедура ОбновитьПоляФормыПоДокументуСБИС(СоставПакетаСБИС)  
	...
	Если	МодульТекущий.ПрименятьФункционалНовыеКонтрагенты(Новый Структура("СоставПакета", СоставПакетаСБИС)) Тогда
		...
		МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "УчетОрганизация").Доступность = Истина;
		МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "УчетКонтрагент").Доступность = Истина;
		МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "УчетГрузополучатель").Доступность = Истина;
		
	КонецЕсли;
	
	ВидимостьЭлементов = 		(ДокументУчетаРазобран	И ИмяРеквизитаВложений = "ВложениеУчета")
							Или (ДокументРазобран		И ИмяРеквизитаВложений = "Вложение"); 
	
	МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТаблицаДокументов.Статус"		).Видимость	= ВидимостьЭлементов;
	МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТаблицаДокументов.СтатусКартинка").Видимость	= ВидимостьЭлементов;
	
	МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ПодготовитьКЗагрузке").Видимость	= Не ВидимостьЭлементов;
	МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ЗагрузитьНаВложении").Видимость	= ВидимостьЭлементов;
...
Процедура УстановитьВидимостьПоВложению(ВложениеСБИС)
	...
	ЭлементПанельЗагрузка	= МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ПанельЗагрузкаЛево");
	ПанельДокумент1С		= МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ПанельЗагрузкаЛево_Документ1С");
	...
	ЭлементПанельЗагрузка.Видимость = ГотовКЗагрузке;
...
Процедура УстановитьПолеДокумент1С_СписокВыбора(ВложениеСБИС)
	...
	Иначе
		...
		Если ЗначениеЗаполнено(РедактируемыеПоляВложения.Документ.Значение) Тогда
			...
			СписокВыбораОпераций.Добавить(ОписаниеНоваяОперация, "Выбрать подходящий документ");
