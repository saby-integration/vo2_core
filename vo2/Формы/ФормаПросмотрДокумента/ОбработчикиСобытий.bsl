...
Процедура СбисСтрокаТабличнаяЧастьПриИзменении(ВходящийКэш, ТабЧасть)
	...
	СтрКонтрагента = Кэш.ОбщиеФункции.РассчитатьЗначениеИзСтруктуры(ПутьКонтрагента, Вложение.СтруктураФайла); 
	
	ИдентификационныйНомер = МодульОбъектаКлиент().Сторона_Получить(СтрКонтрагента, "ИдентификаторЛица"); 
	СтрКонтрагента.Вставить("ИдентификаторЛица", ИдентификационныйНомер);
