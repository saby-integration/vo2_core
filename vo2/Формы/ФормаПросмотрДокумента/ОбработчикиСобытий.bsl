...
Процедура ОбновитьСтрокуТаблицыДокументов(СтрокаТаблицыДокументов)
	...
	Если Не	ПараметрыПросмотр = Неопределено Тогда
		...
		ПараметрыПросмотр.ПараметрыКонтрагент.ПоказатьСозданиеКарточки = Ложь;
	...
	ИначеЕсли	ПараметрыПоВложению.Проблемы.Количество() Тогда
		...
		КонтрагентВложения		= Поля1С.Контрагент; 
		
		Если	КонтрагентВложения.Доступность
			И	КонтрагентВложения.Значение = Неопределено Тогда
			
			ПараметрыПросмотр.ПараметрыКонтрагент.ПоказатьСозданиеКарточки = Истина;
...
Процедура УчетСторонаПриИзменении(Элемент)
	...
	Для каждого ВложениеСБИС Из Вложения Цикл
		...
		Если НЕ ВложениеСбис.СтатусыВложения.Ини Тогда
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьВидимостьПоВложениюСБИС(ВложениеСБИС)
	...
	Если	ЗначениеЗаполнено(ПараметрыНоменклатуры) Тогда
		...
		ВидимостьХарактеристик		= ПараметрыНоменклатуры.ЕстьХарактеристики;
	...
	Если	ДанныеВложенияРазобраны Тогда	
		...
		Если ЗагрузкаПоддерживается Тогда
			...
			МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.Характеристика").Видимость			= ВидимостьХарактеристик;	
	 		МодульТекущий.ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТабличнаяЧасть.ЕдИзмОрг").Видимость					= ВидимостьЕдиниц;
