...
Процедура СохранитьНаДискНажатие(Элемент) Экспорт
	...
	Для Каждого Вложение Из СоставПакета.Вложение Цикл
		...
		Если	ЗначениеЗаполнено(XMLДокумента) Тогда
			...
			ТД.УстановитьТекст(XMLДокумента);
...
Процедура ДобавитьВложение(Команда)
	...
	ПараметрыВыбора = Новый Структура("РежимВыбораФайла, Заголовок, МножественныйВыбор", РежимДиалогаВыбораФайла.Открытие, "Выберите файлы", Истина);
	ОбработкаДиалога = МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("ДобавитьВложение_ПослеДиалога", ЭтаФорма, ПараметрыВыбора);
	МодульОбъектаКлиент().СбисВыбратьФайл(ОбработкаДиалога, ПараметрыВыбора);
	
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВложение_ПослеДиалога(МассивФайлов, ДопПараметры) Экспорт
	...
	Если		МодульОбъектаКлиент().ПараметрSaby("Шифрование")
			И	МодульОбъектаКлиент().ПараметрSaby("ШифроватьВыборочно") Тогда
		
		БезопасныйКаталог	= МодульОбъектаКлиент().ПараметрSaby("БезопасныйКаталог", "Сбис");
		Шифровать			= БезопасныйКаталог = Лев(ДопПараметры.ДиалогВыбора.Каталог, СтрДлина(БезопасныйКаталог));
...
Процедура ЗаполнитьТаблицуНоменклатуры(ВложениеСБИС) Экспорт
	...
	ТаблДок = ДанныеСтруктурыФайла.Поля.Получить("ТабличнаяЧастьСопоставлениеНоменклатуры");
	Для Каждого СтрТабл Из ТаблДок Цикл
...
Процедура сбисИнформацияДляТП(Команда)
	...
	ПараметрыСбора = Новый Структура("Модули", Новый Массив);
	ПараметрыСбора.Модули.Добавить(ЭтаФорма);
	
	МодульОбъектаКлиент().НачатьСборТехИнформации(ПараметрыСбора);
...
Процедура УстановитьВидимостьЭлементовВкладкиТранспортныеУпаковки(СоставПакетаСБИС)
	...
	ВидимостьТранспортныхУпаковок =	СоставПакетаСБИС.Свойство("Расширение", ПроверяемыйПараметр)
									И	ПроверяемыйПараметр.Свойство("Упаковки", ПроверяемыйПараметр)
