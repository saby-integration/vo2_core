...
Функция СписокВложенийДляОперация()
	...
	Для Каждого СтрокаОтмечена Из ТаблицаДокументов.НайтиСтроки(Новый Структура("Отмечен", Истина)) Цикл
		...
		МодульОбъектаКлиент().ВложениеСБИС_Обновить(ВложениеВыбрано, ИзмененияВложения);
...
//
// Параметры:
//  Элемент	 - ЭлементФормы	 - куда было кликнуто
//
&НаКлиенте
Процедура СохранитьНаДискНажатие(Элемент) Экспорт
	...
	ОбщиеФункцииДок	= МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("ОбщиеФункции");
	
	Если	Не СоставПакета.Свойство("Вложение") Тогда
		
		МодульОбъектаКлиент().СбисСообщить("Документ пуст. Отсутствуют вложения для записи на диск!");
		Возврат;
		
	ИначеЕсли Не ДиалогОткрытия.Выбрать() Тогда 
	...
	ИмяZIP = СокрЛП(СтрЗаменить(СтрЗаменить(СтрЗаменить(ЗаголовокПакета,
				":","."),
				"\","_"),
				"/","_"));
	
	// Чтобы правильно расставились разделители в конце пути
	ПутьККаталогу =	ОбщиеФункцииДок.сбисФорматКаталога(ПутьККаталогу,			Кэш.ПараметрыСистемы.Клиент);
	ПутьККаталогу =	ОбщиеФункцииДок.сбисФорматКаталога(ПутьККаталогу + ИмяZip,	Кэш.ПараметрыСистемы.Клиент);
	...
	Для Каждого Вложение Из СоставПакета.Вложение Цикл
		...
		Файл = МодульОбъектаКлиент().ВложениеСБИС_Получить(Вложение, "Файл");
		...
		Если	ЗначениеЗаполнено(XMLДокумента) Тогда
			...
			Если ЗначениеЗаполнено(Файл) Тогда
				...
				Если ТипЗнч(Файл) = Тип("Структура") Тогда   // в SDK 1.1 файл стал не массивом, а объектом
			...
			ИначеЕсли	ЗначениеЗаполнено(СтруктураФайла)
			...
			ТД.Записать(ПутьККаталогу + ИмяФайла, "windows-1251");
			
		ИначеЕсли ЗначениеЗаполнено(ПолноеИмяФайла) Тогда
			
			ИмяФайла = МодульОбъектаКлиент().ВложениеСБИС_Получить(Вложение, "ИмяФайла");
			КопироватьФайл(ПолноеИмяФайла, ПутьККаталогу + ИмяФайла);
			
		ИначеЕсли	ТипЗнч(Файл)=Тип("Структура")
				И	Файл.Свойство("Ссылка")
				И	Файл.Свойство("Имя") Тогда
			
			Кэш.Интеграция.СохранитьВложениеПоСсылкеВФайл(Кэш, Файл.Ссылка, ПутьККаталогу + Файл.Имя);
			
		Иначе
			
			// Неизвестное вложение, хз как обработать
			
		КонецЕсли;
		
	КонецЦикла;
	
	Кэш.ГлавноеОкно.сбисОткрытьКаталог(ПутьККаталогу);
...
Процедура СоздатьКонтрагента(Команда)
	...
	АргументыТочкиВхода		= Новый ФиксированнаяСтруктура("СтрокаТаблДок, СоставПакета, Кэш", СтрокаТаблДок, СоставПакета, МестныйКэш);
	ОписаниеТочкиВходаВФК	= МодульОбъектаКлиент().ПользовательскаяТочкаВхода("ЗаполнитьИзФайла", "ФормаСозданияКонтрагента");
	Если ОписаниеТочкиВходаВФК = Неопределено Тогда
		
		ПараметрыСоздать	= Новый Структура("Вложение, СоставПакета", Вложение, СоставПакета);
		ОбработчикСоздания	= МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("СоздатьКонтрагента_Завершение", ЭтаФорма, АргументыТочкиВхода);
		ФормаСоздания		= МодульОбъектаКлиент().ПолучитьФормуОбработки("ФормаСозданияКонтрагента");
		ФормаСоздания.ОписаниеОповещенияОЗакрытии = ОбработчикСоздания;
		
		ФормаСоздания.ПоказатьПоверх(Новый Структура("Вложение, СоставПакета", Вложение, СоставПакета));
		
	Иначе
		
		Попытка
			
			МодульОбъектаКлиент().ВызватьПользовательскуюТочкуВхода(ОписаниеТочкиВходаВФК, АргументыТочкиВхода);
			
		Исключение
			
			МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке());
...
&НаКлиенте
Процедура СоздатьКонтрагента_Завершение(РезультатСоздания, ДопПараметры) Экспорт
	...
	СтрокаТаблицыДокументов	= ДопПараметры.СтрокаТаблДок;
	
	Контрагент1СПриИзменении_Обработчик(СтрокаТаблицыДокументов, РезультатСоздания);
	
	МодульОбъектаКлиент().СбисОткрытьКарточку1С(РезультатСоздания);
...
Процедура ЗаполнитьДокумент1СВСоставеПакета(ИдентификаторВложения, Документ1С, СтруктураДокумента1С)
	...
	Для Каждого ВложениеПакета Из ВложенияПакета Цикл
		
		Если Не (	ВложениеПакета.Свойство("Идентификатор")
				И	ВложениеПакета.Идентификатор = ИдентификаторВложения) Тогда
			Продолжить;
		КонецЕсли;
		
		ПараметрыОбщие		= МодульОбъектаКлиент().ВложениеСБИС_Получить(ВложениеПакета, "Параметры");
		БылоРедактирование	=	ПараметрыОбщие.РедактироватьДокумент
							Или	ПараметрыОбщие.РедактироватьТабЧасть;
		
		// Очистить временные данные
		СбисОписаниеОбъекта	= Новый Структура("Тип, Ключ, Значение", "ДокументШаблон.СтруктураСвойств", ВложениеПакета.Идентификатор);
		...
		ПараметрыПросмотр	= МодульОбъектаКлиент().ВложениеСБИС_Получить(ВложениеПакета, "Просмотр", "ФормаПросмотрДокумента");
		...
		Если Не БылоРедактирование Тогда	
			
			// Редактируемый документ записывается один раз, при создании нового. При перезаполнении, сопоставление не записывается
			...
			МодульОбъектаКлиент().ВложениеСБИС_Заполнить(ВложениеПакета, Документ1С, "Документ1С");
			МодульОбъектаКлиент().ВложениеСБИС_Записать(ВложениеПакета, ПараметрыЗаписать);
			
		КонецЕсли;
		
		// Обновить поля на форме с учетом нового документа
		...
		МодульОбъектаКлиент().ВложениеСБИС_Обновить(ВложениеПакета, Новый Структура("Поля1С", СтруктураОбновить), ДопПараметрыВложения);
...
Процедура ДобавитьВложение(Команда)
	...
	Для Каждого ИмяФайла Из МассивФайлов Цикл
		...
		ВложениеДляДобавления = МодульОбъектаКлиент().НовыйВложениеСБИС(Новый Структура("Файл", Файл));
		МодульОбъектаКлиент().ВложениеСБИС_Установить(ВложениеДляДобавления, "ВыборочноеШифрование", Шифровать);

		СоставПакета.Вложение.Добавить(ВложениеДляДобавления);
	КонецЦикла;
...
Функция ЗаполнитьДокументДаннымиЭД(Вложение) Экспорт
	...
	Если Найти(сбисЭлементФормы(ЭтаФорма,"ТаблицаДокументов").ТекущиеДанные.Статус, "Сопоставление не требуется")=0 Тогда
		...
		МассивМодулей.Добавить("Файл_" + Вложение.ФорматДляЗагрузки + "_" + Вложение.ВерсияФорматаДляЗагрузки);
		МассивМодулей.Добавить("Файл_Шаблон_" + Вложение.ВерсияФорматаДляЗагрузки);
		МассивМодулей.Добавить("Файл_Шаблон");
		
		МодульДанныхСтроки = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки("ДанныеСтрокиВложенияНаПросмотр", МассивМодулей);
		...
		Если ЗначениеЗаполнено(ТаблДок) Тогда 
			...
			Для Каждого СтрТабл Из ТаблДок Цикл
				...
				СтрТаблНаПросмотр = МодульДанныхСтроки.ДанныеСтрокиВложенияНаПросмотр(Кэш, СтрТабл);
				
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрТаблНаПросмотр);
...
Процедура ЗаполнитьТаблицуНоменклатуры(ВложениеСБИС) Экспорт
	...
	// Костыль для заполнения обратного сопоставления
	...
	сч = 1;
	ТаблДок = ВложениеСБИС.ДанныеСтруктурыФайла.Поля.Получить("ТабличнаяЧастьСопоставлениеНоменклатуры");
	Для Каждого СтрТабл Из ТаблДок Цикл
...
Процедура ПриЗакрытии()
	...
	Если ВыполнитьЗакрытиеФормы Тогда
		
		// Это завершение работы формы
		Возврат;
...
//
// Параметры:
//  СоставПакета - Структура	 - Обрабатываемы пакет (документ)
//
&НаКлиенте  
Процедура ЗаполнитьМаркировку(СоставПакета) Экспорт
	Перем лКонтрагент;
	...
	Если СоставПакета.Свойство("Контрагент", лКонтрагент) Тогда 
		...
		ОписаниеЭлемента = Новый Структура("Форма, Реквизит", ЭтаФорма, "УчастникОборота");
		ИзмененияЭлемента = Новый Структура("Видимость", лИностраннаяОрганизация <> Истина);
		МодульОбъектаКлиент().ИзменитьЭлементФормыОбработки(ОписаниеЭлемента, ИзмененияЭлемента);
...
// Процедура - обновляет состояние проверки марировки на вкладке «Маркировка»
//
&НаКлиенте
Процедура ОбновитьСтатусПроверкиКодовМаркировки()
...
Функция ПолучитьСостояниеПроверки(ЗначениеПроверки)
	...
	ИначеЕсли	ЗначениеПроверки = "GtinSuccess"
...
&НаКлиенте
Процедура УстановитьВидимостьЭлементовВкладкиМаркировка(СоставПакетаСБИС) 
	Перем СтатусГос, СостояниеОперации, КодСостоянияОперации;
	
	МаркировкаЗаполнена = ПараметрыРаботы.МаркировкаЗаполнена;
	
	Если МаркировкаЗаполнена Тогда
		
		Возврат;
		
	КонецЕсли;
	
	СбисРасширение = МодульОбъектаКлиент().СоставПакета_Получить(СоставПакетаСБИС, "Расширение");
	РаботаСГосКонтрактами = МодульОбъектаКлиент().РаботаСГосКонтрактами(СоставПакетаСБИС);
	РаботаСМаркировкой = МодульОбъектаКлиент().РаботаСМаркировкой(СоставПакетаСБИС);
	
	ТекущийРаздел = МестныйКэш.Разделы["р" + МестныйКэш.Текущий.Раздел];    
	
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "Контент.Маркировка").Видимость = Ложь;   
	
	Если НЕ МодульОбъектаКлиент().РаботаСМаркировкой(СоставПакетаСБИС)
		И НЕ РаботаСГосКонтрактами Тогда
		Возврат;
	КонецЕсли;
	...
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ОбновитьСтатусПроверкиКодовМаркировки").Видимость	= РаботаСМаркировкой;
	...
	Если СбисРасширение.Свойство("СостояниеМарк", СтатусГос)
		...
		Если КодСостоянияОперации = "3" Тогда
			...
			МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ОбновитьСтатусПроверкиКодовМаркировки").Видимость	= Ложь;
		КонецЕсли;
...
&НаКлиенте
Процедура УстановитьПараметрыЭлементовГруппыДокументВыбытие(ДопПараметры = Неопределено)

	ЭлементТипДокументаВыбытия				= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТипДокументаВыбытия");
	ЭлементИмяДокументаВыбытия				= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ИмяДокументаВыбытия");
	ЭлементДекорацияДокументВыбытияОт		= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ДекорацияДокументВыбытияОт");
	ЭлементДатаДокументаВыбытия				= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ДатаДокументаВыбытия");
	ЭлементДекорацияДокументВыбытияНомер	= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ДекорацияДокументВыбытияНомер");
	ЭлементНомерДокументаВыбытия			= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "НомерДокументаВыбытия");
	
	Если ЗначениеЗаполнено(ДопПараметры) Тогда
		Если ДопПараметры.Свойство("СкладскиеПараметрыДокумента") Тогда
			СкладскиеПараметрыДокумента = ДопПараметры.СкладскиеПараметрыДокумента;
			ПараметрыДокументаВыбытия = МодульОбъектаКлиент().ПолучитьПараметрыДокументаВыбытия(СкладскиеПараметрыДокумента);
			
			ИзмененияЭлемента = Новый Структура("СписокВыбора", ПараметрыДокументаВыбытия.СписокТиповДокументаВыбытия);
			МодульОбъектаКлиент().ИзменитьЭлементФормыОбработки(ЭлементТипДокументаВыбытия, ИзмененияЭлемента);
			
			ТипДокументаВыбытия		= ПараметрыДокументаВыбытия.ТипДокументаВыбытия;
			ИмяДокументаВыбытия		= ПараметрыДокументаВыбытия.ИмяДокументаВыбытия;
			ДатаДокументаВыбытия	= ПараметрыДокументаВыбытия.ДатаДокументаВыбытия;
			НомерДокументаВыбытия	= ПараметрыДокументаВыбытия.НомерДокументаВыбытия;
		КонецЕсли;
	КонецЕсли;
	
	ПоказыватьПолеИмяДокументаВыбытия	= ТипДокументаВыбытия = "4"; // Прочий документ
	
	ПоказыватьПолеДатаДокументаВыбытия	= ТипДокументаВыбытия = "4" // Прочий документ
											ИЛИ ТипДокументаВыбытия = "2" // Кассовый чек
											ИЛИ ТипДокументаВыбытия = "3"; // Товарный чек
	
	ПоказыватьПолеНомерДокументаВыбытия	= ТипДокументаВыбытия = "4" // Прочий документ
											ИЛИ ТипДокументаВыбытия = "2" // Кассовый чек
											ИЛИ ТипДокументаВыбытия = "3"; // Товарный чек
																			
																		
	ЭлементТипДокументаВыбытия.Видимость			= ЗначениеЗаполнено(ЭлементТипДокументаВыбытия.СписокВыбора);
	ЭлементИмяДокументаВыбытия.Видимость			= ПоказыватьПолеИмяДокументаВыбытия;
	ЭлементДекорацияДокументВыбытияОт.Видимость		= ПоказыватьПолеИмяДокументаВыбытия;
	ЭлементДатаДокументаВыбытия.Видимость			= ПоказыватьПолеДатаДокументаВыбытия;
	ЭлементДекорацияДокументВыбытияНомер.Видимость	= ПоказыватьПолеНомерДокументаВыбытия;
	ЭлементНомерДокументаВыбытия.Видимость			= ПоказыватьПолеНомерДокументаВыбытия;
	
	Если НЕ МестныйКэш.ПараметрыСистемы.Клиент.УправляемоеПриложение Тогда
		ЭлементНадписьТипДокументаВыбытия = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТипДокументаВыбытия.Надпись");
		ЭлементНадписьТипДокументаВыбытия.Видимость = ЗначениеЗаполнено(ЭлементТипДокументаВыбытия.СписокВыбора);
	КонецЕсли;
...
Процедура УстановитьПоложениеЭлемента(НаименованиеЭлемента) 
	...
	ТаблицаСоответствияЭлементов = МодульОбъектаКлиент().СериализоватьЗначениеВТаблицуЗначений(МассивСоответсвий);  	
...
// Процедура получает список транспортных упаковок с документа СБИС
//
// Параметры:
//  СоставПакета - Структура - обрабатываемый пакет (документ)
//	ДопПараметры - Струкрута - дополнительные параметры
//
&НаКлиенте
Процедура ЗаполнитьТранспортныеУпаковки(ДопПараметры = Неопределено) Экспорт
	Перем УпаковкаРазвернуть;	
	
	Если ЗначениеЗаполнено(ДопПараметры) Тогда
		Если ДопПараметры.Свойство("УпаковкаРазвернуть") Тогда
			УпаковкаРазвернуть = ДопПараметры.УпаковкаРазвернуть;
		КонецЕсли;
	КонецЕсли;
	
	СписокПараметровФильтра = Новый Структура;
	СписокПараметровФильтра.Вставить("read_with_pack_rec",	Истина);
	СписокПараметровФильтра.Вставить("docflowId",			СоставПакета.Идентификатор);
	Если УпаковкаРазвернуть <> Неопределено Тогда
		СписокПараметровФильтра.Вставить("Раздел",			УпаковкаРазвернуть.ИдентификаторСбис);
	КонецЕсли;
	
	СписокПараметровВызова = Новый Структура();
	СписокПараметровВызова.Вставить("Фильтр", СписокПараметровФильтра);
	
	Попытка
		СписокТраспортныхУпаковокСНоменклатурой = МодульОбъектаКлиент().ПолучитьТранспортныеУпаковкиСДокумента(СписокПараметровВызова); 
	Исключение
		МодульОбъектаКлиент().СообщитьСбисИсключение(ИнформацияОбОшибке(), "ЗаполнитьТранспортныеУпаковки");
	КонецПопытки;

	ЗаполнитьТаблицуТранспортныхУпаковок(СписокТраспортныхУпаковокСНоменклатурой, УпаковкаРазвернуть);
	ПараметрыРаботы.ТранспортныеУпаковкиЗаполнены = Истина;
...
//
// Параметры:
//  СписокТраспортныхУпаковокСНоменклатурой	 - Массив структур - транспортные упаковки документа СБИС
//
&НаКлиенте
Процедура ЗаполнитьТаблицуТранспортныхУпаковок(СписокТраспортныхУпаковокСНоменклатурой, УпаковкаРазвернуть) 
	
	СтрокиУпаковок = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ТранспортныеУпаковки, "Строки");
	Если УпаковкаРазвернуть <> Неопределено Тогда
		СтрокиУпаковок = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(УпаковкаРазвернуть, "Строки");
	КонецЕсли;
	СтрокиУпаковок.Очистить();
	
	Если НЕ ЗначениеЗаполнено(СписокТраспортныхУпаковокСНоменклатурой) Тогда
		Возврат;
	КонецЕсли;
	
	НомерПП = 1;
	Для Каждого СтрМасс из СписокТраспортныхУпаковокСНоменклатурой Цикл
		
		НоваяСтрока = СтрокиУпаковок.Добавить(); 
		
		НоваяСтрока.ИдентификаторСбис	= СтрМасс.НоменклатураДокументаПК;
		
		Если ЗначениеЗаполнено(СтрМасс.Номенклатура_Наименование) Тогда
			НоваяСтрока.Наименование = СтрМасс.Номенклатура_Наименование;
		Иначе
			НоваяСтрока.Наименование = СтрМасс.ВходящееНаименование_Наименование;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтрМасс.НоменклатураДокументаРасш_Комментарий) Тогда
			НоваяСтрока.НазваниеУпаковки = СтрМасс.НоменклатураДокументаРасш_Комментарий;
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрМасс.Номенклатура_НомНомер) Тогда
			НоваяСтрока.ЭтоУпаковка			= Истина;
			НоваяСтрока.ВместимостьУпаковки	= СтрМасс.SerialNumber_Count;
		КонецЕсли;
		
		НоваяСтрока.Количество			= СтрМасс.Количество;
		НоваяСтрока.ЕдиницаИзмерения	= СтрМасс.НазваниеЕИ;
		НоваяСтрока.Вес					= СтрМасс.NomenclatureExt_Weight;
		НоваяСтрока.Объем				= СтрМасс.NomenclatureExt_Volume;
		НоваяСтрока.Сумма				= СтрМасс.СуммаЦен;
		
		Если ЗначениеЗаполнено(СтрМасс.DnIdsInPack) Тогда
			// Проверяем на наличие номенклатуры в составе упаковки и добавляем пустую строку - заглушку, чтобы можно было нажать на "+"
			ПодчиненнаяСтрока = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(НоваяСтрока, "Строки").Добавить();
		КонецЕсли;
		
		Если УпаковкаРазвернуть <> Неопределено Тогда
			НомерСтроки = УпаковкаРазвернуть.НомерСтроки + "." + НомерПП;
		Иначе
			НомерСтроки = НомерПП;
		КонецЕсли;
...
Процедура УстановитьОграничениеТипаРеквизитов(ВложениеСБИС)
	...
	УстановитьОграничениеТипаНоменклатуры(ВложениеСБИС, УстановленныйРаздел);				
	УстановитьОграничениеТипаХарактеристики(ВложениеСБИС, УстановленныйРаздел);
	УстановитьОграничениеТипаЕдиниц(ВложениеСБИС, УстановленныйРаздел);
	
КонецПроцедуры

&НаКлиенте
Процедура  УстановитьОграничениеТипаНоменклатуры(ВложениеСБИС, ИниТекущий = Неопределено) Экспорт			
	...
	ОписаниеПараметраИзменить = Новый ФиксированнаяСтруктура("Форма, Реквизит", ЭтаФорма, КлючТЧСопоставления + ".Номенклатура");
...
&НаКлиенте
Процедура  УстановитьОграничениеТипаХарактеристики(ВложениеСБИС, ИниТекущий = Неопределено) Экспорт			
	мВидовСправочников = Новый Массив;		
	ВидыСпр = Неопределено;
	...
	ОписаниеПараметраИзменить = Новый ФиксированнаяСтруктура("Форма, Реквизит", ЭтаФорма, КлючТЧСопоставления + ".Характеристика");
	ПараметрыИзменить = Новый Структура;
	ПараметрыИзменить.Вставить("УстановитьОграничениеТипа", ОписаниеТипа);
...
&НаКлиенте
Процедура  УстановитьОграничениеТипаЕдиниц(ВложениеСБИС, ИниТекущий = Неопределено) Экспорт			
	мВидовСправочников = Новый Массив;		
	ВидыСпр = Неопределено;
	...
	ОписаниеПараметраИзменить = Новый ФиксированнаяСтруктура("Форма, Реквизит", ЭтаФорма, КлючТЧСопоставления + ".ЕдИзмОрг");
	ПараметрыИзменить = Новый Структура;
	ПараметрыИзменить.Вставить("УстановитьОграничениеТипа", ОписаниеТипа);
	Попытка
...
Процедура ПослеВыбораИзМеню(ВыбранныйЭлемент, ПараметрыВыбора) Экспорт
	...
	НужноРазобрать		= Истина;
	ПараметрыКоманды	= ВыбранныйЭлемент.Значение;
	Если ТипЗнч(ПараметрыКоманды) = Тип("Структура") Тогда
		
		ИмяФункции = ПараметрыКоманды.ИмяФункции;
		Если ПараметрыКоманды.Свойство("ТребуетРазбора") Тогда
			
			НужноРазобрать = ПараметрыКоманды.ТребуетРазбора;
			
		КонецЕсли;
		
	Иначе
		
		ИмяФункции = ПараметрыКоманды;
		
	КонецЕсли;
	
	Если НужноРазобрать Тогда
	...
	Иначе
		ИмяФункцииСПараметрами = ИмяФункции + "(Кэш, ЭтаФорма)";
	КонецЕсли;
	
	МодульОбработчика = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки(ИмяФункции, 
		Кэш.Разделы.Текущий.ФормаПросмотрДокумента.ФормыРаздела);
	
	Если МодульОбработчика = Ложь Тогда
		МодульОбработчика = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки(ИмяФункции, "РаботаСДокументами1С");	
	КонецЕсли;
	
	Результат = Вычислить("МодульОбработчика." + ИмяФункцииСПараметрами);
...
Процедура СбисПечатьСоставаПакета_ПослеВыбора(СписокРезультат, ДополнительныеПараметры) Экспорт
	...
	Попытка
		...
		МодульОбъектаКлиент().ОтправитьНаборОбъектовНаПечать(ОбъектыПечати, ДополнительныеПараметры);
		
	Исключение
		
		МодульОбъектаКлиент().СбисСообщить(ИнформацияОбОшибке(), "ФормаПросмотрДокумента.СбисПечатьСоставаПакета_ПослеВыбора");
...
Процедура ДокументооборотПерейти(Команда)
	...
	ОбработчикПерехода = МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("сбисПослеВыбораПерехода", ЭтаФорма, Новый Структура("СоставПакета", СоставПакета));
	ПараметрыДействие = Новый Структура("Владелец, ОбработчикРезультат", ЭтаФорма, ОбработчикПерехода);
	
	МодульОбъектаКлиент().СоставПакета_ВыбратьДействиеПоЭтапу(СоставПакета, ПараметрыДействие);
...
Функция  ПараметрыДляФормированияКонтекста(ВходящийКэш)
	...
	МодульОбъектаКлиент().ПодготовитьСтороныКЗагрузкеДокумента(ПараметрыПодготовки, Новый Структура("ЕстьПартнеры", Кэш.ини.Конфигурация.Свойство("Партнеры")));
...
Процедура ЗаполнитьТаблицуДокументов(СоставПакета) Экспорт
	...
	Для СчетчикВложений = 0 По ВложенияДляЗаполнения.Количество() - 1  Цикл
		
		НоваяСтр = ТаблицаДокументов.Добавить();
		НоваяСтр.НомерВложенияВПакете = СчетчикВложений;
		
		ТаблицаДокументовУстановитьСтроку(НоваяСтр, ВложенияДляЗаполнения[СчетчикВложений]);
...
Процедура ОтклонитьУтвердитьНажатие(Элемент)
	...
	Если	ВыбранноеДействие.ТребуетКомментария = "Да"
		...
		ПараметрыПереход	= Новый Структура("СоставПакета", СоставПакета);	
		ОбработчикПерехода	= МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("сбисПослеВыбораПерехода", ЭтаФорма, ПараметрыПереход);
		ПараметрыДействие	= Новый Структура("Владелец, ОбработчикРезультат, Действие", ЭтаФорма, ОбработчикПерехода, ВыбранноеДействие);
...
//
&НаКлиенте
Процедура УстановитьВидимостьЭлементовВкладкиПрослеживаемость(СоставПакетаСБИС)  
	...
	ВидимостьПрослежки =	СоставПакетаСБИС.Свойство("Расширение", СбисРасширение)
	...
	Если СоставПакетаСБИС.Свойство("Расширение",СбисРасширение) 
	...
	Если СоставПакетаСБИС.Свойство("Состояние", СостояниеДокумента)
	...
	ИначеЕсли СоставПакетаСБИС.Свойство("Расширение",СбисРасширение) 
...
&НаКлиенте
Процедура УстановитьКонтрагентыСопоставление(СоставПакетаСБИС)  
	...
	Если СоставПакетаСБИС.Свойство("НашаОрганизация") Тогда
		
		ПакетОрганизация = Кэш.ОбщиеФункции.сбисНазваниеСтороны(СоставПакетаСБИС.НашаОрганизация);
		
	КонецЕсли;
	
	Если СоставПакетаСБИС.Свойство("Контрагент") Тогда
		
		ПакетКонтрагент = Кэш.ОбщиеФункции.сбисНазваниеСтороны(СоставПакетаСБИС.Контрагент);
		
	КонецЕсли;
		
	Грузополучатель = МодульОбъектаКлиент().СоставПакета_Получить(СоставПакетаСБИС, "Грузополучатель");
...
&НаКлиенте
Процедура УстановитьВидимостьЭлементовВкладкиТранспортныеУпаковки(СоставПакетаСБИС)
	Перем ПроверяемыйПараметр;
	
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ТранспортныеУпаковки, "Строки").Очистить();     
	
	ВидимостьТранспортныхУпаковок =	СоставПакетаСБИС.Свойство("Расширение", ПроверяемыйПараметр)
									//И	ПроверяемыйПараметр.Свойство("Прослеживаемость", ПроверяемыйПараметр)
									//И	ПроверяемыйПараметр = "Да" // TODO МАИ пока непонятно, что за признак на пакете
									И Не МодульОбъектаКлиент().ПолучитьЗначениеФичи("АФ_СозданиеТранспортныхУпаковок");
						
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "Контент.ТранспортныеУпаковки").Видимость = ВидимостьТранспортныхУпаковок;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьДопПоляДокумента(СоставПакетаСБИС)

	Состояние = МодульОбъектаКлиент().СоставПакета_Получить(СоставПакетаСБИС, "Состояние");
	Если Состояние <> Неопределено Тогда
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "Контент.ДополнительныеПоля").Видимость = Ложь;
		Возврат;
	КонецЕсли;
	
	ДопПоляДокумента = МодульОбъектаКлиент().СоставПакета_Получить(СоставПакетаСБИС, "ДопПараметрыСБИС");
	Если ДопПоляДокумента = Неопределено Тогда
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "Контент.ДополнительныеПоля").Видимость = Ложь;
		Возврат;
	КонецЕсли;

	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "Контент.ДополнительныеПоля").Видимость = ДопПоляДокумента.Количество();

	ДополнительныеПараметрыДокумента.Очистить();

	Для каждого ДопПоле Из ДопПоляДокумента Цикл
		
		НовСтрока = ДополнительныеПараметрыДокумента.Добавить();
		НовСтрока.ИмяПараметра = ДопПоле.Имя;
		НовСтрока.ЗначениеПараметра = ДопПоле.Значение;
		НовСтрока.Заголовок = ДопПоле.Заголовок;
		НовСтрока.Параметр = ДопПоле.Параметр;
