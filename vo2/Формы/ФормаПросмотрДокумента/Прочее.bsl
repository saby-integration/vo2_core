...
Процедура СохранитьНаДискНажатие(Элемент) Экспорт
	...
	Для Каждого Вложение Из СоставПакета.Вложение Цикл
		...
		Если	Вложение.Свойство("XMLДокумента")
			И	Вложение.XMLДокумента <> "" Тогда
			...
			ИначеЕсли	Вложение.Свойство("СтруктураФайла")
				И		Вложение.СтруктураФайла.Свойство("Файл") Тогда
		...
		ИначеЕсли Вложение.Свойство("ПолноеИмяФайла") Тогда
			...
			КопироватьФайл(Вложение.ПолноеИмяФайла, ПутьККаталогу + Вложение.ИмяФайла);
			
		ИначеЕсли	Вложение.Свойство("Файл")
				И	ТипЗнч(Вложение.Файл)=Тип("Структура")
				И	Вложение.Файл.Свойство("Ссылка")
				И	Вложение.Файл.Свойство("Имя") Тогда
			
			Кэш.Интеграция.СохранитьВложениеПоСсылкеВФайл(Кэш, Вложение.Файл.Ссылка, ПутьККаталогу + Вложение.Файл.Имя);
...
Процедура ЗаполнитьТаблицуДокументов(СоставПакета) Экспорт
	...
	Для СчетчикВложений = 0 По ВложенияДляЗаполнения.Количество() - 1  Цикл
		...
		Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры_ОбратноеСопоставление")
			И ТекущийРаздел = "Продажа"
			И ВложенияДляЗаполнения[СчетчикВложений].Свойство("Параметры") Тогда
			
			Если  ВложенияДляЗаполнения[СчетчикВложений].Параметры.Свойство("НоменклатураСопоставлена")
				И ВложенияДляЗаполнения[СчетчикВложений].Параметры.НоменклатураСопоставлена Тогда
				НоваяСтр.Статус = "Номенклатура сопоставлена"; 
			ИначеЕсли ВложенияДляЗаполнения[СчетчикВложений].Параметры.Свойство("НоменклатураСопоставлена")
				И НЕ ВложенияДляЗаполнения[СчетчикВложений].Параметры.НоменклатураСопоставлена Тогда
				НоваяСтр.Статус = "Не вся номенклатура сопоставлена";
				МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "ТаблицаДокументов.Статус").Доступность = Истина;
			КонецЕсли;
