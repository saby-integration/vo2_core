...
Процедура ВыбратьДействиеСПакетом(ВыбранноеДействие=Неопределено)
	ЕстьАннулирование =  (Не МестныйКэш.Парам.СпособОбмена = 1);
	Если ЕстьАннулирование  
	   И СоставПакета.Параметры.Свойство("ЕстьАннулирование") Тогда 
	  
		Если СоставПакета.Параметры.ЕстьАннулирование = Неопределено Тогда
			ЕстьАннулирование = МодульОбъектаКлиент().СоставПакета_Получить(СоставПакета, "МожноАннулировать");
		ИначеЕсли Не СоставПакета.Параметры.ЕстьАннулирование Тогда
			ЕстьАннулирование = Ложь;
		Иначе 
			ЕстьАннулирование = Истина;
		КонецЕсли; 
	КонецЕсли; 
	ПараметрыПереходов = Новый Структура("СоставПакета, Конфигурация, ЕстьАннулирование, ВыбранноеДействие", 
										  СоставПакета, МестныйКэш.Ини.Конфигурация, ЕстьАннулирование);
