...
Процедура ЗаполнитьДокумент1СВСоставеПакета(ИдентификаторВложения, Документ1С, СтруктураДокумента1С)
	...
	Для Каждого Вложение Из СоставПакета[ИмяРеквизитаВложений] Цикл
		Если Вложение.Свойство("Идентификатор") и Вложение.Идентификатор = ИдентификаторВложения Тогда
			...
			ПараметрыРассчитать = Новый Структура("Ини", Вложение.СтруктураИниФайла);
			
			//Если на инишке вложения стоит признак не запоминать результат загрузки, то пропустить заполнение документа.
			СохранитьСопоставление = Не МодульДокументШаблон.РассчитатьЗначение("СохранитьСопоставление", ПараметрыРассчитать) = Ложь;
			Если Не СохранитьСопоставление Тогда
...
Процедура УстановитьВидимостьЭлементовВкладкиМаркировка() 
	...
	Если НЕ МестныйКэш.ПараметрыСистемы.Клиент.УправляемоеПриложение Тогда
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "НадписьПричинаВывода").Видимость = РаботаСМаркировкой;
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "НадписьНомерКонтракта").Видимость = Ложь;
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "НадписьУчастникОборота").Видимость = РаботаСМаркировкой И ТекущийРаздел = "Продажа";
	КонецЕсли;
...
//
&НаКлиенте
Процедура УстановитьОграничениеТипаРеквизитов(УстановленныйРаздел)
	Перем ОписаниеТипа;
	
	ОписаниеТипов1С = МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("ТипыПолейФильтра");
	
	Если ОписаниеТипов1С.Свойство("ДоговорыКонтрагентов", ОписаниеТипа) Тогда
			
		ОписаниеТипа = ОписаниеТипа.ОписаниеТипа;
	...
	Если ОписаниеТипов1С.Свойство("Контрагент", ОписаниеТипа) Тогда
...
Процедура СбисВыполнитьРазбор(ПараметрыРазбора=Неопределено)
	...
	ОбновитьПоляФормыПоВложению(ЭлементТаблица.ТекущиеДанные.Вложение[0].Значение);
...
Процедура СформироватьРасхождениеНажатие(Команда) Экспорт
	...
	СтрокиОтмечены = ПолучитьОтмеченныеВложения();
...
Процедура ЗагрузитьВложения(РежимНоменклатуры)
	...
	Для Каждого СтрокаТаблДок Из ТаблицаДокументов Цикл
		...
		Если Не ВложениеСБИС.Параметры.РедактироватьСуществующий
			И	ЗначениеЗаполнено(ПодходящийДокумент) Тогда
...
Процедура ЗагрузитьВложенияСоздатьПерезаполнить(РежимДокументов, РежимНоменклатуры) Экспорт
	...
	ОбработчикРезультат = МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("СозданиеДокуметов_ОбработатьРезультат", ЭтаФорма, ПараметрыОбработки);
...
&НаКлиенте
Процедура ОбновитьТаблДок_ПозГК()
	...
	МодульОбъектаКлиент().СоставПакета_Обновить(СоставПакета, Изменения);

КонецПроцедуры

&НаКлиенте
Процедура ПерегенерироватьВложения()

	ОснованиеПакета = МодульОбъектаКлиент().СоставПакета_Получить(СоставПакета, "ОсновнойДокумент1С");
	ПараметрыГенерацииПакетов = Новый Структура("Кэш", Кэш);
	ПараметрыГенерацииПакетов.Вставить("ОснованиеПакета", ОснованиеПакета);

	Для каждого Вложение Из МодульОбъектаКлиент().СоставПакета_Получить(СоставПакета, "Вложение") Цикл
		// Если были изменения во вложении, необходимо записать черновик, иначе нет смысла
		Если МодульОбъектаКлиент().ВложениеСБИС_Получить(Вложение, "Модифицирован") Тогда
			...
			МодульОбъектаКлиент().СоставПакета_Обновить(СоставПакета, Новый Структура("Вложения", РезультатГенерации));
