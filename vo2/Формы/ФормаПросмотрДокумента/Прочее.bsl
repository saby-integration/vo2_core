...
Процедура ЗаполнитьТаблицуДокументов(СоставПакета) Экспорт
	...
	Для СчётчикВложений = 0 По ВложенияДляЗаполнения.Количество() - 1  Цикл
		...
		Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры_ОбратноеСопоставление")
			...
			Если ВложенияДляЗаполнения[СчётчикВложений].Параметры.Свойство("НоменклатураСопоставлена")
				И ВложенияДляЗаполнения[СчётчикВложений].Параметры.НоменклатураСопоставлена Тогда
				НоваяСтр.Статус = "Номенклатура сопоставлена"; 
			ИначеЕсли ВложенияДляЗаполнения[СчётчикВложений].Параметры.Свойство("НоменклатураСопоставлена")
				И НЕ ВложенияДляЗаполнения[СчётчикВложений].Параметры.НоменклатураСопоставлена Тогда
