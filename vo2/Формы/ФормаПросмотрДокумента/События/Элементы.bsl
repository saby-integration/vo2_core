...
Процедура СинонимДокументаПриИзменении(Элемент)
	...
	ИзмененияВложения		= Новый ФиксированнаяСтруктура("ПодразделИни, ЗапомнитьВыборДокумента", ТипДокумента, ЗапомнитьВыборДокумента);
...
&НаКлиенте
Процедура ЗапомнитьВыборДокументаПриИзменении(Элемент)
	
	ДанныеДокументовПоУмолчанию = Новый Структура("ЗапомнитьВыборДокумента", ЗапомнитьВыборДокумента);
	МодульОбъектаКлиент().ВложениеСБИС_Обновить(Вложение, ДанныеДокументовПоУмолчанию);
...
Процедура Документ1СВыборВариантаСписокНажатие(Элемент)
	...
	ВложениеСБИС				= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма,"ТаблицаДокументов").ТекущиеДанные.Вложение[0].Значение;
	ПараметрыПросмотр			= МодульОбъектаКлиент().ВложениеСБИС_Получить(ВложениеСБИС, "Просмотр", "ФормаПросмотрДокумента");
	ВариантыОбработкиДокументов = ПараметрыПросмотр.ВариантыОбработкиДокумента;
...
Процедура Документ1СОчистка(Элемент, СтандартнаяОбработка)
	...
	ВложениеСбис = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма,"ТаблицаДокументов").ТекущиеДанные.Вложение[0].Значение;
	ДанныеПросмотраДокумента	= МодульОбъектаКлиент().ВложениеСБИС_Получить(ВложениеСБИС, "Просмотр", "ФормаПросмотрДокумента");
...
Процедура Контрагент1СПриИзменении(Элемент)
	...
	СтрокаТаблицыДокументов	= МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма,"ТаблицаДокументов").ТекущиеДанные;
	
	Контрагент1СПриИзменении_Обработчик(СтрокаТаблицыДокументов, Контрагент1С)
	
КонецПроцедуры

&НаКлиенте
Процедура Контрагент1СПриИзменении_Обработчик(СтрокаТаблицыДокументов, НовыйКонтрагент1С)

	ВложениеСбис = СтрокаТаблицыДокументов.Вложение[0].Значение;

	СтруктураОбновить = Новый Структура;
	СтруктураОбновить.Вставить("Контрагент", НовыйКонтрагент1С);

	ДопПараметрыВложения = Новый ФиксированнаяСтруктура("СоставПакета", СоставПакета);
	МодульОбъектаКлиент().ВложениеСБИС_Обновить(ВложениеСбис, Новый Структура("Поля1С", СтруктураОбновить), ДопПараметрыВложения);
	
	ТаблицаДокументовУстановитьСтроку(СтрокаТаблицыДокументов, ВложениеСБИС);
	ОбновитьПоляФормыПоВложениюСБИС(ВложениеСБИС);
