...
Процедура ПослеОтправки(ВходящийКэш) Экспорт 
	...
	Иначе    // если отправилось без ошибок, закрываем окно просмотра
		...
		Если ЭтаФорма.Открыта() Тогда
			ЭтаФорма.Закрыть();
			МодульОбъектаКлиент().СбисСообщить("Документ успешно отправлен");
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ СопоставленияСохранены Тогда
...
Процедура ПередЗакрытием_ПослеДиалога(РезультатДиалога, ДопПараметры) Экспорт
	...
	Если РезультатДиалога = КодВозвратаДиалога.Да Тогда
		ЗаписатьСопоставлениеНоменклатуры_Служебный(Вложение, Новый Структура("СоставПакета", СоставПакета));
		Закрыть();
