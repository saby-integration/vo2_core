...
Процедура ЗагрузкаВ1С_Начало() Экспорт
	...
	Если МодульОбъектаКлиент().ПолучитьЗначениеФичи(Новый Структура("НазваниеФичи", "РасширенныйФункционалСопоставленияНоменклатуры")) Тогда
		ЗаписатьСопоставлениеНоменклатуры_Служебный(Вложение);
	КонецЕсли; 
...
Процедура ЗагрузкаВ1С_ВыбранРежимДокумента(Ответ, ДопПараметры) Экспорт
	...
	Если	ВыбранныйРежимЗагрузкиДокумента = 1 Тогда
		...
		Для Каждого ВложениеТчОтмечено Из ДопПараметры.Вложение Цикл
			...
			РедактируемыеПоляВложения	= МодульОбъектаКлиент().ВложениеСБИС_Получить(ВложениеТчОтмечено, "Поля1С", Новый Структура);
