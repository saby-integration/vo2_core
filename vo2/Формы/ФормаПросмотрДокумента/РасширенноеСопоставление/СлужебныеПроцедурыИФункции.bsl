...
Процедура ПослеРучнойОбработкиСопоставления(Результат, ДопПараметры = Неопределено) Экспорт
	...
	ИскомыеСтрокиНоменклатуры = СопоставлениеНоменклатуры.НайтиСтроки(ОтборНоменклатуры);
...
Процедура ОбновитьСтатусыСопоставления(ВложениеСБИС)
	...
	Для Каждого ПозицияНоменклатурыСБИС Из ВложениеСБИС.КлассыСопоставленияНоменклатур Цикл
		...
		НоменклатураВТабличнойЧасти = СопоставлениеНоменклатуры.НайтиСтроки(ОтборНоменклатуры);
			...
			Для Каждого СтрокаНоменклатуры Из НоменклатураВТабличнойЧасти Цикл  
				...
				Для Каждого Номенклатура1С ИЗ ПозицияНоменклатурыСбис.Номенклатура1С Цикл
					Если ЗначениеЗаполнено(Номенклатура1С.Ключ) 
						И ПозицияНоменклатурыСбис.НеобходимоСопоставление
						И НЕ Номенклатура1С.Значение.Количество() Тогда 
...
Функция СопоставленияСохраненыПередВыходом() 
	...
	Для Каждого СтрокаНоменклатуры Из СопоставлениеНоменклатуры Цикл
		Если СтрокаНоменклатуры.Статус = 2 Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ЗаписатьСопоставлениеНоменклатуры_Служебный(ВложениеСБИС, ДопПараметры)
	
	Если	НЕ МодульОбъектаКлиент().ИспользоватьНовоеСопоставлениеНоменклатуры()
		ИЛИ	НЕ ВложениеСБИС.Свойство("КлассыСопоставленияНоменклатур") Тогда
	...
	ПараметрыДляОбновления.Вставить("Контрагент",	ДопПараметры.СоставПакета.Контрагент);  
	ПараметрыДляОбновления.Вставить("Номенклатура", ВложениеСБИС.КлассыСопоставленияНоменклатур);
	...
	ВложениеСБИС.КлассыСопоставленияНоменклатур = ПараметрыДляОбновления.Номенклатура;
	...
	// Костыль чтобы не словить аварию на ОП. Проблема в ДБФ, времени быстро разобраться с этим - нет 
	// https://online.saby.ru/opendoc.html?guid=6c9b8551-df85-4b97-9edc-ee3fc1653952&client=3
	Для Каждого КлассСопоставления Из ВложениеСБИС.КлассыСопоставленияНоменклатур Цикл
		КлассСопоставления.НеобходимоСопоставление = Ложь;
	КонецЦикла;
	// -----------------------------------------------------------------------------------------------
	ОбновитьСтатусыСопоставления(ВложениеСБИС); 
