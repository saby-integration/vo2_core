...
Процедура ТипыОшибокВыбор(Элемент, Строка, СтандартнаяОбработка)
	...
	ИначеЕсли Найти(ИмяПоля, "РешениеОшибки") Тогда
		...
		Если Элемент.ТекущиеДанные.РешениеОшибки = "Решение ошибки" Тогда
			
			ЗапуститьПриложение(Элемент.ТекущиеДанные.СсылкаНаРешение);
			
		ИначеЕсли Элемент.ТекущиеДанные.РешениеОшибки = "Сохранить данные об ошибке" Тогда
			...
			Если ЗначениеЗаполнено(КаталогВыгрузкиФайла) Тогда
				JSONОшибки = МодульОбъектаКлиент().СбисИсключение_ОшибкаВJSON(Элемент.ТекущиеДанные.СтруктураОшибки[0].Значение);
