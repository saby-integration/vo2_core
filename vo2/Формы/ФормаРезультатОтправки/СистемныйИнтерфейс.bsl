...
Процедура СбисУстановитьФорму(ПараметрыОткрытия)
	Перем МассовоеДействие;
	
	Если МодульОбъектаКлиент().ЭтоТипСБИС(ПараметрыОткрытия, "МассоваяОперация") Тогда
		
		МассовоеДействие = ПараметрыОткрытия;
	
	ИначеЕсли Не ПараметрыОткрытия.Свойство("МассоваяОперация", МассовоеДействие) Тогда
		
		МассовоеДействие = Кэш.РезультатОтправки;
		
	КонецЕсли;
	
	Если МодульОбъектаКлиент().МассоваяОперация_Получить(МассовоеДействие, "Версия") = 2 Тогда
		
		РезультатОтправки = МодульОбъектаКлиент().МассоваяОперация_Выгрузить(МассовоеДействие);
		
	Иначе
		
		РезультатОтправки = МассовоеДействие;
