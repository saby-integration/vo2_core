...
Процедура СбисУстановитьФорму(ПараметрыОткрытия)
	...
	ЦенаВключаетНДС				= Цена * КоличествоСБИС	= СуммаБезНал + СуммаНДСВсего;
	
	Если ЦенаВключаетНДС Тогда
		НДССверху				= Цена * КоличествоСБИС = СуммаВсего;
	Иначе
		НДССверху				= Цена * КоличествоСБИС * КоэффициентПриведенияНДС(СтавкаНДС) = СуммаВсего;
	КонецЕсли;
