...
Процедура Сохранить(Команда)
	...
	Результат = Новый Структура("ИмяПрограммы,ТипПрограммы,ПутьКПрограмме,ПодписьНаСервере");
	ЗаполнитьЗначенияСвойств(Результат, Этаформа);
	ФормаЗакрытьУстановитьСтатусОк(Результат, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура СбисУстановитьФорму(Отказ)
	
	ЛокальныйКэш = МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш();
	Настройки.Очистить();
	сбисСписокНастроек(Настройки);
	ЭлементНаименование = МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "Наименование");
	...
	КонецЦикла;
	ЗаполнитьЗначенияСвойств(Этаформа, ЛокальныйКэш.Парам.НастройкиКриптографии, "ИмяПрограммы,ТипПрограммы,ПутьКПрограмме,ПодписьНаСервере");
