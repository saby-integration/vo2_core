...
Процедура ОбработчикТабличногоДокумента(Кэш, Контекст, Параметр) Экспорт
	...
	Для Каждого Стр Из ТабЧастьДокумента Цикл  
		...
		РасхождениеДебет = 0; 
		РасхождениеКредит = 0;  
		Если Контекст.ВложениеКонтрагента.СтруктураФайла.Файл.Документ.ТаблДок.Свойство("СтрТабл") Тогда
			...
			Для Каждого СтрокаКонтрагента из ТаблДокКонтрагента Цикл
				Если    СтрокаКонтрагента.Свойство("Примечание")
					И	СтрокаКонтрагента.Примечание = НоваяСтрока.Примечание Тогда 
...
Процедура ПропатчитьСтруктуруВложения(ВложениеСБИС, ДопПараметры) Экспорт
	...
	УдаляемыеУзлы.Добавить("Файл.Документ.Получатель.Адрес.ФиасИд");
	УдаляемыеУзлы.Добавить("Файл.Документ.Отправитель.Адрес.ФиасИд");
