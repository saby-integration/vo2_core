...
Функция СформироватьДокументДляГенератора(ПараметрыФормированияВходящие, Кэш) Экспорт
	...
	Если Файл_Шаблон.ФайлДокумент_ОтправительГрузоотправительОнЖе(ТекДокумент) Тогда
...
Процедура ПроверитьПривестиСтороныПоИмени(ТекДокумент, ПутьСтороны)  
	...
	Сторона = ТекДокумент[ПутьСтороны];  
	
	Если Сторона.Свойство("ОнЖе") Тогда
		
		Сторона.Удалить("ОнЖе");
		
	КонецЕсли;
	
	Если НЕ Сторона.Свойство("СвФЛ") Тогда 
		
		Возврат;     
		
	КонецЕсли;
	
	ЭтоФизЛицо = СторонаФизическоеЛицо(Сторона);
	
	Если НЕ ЭтоФизЛицо Тогда 
		
		СвИП = Новый Структура; 
		
		ФункцииДокументов.сбисСкопироватьСтруктуруНаКлиенте(СвИП, Сторона.СвФЛ);
		
		Сторона.Вставить("СвИП", СвИП);   
		Сторона.Удалить("СвФЛ");
...
КонецПроцедуры	          
 ...
 &НаКлиенте
 Функция СторонаФизическоеЛицо(Сторона) 
	 
	Перем ДопПараметрыСтороны;
	 
	Если НЕ Сторона.Свойство("Параметр", ДопПараметрыСтороны) Тогда 
	...
	Для Каждого Параметр Из ДопПараметрыСтороны Цикл 
		...
		Если Параметр.Имя = "ЧастноеЛицо" 
			И Параметр.Значение = "Да" Тогда
