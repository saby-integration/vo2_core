...
Функция ПолучитьДанныеИзДокумента1С(Кэш,Контекст) Экспорт
	Попытка	
		...
		Если Не Док.Файл.Документ.Свойство(ПолучательРоль) Тогда
			МодульОбъектаКлиент().ВызватьСбисИсключение(721, "Файл_ОтчКмт_3_01.ПолучитьШапкуИзДокумента1С",,,"Не удалось определить ИНН получателя документа " + Строка(Контекст.Документ));
		КонецЕсли;
		Если Не Док.Файл.Документ.Свойство(ОтправительРоль) Тогда
			МодульОбъектаКлиент().ВызватьСбисИсключение(721, "Файл_ОтчКмт_3_01.ПолучитьШапкуИзДокумента1С",,,"Не удалось определить ИНН отправителя документа " + Строка(Контекст.Документ));
		КонецЕсли;
	...
	Исключение
		ИсключениеФормирования = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "Файл_ОтчКмт_3_01.ПолучитьШапкуИзДокумента1С");
...
Процедура ПропатчитьСтруктуруВложения(ВложениеСБИС, ДопПараметры) Экспорт
	...
	УдаляемыеУзлы.Добавить("Файл.Документ.Комиссионер.Адрес.АдрРФ.НаимРегион");
	УдаляемыеУзлы.Добавить("Файл.Документ.Комитент.Адрес.АдрРФ.НаимРегион");

	УдаляемыеУзлы.Добавить("Файл.Документ.Комиссионер.Адрес.АдрИно.НаимСтран");
	УдаляемыеУзлы.Добавить("Файл.Документ.Комитент.Адрес.АдрИно.НаимСтран");
	
	УдаляемыеУзлы.Добавить("Файл.Документ.Комиссионер.Адрес.ФиасИд");
	УдаляемыеУзлы.Добавить("Файл.Документ.Комитент.Адрес.ФиасИд");
