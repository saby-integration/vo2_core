...
Функция ПолучитьДанныеИзДокумента1С(Кэш,Контекст) Экспорт
	...
	Попытка	
		...
		// Сделать проверку по фиче Обратного сопоставления 
		Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры_ОбратноеСопоставление") Тогда
			Контекст.Вставить("ИспользоватьСопоставлениеНоменклатуры", Кэш.ОбщиеФункции.РассчитатьЗначение("ИспользоватьСопоставлениеНоменклатуры", Контекст.ФайлДанные) = Истина);
		Иначе
			Контекст.Вставить("ИспользоватьСопоставлениеНоменклатуры", Ложь);
		КонецЕсли;
		...
		ПараметрыВложения = Новый Структура("ФормаСопоставленияНоменклатуры, ИспользоватьСопоставлениеНоменклатуры", "ФормаОбратногоСопоставления", Контекст.ИспользоватьСопоставлениеНоменклатуры);
		Вложение.Вставить("Параметры", ПараметрыВложения);
		...
		Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры_ОбратноеСопоставление")
			...
			Иначе
				Сообщить("Основание не заполнено");
			КонецЕсли;
...
Процедура ДобавитьМПараметр(Кэш, Контекст, Получатель) Экспорт
	...
	Если Контекст.Свойство("мПараметр") Тогда
		...
		Для каждого Параметр Из СоответствиеДобавленных Цикл
			Получатель.Параметр.Добавить(Параметр.Значение);
		КонецЦикла;
