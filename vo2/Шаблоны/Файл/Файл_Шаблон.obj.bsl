...
Функция РазложитьДанныеПоКонтуру(РезультатПоКонтуру, Документ, ТаблДок)
	...
	Если РезультатПоКонтуру.ОснованияКонтура.Количество() Тогда
		...
		Для Каждого Парам Из Документ.Параметр Цикл
			...
			Если Парам.Имя = "номер_акта" И Не ЗначениеЗаполнено(Парам.Значение) Тогда
				
				Парам.Значение = Основание.НомерДокумента;	
				
			КонецЕсли;
			
			Если Парам.Имя = "дата_акта" И Не ЗначениеЗаполнено(Парам.Значение) Тогда
				
				Парам.Значение = Формат(Основание.ДатаДокумента, "ДЛФ=Д");	
...
Функция СформироватьПакетКорреспонденции(Кэш=Неопределено, ПараметрыФормированияПакета=Неопределено, Отказ=Неопределено) Экспорт
	...
	Для Каждого Строка Из СписокФайлов Цикл
		...
		Если	Не ЗначениеЗаполнено(СбисВложение)
			...
			Файл = Новый Файл(Строка.ПолноеИмяФайла);
			ВложениеДляДобавления = МодульОбъектаКлиент().НовыйВложениеСБИС(Новый Структура("Файл", Файл));
			КорреспонденцияКОтправке.Добавить(ВложениеДляДобавления);
			Продолжить;
		КонецЕсли;
		...
		// TODO переделать формирование XML на полноценное вложение, а не с прикрепленным файлом
		...
		ВложениеСБИС = МодульОбъектаКлиент().НовыйВложениеСБИС(XML_Описание);
		КорреспонденцияКОтправке.Добавить(ВложениеСБИС);
...
#Область include_core2_vo2_Файл_Шаблон_3_01
#КонецОбласти
