...
Функция ПолучитьДанныеИзДокумента1С(Кэш,Контекст) Экспорт
	...
	Попытка	
		...
		Если Контекст.ИспользоватьСопоставлениеНоменклатуры 
			И МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры_ОбратноеСопоставление") Тогда	
			...
			НаборСопоставлений = СформироватьНаборСопоставленийНоменклатурыПоТабличнойЧасти(Вложение, Кэш);	
			Вложение.Вставить("КлассыСопоставленияНоменклатур", НаборСопоставлений);	
			НоменклатураСопоставлена = МодульОбъектаКлиент().СопоставлениеНоменклатуры_ВсеНоменклатурыСопоставлены(НаборСопоставлений);
