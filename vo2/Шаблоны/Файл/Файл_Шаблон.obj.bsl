...
Функция ПолучитьДанныеИзДокумента1С(Кэш, Контекст, ДопПараметры = Неопределено) Экспорт
	...
	Попытка	
		...
		Если 	Контекст.ФайлДанные.Свойство("мСвязанноеВложение") И ЗначениеЗаполнено(Контекст.ФайлДанные.мСвязанноеВложение)
			...
			ДопПараметры.Вставить("ОсновныеФормыВФК", ПараметрыВложения.НаборМодулейДляФормата);
	
			ПолучитьДанныеИзДокумента1С_мСвязанноеВложение(Кэш, Контекст, ДопПараметры);
		КонецЕсли;
...
Процедура ПолучитьДанныеИзДокумента1С_мСвязанноеВложение(Кэш, Контекст, ДопПараметры) Экспорт
	...
	Для Каждого СвязанноеВложение Из Контекст.ФайлДанные.мСвязанноеВложение Цикл
		...
		//СвязанноеВложение_Ини		= МодульОбъектаКлиент().ИниПоПараметрам(СвязанноеВложение_ИмяИни);
