...
Функция ПолучитьДанныеИзДокумента1С(Кэш, Контекст, ДопПараметры = Неопределено) Экспорт
	...
	Попытка	
		...
		Если Контекст.ФайлДанные.Свойство("мСторона") Тогда
			...
			Если НЕ Контекст.Свойство("ЗаполнятьГрузотпрГрузполуч")
				 ИЛИ (Контекст.Свойство("ЗаполнятьГрузотпрГрузполуч")
				 И Контекст.ЗаполнятьГрузотпрГрузполуч = Истина) Тогда
					
				Если Контекст.ФайлДанные.мСторона.Свойство("Грузоотправитель")
					 И НЕ Стороны.Свойство("Грузоотправитель") Тогда
					...
					Если Стороны.Свойство(КопироватьГрузоотправителяС) Тогда
						
						Стороны.Вставить("Грузоотправитель", МодульОбъектаКлиент().СторонаSABY_Скопировать(Стороны[КопироватьГрузоотправителяС]));		
						
					КонецЕсли;	
					
				КонецЕсли; 
				
				Если Контекст.ФайлДанные.мСторона.Свойство("Грузополучатель") И НЕ Стороны.Свойство("Грузополучатель") Тогда
					...
					Если Стороны.Свойство(КопироватьГрузополучателяС) Тогда
						
						Стороны.Вставить("Грузополучатель", МодульОбъектаКлиент().СторонаSABY_Скопировать(Стороны[КопироватьГрузополучателяС]));
			...
			Для Каждого СторонаДанные Из Стороны Цикл
				...
				Док.Файл.Документ.Вставить(СторонаДанные.Ключ, МодульОбъектаКлиент().СторонаSABY_Выгрузить(СторонаДанные.Значение, "3.01"))
...
Функция РазложитьДанныеПоКонтуру(РезультатПоКонтуру, Документ, ТаблДок)
	...
	Для Каждого Позиция Из ТаблДок Цикл
		...
		Хар = Строка(ФункцииМодуляОбъекта.СбисПолучитьСвойствоОбъекта(Позиция, "Характеристика1С"));
	...
	// ОбработкаGLN
	Документ.Отправитель.GLN = РезультатПоКонтуру.ДанныеGLN.GLN_Отправитель;
	Документ.Получатель.GLN = РезультатПоКонтуру.ДанныеGLN.GLN_Получатель;
	Документ.Грузоотправитель.GLN = РезультатПоКонтуру.ДанныеGLN.GLN_Грузоотправитель;
	Документ.Грузополучатель.GLN = РезультатПоКонтуру.ДанныеGLN.GLN_Грузополучатель;
	...
	Если РезультатПоКонтуру.ОснованияКонтура.Количество() Тогда
		...
		ПодготовкаОснования.Вставить("Название", "Заказ");
		...
		Документ.Основание.Добавить(ПодготовкаОснования);                            
	
		Для Каждого Парам Из Документ.Параметр Цикл
			
			Если Парам.Имя = "Срок" И Парам.Значение = Неопределено Тогда
				
				Парам.Значение = Формат(Основание.ДатаПоставки, "ДЛФ=Д");	
				
			КонецЕсли;
			
			Если Парам.Имя = "номер_акта" И Парам.Значение = Неопределено Тогда
				
				Парам.Значение = Основание.НомерДокумента;	
	...
	Для Каждого Параметр Из Контекст.ФайлДанные.мТаблДетал Цикл
		...
		Если ТипЗнч(ТабЧастьДокумента) = Тип("Массив") Тогда
			...
			Для Каждого Стр Из ТабЧастьДокумента Цикл
				...
				ДобавитьМПараметр(Кэш, Стр, НоваяСтрока);
...
// Процедура - добавляет параметры в обработку
//
// Параметры:
//  Кэш			 - ЛокальныйКэш	 - Структура параметров обработки
//  Контекст	 - Струкутра	 - контекст расчета документа
//  Получатель	 - Сторона		 - структура стороны
//
&НаКлиенте
Процедура ДобавитьМПараметр(Кэш, Контекст, Получатель) Экспорт

	Если Контекст.Свойство("мПараметр") Тогда
		
		СоответствиеДобавленных = Новый Соответствие;
		ПорядокДобавленных		= Новый Массив;

		Если Получатель.Свойство("Параметр") Тогда
			Для Каждого Параметр Из Получатель.Параметр Цикл
				
				СоответствиеДобавленных.Вставить(Параметр.Имя, Параметр);
				НомерДобавлен = ПорядокДобавленных.Найти(Параметр.Имя);
				Если Не НомерДобавлен = Неопределено Тогда
					
					ПорядокДобавленных.Удалить(НомерДобавлен);
					
				КонецЕсли;
				ПорядокДобавленных.Добавить(Параметр.Имя);
		...
		Для Каждого Элемент Из Контекст.мПараметр Цикл
			...
			СоответствиеДобавленных.Вставить(Элемент.Ключ, Параметр);
			НомерДобавлен = ПорядокДобавленных.Найти(Элемент.Ключ);
			Если Не НомерДобавлен = Неопределено Тогда
				
				ПорядокДобавленных.Удалить(НомерДобавлен);
				
			КонецЕсли;
			ПорядокДобавленных.Добавить(Элемент.Ключ);
			
		КонецЦикла;
		
		Получатель.Вставить("Параметр", Новый Массив);
		
		Для каждого КлючПоПорядку Из ПорядокДобавленных Цикл
			
			Получатель.Параметр.Добавить(СоответствиеДобавленных.Получить(КлючПоПорядку));
			
		КонецЦикла;
		
	КонецЕсли;

КонецПроцедуры // ДобавитьМПараметр()
...
Функция СформироватьРасхождениеНаСервере(СтруктураФайлаКонтрагента, СтруктураФайлаНаша) Экспорт
	...
	Если СтруктураФайлаНаша.Файл.Документ.ТаблДок.СтрТабл.Количество()
		И СтруктураФайлаНаша.Файл.Документ.ТаблДок.СтрТабл[0].Свойство("ПорНомерВФайлеКонтрагента") Тогда
	...
	ПорядокСтрокТЧКонтрагента = Новый Массив;
	Для Каждого СтрТабл Из СтруктураФайлаКонтрагента.Файл.Документ.ТаблДок.СтрТабл Цикл
		Если СтрТабл.Свойство("Идентификатор") и ЗначениеЗаполнено(СтрТабл.Идентификатор) И НЕ ЕстьПорНомерВФайлеКонтрагента Тогда
			...
			ИдентификаторСтроки = СтрТабл.Идентификатор;
		...
		Иначе
			...
			ТабЧастьКонтрагента.Вставить(ИдентификаторСтроки, СтрТабл);	
		КонецЕсли;
		
		ПорядокСтрокТЧКонтрагента.Добавить(ИдентификаторСтроки);
		
	КонецЦикла;
	
	ЛишниеАтрибутыСтроки = Новый Массив;
	// Эти поля могли добавлиться в структуру файла при загрузке
	// или в момент формирования структуры нашей ТЧ, при выгрузке расхождения они не нужны
	ЛишниеАтрибутыСтроки.Добавить("Номенклатура");
	ЛишниеАтрибутыСтроки.Добавить("ХарактеристикаНоменклатуры");
	ЛишниеАтрибутыСтроки.Добавить("ПорНомерВФайлеКонтрагента");
	ЛишниеАтрибутыСтроки.Добавить("ЕдИзм1С");
	ЛишниеАтрибутыСтроки.Добавить("ИмяТЧ");
	ЛишниеАтрибутыСтроки.Добавить("НомерСтрокиТЧ");
	ЛишниеАтрибутыСтроки.Добавить("НеЗагружать");
	...
	Для Каждого ИдентификаторСтроки Из ПорядокСтрокТЧКонтрагента Цикл
		
		СтрТабл = ТабЧастьКонтрагента.Получить(ИдентификаторСтроки);
		...
		НоваяСтрока = Новый Структура();
		Если СтрТабл.Свойство("КодПокупателя", КодПокупателя) Тогда
		...
		СтрТабл.Свойство("Идентификатор", Идентификатор);  
		
		
		Если ЕстьПорНомерВФайлеКонтрагента Тогда
			Отбор = Новый Структура("ПорНомерВФайлеКонтрагента", СтрТабл.ПорНомер);
		Иначе
			Если	ЗначениеЗаполнено(КодПокупателя) И КодПокупателя <> "-"
				И	ТабЧастьНаша.Колонки.Найти("КодПокупателя") <> Неопределено Тогда
				
				Отбор = Новый Структура("КодПокупателя", КодПокупателя);
				
			ИначеЕсли	ЗначениеЗаполнено(Номенклатура) или ЗначениеЗаполнено(ХарактеристикаНоменклатуры)
					И	ТабЧастьНаша.Колонки.Найти("Номенклатура") <> Неопределено Тогда 
		...
		НайденныеСтроки = ?(ТабЧастьНаша.Колонки.Количество() > 1, ТабЧастьНаша.НайтиСтроки(Отбор), Новый Массив);
		Если НайденныеСтроки.Количество()>0 Тогда 
			//НайденнаяСтрока = НайденныеСтроки[0];
			Для Каждого НайденнаяСтрока из НайденныеСтроки Цикл  
				Если НоваяСтрока.Свойство("Цена") и не НоваяСтрока.Цена = НайденнаяСтрока.Цена Тогда
					Продолжить;
				КонецЕсли;
				
				НайденнаяСтрока.ЕстьВФайлеКонтрагента = Истина;
				
				Если не НоваяСтрока.Свойство("Идентификатор") Тогда
					Для Каждого Поле Из СтруктураФайлаНаша.Файл.Документ.ТаблДок.СтрТабл[0] Цикл
						Если ЛишниеАтрибутыСтроки.Найти(Поле.Ключ) = Неопределено Тогда
							НоваяСтрока.Вставить(Поле.Ключ, НайденнаяСтрока[Поле.Ключ]);
						КонецЕсли;
					КонецЦикла;     
				Иначе   
					Попытка
						НоваяСтрока.Кол_во		= Формат((Число(НоваяСтрока.Кол_во) + НайденнаяСтрока.Кол_во), "ЧЦ=17; ЧДЦ=3; ЧРД=.; ЧГ=0; ЧН=0.000");
 						НоваяСтрока.Сумма 		= Формат((Число(НоваяСтрока.Сумма) + НайденнаяСтрока.Сумма), "ЧЦ=17; ЧДЦ=2; ЧРД=.; ЧГ=0; ЧН=0.00");   
						НоваяСтрока.СуммаБезНал = Формат((Число(НоваяСтрока.СуммаБезНал) + НайденнаяСтрока.СуммаБезНал), "ЧЦ=17; ЧДЦ=2; ЧРД=.; ЧГ=0; ЧН=0.00");
						НоваяСтрока.НДС.Сумма 	= Формат((Число(НоваяСтрока.НДС.Сумма) + НайденнаяСтрока.НДС.Сумма), "ЧЦ=17; ЧДЦ=2; ЧРД=.; ЧГ=0; ЧН=0.00");
					Исключение
					КонецПопытки;
				КонецЕсли;
...
//KES МОТП ИСМП--->
&НаКлиенте
Функция ОбработатьСложныйУзел(СложныйУзел,СтараяСтрока,НоваяСтрока,Кэш) Экспорт
...
Функция ПолучитьКартуСравнения() Экспорт
	...
	КлючиПоиска = "Номенклатура, Название";
	Карта.ТаблДок = Новый Структура("СтрТабл, ИтогТабл");
	Карта.ТаблДок.СтрТабл = Новый Структура("КлючПоиска, КлючСравнения");
	
	//УсловиеНоменклатура	= Новый Структура("Номенклатура",	"Есть,Заполнено");
	УсловиеНаименование	= Новый Структура("Название",		"");
	//УсловияПоиска = Новый Структура("Или", Новый Массив);
	//УсловияПоиска.Или.Добавить(УсловиеНоменклатура);
	//УсловияПоиска.Или.Добавить(УсловиеНаименование);
	// В качестве ключа задаём 
	Карта.ТаблДок.СтрТабл.КлючПоиска		= УсловиеНаименование;
