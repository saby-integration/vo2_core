...
Функция ПолучитьДанныеИзДокумента1С(Кэш, Контекст, ДопПараметры = Неопределено) Экспорт
	...
	Попытка	
		...
		Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры") Тогда    
			...
			ТипОснования = "Заказ";                            
            Основание = НайтиОснованиеПоТипу(Док, ТипОснования);
		...
		ОбработчикВФК = МодульОбъектаКлиент().ПользовательскаяТочкаВхода("сбисПослеФормированияДокумента", ОсновныеФормыВФК);
		Если ОбработчикВФК <> Неопределено Тогда
			Аргумент = Новый Массив;
			Аргумент.Добавить(Док);
			Аргумент.Добавить(Кэш);
			Аргумент.Добавить(Контекст);
			МодульОбъектаКлиент().ВызватьПользовательскуюТочкуВхода(ОбработчикВФК, Аргумент);
			Вложение.СтруктураДокумента = Аргумент[0]; // на случай, если Док поменялся в функции сбисПослеФормированияДокумента
		КонецЕсли;
		Возврат Истина;
...
Процедура ПолучитьДанныеИзДокумента1С_мСвязанноеВложение(Кэш, Контекст, ДопПараметры) Экспорт
	...
	ОбработчикВФК = МодульОбъектаКлиент().ПользовательскаяТочкаВхода("ОбработчикСвязанныхВложений", ОсновныеФормыВФК, ДопПараметры);
	Если ОбработчикВФК = Неопределено Тогда
		...
		КонецЦикла;
	Иначе
		ДопПараметрыОбработчикаВФК = Новый Структура("ТипыСвязанныхВложений", ТипыСвязанныхВложений);
		Аргумент = Новый Массив;
		Аргумент.Добавить(ПакетыСвязанныхВложений);
		Аргумент.Добавить(Контекст);
		Аргумент.Добавить(Кэш);
		Аргумент.Добавить(ДопПараметрыОбработчикаВФК);
		МодульОбъектаКлиент().ВызватьПользовательскуюТочкуВхода(ОбработчикВФК, Аргумент);
	КонецЕсли;
