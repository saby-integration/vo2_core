...
Функция ПолучитьДанныеИзДокумента1С(Кэш, Контекст) Экспорт
	Перем Файл_Формат, Файл_ВерсияФормата, ВходящийКонтекстРасчета;
	...
	Попытка	
		...
		Для Каждого Параметр Из Контекст.ФайлДанные.мТаблДок Цикл
			...
			Если Перезаполнить5А Тогда
				ДокПодтвПоИни = Неопределено;
				Контекст.ФайлДанные.мОснование.Свойство("ДокПодтвОтгр", ДокПодтвПоИни);
...
Процедура ДополнитьДаннымиДокПодтвОтгр(Кэш, Контекст, ВременныйКонтекст, ДопПараметры) Экспорт
	...
	Для Каждого Элемент Из ВременныйКонтекст.ДокументДанные.мФайл Цикл
		Если Элемент.Значение.Свойство("мТаблДок")
			...
			Основание.Вставить("Основание_Дата",		Документ_Дата);
			Основание.Вставить("Основание_Номер",		Документ_Номер);
