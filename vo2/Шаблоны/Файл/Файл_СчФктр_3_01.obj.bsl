	...
	Попытка	
		...
		Для Каждого Параметр Из Контекст.ФайлДанные.мТаблДок Цикл
			...
			Для Каждого ДокОсн Из ТабЧасть Цикл
				...
				Если Перезаполнить5А Тогда
					ДопПараметры = Новый Структура("ДокПодтвПоИни, Перезаполнить5А", ДокПодтвПоИни, Перезаполнить5А);
					ДополнитьДаннымиДокПодтвОтгр(Кэш, Контекст, ВременныйКонтекст, ДопПараметры);
					Перезаполнить5А = ДопПараметры.Перезаполнить5А;
				КонецЕсли;
