...
Функция ПолучитьДанныеИзДокумента1С(Кэш, Контекст, ДопПараметры = Неопределено) Экспорт
	...
	Попытка	
		...
		Для Каждого Параметр Из Контекст.ФайлДанные.мТаблДок Цикл
			...
			Для Каждого ДокОсн Из ТабЧасть Цикл
				...
				Если Не ДокОсн.Свойство("ТаблДок_ДокументОснование") Тогда
					
					// Это не ТЧ  оснований для забора номенклатуры
					Прервать;
				...
				Если Не Кэш.ини.Свойство(ИниДокумента) Тогда
					ОшибкаПодбораИни = МодульОбъектаКлиент().НовыйСбисИсключение(747101,"Файл_СчФктр_3_01.ПолучитьДанныеИзДокумента1С",,, 
					"Не установлен файл настроек для формирования документов данного типа. Документ:" + строка(ДокОснование));
					Контекст.СоставПакета.Вставить("Ошибка", ОшибкаПодбораИни);
					Возврат Ложь;
				КонецЕсли;
				...
				Если	Не Контекст.ФайлДанные.мСторона.Свойство("ОтпускПроизвёл") // берем ОтпускПроизвёл с основания СФ
					Или	Не ЗначениеЗаполнено(Контекст.ФайлДанные.мСторона.ОтпускПроизвёл.Сторона) Тогда
					...
					Если ВременныйКонтекст.ДокументДанные.Свойство("мФайл")	Тогда
						...
						Для Каждого Элемент Из ВременныйКонтекст.ДокументДанные.мФайл Цикл
							Если Элемент.Значение.Свойство("мТаблДок") Тогда
								Если (Элемент.Значение.мТаблДок.Свойство("Товары") и Элемент.Значение.мТаблДок.Товары.Количество()>0) или (УказанТипНоменклатуры и КолТоваров>0) Тогда
									Если Элемент.Значение.Свойство("мСторона") и Элемент.Значение.мСторона.Свойство("ОтпускПроизвёл") Тогда
										Контекст.ФайлДанные.мСторона.Вставить("ОтпускПроизвёл", Новый Структура);
										Кэш.ОбщиеФункции.сбисСкопироватьСтруктуруНаКлиенте(Контекст.ФайлДанные.мСторона.ОтпускПроизвёл,Элемент.Значение.мСторона.ОтпускПроизвёл);
										НайденоОснованиеСоСтороной = Истина;
									КонецЕсли;
									Если НайденоОснованиеСоСтороной Тогда    // прерываем, когда заполнили данные с какого-либо основания
										Прервать;
		...
		Если Контекст.ФайлДанные.Свойство("мТаблДок") И Контекст.ФайлДанные.мТаблДок.Свойство("ИсходныеНомераСтрок") Тогда
			МассивИсходныхНомеровСтрок = Контекст.ФайлДанные.мТаблДок.ИсходныеНомераСтрок;
			Если	МассивИсходныхНомеровСтрок.Количество() > 0
				...
				Для ИндСтр = 0 По Контекст.ТаблДок.СтрТабл.ВГраница() Цикл
					...
					Иначе
						Контекст.ТаблДок.СтрТабл[ИндСтр].Вставить("ПорНомТовВСЧФ", Формат(МассивИсходныхНомеровСтрок[ИндСтр].ТаблДок_НомерСтроки, "ЧГ=0"));
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	...
	Исключение
		ИсключениеФормирования = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), НазваниеКратко() + ".ПолучитьДанныеИзДокумента1С");
...
Функция РазложитьДанныеПоКонтуру(РезультатПоКонтуру, Документ, ТаблДок)
	...
	//ОбработкаGLN
	Если Документ.мСторона.Отправитель.Свойство("Сторона_GLN") Тогда
		Документ.мСторона.Отправитель.Сторона_GLN = РезультатПоКонтуру.ДанныеGLN.GLN_Отправитель;
	КонецЕсли;
	
	Если Документ.мСторона.Получатель.Свойство("Сторона_GLN") Тогда
		Документ.мСторона.Получатель.Сторона_GLN = РезультатПоКонтуру.ДанныеGLN.GLN_Получатель;
	КонецЕсли;	
	
	Если Документ.мСторона.Грузополучатель.Свойство("Сторона_GLN") Тогда
...
Функция ИсточникДанных_ПолучитьДанныеСтороныПоИмени(Кэш, Вложение, ИмяСтороны) Экспорт
	...
	Перем СвФлЮл, лОГРНИП, лДатаОГРН, ПараметрДатаОГРН;
	...
	Если ДанныеСтороны.Свойство("Параметр")
		И ЗначениеЗаполнено(ДанныеСтороны.Параметр) Тогда
	
		ПараметрДатаОГРН = МодульОбъектаКлиент().НайтиВМассивеСтруктурПоКлючу(ДанныеСтороны.Параметр,
																			"ДатаОГРН",
																			"Имя");
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрДатаОГРН)
		И ЗначениеЗаполнено(ПараметрДатаОГРН.Значение) Тогда
		 
		СвФлЮл.ОГРН = СвФлЮл.ОГРН + " " + ПараметрДатаОГРН.Значение;
		СвФлЮл.Вставить("ДатаОГРН", ПараметрДатаОГРН.Значение);
