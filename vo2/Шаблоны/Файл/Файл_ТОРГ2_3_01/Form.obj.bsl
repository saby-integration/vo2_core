...
Функция СформироватьРасхождение(ДанныеВложений, Кэш) Экспорт 
	...
	Если НЕ МодульОбъектаКлиент().ПараметрSaby("ИспользоватьГенератор") Тогда
	
		ИсключениеСбис = МодульОбъектаКлиент().НовыйСбисИсключение(, 
																	"Файл_ТОРГ2_3_01.СформироватьРасхождение",
																	715201,
																	"Ошибка формирования документа",
																	"Использование генератора ФЭД отключено");
		ВызватьИсключение МодульОбъектаКлиент().СбисИсключение_ВСтроку(ИсключениеСбис);
