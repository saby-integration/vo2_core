...
Функция НовыйСбисИсключение(БазоваяОшибка=Неопределено,	СбисИмяКоманды,
	...
	// Спилить это в 4100
	Если ТипЗнч(БазоваяОшибка) = Тип("Структура") И БазоваяОшибка.Свойство("ДопПоля") Тогда         
		 СбисИсключение.Вставить("ДопПоля", БазоваяОшибка.ДопПоля);
	КонецЕсли;
...
Функция СбисИсключение_ИнформацияОбОшибке(БазоваяОшибка)
	...
	Попытка
		Если ТипЗнч(БазоваяОшибка.Описание) = Тип("Строка")
			...
			Возврат СбисИсключение_ИзСтроки(БазоваяОшибка.Описание);
		КонецЕсли;
	Исключение
