...
Функция НовыйСбисИсключение(БазоваяОшибка=Неопределено,	СбисИмяКоманды = Неопределено,
	...
	Если Не СбисОсновнаяОшибка.Свойство("stack", СбисСтек) Тогда
		...
		СбисИсключение_ДобавитьВСтек(СбисИсключение, СбисСтек, СбисОсновнаяОшибка);
		
	КонецЕсли;
	СбисИсключение.Вставить("stack", СбисСтек);
	
	Если СбисИмяКоманды = Неопределено Тогда
		
		 // Иначе это перегенерация без изменения стека
		 Если СбисСтек.Количество() Тогда
			 
		 	ЗаполнитьЗначенияСвойств(СбисСтек[0], СбисИсключение);
			
		КонецЕсли;
		
	Иначе
		
		// Если новая команда, то просто добавить в стек.
		СбисИсключение_ДобавитьВСтек(СбисИсключение, СбисСтек, СбисИсключение);
...
Функция ПерегенерироватьСбисИсключение_ПрочитатьДокумент(СбисИсключение, ДопПараметры = Неопределено) Экспорт
	...
	ИсключениеРезультат = СбисИсключение_Пустое();
	ЗаполнитьЗначенияСвойств(ИсключениеРезультат, СбисИсключение);
	...
	ИсключениеРезультат.ext_code	= РасширениеКод;
	ИсключениеРезультат.message		= СбисИсключение_СообщениеПоДополнительномуКоду(ИсключениеРезультат.code, ИсключениеРезультат.ext_code);
	
	Если	Не ИсключениеРезультат.code		= СбисИсключение.code
		Или	Не ИсключениеРезультат.ext_code = СбисИсключение.ext_code Тогда
...
Функция СбисИсключение_Пустое()
	...
	Возврат Новый Структура("code, message, details, methodName, dump, help, base, data, ext_code", 100);
...
Процедура СбисИсключение_ДобавитьВСтек(СбисИсключение, СбисСтек = Неопределено, ИсключениеДобавить)
	Перем СтекДобавить;
	
	Если СбисСтек = Неопределено Тогда
...
Функция СбисИсключение_СообщениеПоДополнительномуКоду(ОсновнойКодОшибки, ДополнительныйКодОшибки) Экспорт
	...
	Если ОсновнойКодОшибки = 779 Тогда
		
		Если ДополнительныйКодОшибки = "001" Тогда
			
			Возврат "Не удалось прочитать документ. Возможно, что он недоступен для Вашего пользователя, или был удален.";
			
		ИначеЕсли ДополнительныйКодОшибки = "002" Тогда
...
Функция СбисИсключение_Структура(БазоваяОшибка)
	...
	Если ЭтоТипСбис(БазоваяОшибка, "SbisWarning") Тогда
		...
		СбисОсновнаяОшибка.data.Удалить("classid");
		Если	СбисОсновнаяОшибка.code = 100
			И	БазоваяОшибка.Свойство("details")
			И	Лев(БазоваяОшибка.details, "1") = "{" Тогда
			
			// Не удалось определить ошибку по ClsID БЛ, проверить наличие запакованного ExtException.
			ЗапакованнаяОшибка = СбисПрочитатьJSON(БазоваяОшибка.details);
...
Функция СбисИсключение_Соответствие(БазоваяОшибка)
	...
	Для Каждого КлючОшибки Из СбисОсновнаяОшибка Цикл
		СбисОсновнаяОшибка[КлючОшибки.Ключ] = БазоваяОшибка[КлючОшибки.Ключ];	
	КонецЦикла;	
	
	Если СбисОсновнаяОшибка.details = Неопределено Тогда
		СбисОсновнаяОшибка.Вставить("details",БазоваяОшибка["detail"]);	
	КонецЕсли;
