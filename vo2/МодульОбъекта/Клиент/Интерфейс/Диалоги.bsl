...
Процедура СообщитьСбисИсключение(ИсключениеВходящее, ДопПараметры=Неопределено) Экспорт
	...
	Если СбисИсключение.Свойство("help") И ТипЗнч(СбисИсключение.help) = Тип("Структура") Тогда
		лРешениеПроблемы = СбисИсключение.help.action;
	Иначе
	   	лРешениеПроблемы = СбисИсключение.help;
	КонецЕсли;
	...
	Иначе
		ПараметрыСообщения.Вставить("ТекстСсылка",	"Сохранить данные об ошибке");
		ПараметрыСообщения.Вставить("СтруктураОшибки",	СбисИсключение);
		ПараметрыСообщения.Вставить("АдресСсылка",	"Сохранить ошибку");
	КонецЕсли;
