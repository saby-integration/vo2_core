...
Процедура СбисСообщить(ПараметрыСообщения, ДопПараметры=Неопределено) Экспорт
	...
	ИначеЕсли ЭтоТипСбис(ПараметрыСообщения, "ExtException") Тогда
	...
	Если ПараметрSABY("ТихийРежим") Тогда
...
Процедура СообщитьСбисИсключение(ИсключениеВходящее, ДопПараметры=Неопределено) Экспорт
	...
	Если		ТипЗнч(ИсключениеВходящее) = Тип("ИнформацияОбОшибке")
		Или Не	ЭтоТипСбис(ИсключениеВходящее, "ExtException") Тогда
		...
		КонецЕсли;
		СбисИсключение = NewExtException(ИсключениеВходящее, ИмяКомандыВСтэк);
	...
	Если Не ДопПараметры = Неопределено Тогда
		...
		Если 	ДопПараметры.Свойство("Отложенно")
			...
			Исключение
				...
				СообщитьСбисИсключение(NewExtException(ИнформацияОбОшибке(), "МодульОбъектаКлиент.ПодключитьОтложенноеДействие"), Новый Структура("СтатусСообщения", "message"));
	...
	Если	ОбъектSABY_ЭтоТип(СбисИсключение, "UnauthorizedError")
		Или	ОбъектSABY_ЭтоТип(СбисИсключение, "TransportError") Тогда
		
		лОтправлятьОшибку = Ложь;
		
	ИначеЕсли лОтправлятьОшибку = Неопределено Тогда
		
		лОтправлятьОшибку = Истина;
		
	КонецЕсли;
	
	ТекстОшибки = ExtException_Dump(СбисИсключение, "сообщение");
	...
	КонецЕсли;
	СтатусОшибки = НРег(СтатусОшибки);
	
	Если ЗначениеЗаполнено(ГлобальныйКэш) Тогда
		
		Если ПараметрSABY("РежимОтладки") Тогда
	...
	Иначе
		Если НЕ (	ДопПараметры.Свойство("НеВызыватьРекурсивно")
			...
			ИсключениеДоп = NewExtException(,"МодульОбъектКлиент.СообщитьСбисИсключение",,"Не определен ГлобальныйКэш");
	...
	ПараметрыСообщения.Вставить("СтруктураОшибки",	СбисИсключение);
	ПараметрыСообщения.Вставить("ФормаВладелец",	лФормаВладелец);
	
	Если	СтатусОшибки = "warning"
		Или	СтатусОшибки = "предупреждениеsaby" Тогда
	...
	ИначеЕсли	СтатусОшибки = "attention"
			Или	СтатусОшибки = "внимание" Тогда
	...
	ИначеЕсли	СтатусОшибки	= "platform_warning"
			Или	СтатусОшибки	= "предупреждение" Тогда
...
Процедура СбисПоказатьВводЧисла(ОписаниеОповещения, Число, Подсказка = "", Длина = 0, Точность = 0) Экспорт
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если РезультатДиалога Тогда
			РезультатДиалога = Число;
		КонецЕсли;
		ПодключитьОтложенноеДействие(ОписаниеОповещения, РезультатДиалога);
...
// 
&НаКлиенте
Процедура СбисВыбратьФайл(ОписаниеОповещения, ПараметрыВыбораФайла) Экспорт
	Перем РежимВыбораФайла;
	
	Если	Не	ПараметрыВыбораФайла.Свойство("РежимВыбораФайла", РежимВыбораФайла)
		Или		РежимВыбораФайла = Неопределено
		Или	Не	ТипЗнч(РежимВыбораФайла) = Тип("РежимДиалогаВыбораФайла") Тогда
		
		РежимВыбораФайла = РежимДиалогаВыбораФайла.Открытие;
	...
	КонецЕсли;
	Если	ПараметрыВыбораФайла.Свойство("МножественныйВыбор")
		И	ПараметрыВыбораФайла.МножественныйВыбор Тогда
	...
	Если ТипЗнч(ОписаниеОповещения.ДополнительныеПараметры) = Тип("Структура") Тогда
		...
		// Пробросить диалог открытия в параметры метода.
		ОписаниеОповещения.ДополнительныеПараметры.Вставить("ДиалогВыбора", ДиалогОткрытия);
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Если ДиалогОткрытия.Выбрать() Тогда
			...
			Если РежимВыбораФайла = РежимДиалогаВыбораФайла.ВыборКаталога Тогда
				...
				РезультатВыбора = ДиалогОткрытия.Каталог;
				
			Иначе
				
				РезультатВыбора = ДиалогОткрытия.ВыбранныеФайлы;
				
			КонецЕсли;
			
		КонецЕсли;
		
		ВыполнитьСбисОписаниеОповещения(РезультатВыбора, ОписаниеОповещения);
