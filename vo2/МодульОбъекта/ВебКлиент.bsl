...
// Область содержит особенности работы с Веб-клиентом
...
Функция СсылочныйТипСтрокойВызовКлиента(Ссылка, ДопПараметры = Неопределено) 
	...
	ИначеЕсли Найти(СтрПолучитьСтроку(ЧастиСтроки, 1), "ChartOfCharacteristicTypesRef") Тогда

		ЧастиСтроки = СтрЗаменить(ЧастиСтроки, СтрПолучитьСтроку(ЧастиСтроки, 1), "ПланВидовХарактеристикСсылка");
