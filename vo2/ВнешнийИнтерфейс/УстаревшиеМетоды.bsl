	...
	Если ЗапускРоботом Тогда
		...
		Иначе
			Если	Не ПараметрыАвторизации.Свойство("Логин")
				...
				Возврат Ложь;
			КонецЕсли;
			РасшифрованныйПароль = Кэш.ГлавноеОкно.ТранспортИнтеграции.РасшифроватьXOR(ПараметрыАвторизации.Пароль, Новый Структура("password", ""));
			Результат = Кэш.Интеграция.АвторизоватьсяПоЛогинуПаролю(Кэш,ПараметрыАвторизации.Логин,РасшифрованныйПароль,Отказ);
		КонецЕсли;
		Если Отказ Тогда
