...
Процедура ExecuteActionEx(АсинхроннаяСБИСКоманда, ДопПараметры) Экспорт
	...
	Если	Действие.ТребуетПодписания = "Да"
		...
		Если Не СертификатВыбран и ТекстОшибки <> "" Тогда
			...
			МодульОбъектаКлиент().ВызватьСбисИсключение(775, "ФормаЭП.сбисВыбратьПодходящийСертификат",,,ТекстОшибки);
	...
	ПараметрыОтправки	= Новый Структура("Кэш, document_in, РезультатОтправки", ДопПараметры.Кэш, document_in);
	ПараметрыОтправки.РезультатОтправки = АсинхроннаяСБИСКоманда.АргументВызова.ДокументОтправить;
	
	ОбработчикОтправки	= МодульОбъектаКлиент().НовыйСбисОписаниеОповещения("PrepareAction",
																			ДопПараметры.Кэш.ТекущийСеанс.Модули.Интеграция,
																			ПараметрыОтправки);
	Сбис3Команда		= МодульОбъектаКлиент().МассоваяОперация_НовыйАсинхроннаяСбисКоманда(	ДопПараметры.РезультатОтправки,
