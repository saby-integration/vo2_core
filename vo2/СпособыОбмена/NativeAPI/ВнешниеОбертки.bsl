...
Функция СБИС_СформироватьHTMLИзXML(Кэш, ПараметрыЗапроса, ДопПараметры = Неопределено, Отказ) Экспорт
	...
	param = Новый Структура("XML");
	ЗаполнитьЗначенияСвойств(param, ПараметрыЗапроса);
	param.XML = СериализоватьИзСтроки_ВBase64(Кэш, param.XML,, Отказ);
	Если Отказ Тогда 
		Возврат МодульОбъектаКлиент().NewExtException(	param.XML,
														МодульОбъектаКлиент().ПараметрSABY("ИнтеграцияИмя") + ".СБИС.СформироватьHTMLИзXML");
	КонецЕсли;
	...
	Если Отказ Тогда
		Возврат МодульОбъектаКлиент().NewExtException(	СтруктураРезультата,
														МодульОбъектаКлиент().ПараметрSABY("ИнтеграцияИмя") + ".СБИС.СформироватьHTMLИзXML");
	КонецЕсли;
	...
	Если Отказ Тогда 
		Возврат МодульОбъектаКлиент().NewExtException(	html_text,
														МодульОбъектаКлиент().ПараметрSABY("ИнтеграцияИмя") + ".СБИС.СформироватьHTMLИзXML");
	КонецЕсли;
...
&НаКлиенте
Функция СБИС_СформироватьHTMLИзXML_Локально(Кэш, ПараметрыЗапроса, ДопПараметры = Неопределено, Отказ) Экспорт

	Возврат СБИС_СформироватьHTMLИзXML(Кэш, ПараметрыЗапроса, ДопПараметры, Отказ);
...
Функция ExtSysOrganization_WriteMapping(params, ДопПараметры, Отказ = Ложь) Экспорт   
	...
	Результат = сбисОтправитьИОбработатьКоманду(Кэш, "ExtSysOrganization.WriteMapping", params, ДопПараметрыВызова, Отказ);   
