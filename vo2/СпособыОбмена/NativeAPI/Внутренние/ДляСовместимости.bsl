			...
			Если Лев(СтруктураРезультата.message, 27) = "Не удалось найти сотрудника" Тогда
				...
				Попытка
					...
					Если СоздаватьСотрудника Тогда
						...
						Если Отказ Тогда
							Для Каждого Ошибка Из РезультатОтправки.ДетализацияОшибок Цикл
								...
								Возврат МодульОбъектаКлиент().СбисИсключение_Выгрузить(Ошибка.Значение[0].СтруктураОшибки, "Сообщение");
					...
					Иначе
						...
						Возврат МодульОбъектаКлиент().СбисИсключение_Выгрузить(СтруктураРезультата, "Сообщение");
...
&НаКлиенте
Функция сбисПодписатьВложения(МестныйКэш, attachmentList, ПараметрыДействия, Отказ = Неопределено) Экспорт
