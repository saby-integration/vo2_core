...
&НаСервере
Функция ПолучитьНастройкиАвторизацииДляТекущегоПользователяНаСервере(КэшПарам, ДанныеСотрудника,Отказ=Ложь) Экспорт
 	// Функция активирует серверные сертификаты для определенного списка ИНН	
	Попытка
...
Функция ПолучитьТокенДляТекущегоПользователяНаСервере(КэшПарам, ДанныеСотрудника,Отказ=Ложь) Экспорт
	...
	Настройки = ПолучитьНастройкиАвторизацииДляТекущегоПользователяНаСервере(КэшПарам, ДанныеСотрудника,Отказ=Ложь); 	
...
Функция сбисВыполнитьДействие(Кэш, СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета) Экспорт
	...
	Если Действие.ТребуетПодписания="Да" Тогда
		...
		Если Сертификат = Неопределено Тогда 
			Кэш.ГлавноеОкно.СбисСообщитьОбОшибке(Кэш, Кэш.ОбщиеФункции.СбисИсключение(, "API.СбисВыполнитьДействие", 755,, "Отсутствует сертификат ЭЦП для утверждения/отклонения пакета документов " + ПредставлениеПакета));
			Возврат Ложь;
		КонецЕсли;
	...
	Для Каждого Вложение Из СоставПакета.Вложение Цикл
		Если Вложение.Свойство("Зашифрован") И Вложение.Зашифрован = "Да" Тогда
			Кэш.ГлавноеОкно.СбисСообщитьОбОшибке(Кэш, Кэш.ОбщиеФункции.СбисИсключение(, "API.сбисВыполнитьДействие", 601, "Для корректной обработки зашифрованных документов выберите способ обмена ""extSDK"" на вкладке Настройки."));
			Возврат Ложь;
		КонецЕсли;
		Прервать;
	КонецЦикла;
...
Функция ОтправитьПакетыДокументов(Кэш, МассивПакетов, РезультатОтправки=Неопределено) Экспорт
	...
	Для СчетчикПакетов=0 По КоличествоПакетов-1 Цикл
		...
		Если МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ПоддержкаОбменаЕИС") = Истина
			И СоставПакета.Свойство("НеЗапускатьВДокументооборот")
			И СоставПакета.НеЗапускатьВДокументооборот = "Да" Тогда
			document.ДопПоля = document.ДопПоля + ",Расширение,ЕИС";
		КонецЕсли;
				...
				Для Каждого Вложение Из СоставПакета.Вложение Цикл
					Если Вложение.Свойство("Документы1С") Тогда
						Для Каждого Документ1С Из Вложение.Документы1С Цикл
							...
							ДанныеПоСтатусам.Добавить(Новый Структура("СтруктураСвойств, Документ1С, ИдАккаунта", СтруктураСвойств, Документ1С.Значение, ПолучитьИдТекущегоАккаунта(Кэш)));
						КонецЦикла;
					КонецЕсли;
				КонецЦикла;
