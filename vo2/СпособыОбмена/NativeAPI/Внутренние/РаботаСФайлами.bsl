...
Функция ПолучитьHTMLВложения(Кэш, ИдДок, Вложение) Экспорт
	Если Вложение.Свойство("Зашифрован") И Вложение.Зашифрован = "Да" тогда
		Возврат "<HTML><BODY scroll=no>Документ зашифрован.</br>" + 
			"Для работы с зашифрованными документами выберите способ обмена ""extSDK2"" на вкладке Настройки.</BODY></HTML>";
	КонецЕсли;
...
Функция ПолучитьДанныеФайлаНаКлиенте(ПараметрыПолучения, ПараметрыФайла, Отказ)
	...
	//	Или	Лев(РезультатТекст,2) <> "<?" Тогда
	//	для PDF, JPEG файлjd иногда возвращается крокозябра, которая потом вызывает ошибку передачи данных между клиентом и сервером,
	//	несмотря на то, что это строка
		РезультатТекст = "";
	КонецЕсли;
