...
Функция СбисПолучитьОтвет_Синхронно(Кэш, Команда, ДопПараметры, Отказ)
	...
	Пока Истина Цикл
		...
		ИначеЕсли ТекущаяДата() - Команда.ВремяВызова > Команда.ВремяОжиданияОтвета Тогда
			...
			Отказ = Истина;
			Результат = МодульОбъектаКлиент().НовыйСбисИсключение(775101,"SabyHttpsClient.СбисПолучитьОтвет_Синхронно",,,
			"Нам не удалось получить ответ от плагина в течении " + Строка(Команда.ВремяОжиданияОтвета) + " секунд");
			Прервать;
		Иначе
			Продолжить;
		КонецЕсли;
