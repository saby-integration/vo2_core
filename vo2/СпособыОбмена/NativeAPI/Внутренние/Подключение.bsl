...
Функция Включить(Кэш, ДопПараметры=Неопределено, Отказ = Ложь) Экспорт 
	...
	МодульОбъектаКлиент().ВнешняяОбработка_ПодключитьКомпоненту(Новый Структура("ИмяОбработки, ИмяМакета, ИмяКомпоненты","SABYHttpClient", "SABYHttpsClient", "SABYHttpsClient"));
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("Версия",						Компонента.ВерсияКомпоненты);
