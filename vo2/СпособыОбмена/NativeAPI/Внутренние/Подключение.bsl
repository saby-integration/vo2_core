...
Функция Включить(Кэш, ДопПараметры=Неопределено, Отказ = Ложь) Экспорт 
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("НастройкиКриптографии",			МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("НастройкиКриптографии"));
...
Функция СведенияОбИнтеграции() Экспорт
	...
	МодульАПИ = МодульОбъектаКлиент().ПолучитьФормуОбработки("API");	
	
	СведенияОбИнтеграции = МодульАПИ.СведенияОбИнтеграции();
	СведенияОбИнтеграции.Имя = "SABYHttpClient";
	СведенияОбИнтеграции.Параметры.МногопоточныйРежим = Истина;
	СведенияОбИнтеграции.Параметры.ИмяКоннкетора = "1c-native-http";
 	СведенияОбИнтеграции.Параметры.ИмяВнутреннегоМодуля = "SABYHttpClient";// Имя для поиска функций вызова
...
Функция СбисПолучитьОтвет_Синхронно(Кэш, Команда, ДопПараметры, Отказ)
	...
	Пока Истина Цикл
		...
		ИначеЕсли ТекущаяДата() - Команда.ВремяВызова > Команда.ВремяОжиданияОтвета Тогда
			...
			Отказ = Истина;
			Результат = Кэш.ОбщиеФункции.СбисИсключение(,
														Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя + ".СБИС_ПолучитьОтвет_Синхронно",
														775,
														,
														Команда.Метод + "() не вернул ответ за разумное время",
														,
...
&НаКлиенте
Функция СбисОтправитьИОбработатьКоманду(Кэш, Метод, ПараметрыМетода, ДопПараметры = Неопределено, Отказ = Ложь) Экспорт
	...
	Если Отказ Тогда
		...
		ЗаписьВСтэк	= МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("ИнтеграцияИмя") + ".СбисОтправитьИОбработатьКоманду";
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ИД, ЗаписьВСтэк);
...
//
// Параметры:
//  Кэш				 - ЛокальныйКэш	 - струкутра параметров обработки
//  ПараметрыВызова	 - Струкутра	 - результат: результат авторизации по логину/сертификату, ПроверкаВыполнена: Булево - уже переключали домен
//  Отказ			 - Булево		 - флаг ошибки
// 
// Возвращаемое значение:
//  Струкутра - ПараметрыВызова.Результат
//
&НаКлиенте
Функция сбисПереключитьДомен(Кэш, ПараметрыВызова, Отказ) Экспорт
	...
	Если	Кэш.СБИС.ПараметрыИнтеграции.РезервныйДомен
		Или	(	ПараметрыВызова.Свойство("ПроверкаВыполнена")
			И	ПараметрыВызова.ПроверваВыполнена) Тогда
			
		//Уже включен резервный домен, или проверка была выполнена выходим
	...
	КонецЕсли;
	ПараметрыВызова.Вставить("Результат", Кэш.Интеграция.сбисОтправитьИОбработатьКоманду(	Кэш,
																							ПараметрыВызова.Метод,
																							ПараметрыВызова.Параметры,
																							ПараметрыВызова.ДополнительныеПараметры,
																							Отказ));
