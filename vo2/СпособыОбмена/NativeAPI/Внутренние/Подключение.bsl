...
Функция СбисКомпонента_Команда(Кэш, Метод, ПараметрыМетода, ДопПараметры, Отказ)
	...
	Запрос.Вставить("Url", "https://" + Кэш.СБИС.АдресСервера + Прав(АдресРесурса,СтрДлина(АдресРесурса)));
...
Функция СбисОбработатьРезультат_Синхронно(Кэш, Команда, ДопПараметры, Отказ)
	...
	Если СбисРезультат.Type = "text" И НЕ (СбисРезультат.Status = 200 ИЛИ СбисРезультат.Status = 201) Тогда	
		Отказ = Истина;
		Результат = МодульОбъектаКлиент().НовыйСбисИсключение(СбисРезультат.Data, Команда.Метод, СбисРезультат.Status);
		
	ИначеЕсли СбисРезультат.Data.Свойство("error", Результат) Тогда
