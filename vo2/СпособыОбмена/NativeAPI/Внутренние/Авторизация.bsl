...
&НаКлиенте
Функция ЗакрытьСессию(Кэш, Отказ=Ложь) Экспорт
	
	ДопПараметры		= Новый Структура("АдресРесурса", "/auth/service/");
	РезультатЗакрытия	= сбисОтправитьИОбработатьКоманду(Кэш, "СБИС.Выход", Новый Структура, ДопПараметры, Отказ);
...
Функция ПолучитьСписокСертификатовДляАвторизации(Кэш,ТекстОшибки) Экспорт
	...
	РезультатЗапроса.Свойство("Сертификат", ObjectList);
	...
	СтрокаШаблонСерт = "[ФИО].([Должность])., [Название].(ИНН [ИНН])., действителен до [ДействителенПо]";
	...
	Для сч = 0 По Размер - 1 Цикл
		...
		Если Сертификат.Аутентификация.Зарегистрирован = "Да" Тогда
			Если Сертификат.Свойство("ДействителенПо") Тогда
				...
				КонецЕсли;
			КонецЕсли;
			СписокСертификатов.Добавить(Сертификат, Кэш.ОбщиеФункции.ПолучитьПредставлениеСертификата(Сертификат, СтрокаШаблонСерт, Истина, "ДЛФ=DD"));
		КонецЕсли;
	КонецЦикла;
