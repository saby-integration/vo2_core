...
функция сбисЗаписатьСотрудников(Кэш, ДанныеСотрудников, Отказ) Экспорт
	// функция активирует серверные сертификаты для определенного списка ИНН	
	СбисОшибка = Кэш.ОбщиеФункции.СбисИсключение(,"SDK2.сбисЗаписатьСотрудников",,"Загрузка сотрудников в Saby дступна только при способе обмена ExtSDK, ExtSDK2 или API.","Изменить способ обмена можно на вкладке ""Настройки"".");
...
Функция Включить(Кэш, ДопПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	Попытка
		...
		Если Кэш.ОбщиеФункции.ЭтоНоваяВерсия(ТребуемаяВерсия, ВерсияSDK) Тогда
			...
			ПараметрыСообщения.Вставить("АдресСсылка", "https://saby.ru/help/integration/sdk/");
			ПараметрыСообщения.Вставить("ТекстСсылка", "Скачать Saby SDK");
			ПараметрыСообщения.Вставить("Текст",	"Не зарегистрирована новая версия ActiveX компоненты Saby SDK."+Символы.ПС+
			...
			Кэш.ГлавноеОкно.сбисПолучитьФорму("ФормаПредупреждения").ПоказатьПоверх(ПараметрыСообщения);
	...
	Исключение
		...
		ПараметрыСообщения.Вставить("АдресСсылка", "https://saby.ru/help/integration/sdk/");
		ПараметрыСообщения.Вставить("ТекстСсылка", "Скачать Saby SDK");
		ПараметрыСообщения.Вставить("Текст",	"Не зарегистрирована ActiveX компонента Saby SDK."+Символы.ПС+
...
Функция СведенияОбИнтеграции() Экспорт
	...
	ПараметрыИнтеграции.Вставить("Актуален",					Ложь);
	
	Возврат Новый Структура("Имя, Версия, Параметры", "SDK2", "(Снят с поддержки)", ПараметрыИнтеграции);
...
//Авторизуется на online.saby.ru по логину/паролю	
&НаКлиенте
...
//Авторизуется на online.saby.ru по сертификату		
&НаКлиенте
...
//Подтверждаем авторизацию на online.saby.ru по коду		
&НаКлиенте
...
#Область include_core2_vo2_СпособыОбмена_SDK2_Вызовы
#КонецОбласти

#Область include_core2_vo2_СпособыОбмена_SDK2_ВнешниеОбертки
#КонецОбласти

#Область include_core2_vo2_СпособыОбмена_SDK2_Сериализатор
#КонецОбласти

// Отправляет сформированные пакеты документов	
&НаКлиенте
Процедура Отправка_ПачкаДокументов(ПараметрыОбработки, Кэш) Экспорт
	Перем ДопПараметрыСтороны;
	
	КомандыОтправки	= ПараметрыОбработки.Отправки;
	ЧислоПотоков	= МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("Потоки_МаксимальноеКоличество");
	ДанныеОтправки	= Новый Структура("Документ, ЧислоПотоков", Новый Массив, ЧислоПотоков);
	НеЗапускатьВДо	= Ложь;
	Для Каждого КомандаВызоваОтправки Из КомандыОтправки Цикл
		ПакетКОтправке	= КомандаВызоваОтправки.АргументВызова;
		СтатусПакета	= МодульОбъектаКлиент().ОтправкаДокументаСБИС_Получить(ПакетКОтправке, "СтатусПакета");
		СоставПкета		= МодульОбъектаКлиент().ОтправкаДокументаСБИС_Получить(ПакетКОтправке, "СоставПакета");
		Если Не СтатусПакета = "Отправлен" Тогда//Статус проставили, когда заняли поток.
			Продолжить;
		КонецЕсли;
		НеЗапускатьВДо	= НеЗапускатьВДо Или СоставПкета.НеЗапускатьВДокументооборот = Истина;	
		
		ДанныеОтправки.Документ.Добавить(МодульОбъектаКлиент().ОтправкаДокументаСБИС_Получить(ПакетКОтправке, "ДокументОтправить"));
	КонецЦикла;
	Если НеЗапускатьВДо Тогда
		ДанныеОтправки.Вставить("НеЗапускатьВДокументооборот", "Да");
	КонецЕсли;
	
	param = СериализоватьСтруктуруВObject(ДанныеОтправки, Кэш);
	ПараметрыОбработки.Вставить("РезультатПачки",  Кэш.docflow.WriteDocumentsEx(param));
...
Процедура WriteDocumentEx_Error(АсинхронноеСбисСобытие, ПараметрыОбработки) Экспорт
	...
	Если	РезультатВызова.message = "Необходимо аутентифицироваться для выполнения запросов к серверу" 
		Или	Лев(РезультатВызова.message, 18) = "Ошибка авторизации" Тогда
		ПараметрыОбработки.Вставить("ПотеряАвторизации", Истина);
	КонецЕсли;	
...
// Отправляет сформированные пакеты документов	
&НаКлиенте
Процедура Отправка_ОбработкаРезультатаПачкиДокументов(ПараметрыОбработки, Кэш) Экспорт
	Перем ДанныеСобытияОтвета;
	
	РезультатОтправки	= ПараметрыОбработки.РезультатОтправки;
	РезультатПачки		= ПараметрыОбработки.РезультатПачки;
	
	Если РезультатПачки = Неопределено Тогда
		
		ДетализацияОшибки	= ПолучитьСообщениеОбОшибке(Ложь);
		МодульОбъектаКлиент().ВызватьСбисИсключение(, "SDK2.Отправка_ОбработкаРезультатаПачкиДокументов", ,ДетализацияОшибки.ТекстОшибки, ДетализацияОшибки.ИнформацияОбОшибке);

	КонецЕсли;
	
	РезультатПачки		= СериализоватьObjectВСтруктуру(РезультатПачки);
	СбисМодульОбработки	= МодульОбъектаКлиент();
	
	ИндексЗаписей = 0;
	Для Каждого ЭлементОтвета Из РезультатПачки.Реестр Цикл
		АсинхроннаяСбисКоманда = ПараметрыОбработки.Отправки[ИндексЗаписей];
		
		Если		ЭлементОтвета.Свойство("Документ", ДанныеСобытияОтвета) Тогда
			АсинхронноеСбисСобытие = СбисМодульОбработки.НовыйАсинхронноеСбисСобытие(АсинхроннаяСбисКоманда.Идентификатор, ДанныеСобытияОтвета, "Message");
			СбисМодульОбработки.АсинхроннаяСбисКоманда_ВызватьСобытие(АсинхроннаяСбисКоманда, АсинхронноеСбисСобытие);
		ИначеЕсли	ЭлементОтвета.Свойство("Ошибка", ДанныеСобытияОтвета) Тогда
			ОшибкаОтправки = СбисМодульОбработки.НовыйСбисИсключение(, "SDK2.Отправка_ОбработкаРезультатаПачкиДокументов",,ДанныеСобытияОтвета.Описание, ДанныеСобытияОтвета.ОписаниеРасширенное);
			АсинхронноеСбисСобытие = СбисМодульОбработки.НовыйАсинхронноеСбисСобытие(АсинхроннаяСбисКоманда.Идентификатор, ОшибкаОтправки, "Error");
		Иначе
			ОшибкаОтправки = СбисМодульОбработки.НовыйСбисИсключение(, "SDK2.Отправка_ОбработкаРезультатаПачкиДокументов");
			АсинхронноеСбисСобытие = СбисМодульОбработки.НовыйАсинхронноеСбисСобытие(АсинхроннаяСбисКоманда.Идентификатор, ОшибкаОтправки, "Error");
		КонецЕсли;
		
		СбисМодульОбработки.МассоваяОперация_ОсвободитьПоток(ПараметрыОбработки.РезультатОтправки, АсинхронноеСбисСобытие);
...
Процедура Отправка_ОбработатьВнешнийФайл(Кэш, ДанныеФайла, ПараметрыПодготовки) Экспорт
	...
	StreamHelper = Новый COMОбъект("SBIS.StreamHelper");
	ДанныеФайла.Файл.Вставить("ДвоичныеДанные", StreamHelper.FileToBase64(ДанныеФайла.Вложение.ПолноеИмяФайла)); 
...
Процедура Отправка_ОбработатьXMLФайл(Кэш, ДанныеФайла, ПараметрыПодготовки) Экспорт
	...
	StreamHelper = Новый COMОбъект("SBIS.StreamHelper");
	ДанныеФайла.Файл.Вставить("ДвоичныеДанные", StreamHelper.StringToBase64(ДанныеФайла.Вложение.XMLДокумента)); 
...
Функция СБИС_СписокДокументов(Кэш, filter, ДопПараметры, Отказ) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКоманду(Кэш, "ReadDocuments", Новый Структура("Фильтр", filter), ДопПараметры, Отказ);
	Если Отказ Тогда
...
Функция СБИС_СписокДокументовПоСобытиям(Кэш, filter, ДопПараметры, Отказ) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКоманду(Кэш, "ReadDocumentsByEvents", Новый Структура("Фильтр", filter), ДопПараметры, Отказ);
	Если Отказ Тогда
