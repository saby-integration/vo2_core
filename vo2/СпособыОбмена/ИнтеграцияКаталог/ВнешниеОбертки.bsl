...
Функция СБИС_СписокИзменений(Кэш, filter, ДопПараметры, Отказ) Экспорт
	Перем ФайлПоследнегоСобытия, СписокСостоянийСобытия, КодСостоянияСобытия;
	...
	КодыСостоянийПропустить	= ",12,13,16,17,30,31,"; // Коды состояний коннекта, которые мы пропустим.
	...
	Для Каждого Файл Из ФайлыСтатусов Цикл
		...
		Если	оОрганизация = Неопределено
			Или оОрганизация = Ложь Тогда
			
			ПропуститьФайл = Истина;
			
		КонецЕсли;
		
		// Привести дату к общему формату событий
		Если оДокумент.Свойство("Дата") Тогда
			
			оДокумент.Дата = МодульОбъектаКлиент.ДатаИзСтроки(оДокумент.Дата);
			
		КонецЕсли;
		
		Если оДокумент.Свойство("ДатаВремяСоздания") Тогда
			
			оДокумент.ДатаВремяСоздания	= МодульОбъектаКлиент.ДатаИзСтроки(оДокумент.ДатаВремяСоздания);
			
		КонецЕсли;
		
		Если оДокумент.Свойство("Событие", СписокСостоянийСобытия) Тогда
			
			// Очистить события коннекта
			ИндексСобытия = 0;
			Пока ИндексСобытия < СписокСостоянийСобытия.Количество() Цикл
				
				СобытиеДокумента = оДокумент.Событие[ИндексСобытия];
				Если	СобытиеДокумента.Свойство("Состояние", КодСостоянияСобытия)
					И	КодСостоянияСобытия.Свойство("Код", КодСостоянияСобытия)
					И	Найти(КодыСостоянийПропустить, "," + КодСостоянияСобытия + ",") Тогда 
					
					СписокСостоянийСобытия.Удалить(ИндексСобытия);
					Продолжить;
			...
			ПропуститьФайл = Не СписокСостоянийСобытия.Количество();
...
Функция СБИС_ПолучитьИнформациюОТекущемПользователе(Кэш, param, ДопПараметры, Отказ) Экспорт
	...
	Результат.Вставить("НомерАккаунта", "");
	Возврат Результат;
КонецФункции    
...
Функция СБИС_СписокДокументовПоСобытиям(Кэш, filter, ДопПараметры, Отказ) Экспорт
	...
	// Получает список документов по событиям с online.saby.ru	
