...
Процедура Отправка_ОбработатьВнешнийФайл(Кэш, ДанныеФайла, ПараметрыПодготовки) Экспорт
	...
	Если Не ПараметрыПодготовки.Свойство("КаталогОбмена", КаталогОтправки) Тогда
	...
	Если Вложение.Свойство("Дата") Тогда
		ДанныеФайла.ВложениеВПакет.Вставить("Дата",		Вложение.Дата);
	КонецЕсли;
	Если Вложение.Свойство("Номер") Тогда
		ДанныеФайла.ВложениеВПакет.Вставить("Номер",	Вложение.Номер);
	КонецЕсли;
	
	ИмяФайла			= Вложение.ИмяФайла;
	СбисСсылкаНаФайл	= Вложение.Идентификатор + "_" + ИмяФайла;
	КопироватьФайл(Вложение.ПолноеИмяФайла, КаталогОтправки + СбисСсылкаНаФайл);

	ДанныеФайла.ВложениеВПакет.Файл.Вставить("СсылкаНаФайл", СбисСсылкаНаФайл);
