	...
	Если НЕ Кэш.Свойство("СБИСПлагин") Тогда
		...
		Пока Статус = "Connecting" ИЛИ Статус =  "Closed"  Цикл
			...
			Если ТекущаяДата() - ОжиданиеОтвета > НачДата Тогда
				Сообщить("не сумели подключиться к Saby плагину за разумное время");
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
...
Функция СформироватьПотокОтправки(Поток, Кэш)
	...
	Если Ответы.Количество() = 0 Тогда
		Сообщить("Не удалось получить расширение ExtCom1C. Возможно, Saby плагин не запущен.");
		Возврат "Не удалось получить расширение ExtCom1C. Возможно, Saby плагин не запущен.";
	КонецЕсли;
