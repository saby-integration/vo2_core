...
Процедура ExecuteActionEx(АсинхроннаяСБИСКоманда, ДопПараметры) Экспорт
	...
	document_in.Вставить( "Идентификатор", АсинхроннаяСБИСКоманда.АргументВызова.СтатусЭДО.ИдПакета);	
...
Процедура ProcessServiceStagesEx(АсинхроннаяСБИСКоманда, ДопПараметры) Экспорт
	...
	РезультатОбработки = ОбработатьСлужебныеДокументыПоПакету(	МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш(),
																АсинхроннаяСБИСКоманда.АргументВызова);
	АсинхронноеСбисСобытие = МодульОбъектаКлиент().НовыйАсинхронноеСбисСобытие(АсинхроннаяСБИСКоманда, РезультатОбработки, "Message");
	МодульОбъектаКлиент().АсинхроннаяСбисКоманда_ВызватьСобытие(АсинхроннаяСбисКоманда, АсинхронноеСбисСобытие);
