...
Процедура WriteDocumentEx(АсинхроннаяСБИСКоманда, ДопПараметры) Экспорт
	...
	Если DocumentOut.Свойство("НаборДопПолей") Тогда
		Если DocumentOut.НаборДопПолей.Свойство("ЗаписатьДокумент") Тогда
			...
			ДопПоляЗаписатьДокумент.Добавить("ДополнительныеПоляВерсия1");
		КонецЕсли;
		Если DocumentOut.НаборДопПолей.Свойство("ПодготовитьДействие") Тогда
			...
			ДопПоляПодготовитьДействие.Добавить("ДополнительныеПоляВерсия1");
		КонецЕсли;
	...
	Если СтруктураРезультата.Этап[0].Действие[0].свойство("Сертификат") Тогда 
		Если ЗначениеЗаполнено(СертификатДляПодписания) и СтруктураРезультата.Этап[0].Свойство("Вложение") Тогда
			...
			РезультатПодписания = сбисПодписатьВложения(Кэш, prepared_document.Этап[0].Вложение,ПараметрыДействия, Отказ);
			Если Отказ Тогда                                                                                              
				ОшибкаПодписания = МодульОбъектаКлиент().НовыйСбисИсключение(РезультатПодписания, "API.WriteDocumentEx",,,);
				ВызватьИсключение МодульОбъектаКлиент().СбисИсключение_Выгрузить(РезультатПодписания);
			КонецЕсли;
...
Процедура Отправка_ОбработатьXMLФайл(Кэш, ДанныеФайла, ПараметрыПодготовки) Экспорт
	...
	Кодировка = ?(Найти(ДанныеФайла.Вложение.XMLДокумента, "encoding=""UTF-8"""), "UTF-8", "windows-1251");
	ДвоичныеДанныеXML = СбисСтрокаВДвоичныеДанныеКлиент(Кэш, ДанныеФайла.Вложение.XMLДокумента, Кодировка, Ложь);
