...
Процедура WriteDocumentEx(АсинхроннаяСБИСКоманда, ДопПараметры) Экспорт
	...
	Если DocumentOut.Свойство("ДопПоля") Тогда
		DocumentOut.ДопПоля.Добавить("ЭтапВернутьВсеСертификаты");
	Иначе
		ДопПоля = Новый Массив;
		ДопПоля.Добавить("ЭтапВернутьВсеСертификаты");
		DocumentOut.Вставить("ДопПоля", ДопПоля);
	КонецЕсли;
	
	Если НеЗапускатьВДО И ПоддержкаОбменаЕИС Тогда
		DocumentOut.ДопПоля.Добавить("Расширение");
		DocumentOut.ДопПоля.Добавить("ЕИС");
	КонецЕсли;
	ДопПоля = МодульОбъектаКлиент().СбисСтрСоединить(DocumentOut.ДопПоля, ",");
	DocumentOut.Вставить("ДопПоля",ДопПоля);
