...
Процедура WriteDocumentEx(АсинхроннаяСБИСКоманда, ДопПараметры) Экспорт
	...
	ИначеЕсли	DocumentOut.Свойство("НеЗапускатьВДокументооборот")
			И	DocumentOut.НеЗапускатьВДокументооборот = "Да" Тогда
...
Функция Отправка_ПодпискиСобытия(ПараметрыСобытий) Экспорт
	...
	Результат.Вставить("Message",		Новый Структура("Функция",	"WriteDocumentEx_Message"));
	Возврат Результат;
