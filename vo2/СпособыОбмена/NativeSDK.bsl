...
#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеВызовы_Устаревшее
#КонецОбласти 
...
Процедура СБИСПлагин_ОбработатьПросроченныеВызовы(Кэш)
	...
	Для Каждого ИдЗапросПросрочен Из ЗапросыПросрочены Цикл
		...
		ОтветПросрочено = МодульОбъектаКлиент().НовыйСбисИсключение(775101,"SabyPluginConnector.СБИСПлагин_ОбработатьПросроченныеВызовы",,,
		"Нам не удалось получить ответ от плагина в течении " + Строка(Сбис3КомандаПросрочена.ВремяОжиданияОтвета) + " секунд");
...
Функция СБИСПлагин_ПолучитьОтвет_Синхронно(Кэш, СБИС3Команда, ДопПараметры, Отказ) Экспорт
	...
	Пока Истина Цикл
		...
		ИначеЕсли ТекущаяДата() - СБИС3Команда.ВремяВызова > СБИС3Команда.ВремяОжиданияОтвета Тогда
			...
			Отказ = Истина;
			Результат = МодульОбъектаКлиент().НовыйСбисИсключение(775101,"SabyPluginConnector.СБИСПлагин_ПолучитьОтвет_Синхронно",,,
...
Функция СБИСПлагин_ОсвободитьПоток(Кэш, СбисОписаниеСобытия) Экспорт
	...
	Если Не КомандаВыполнена = Неопределено Тогда
		...
		Если Кэш.Парам.РежимОтладки Тогда
			...
			Если СбисОписаниеСобытия.Свойство("ПараметрыСобытия") Тогда
				
				ПараметрыСобытия = СбисОписаниеСобытия.ПараметрыСобытия;
				
			КонецЕсли;
	
			лОтладочныеДанные = Новый Структура;
			
			лОтладочныеДанные.Вставить("Тип", 				СбисТипОтвета);
			лОтладочныеДанные.Вставить("Модуль", 			КомандаВыполнена.Модуль); 
			лОтладочныеДанные.Вставить("Вызов", 			КомандаВыполнена.Метод); 
			лОтладочныеДанные.Вставить("Время", 			ТекущаяДата()); 
			лОтладочныеДанные.Вставить("Идентификатор", 	КомандаВыполнена.Идентификатор); 
			лОтладочныеДанные.Вставить("Сообщение", 		СбисТекстОтвета); 
			лОтладочныеДанные.Вставить("ПараметрыСобытия", 	ПараметрыСобытия);
...
Функция СБИСПлагин_ЧтениеОтветовИВызовСобытий(Кэш, Сбис3КомандыОжидаются=Неопределено, ДопПараметры=Неопределено, Отказ)
	...
	Для Каждого Сбис3СобытиеОтвет Из СбисСобытия.Response Цикл
		...
		Если ДопПараметры.Свойство("ОчиститьПараметры", ОчиститьПараметры) Тогда
			
			ПараметрыСобытия.Вставить("ОчиститьПараметры", ОчиститьПараметры); 
			
		КонецЕсли;
		
		ПараметрыКоманды = Новый Структура;  
		
		ПараметрыКоманды.Вставить("Идентификатор", ИдентификаторСобытия);
		ПараметрыКоманды.Вставить("Тип", Сбис3СобытиеОтвет.type); 
		ПараметрыКоманды.Вставить("Ответ", Сбис3СобытиеОтвет.data); 
		ПараметрыКоманды.Вставить("ПараметрыСобытия", ПараметрыСобытия);
		
		КомандаОбрабатываемогоОтвета = СБИСПлагин_ОсвободитьПоток(Кэш, ПараметрыКоманды);  
