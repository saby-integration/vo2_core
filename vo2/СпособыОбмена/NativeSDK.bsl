...
#Область include_core_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки
#КонецОбласти
...
Функция Включить(Кэш, ДопПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	ПараметрыВключения = Новый Структура("ВремяОжиданияОтвета, ВремяОжиданияОтвета_Отправка, КоличествоПотоков, ИнтеграцияИмя", 60, 120, 10, "SabyPluginConnector");
	...
	Возврат СбисОценитьРезультатВключения(Кэш, РезультатЗапроса, ДопПараметры, Отказ);
КонецФункции
...
Функция ПолучитьМодульИнтеграции(Кэш,ДопПараметры,Отказ) Экспорт 
	...
	МодульОбъектаКлиент().ВнешняяОбработка_ПодключитьКомпоненту(Новый Структура("ИмяОбработки, ИмяМакета, ИмяКомпоненты",
																	"SABYPluginConnector","SABYPluginConnector", "SABYPluginConnector"));	
	...
	Попытка
		Компонента = Новый("AddIn.SabyPluginConnector.SabyPluginConnector");
	Исключение
		Отказ = Истина;
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ОписаниеОшибки());
	КонецПопытки; 
...
Функция СБИСПлагин_ЧтениеОтветовИВызовСобытий(Кэш, Сбис3КомандыОжидаются=Неопределено, ДопПараметры=Неопределено, Отказ)
	...
	Для Каждого Сбис3СобытиеОтвет Из СбисСобытия.Response Цикл
		ОтветОбработан = Ложь;
		////костыли
		//Если НЕ Сбис3СобытиеОтвет.Свойство("QueryId") И Сбис3СобытиеОтвет.data.Свойство("QueryId") Тогда
		//	Сбис3СобытиеОтвет.Вставить("queryID", Сбис3СобытиеОтвет.data.QueryId);
		//КонецЕсли;
...
#Область include_core_vo2_СпособыОбмена_NativeSDK_XMLКонвертер
#КонецОбласти
