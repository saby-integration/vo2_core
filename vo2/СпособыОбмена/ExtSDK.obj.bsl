...
Функция ПрочитатьДокумент(Кэш, ИдДок, ДопПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	ПараметрыЧтения = Новый Структура("Идентификатор, ДопПоля",ИдДок, "ЭтапВернутьВсеСертификаты,Лицо2,Лицо3,НеПерегенерироватьВложения");
	...
	Если РезультатЗапроса = "" Тогда
		...
		Для Каждого КлючИЗначениеОтвет Из МодульОбработкиВызова.ГлобальныйКэш.СБИС.ДанныеИнтеграции.НеизвестныеОтветы Цикл
			Для Каждого ЗначениеСобытия Из КлючИЗначениеОтвет.Значение Цикл
				...
				Если ЗначениеОтвет.Тип = "Error" Тогда
