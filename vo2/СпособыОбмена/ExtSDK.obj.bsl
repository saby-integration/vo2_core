...
Функция СведенияОбИнтеграции() Экспорт
	...
	"ExtSdk", МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("Версия"), ЭтаФорма, ПараметрыИнтеграции);
...
Функция ПолучитьМодульИнтеграции(Кэш,ДопПараметры,Отказ) Экспорт
	...
	РезультатЗапроса = СБИСПлагин_GetModule(Кэш, МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("ИнтеграцияИмя"), Отказ);
	Если Отказ Тогда
...
Функция ЗавершитьExtSDK(Кэш, ДопПараметры, Отказ) Экспорт
	Попытка
		...
		МодульИнтеграции = Кэш.СБИС.ДанныеИнтеграции.Объекты[Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя];
		Кэш.СБИС.ДанныеИнтеграции.Объекты.Плагин.ReleaseModule(МодульИнтеграции);
...
Функция СБИСПлагин_ПрочитатьДокументКакHTML(Кэш, param, ДопПараметры, Отказ) Экспорт
	...
	Если Отказ Тогда
		Возврат Кэш.ОбщиеФункции.сбисИсключение(,  Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя + ".СБИСПлагин_ПрочитатьДокументКакHTML", , "Не удалось сформировать печатную форму. Повторите попытку позднее", , Новый Структура("Error", Результат));
	КонецЕсли;
	Возврат Результат
КонецФункции	
