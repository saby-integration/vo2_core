...
Функция УстановитьКаталогОтладки(Кэш) Экспорт
	...
	Исключение
		СбисИсключение = Кэш.ОбщиеФункции.СбисИсключение(, "ExtSDK.УстановитьКаталогОтладкиРекурсивно", 772, "Ошибка работы с файловой системой", ОписаниеОшибки());
...
Функция ЗавершитьExtSDK(Кэш, ДопПараметры, Отказ) Экспорт
	...
	Попытка
		...
		Если Кэш.СБИС.ДанныеИнтеграции.Объекты.Свойство(ИмяИнтеграции, ИдМодуляИнтеграции) Тогда
			Кэш.СБИС.ДанныеИнтеграции.Объекты.Плагин.ReleaseModule(ИдМодуляИнтеграции);
		КонецЕсли;
...
Функция СБИСПлагин_GetModule(Кэш, ИмяМодуля, Отказ)
	...
	Попытка 
		...
		Если РезультатЗапроса = "" Тогда
			...
			Кэш.СБИС.ДанныеИнтеграции.Объекты.Плагин.ReadAllXml2();
			РезультатЗапроса = Кэш.СБИС.ДанныеИнтеграции.Объекты.Плагин.GetModule(ИмяМодуля);
	...
	Если РезультатЗапроса = "" Тогда
		...
		Для Каждого КлючИЗначениеОтвет Из МодульОбработкиВызова.ГлобальныйКэш.СБИС.ДанныеИнтеграции.НеизвестныеОтветы Цикл
			Для Каждого ЗначениеСобытия Из КлючИЗначениеОтвет.Значение Цикл
				Если МодульОбъектаКлиент().ЭтоТипСбис(ЗначениеСобытия, "АсинхронноеСбисСобытие") Тогда
		...
		Для Каждого КлючОчистить Из КлючиОчистить Цикл 
			МодульОбработкиВызова.ГлобальныйКэш.СБИС.ДанныеИнтеграции.НеизвестныеОтветы.Удалить(КлючОчистить);
		КонецЦикла;
...
Процедура Отправка_ОбработатьОшибкуОтправки(Кэш, Сбис3КомандаОтправки, СтруктураОшибки)
	...
	Если	СтруктураОшибки.details = "Необходимо аутентифицироваться для выполнения запросов к серверу"
		...
		ИначеЕсли	(	Кэш.Парам.ВходПоСертификату И Кэш.Парам.ЗапомнитьСертификат) 
			...
			// переавторизуемся
			формаАвторизации = Кэш.ГлавноеОкно.сбисПолучитьФорму("ФормаАвторизация");
