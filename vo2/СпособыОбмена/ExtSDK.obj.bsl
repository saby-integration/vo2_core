...
Функция СБИСПлагин_ПолучитьОтвет_Синхронно(Кэш, СБИС3Команда, ДопПараметры, Отказ) Экспорт
	...
	Пока Истина Цикл
		...
		ИначеЕсли ТекущаяДата() - СБИС3Команда.ВремяВызова > СБИС3Команда.ВремяОжиданияОтвета Тогда
			...
			Отказ = Истина;
			Результат = Кэш.ОбщиеФункции.СбисИсключение(, Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя + ".СБИСПлагин_ПолучитьОтвет_Синхронно",775,, СБИС3Команда.Метод + "() не вернул ответ за разумное время",,"https://sbis.ru/help/integration/1C_set/modul/typical_errors/plugin_answer");
			Прервать;
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
...
Функция ПолучитьМодульИнтеграции(Кэш, ДопПараметры, Отказ) Экспорт
	...
	Если РезультатЗапроса = "Unknown module" Тогда
		...
		Отказ = Истина;
		ТекстСообщения = "Модуль """ + ИмяМодуляПолучить + """ не найден в СБИС.Плагин";
...
Функция СбисОценитьРезультатВключения(Кэш, Результат, ДопПараметры, Отказ) Экспорт
	...
	ИначеЕсли Результат.code = 767 Тогда
		...
		АдресСсылкаРешение			= "https://sbis.ru/help/integration/1C_set/modul/typical_errors/activex_component";
		
	ИначеЕсли Результат.code = 775 Тогда
		
		сбисОткрытьПредупреждение	= Истина;
		ТекстПредупреждение			= Результат.message + " (" + Результат.details + ")";
		АдресСсылкаРешение			= "https://online.sbis.ru/reg/?check=ExtSDK2";
