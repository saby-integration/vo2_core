...
Процедура Disconnected(РезультатВызова, ПараметрыОбработки) Экспорт 
	...
	Если Кэш.СБИС.ОбменВключен Тогда
		Кэш.СБИС.Вставить("ОбменВключен", Ложь); 
		ОткрытиеФормыЗадано = Ложь;
		ОтложенныеОперации = МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("СбисОтложенныеОперации"); 
		Если Не ОтложенныеОперации = Неопределено Тогда
			ОтложенныеОперацииСоотвествие = ОтложенныеОперации.Процедуры;
			Для каждого ЗначениеОперации Из ОтложенныеОперацииСоотвествие Цикл
				Если ЗначениеОперации.Значение.ОписаниеОповещения.ИмяПроцедуры = "ОткрытьФормуНастроекСоединения" Тогда
					ОткрытиеФормыЗадано = Истина; 
					Прервать;
				КонецЕсли; 
			КонецЦикла;
		КонецЕсли;
		Если Не ОткрытиеФормыЗадано Тогда
			
			// Откроем форму настроек соединения для смены способа обмена
			...
			ОтложенноВыполнить = МодульОбъектаКлиент().НовыйОтложенноеДействие(ПараметрыОткрытияФормы); 
			МодульОбъектаКлиент().ПодключитьОтложенноеДействие(ОтложенноВыполнить);  
