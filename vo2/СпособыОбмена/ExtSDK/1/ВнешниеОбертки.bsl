...
Функция СБИС_СохранитьПоСсылкеВФайл(Кэш, ПараметрыФайла, ДопПараметры, Отказ) Экспорт
	...
	Если Отказ Тогда 
		СообщениеОбОшибке = "Не удалось скачать файл по ссылке: " + ПараметрыЗаписи.uri_in;
...
Функция СБИС_СписокДокументов(Кэш, filter, ДопПараметры, Отказ) Экспорт
	...
	Результат = Кэш.Интеграция.СБИСПлагин_СписокДокументов(Кэш, filter, ДопПараметры, Отказ);
	Если Отказ Тогда
