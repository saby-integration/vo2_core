...
Функция СбисОтправитьИОбработатьКоманду(Кэш,Метод,ПараметрыМетода=Неопределено,ДопПараметры,Отказ) Экспорт
	...
	ДопПараметрыДляВызовов	= Новый Структура("Кэш, ДополнительныеПараметры, Метод, ИмяМодуля", Кэш, ДопПараметры, Метод, СведенияОбИнтеграции.Параметры.ИмяВнешнегоМодуля);
	...
	СобытияКоманды.Вставить("CriticalError",	Новый Структура("Функция, Модуль",					"СбисОтправитьИОбработатьКоманду_Ошибка",	ЭтаФорма));
	...
	Если	Результат = Неопределено
		ИЛИ	Результат.Тип = "Error"
		Или	Результат.Тип = "CriticalError"Тогда
		Отказ = Истина;
	КонецЕсли;
		
	Если Результат = Неопределено Тогда
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(,
			АсинхроннаяСбисКоманда.ПараметрыСобытий.ИмяМодуля + "." + АсинхроннаяСбисКоманда.АргументВызова.Метод,
			775,
			"Не получен ответ от SABY плагина");
	Иначе
...
Процедура СбисОтправитьИОбработатьКоманду_Ответ(РезультатВызова, ПараметрыОбработки) Экспорт
	// Не удалять. Если нужно обрабатывать ответ - добавь свой код.
КонецПроцедуры
...
Функция НовыйСбисПлагинКоманда(Кэш, АсинхроннаяСбисКоманда, ДопПараметры=Неопределено) Экспорт
	...
	СбисИдЗапроса		= МодульОбъектаКлиент.ПрикладнойОбъектSABY_Получить(АсинхроннаяСбисКоманда, "Идентификатор");
	СбисМодуль			= СведенияОбИнтеграции.Параметры.ИмяВнешнегоМодуля;
	Если СбисПараметрыМетода = Неопределено Тогда
		СбисПараметрыМетода = Новый Структура;
	КонецЕсли;
	
	//КомандаПлагина = МодульОбъектаКлиент().НовыйПрикладнойОбъектSABY("КомандаТранспорта");
	//КомандаПлагина.Вставить("Метод",		СбисМетод);
	//КомандаПлагина.Вставить("Параметры", 	СбисПараметрыМетода);
	//КомандаПлагина.Вставить("Модуль",		СбисМодуль);
	//КомандаПлагина.Вставить("События",		Новый Структура);
	//КомандаПлагина.Вставить("Ответ");
	//КомандаПлагина.Вставить("Аккаунт");
	//
	//МодульОбъектаКлиент().ПрикладнойОбъектSABY_Установить(КомандаПлагина, "Идентификатор",			СбисИдЗапроса);
	//МодульОбъектаКлиент().ПрикладнойОбъектSABY_Установить(КомандаПлагина, "ВремяОжиданияОтвета",	СбисОжиданиеОтвета);
	//
	//Если ДопПараметры = Неопределено Тогда
	//	
	//	КомандаПлагина.Аккаунт = Кэш.СБИС.ДанныеИнтеграции.Объекты["Форма_" + СбисМодуль].СбисИдАккаунта(Кэш);
	//	Возврат КомандаПлагина;
	//	
	//КонецЕсли;
