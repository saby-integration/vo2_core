...
Процедура WriteDocumentEx_After_Call(РезультатВызова, ПараметрыВызова) Экспорт
	...
	Отправка_ДобавитьСтатусВРезультатПоПакету(Кэш, РезультатВызова.Идентификатор,		ПараметрыВызова.СоставПакета);
	Кэш.РезультатОтправки.СформированныеПакеты.Вставить(РезультатВызова.Идентификатор,	ПараметрыВызова.СоставПакета); // складываем сюда пакеты, чтобы можно было переотправить. После получения ответов - чистим, чтобы не переполнилась память
КонецПроцедуры
...
Процедура WriteDocumentEx_FatalError(РезультатВызова, ПараметрыОбработки) Экспорт
	...
	КонецПопытки;
	ПараметрыОбработки.Кэш.РезультатОтправки.ДетализацияОтправки.Удалить(ПараметрыОбработки.Команда.Идентификатор);
