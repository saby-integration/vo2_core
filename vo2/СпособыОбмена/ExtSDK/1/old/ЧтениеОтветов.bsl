...
Функция СБИСПлагин_ЧтениеОтветовИВызовСобытий(Кэш, Сбис3КомандыОжидаются=Неопределено, ДопПараметры=Неопределено, Отказ)
	...
	Для Каждого Сбис3СобытиеИзСтека Из СбисСобытия Цикл
		...
		ИдентификаторСобытия	= МодульОбъектаКлиент().АсинхронноеСбисСобытие_Получить(Сбис3СобытиеОтвет, "Идентификатор");
...
Процедура СБИСПлагин_ЗанятьПоток(Кэш, ИдентификаторПотока, Сбис3Команда) Экспорт
	...
	Если Кэш.Парам.РежимОтладки Тогда
		...
		лОтладочныеДанные = Новый Структура("Тип, Модуль, Вызов, Время, Идентификатор, Сообщение", "CALL", Сбис3Команда.Модуль, Сбис3Команда.Метод, Сбис3Команда.ВремяВызова, ИдентификаторОтправки, Кэш.РаботаСJson.ПреобразоватьЗначениеВJson(Сбис3Команда.Параметры));
		ДанныеВЛог = Новый Массив;
		ДанныеВЛог.Добавить(лОтладочныеДанные);
		Кэш.ОбщиеФункции.сбисСохранитьОтладочныеДанные(Кэш, Новый Структура("Log", ДанныеВЛог));
	КонецЕсли;
...
Функция СБИСПлагин_ОсвободитьПоток(Кэш, СбисОписаниеСобытия) Экспорт
	ИдентификаторСобытия	= МодульОбъектаКлиент().АсинхронноеСбисСобытие_Получить(СбисОписаниеСобытия, "Идентификатор");
	...
	Если Не КомандаВыполнена = Неопределено Тогда
		...
		Если Кэш.Парам.РежимОтладки Тогда
			...
			КонецЕсли;
			лОтладочныеДанные = Новый Структура("Тип, Модуль, Вызов, Время, Идентификатор, Сообщение", СбисТипОтвета, КомандаВыполнена.Модуль, КомандаВыполнена.Метод, ТекущаяДата(), ИдентификаторСобытия, СбисТекстОтвета);
			ДанныеВЛог = Новый Массив;
			ДанныеВЛог.Добавить(лОтладочныеДанные);
			Кэш.ОбщиеФункции.сбисСохранитьОтладочныеДанные(Кэш, Новый Структура("Log", ДанныеВЛог));
		КонецЕсли;
