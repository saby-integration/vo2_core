...
#Область include_core2_vo2_СпособыОбмена_ExtSDK_2_ВнешниеОбертки_СинхронныеОперации_СБИС
#КонецОбласти
...
Функция СБИС_СериализоватьСтрокуВBase64(Кэш, ПараметрыСериализовать, ДопПараметры, Отказ) Экспорт
	Возврат Форма_ExtSDK.СБИС_СериализоватьСтрокуВBase64(Кэш, ПараметрыСериализовать, ДопПараметры, Отказ);
КонецФункции

&НаКлиенте
Функция СБИС_СериализоватьФайлВBase64(Кэш, ПараметрыСериализовать, ДопПараметры, Отказ) Экспорт
	Возврат Форма_ExtSDK.СБИС_СериализоватьФайлВBase64(Кэш, ПараметрыСериализовать, ДопПараметры, Отказ);
КонецФункции
...
Функция Интеграция_ФЭДМультиСгенерировать(ПараметрыДокумента, НаборПодстановок, ДопПараметры) Экспорт
	...
	Исключение
		...
		МодульОбъектаКлиент().ВызватьСбисИсключение(ИнфоОбОшибке, МодульОбъектаКлиент().ПараметрSaby("ИнтеграцияИмя") + ".Интеграция_ФЭДМультиСгенерировать");
	КонецПопытки;
...
&НаКлиенте
Функция СБИС_ЗаписатьДокумент(Кэш, document_in, ДопПараметры=Неопределено, Отказ=Ложь) Экспорт 
	...
	МассивПараметров = Новый Массив;
	МассивПараметров.Добавить(document_in);

	СтруктураПараметровЗапроса	= Новый Структура("Документ", document_in);
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "СБИС.ЗаписатьДокумент", МассивПараметров, ДопПараметры, Отказ);
	Если Отказ Тогда
		Возврат Кэш.ОбщиеФункции.сбисИсключение(Результат,  МодульОбъектаКлиент().ПараметрSaby("ИнтеграцияИмя") + ".СБИС_ЗаписатьДокумент");
	КонецЕсли;
	Возврат Результат;
...
#Область include_core2_vo2_СпособыОбмена_ExtSDK_2_ВнешниеОбертки_Госсистемы
#КонецОбласти
...
#Область include_core2_vo2_СпособыОбмена_ExtSDK_2_ВнешниеОбертки_РаботаСЗаказами
#КонецОбласти
