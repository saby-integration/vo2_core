...
Функция СБИС_ОбработатьСобытиеОтвета(Сбис3СобытиеОтвет, ДопПараметры=Неопределено) Экспорт
	...
	Если		Сбис3СобытиеОтвет.type = "Message" Тогда	//parse_Message
		...
		ВерсияФормата = Сбис_ФорматОбработкиОтвета(Сбис3СобытиеОтвет, ДопПараметры=Неопределено);
...
Функция Сбис_ФорматОбработкиОтвета(Сбис3СобытиеОтвет, ДопПараметры=Неопределено)
	АсинхронныйПоток = МодульОбъектаКлиент().АсинхронноеСбисСобытие_ПолучитьПоток(Сбис3СобытиеОтвет);
	Если ТипЗнч(АсинхронныйПоток) <> Тип("Структура") Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если АсинхронныйПоток.Свойство("ПараметрыСобытий") = Ложь Тогда
		Возврат Неопределено;
	КонецЕсли;
	Если АсинхронныйПоток.ПараметрыСобытий.Свойство("ДополнительныеПараметры") = Ложь Тогда 
		Возврат Неопределено;
	КонецЕсли;	             
	Если АсинхронныйПоток.ПараметрыСобытий.ДополнительныеПараметры.Свойство("ВерсияФорматаОтвета") = Ложь Тогда
		Возврат Неопределено;	
	КонецЕсли;
	Возврат АсинхронныйПоток.ПараметрыСобытий.ДополнительныеПараметры.ВерсияФорматаОтвета;
КонецФункции
