...
//
&НаКлиенте
Функция СБИС_ПрочитатьПулОтветов(КэшСБИС, ВерсияФормата = Неопределено) Экспорт
	
	Попытка
		СтрокаXML	= ПлагинКоннектор.ReadAllXml2();
		СбисСобытия	= СБИС_РазобратьОтвет(СтрокаXML, ВерсияФормата);
	Исключение
...
Функция СБИС_ОбработатьОтвет(Сбис3СобытиеОтвет, ДопПараметры=Неопределено) Экспорт
	...
	ТретийФормат =	ОбработанноеСобытие.Свойство("ВерсияФормата")
				И	ОбработанноеСобытие.ВерсияФормата = 3;
				
	Если ТретийФормат Тогда
		ОбрезатьЛишнее = 	ОбработанноеСобытие.data.Получить("Stat") <> Неопределено
						И	ОбработанноеСобытие.data.Получить("Result") <> Неопределено;
	Иначе
		ОбрезатьЛишнее = 	ОбработанноеСобытие.data.Свойство("Stat")
						И	ОбработанноеСобытие.data.Свойство("Result");
	КонецЕсли;
				
	
	Если ОбрезатьЛишнее Тогда
		// Обрезать лишнее, нужен только результат
		ОбработанноеСобытие.data = ОбработанноеСобытие.data["Result"];
	КонецЕсли;
...
Функция СБИС_ОбработатьСобытиеОтвета(Сбис3СобытиеОтвет, ДопПараметры=Неопределено) Экспорт
	...
	Если		Сбис3СобытиеОтвет.type = "Message" Тогда	//parse_Message
		...
		Сбис3СобытиеОтвет.Вставить("Идентификатор", Сбис3СобытиеОтвет.queryId);
		СБИС_РазобратьСобытиеОтвета(Сбис3СобытиеОтвет, Сбис_ФорматОбработкиОтвета(Сбис3СобытиеОтвет));
		
	ИначеЕсли	Сбис3СобытиеОтвет.type = "Event" Тогда		//parse_Event
		
		СБИС_РазобратьСобытиеОтвета(Сбис3СобытиеОтвет);
		...
		Иначе	
			...
			Если ТипЗнч(Сбис3СобытиеОтвет.data) = Тип("Строка") Тогда
				...
				Если	Найти(Сбис3СобытиеОтвет.data, "disconnect")
					...
					Или	Сбис3СобытиеОтвет.data = "Server not found"
					Или	(	Найти(Сбис3СобытиеОтвет.data, "Error method ExtSdk2.")
						И	Найти(Сбис3СобытиеОтвет.data, "in CallMethod")) Тогда  
						Сбис3СобытиеОтвет.data = МодульОбъектаКлиент().НовыйСбисИсключение(
						412,	"МодульОбъектаКлиент.СБИСПлагин_ЧтениеОтветовИВызовСобытий",,
						, "Работа Saby плагина была остановлена. Перезапустите Saby Плагин для дальнейшей работы.",, "https://online.saby.ru/reg/?check=ExtSDK2");
				...
				КонецЕсли;
			ИначеЕсли ТипЗнч(Сбис3СобытиеОтвет.data) = Тип("Структура") 
					И	Сбис3СобытиеОтвет.data.Свойство("detail")
	...
	// Может быть как Эвент, так и Эррор
	Если ТипЗнч(Сбис3СобытиеОтвет.data) = Тип("Структура")
		И	Сбис3СобытиеОтвет.data.Свойство("message") Тогда 
		
		Если Найти(Сбис3СобытиеОтвет.data.message, "В аккаунте отсутствует лицензия или имеется задолженность") Тогда
			
			СбисИсключение = МодульОбъектаКлиент().НовыйСбисИсключение(Сбис3СобытиеОтвет.data,
			"МодульОбъектаКлиент.СБИСПлагин_ЧтениеОтветовИВызовСобытий",
			335);
			
			Сбис3СобытиеОтвет.data.detail = Сбис3СобытиеОтвет.data.message;
			Сбис3СобытиеОтвет.data.code = 335;
			//ДопПараметры = Новый Структура("queryID", "No license");
			АсинхСобытие = МодульОбъектаКлиент().НовыйАсинхронноеСбисСобытие("No license", СбисИсключение, 
			"Event", ДопПараметры);
			МодульОбъектаКлиент().ГлобальныйКэш_ДобавитьСобытие(АсинхСобытие);
			
		ИначеЕсли Найти(Сбис3СобытиеОтвет.data.message, "авторизоваться") Тогда
			
			СбисИсключение = МодульОбъектаКлиент().НовыйСбисИсключение(Сбис3СобытиеОтвет.data,
			"МодульОбъектаКлиент.СБИСПлагин_ЧтениеОтветовИВызовСобытий",
			304103, "Недействительная сессия", "Ошибка авторизации при вызове RPC-функции");
			
			Сбис3СобытиеОтвет.data.detail = Сбис3СобытиеОтвет.data.message;
			Сбис3СобытиеОтвет.data.code = 304103;
			//ДопПараметры = Новый Структура("queryID", "No license");
			АсинхСобытие = МодульОбъектаКлиент().НовыйАсинхронноеСбисСобытие("UnValidToken", СбисИсключение, 
			"Event", ДопПараметры);
			МодульОбъектаКлиент().ГлобальныйКэш_ДобавитьСобытие(АсинхСобытие);
...
&НаКлиенте
Процедура СБИС_РазобратьСобытиеОтвета(Сбис3СобытиеОтвет, ВерсияФормата = 2)
	
	Сбис3СобытиеОтвет.Вставить("ВерсияФормата", ВерсияФормата);
	Сбис3СобытиеОтвет.data = СБИС_РазобратьОтвет(Сбис3СобытиеОтвет.data, ВерсияФормата);
...
Функция СБИС_ИдЗапросаВызова(Кэш) Экспорт
	...
	Возврат Строка(ПлагинКоннектор.GenerateQueryID());
	
КонецФункции

// Функция по потоку определяет формат желаемого ответа
//
// Параметры:
//  Сбис3СобытиеОтвет	- Сообщение из пула ответов	 - 
// 
// Возвращаемое значение:
// Формат ответа  - произвольный
//
&НаКлиенте
Функция Сбис_ФорматОбработкиОтвета(Сбис3СобытиеОтвет, ДопПараметры = Неопределено)
	
	ВерсияФорматаОтвета = МодульОбъектаКлиент().АсинхронноеСбисСобытие_ПолучитьПоток(Сбис3СобытиеОтвет);
	Если		ВерсияФорматаОтвета = Неопределено
		Или	Не	ВерсияФорматаОтвета.Свойство("ПараметрыСобытий",		ВерсияФорматаОтвета)
		Или	Не	ВерсияФорматаОтвета.Свойство("ДополнительныеПараметры", ВерсияФорматаОтвета)
		Или	Не	ВерсияФорматаОтвета.Свойство("ВерсияФорматаОтвета",		ВерсияФорматаОтвета) Тогда
		
		Возврат Неопределено;
		
	КонецЕсли;
	
	Возврат ВерсияФорматаОтвета;

КонецФункции

&НаКлиенте
Функция СБИС_РазобратьОтвет(ДанныеИзПулаОтвета, ВерсияФормата = Неопределено) Экспорт
	...
	Попытка
		Если ВерсияФормата = 3 Тогда
			Результат = XmlSerializer_DecodeNode3(Node, Новый Структура("Тип,Путь",Node.Имя,"Ответ"));
		Иначе	
...
Функция СБИС_ОтправитьКомандуБезОбработки(Кэш, ПараметрыСБИС3Команды, ДопПараметры, Отказ) Экспорт
	...
	Если Не Кэш.СБИС.ДанныеИнтеграции.Свойство("Объекты", ПараметрПроверка) Тогда
		Отказ = Истина;
		Возврат Кэш.ОбщиеФункции.СбисИсключение(,Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя + ".СБИС_ОтправитьКомандуБезОбработки", 700,,"Не подключена подсистема взаимодействия с Saby плагин");
	...
	Иначе
		...
		Если Не ПараметрПроверка.Свойство(ПараметрыСБИС3Команды.Модуль, Sbi3Module_ID) Тогда
			Sbi3Module_ID = Форма_ExtSDK.СБИСПлагин_GetModule(Кэш, ПараметрыСБИС3Команды.Модуль, Отказ);
			Если Отказ Тогда
