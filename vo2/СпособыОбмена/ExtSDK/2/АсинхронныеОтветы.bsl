...
Функция СБИС_ОбработатьСобытиеОтвета(Сбис3СобытиеОтвет, ДопПараметры=Неопределено) Экспорт
	...
	Если ТипЗнч(Сбис3СобытиеОтвет.data) = Тип("Структура")
			И	Сбис3СобытиеОтвет.data.Свойство("message")
			И	Найти(Сбис3СобытиеОтвет.data.message, "В аккаунте отсутствует лицензия или имеется задолженность") Тогда
		...
		ОтложенныйСбис3СобытиеОтвет = Новый Структура;
		ОтложенныйСбис3СобытиеОтвет.Вставить("data", СбисИсключение);
		ОтложенныйСбис3СобытиеОтвет.Вставить("queryID", "No license");
		ОтложенныйСбис3СобытиеОтвет.Вставить("type", "Event");
		ОтложенныйСбис3СобытиеОтвет.Вставить("Идентификатор", Сбис3СобытиеОтвет.Идентификатор);
		
		ГенерируемоеСобытие = Новый Структура("Сбис3СобытиеОтвет, ВПроцессе", ОтложенныйСбис3СобытиеОтвет, Ложь);
		Кэш.СБИС.ДанныеИнтеграции.ГенерируемыеСобытия.Вставить(ОтложенныйСбис3СобытиеОтвет.Идентификатор, ГенерируемоеСобытие);
