...
Функция Включить(Кэш, ДопПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	РезультатВключения = ФормаExtSDK.Включить(Кэш, ДопПараметры, Отказ);
...
Функция СведенияОбИнтеграции() Экспорт
	...
	ФормаExtSDK = МодульОбъектаКлиент().МодульИнтеграцииСБИС("ExtSDK");
...
Функция СБИС_СписокДокументовПоСобытиям(Кэш, filter, ДопПараметры, Отказ) Экспорт
	...
	Возврат Кэш.СБИС.ДанныеИнтеграции.Объекты.Форма_ExtSDK.СБИС_СписокДокументовПоСобытиям(Кэш, filter, ДопПараметры, Отказ);
	
КонецФункции

#Область include_core2_vo2_СпособыОбмена_ExtSDKCrypto_1_ВнешниеОбертки
#КонецОбласти
