...
Функция ExtSysNom_MoveToPack(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.Filter);
...
Функция ExtSysNom_GetGln(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.GetGln", ПараметрыМетода, ДопПараметрыВызова, Отказ);   
...
Функция ExtSysNom_GeneratePackage(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.doc_id);
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.nom_id);
	Если ВходящийПараметрМетода.Свойство("template_name") Тогда
		ПараметрыМетода.Добавить(ВходящийПараметрМетода.template_name);
	КонецЕсли;
...
Функция РеалИсх_GeneratePack(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "РеалИсх.GeneratePack", ПараметрыМетода, ДопПараметры, Отказ);   
