...
#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки_АсинхроннаяОтправка
#КонецОбласти

#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки_СопоставлениеНоменклатуры
#КонецОбласти

#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки_АсинхронныеОперации
#КонецОбласти
 
#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки_СинхронныеОперации
#КонецОбласти 
...
&НаКлиенте
Функция СБИС_СформироватьHTMLИзXML(Кэш, ПараметрыЗапроса, ДопПараметры = Неопределено, Отказ) Экспорт
			
	РезультатЗапроса = СбисОтправитьИОбработатьКоманду(Кэш, "GenerateHTMLFromXML", Новый Структура("XML", ПараметрыЗапроса.XML), ДопПараметры, Отказ);
		
	Если Отказ Тогда

		Возврат МодульОбъектаКлиент().NewExtException(РезультатЗапроса, "ExtSDK2.СформироватьHTMLИзXML");
...
&НаКлиенте
Функция СБИС_СформироватьHTMLИзXML_Локально(Кэш, ПараметрыЗапроса, ДопПараметры, Отказ) Экспорт

	РезультатЗапроса = СбисОтправитьИОбработатьКоманду(Кэш, "GenerateHTMLFromXMLClient", Новый Структура("Parameter", ПараметрыЗапроса), ДопПараметры, Отказ);
...
Функция СБИС_СохранитьПоСсылкеВФайл(Кэш, ПараметрыФайла, ДопПараметры, Отказ) Экспорт
	...
	Если Отказ Тогда 
		СообщениеОбОшибке = "Не удалось скачать файл по ссылке: " + ПараметрыЗаписи.Url;
...
&НаКлиенте
Функция СБИС_СписокДокументов(Кэш, filter, ДопПараметры, Отказ)
	
	Результат = СБИСПлагин_СписокДокументов(Кэш, filter, ДопПараметры, Отказ);
	Если Отказ Тогда
		Результат = Кэш.ОбщиеФункции.СбисИсключение(Результат, Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя + ".СБИС_СписокДокументов");
	КонецЕсли;
	Возврат Результат;
...
Функция Интеграция_ФЭДСгенерировать(Кэш, ПараметрыДокумента, Генератор, ДопПараметры, Отказ) Экспорт
	...
	Иначе
		Результат = МодульОбъектаКлиент().ПрочитатьЗначениеПоBase64Клиент(Результат);
	КонецЕсли;
...
#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки_Госсистемы
#КонецОбласти
...
Функция ZakupkiGovAPI_GetContractInfo(Кэш, ПараметрыМетода, ДопПараметры, Отказ = Ложь) Экспорт
	...
	ДопПараметры.Вставить("АдресРесурса", "/service/");

	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi(Кэш, "ZakupkiGovAPI.GetContractInfo", ПараметрыМетода, ДопПараметры, Отказ);
...
Функция ExtSysAdditionalFields_GetList(Кэш, ПараметрыМетода, ДопПараметры, Отказ = Ложь) Экспорт
	...
	ДопПараметрыВызова = Новый Структура("ВернутьОшибку, СообщатьПриОшибке", Истина, Ложь);  
	ПараметрыКоманды = Новый Массив();  
	ПараметрыКоманды.Добавить(ПараметрыМетода.docflow_id);
	ПараметрыКоманды.Добавить(ПараметрыМетода.params);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysAdditionalFields.GetList", ПараметрыКоманды, ДопПараметрыВызова, Отказ);
	
	Если Отказ Тогда
		Возврат Кэш.ОбщиеФункции.сбисИсключение(Результат,  Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя + ".ExtSysAdditionalFields_GetList");
	КонецЕсли;
...
Функция SearchNomenclatureCollation(Кэш, params, ДопПараметры, Отказ=Ложь) Экспорт 
	...
	Возврат БазоваяИнтеграция.СбисОтправитьИОбработатьКоманду(Кэш, "CallSabyApi2", ПараметрыВызова, ДопПараметры,Отказ);
...
Функция ExtSysDocument_Changed(params, ДопПараметры, Отказ = Ложь) Экспорт   
	...
	ПараметрыКоманды = Новый Массив();   
	ПараметрыКоманды.Добавить(params.ИдДок);    
	ПараметрыКоманды.Добавить(params.ФлагИзменен);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysDocument.Changed", ПараметрыКоманды, ДопПараметрыВызова, Отказ);   
...
Функция ExtSysDocument_SetDevStatus(params, ДопПараметры, Отказ = Ложь) Экспорт   
	...
	ПараметрыКоманды = Новый Массив();   
	ПараметрыКоманды.Добавить(params.ИдДок);    
	ПараметрыКоманды.Добавить(params.ФлагРасхождения);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysDocument.SetDevStatus", ПараметрыКоманды, ДопПараметрыВызова, Отказ);   
...
Функция ExtSysDocument_WriteMapping(params, ДопПараметры, Отказ = Ложь) Экспорт   
	...
	ДопПараметрыВызова = Новый Структура("ВернутьОшибку, СообщатьПриОшибке", Истина, Ложь);  
	ПараметрыКоманды = Новый Массив();   
	ПараметрыКоманды.Добавить(params.Data);    
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysDocument.WriteMapping", ПараметрыКоманды, ДопПараметрыВызова, Отказ);   
...
//
&НаКлиенте
Функция ExtSysOrganization_WriteMapping(params, ДопПараметры, Отказ = Ложь) Экспорт   
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysOrganization.WriteMapping", ПараметрыКоманды, ДопПараметрыВызова, Отказ);   
	
	Если Отказ Тогда 
		
		ИнтеграцияИмя  = МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ИнтеграцияИмя");
		Результат = МодульОбъектаКлиент().НовыйСбисИсключение(Результат,  ИнтеграцияИмя + ".ExtSysOrganization_WriteMapping"); 
...
Функция ExtSysLicense_GetInfo(params,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода.Добавить(params);
	
	ДопПараметрыВызова = Новый Структура("ВернутьОшибку, СообщатьПриОшибке", Истина, Ложь);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysLicense.GetInfo", ПараметрыМетода, ДопПараметрыВызова, Отказ);   
...
Функция ExtSysLicense_GetPaymentLink(params,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysLicense.GetPaymentLink", ПараметрыМетода, ДопПараметрыВызова, Отказ);   
...
Функция ExtSys_GetImageByLink(params,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSys.GetImageByLink", ПараметрыМетода, ДопПараметрыВызова, Отказ);   
...
Функция ExtSysPerson_GetStaffPhoto(params,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysPerson.GetStaffPhoto", ПараметрыМетода, ДопПараметрыВызова, Отказ);   
...
Функция ExtSysNom_CustomDocNomUpdate(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.Filters);
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.AddFields);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.CustomDocNomUpdate", ПараметрыМетода, ДопПараметры, Отказ);   
...
Функция ExtSysNom_MoveToPack(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.Filters);
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.RootId);
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.AddFields);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.MoveToPack", ПараметрыМетода, ДопПараметры, Отказ);   
...
Функция ExtSysNom_UnPack(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.UnPack", ПараметрыМетода, ДопПараметры, Отказ);   
...
Функция ExtSysNom_DeleteDnBatchAndRecalcQty(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.DeleteDnBatchAndRecalcQty", ПараметрыМетода, ДопПараметры, Отказ);   
...
Функция ExtSysNom_CustomCreate(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.Метод);
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.Фильтр);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.CustomCreate", ПараметрыМетода, ДопПараметры, Отказ);   
...
Функция ExtSysNom_GetGln(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.Документ); 
	
	ДопПараметрыВызова = Новый Структура("ВернутьОшибку, СообщатьПриОшибке", Истина, Ложь); 
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.GetGln", ПараметрыМетода, ДопПараметры, Отказ);   
...
Функция ExtSysNom_GeneratePackage(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.GeneratePackage", ПараметрыМетода, ДопПараметры, Отказ);   
...
Функция ExtSysNom_GeneratePack(Кэш, ВходящийПараметрМетода,  ДопПараметры, Отказ = Ложь) Экспорт
	...
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.filters);
	ПараметрыМетода.Добавить(ВходящийПараметрМетода.params);
	
	Результат = СбисОтправитьИОбработатьКомандуCallSabyApi2(Кэш, "ExtSysNom.GeneratePack", ПараметрыМетода, ДопПараметры, Отказ);   
...
#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки_РаботаСЗаказами
#КонецОбласти

#Область include_core2_vo2_СпособыОбмена_NativeSDK_ВнешниеОбертки_ПрикладнаяСтатистика
#КонецОбласти
