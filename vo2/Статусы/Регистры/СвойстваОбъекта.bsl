...
Функция СбисПолучитьКлючСвойстваДокументаНаКлиенте(ОписаниеСвойства)
	...
	Если	ОписаниеСвойства.Свойство("Регламент", СбисРегламент)
		...
		СимволовОстаток = ПараметрыСоединения.ДлинаИмениПланаВидовХарактеристикСвойствОбъектов - СтрДлина(КлючСвойства) - 1;
		СбисРегламент	= Сред(СбисРегламент, 1, СимволовОстаток);
...
Процедура ДобавитьЗаписиВПланВидовХарактеристикНаСервере()
	...
	СправочникСвойствОбъектов 					= ПараметрыСоединения.ИмяСправочникаСвойствОбъектов; 
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ОшибкаСоздания = МодульОбъектаСервер().НовыйСБисИсключение(ИнформацияОбОшибке(),"Статусы_Регсистры.ДобавитьЗаписиВПланВидовХарактеристикНаСервере",767301,
		"Ошибка создания дополнительных свойств");	
		ВызватьИсключение МодульОбъектаСервер().СбисИсключение_Выгрузить(ОшибкаСоздания);
	КонецПопытки;
	
	Если Отказ = Истина Тогда
		
		ОшибкаСоздания = МодульОбъектаСервер().НовыйСБисИсключение(502100,"Статусы_Регсистры.ДобавитьЗаписиВПланВидовХарактеристикНаСервер",,, "Возникли проблемы при создании дополнительных свойств для документов и контрагентов.");	
		ВызватьИсключение МодульОбъектаСервер().СбисИсключение_Выгрузить(ОшибкаСоздания);
...
Функция СбисДобавитьЗаписьВПланВидовХарактеристик(ОписаниеСвойства)
	...
	Если	(	Не ОписаниеСвойства.Свойство("Назначение", НазначениеСвойства)
			Или	Не ЗначениеЗаполнено(НазначениеСвойства))
		И	Не Метаданные.ПланыВидовХарактеристик.Найти("НазначенияСвойствКатегорийОбъектов") = Неопределено Тогда
	...
	СтруктураСвойства = Новый Структура("Виден, Доступен, ЭтоДополнительноеСведение", Истина, Истина, Истина);
	СтруктураСвойства.Вставить("Ссылка",				НайденнаяЗапись);
	СтруктураСвойства.Вставить("Заголовок",				КлючСвойства);
	СтруктураСвойства.Вставить("Наименование",			КлючСвойства);
	СтруктураСвойства.Вставить("Имя",					КлючСвойства);
	СтруктураСвойства.Вставить("Комментарий",			"Свойство для работы обработки Saby");
	
	Если Не Метаданные.ПланыВидовХарактеристик[ИмяПланаВидовХарактеристикСвойствОбъектов].Реквизиты.Найти("ВидСвойства") = Неопределено Тогда
		
		СтруктураСвойства.Вставить("ВидСвойства", Перечисления.ВидыСвойств.ДополнительныеСведения);
		
	КонецЕсли;
	
	Если Не НазначениеСвойства = Неопределено Тогда
		
		СтруктураСвойства.Вставить("НазначениеСвойства", НазначениеСвойства);
		
	КонецЕсли;
	
	Если	НайденнаяЗапись.Пустая()
		Или МодульОбъектаСервер().ДанныеОбъектовОтличаются(НайденнаяЗапись, СтруктураСвойства) Тогда
		
		ДопОписаниеСвойства = Новый Структура("ТипОбъекта, ПодТипОбъекта", "ПланВидовХарактеристик", ПараметрыСоединения.ИмяПланаВидовХарактеристикСвойствОбъектов);
		Попытка
			
			МодульОбъектаСервер().ОбновитьОбъектПоНашимДанным(СтруктураСвойства, ДопОписаниеСвойства);
			
		Исключение
			
			ВызватьИсключение МодульОбъектаСервер().СбисИсключение_Выгрузить(МодульОбъектаСервер().НовыйСбисИсключение(ИнформацияОбОшибке(), "МодульОбъектаСервер.НовыйОбъект1С", 709101, 
	...
	Возврат СтруктураСвойства.Ссылка;
