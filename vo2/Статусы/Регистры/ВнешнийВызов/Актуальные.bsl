...
Функция СбисЗаписатьИзмененияПоДокументам1С(Кэш, МассивДокументов, ДопПараметры)	Экспорт
	Перем СбисСтатусРегламент, СбисИдАккаунта, ОбъектМассовоеДействие;
	...
	ДопПараметры.Свойство("МассовоеДействие",	ОбъектМассовоеДействие);
	...
	Для Каждого СтрокаРезультатОбработки Из РезультатОбработки Цикл
		...
		Для Каждого ОбработаннаяСтрока Из СтрокаРезультатОбработки.Значение Цикл
			...
			Если ОбработаннаяСтрока.Отказ Тогда
				
				ИсключениеОбработкиСтатуса = МодульОбъектаКлиент().НовыйСбисИсключение(ОбработаннаяСтрока.Ошибка, "Статусы_Региcтры.СбисЗаписатьИзмененияПоДокументам1С");
				Если ОбъектМассовоеДействие = Неопределено Тогда
					
					МодульОбъектаКлиент().СбисСообщить(ИсключениеОбработкиСтатуса);
				...
				Иначе
					...
					Кэш.ОбщиеФункции.РезультатДействия_ДобавитьОшибку(Кэш, ОбъектМассовоеДействие, СтрокаДетализации, ИсключениеОбработкиСтатуса);
...
Функция СбисПрочитатьЗначениеСвойстваДокумента(ПараметрыСвойства, Кэш=Неопределено)	Экспорт
	...
	Возврат ?(ЗначениеСвойств = Неопределено, Неопределено, ЗначениеСвойств.Получить(ПараметрыСвойства.Имя));
КонецФункции
...
Функция СбисРазобратьИдентификатор(ИдПакета, ДопПараметры=Неопределено)				Экспорт
	...
	Иначе
		...
		// В составном ИД, ид аккаунта всегда в конце
	    ИдАккаунта = СтрПолучитьСтроку(ИдПакета, СтрЧислоСтрок(ИдПакета));
...
&НаКлиенте
Функция _Удалить_СбисПолучитьСписокДокументов1С(ПараметрыСпискаДокументов, Кэш)		Экспорт
	ТекстЗапроса = Кэш.ОбщиеФункции.РассчитатьЗначение("ЗапросСпискаДокументов", ПараметрыСпискаДокументов.Ини);
	
	Реестры1С_ХранениеСтатусов_ДополнитьТекстЗапроса(ТекстЗапроса);
...
// Процедура - дополняет текст запроса из ини ключами свойств
//
// Параметры:
//  ТекстЗапроса - Строка	 - текст для запроса
//
&НаКлиенте
Процедура Реестры1С_ХранениеСтатусов_ДополнитьТекстЗапроса(ТекстЗапроса)								Экспорт
