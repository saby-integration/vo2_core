...
Процедура УстановитьПараметрыРегистры(Кэш)
	...
	ИмяСправочникаСвойствОбъектов				= Кэш.ОбщиеФункции.РассчитатьЗначение("СправочникСвойствОбъектов",					ИниКонфигурация);	
	СтатусРегламент 							= Кэш.ОбщиеФункции.РассчитатьЗначение("СтатусРегламент",							ИниКонфигурация);
	
	Если ИмяСправочникаСвойствОбъектов = Неопределено Тогда
		
		ИмяСправочникаСвойствОбъектов = "ЗначенияСвойствОбъектов";

	КонецЕсли;
	
	ПараметрыЗаполнитьСервер = Новый Структура("ДлинаИмениПланаВидовХарактеристикСвойствОбъектов");
	ПараметрыЗаполнитьСервер.Вставить("ИмяРегистраСвойствОбъектов", 				ИмяРегистраСвойствОбъектов);
	ПараметрыЗаполнитьСервер.Вставить("ПоддержкаСвойстваСпособДоставки", 			Истина);
	ПараметрыЗаполнитьСервер.Вставить("ИмяПланаВидовХарактеристикСвойствОбъектов", 	ИмяПланаВидовХарактеристикСвойствОбъектов);
	ПараметрыЗаполнитьСервер.Вставить("ИмяСправочникаСвойствОбъектов", 				ИмяСправочникаСвойствОбъектов);

	ЗаполнитьПараметрыСтатусовНаСервере(ПараметрыЗаполнитьСервер);
	...
	"	КодСервиса, МассивСтатусРегламент, СвойстваВРазрезеКодаСервиса, СвойстваВРазрезеАккаунта, 
	|	ИмяРегистраСвойствОбъектов, ИмяПланаВидовХарактеристикСвойствОбъектов, ИмяСправочникаСвойствОбъектов, 
	|	ДлинаИмениПланаВидовХарактеристикСвойствОбъектов, ПоддержкаСвойстваСпособДоставки"
	,КодСервиса, МассивСтатусРегламент, СвойстваВРазрезеКодаСервиса, СвойстваВРазрезеАккаунта);
	ЗаполнитьЗначенияСвойств(ПараметрыСоединения, ПараметрыЗаполнитьСервер);
...
// Процедура - заполнить параметры статусов на сервере
//
// Параметры:
//  ПараметрыЗаполнить	 - Структура	 - набор параметров, который надо обработать
//
&НаСервере
Процедура ЗаполнитьПараметрыСтатусовНаСервере(ПараметрыЗаполнить)
	
	Если ПараметрыЗаполнить.ИмяРегистраСвойствОбъектов = Неопределено Тогда
		
		ПараметрыЗаполнить.ИмяРегистраСвойствОбъектов = ИмяРегистраСвойствОбъектовНаСервере();
		
	КонецЕсли;

	Если ПараметрыЗаполнить.ИмяПланаВидовХарактеристикСвойствОбъектов = Неопределено Тогда
		
		ПараметрыЗаполнить.ИмяПланаВидовХарактеристикСвойствОбъектов = ИмяПланаВидовХарактеристикСвойствОбъектовНаСервере();
		
	КонецЕсли;

	Если Не МодульОбъектаСервер().СбисСуществуетОбъектКонфигурацииСервер(	ПараметрыЗаполнить.ИмяСправочникаСвойствОбъектов,
																			Новый Структура("КлассМетаданных", "Справочники")) Тогда
		
		ПараметрыЗаполнить.ПоддержкаСвойстваСпособДоставки = Ложь;
		
	КонецЕсли;
	
	МетаданныеСвойств = МодульОбъектаСервер().ЭлементКоллекцииМетаданныхПоПараметрам(
									ПараметрыЗаполнить.ИмяПланаВидовХарактеристикСвойствОбъектов,
									Новый Структура("КлассМетаданных", "ПланыВидовХарактеристик"));
									
	Если МетаданныеСвойств = Неопределено Тогда
										
		СбисИсключение = МодульОбъектаСервер().НовыйСбисИсключение(	779,
																	"Статусы_Регистры.ЗаполнитьПараметрыСтатусовНаСервере",
																	,
																	,
																	"Не найден план видов характеристик дополнительных свойств """ + ПараметрыЗаполнить.ИмяПланаВидовХарактеристикСвойствОбъектов + """");
		ВызватьИсключение МодульОбъектаСервер().СбисИсключение_Представление(СбисИсключение);
		
	КонецЕсли;
									
	ПараметрыЗаполнить.ДлинаИмениПланаВидовХарактеристикСвойствОбъектов = МетаданныеСвойств.ДлинаНаименования;
	
КонецПроцедуры

&НаСервере
Функция ИмяРегистраСвойствОбъектовНаСервере()
	
	ДопПараметры = Новый Структура("КлассМетаданных", "РегистрыСведений");
	
	Если		МодульОбъектаСервер().СбисСуществуетОбъектКонфигурацииСервер("ДополнительныеСведения", ДопПараметры) Тогда
		Возврат "ДополнительныеСведения";
	ИначеЕсли   МодульОбъектаСервер().СбисСуществуетОбъектКонфигурацииСервер("ЗначенияСвойствОбъектов", ДопПараметры) Тогда
		Возврат "ЗначенияСвойствОбъектов";
	Иначе	
		ВызватьИсключение("Не найден регистр для хранения статусов и идентификаторов Saby");
	КонецЕсли;
	
КонецФункции

&НаСервереБезКонтекста
Функция ИмяПланаВидовХарактеристикСвойствОбъектовНаСервере()
	...
	Иначе
		ВызватьИсключение("Не найден план дополнительных свойств для настройки хранения статусов и идентификаторов Saby");
	КонецЕсли
КонецФункции
