...
Функция ПолучитьСоединениеDBF(ПараметрыСоединенияВходящие=Неопределено, ДопПараметры=Неопределено) Экспорт
	...
	Исключение
		...
		ВызватьИсключение МодульОбъектаСервер.СбисИсключение_ВСтроку(ИнфоОбОшибке, "Статусы_ДБФ.ПолучитьСоединениеDBF");
	КонецПопытки;
	Возврат БД;
КонецФункции
	...
	Если ОписаниеФайла.Есть Тогда	
		Если Не ОписаниеФайла.Проверен Тогда
			...
			ИначеЕсли	Recordset.Fields.Count = 4 Тогда
				...
				Исключение
					ИнфоОбОшибке = ИнформацияОбОшибке();
					МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "Статусы_ДБФ.ПроверитьФайлСтатусовДБФ.ПереместитьФайл", , "Не удалось обновить файл сопоставления");
				КонецПопытки;
				...
				Если Не ОписаниеФайла.Есть Тогда
					...
					МодульОбъектаСервер().ВызватьСбисИсключение(, "Статусы_ДБФ.ПроверитьФайлСтатусовДБФ", 772,, "Не удалось создать файл статусов " + СоединениеСДБФ.Путь + ПараметрыСоединения.ОписаниеСоединенияДБФ.Параметры.ИмяФайла);
				КонецЕсли;
				...
				Исключение
					ПереместитьФайл(СоединениеСДБФ.Путь + ИмяФайлаБэкапа, СоединениеСДБФ.Путь + ИмяФайлаСтатусов);
					ИнфоОбОшибке = ИнформацияОбОшибке();
					МодульОбъектаСервер().ВызватьСбисИсключение(ИнфоОбОшибке, "Статусы_ДБФ.ПроверитьФайлСтатусовДБФ");
				КонецПопытки;
			КонецЕсли;           
