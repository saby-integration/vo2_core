...
&НаКлиенте
Функция НайтиДокументы1СПоПакетуСБИС(ПолныйСоставПакета, КэшИни, КаталогНастроек, ДопПараметры = Неопределено) Экспорт
	
	Попытка
		
		Возврат НайтиДокументы1СПоПакетуСБИСНаСервере(ПолныйСоставПакета);
