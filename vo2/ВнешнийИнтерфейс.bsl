...
Функция сбисВыполнитьКоманду(Кэш, МассивИд, ИмяКоманды, ДополнительныеПараметры = Неопределено) Экспорт	  
	...
	Для Каждого Идентификатор Из МассивИд Цикл
		...
		Исключение
			...
			Ошибка	= МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "ВнешнийИнтерфейс.сбисВыполнитьКоманду");
		КонецПопытки;
		РезультатДетализация = Новый Структура("Ид, КомандаВыполнена", ИдПакета, КомандаВыполнена);
		Если Отказ Тогда
			РезультатДетализация.Вставить("Ошибка", Ошибка);
		КонецЕсли;
		Результат.РезультатВыполнения.Добавить(РезультатДетализация);
	КонецЦикла;
	Возврат Результат; 
...
Функция сбисВыполнитьДействие(Кэш, МассивПакетов, ИмяКоманды, Комментарий) Экспорт
	...
	Для Каждого СоставПакета Из МассивПакетов Цикл
		...
		РезультатДействие	= Новый Структура("Ид, ДействиеВыполнено", СоставПакета.Идентификатор, Ложь);
		Если СоставПакета.Свойство("Этап") И СоставПакета.Этап.Количество() > 0 И СоставПакета.Этап[0].Свойство("Действие") Тогда
			Для Каждого Действие из СоставПакета.Этап[0].Действие Цикл
				...
				Если Не Действие.Название = ИмяКоманды Тогда
				...
				Попытка
					...
					РезультатДействие.ДействиеВыполнено = Кэш.Интеграция.сбисВыполнитьДействие(Кэш, СоставПакета, СоставПакета.Этап[0], Действие, Комментарий, "");
					
				Исключение
					
					ОшибкаВыполнить = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "ВнешинйИнтерфейс.сбисВыполнитьДействие");
					РезультатДействие.Вставить("Сообщение", МодульОбъектаКлиент().СбисИсключение_Выгрузить(ОшибкаВыполнить, "Сообщение"));
		...
		Если НеНашлиДействие Тогда
			...
			РезультатДействие.Вставить("Сообщение", "У документа отсутствует требуемое действие """ + ИмяКоманды + """.");
			
		КонецЕсли;
		
		Результат.РезультатВыполнения.Добавить(РезультатДействие);
...
Процедура сбисПолучитьРеестрДокументовОнлайна(ПараметрыФильтрацииВходящие, ДопПараметры) Экспорт
	...
	Если Не ПараметрыФильтрацииВходящие.Свойство("Фильтр", ФильтрВходящий) Тогда
		...
		Если	ПараметрыФильтрацииВходящие.Свойство("Раздел", Раздел)
			...
			ЛокальныйКэш.Текущий = ОбработкаМодуль.НовыйРазделГО(Раздел);
			
		ИначеЕсли ЛокальныйКэш.Текущий = Неопределено Тогда
			
			//Проинитить раздел по-умолчанию, если не установлено и не передано.
			ЛокальныйКэш.Текущий = ОбработкаМодуль.НовыйРазделГО("АккордеонПолученные11");
		...
		Иначе
			...
			ФильтрРаздела = МодульОбъектаКлиент().РазделГО_Получить(Раздел, "Фильтр");
			ФильтрРаздела.Вставить("ЗаписейНаСтранице", МодульОбъектаКлиент().ПолучитьЗначениеПараметраСБИС("ЗаписейНаСтранице"));
			ФильтрРаздела.Вставить("ФильтрСтраница", ЛокальныйКэш.ГлавноеОкно.ФильтрСтраница);
			ФильтрВходящий	= ОбработкаМодуль.СформироватьФильтрДляРеестраОнлайна(ФильтрРаздела, ПараметрыДляФильтра)
	...
	Иначе
		...
		ВызватьИсключение ОбработкаМодуль.СбисИсключение_Выгрузить(ОшибкаВызова);
	...
	Если Реестр = "СписокСобытий" Тогда
		...
		Результат = ОбработкаМодуль.ГлобальныйКэш.ТекущийСеанс.Модули.Интеграция.СБИС_СписокДокументовПоСобытиям(ЛокальныйКэш, ФильтрВходящий, ДопПараметрыВызова, Отказ);
		Если Отказ Тогда
			
			ОшибкаЧтенияСобытий = ОбработкаМодуль.НовыйСбисИсключение(Результат, "ВнешнийИнтерфейс.сбисПолучитьСписокДокументов");
			ВызватьИсключение ОбработкаМодуль.СбисИсключение_Выгрузить(ОшибкаЧтенияСобытий);
...
Процедура сбисЗапуститьОтправку(СписокДокументовКОтправке, ДопПараметры) Экспорт
	...
	ИначеЕсли	ДопПараметры.Свойство("ИмяИни") Тогда
		...
		УстановитьРазделДляДействия(ДопПараметры, ВладелецФормы.Кэш);
	...
	ИначеЕсли	Не ВладелецФормы.Кэш.Текущий.Свойство("Форма") Тогда
		...
		ВладелецФормы.Кэш.Текущий = МодульОбъектаКлиент().НовыйРазделГО("АккордеонПродажаДок033");
		МодульОбъектаКлиент().РазделГО_Вставить(ВладелецФормы.Кэш.Текущий, "ТипДок", "");
...
Функция сбисСопоставление(Кэш, Фильтр, ДополнительныеПараметры = Неопределено) Экспорт	
	...
	Пока ГлавноеОкно.ФильтрЕстьЕще Цикл
		...
		ПараметрыСписка = Новый Структура("Фильтр", Кэш.ОбщиеФункции.ПолучитьФильтрСобытий(Кэш, Новый Структура("ТипРеестра", "Входящие")));
 		СтруктураДляОбновленияФормы = МодульОбъектаКлиент().ПолучитьСписокДокументовПоСобытиямСБИС(ПараметрыСписка);
		Если СтруктураДляОбновленияФормы.Количество()=0 Тогда
			Возврат "Ошибка при получении списка документов с online.saby.ru";
		КонецЕсли;
		СписокСтрок = Новый СписокЗначений;
		СписокСтрок.ЗагрузитьЗначения(СтруктураДляОбновленияФормы.Таблица_РеестрСобытий);
		СписокДокументовДляСопоставления = фрмСопост.ПодготовитьСписокДокументовКСопоставлению(СписокСтрок);
...
Функция сбисОткрытьДокументПоИдентификатору(Кэш, ДополнительныеПараметры) Экспорт 
	...
	Если ДокументСБИС_Ид = Неопределено Тогда
		//ИД документа СБИС, в допсвойствах этого объекта 1С нет, так что дальше не обрабатываем
		Сообщить("У этого документа нет идентификатора Saby, невозможно отобразить электронный документ");
	...
	ИначеЕсли ОшибкаОткрытияДокумента Тогда 
		...
		Сообщить(МодульОбъектаКлиент().СбисИсключение_Выгрузить(СоставПакета));
...
// Функция выполняет переданный алгоритм Blockly
//	Параметры:
//		ПараметрыВызова - Структура - содержит параметры выполнения алгоритма Blockly (algorithm, endpoint, params, connection_uuid)
//		ДополнительныеПараметры - Структура - дополнительные параметров, необязательный
//
//	Возвращаемое значение:
//	 - Структура - результат выполнения алгоритма
&НаКлиенте
Функция сбисВыполнитьАлгоритмBlockly(ПараметрыВызова, ДополнительныеПараметры = Неопределено) Экспорт 
	ОбработкаМодуль		= МодульОбъектаКлиент();
	Кэш = ОбработкаМодуль.ПолучитьТекущийЛокальныйКэш();
	Кэш.ГлавноеОкно.ТранспортИнтеграции = ОбработкаМодуль.ПолучитьФормуОбработки("Transport");
	Кэш.ГлавноеОкно.ТранспортИнтеграции.ИнициироватьТранспорт();
	Кэш.ГлавноеОкно.BlocklyExecutor = ОбработкаМодуль.ПолучитьФормуОбработки("BlocklyExecutor");
	Кэш.ГлавноеОкно.BlocklyExecutor.ИнициироватьТранспорт();
	сбисСброситьКэшНаСервере();
	Возврат Кэш.ГлавноеОкно.BlocklyExecutor.API_BLOCKLY_RUN(ПараметрыВызова);
КонецФункции	

&НаСервере
Процедура сбисСброситьКэшНаСервере() 
	МодульОбъекта	= МодульОбъектаСервер();
	МодульОбъекта.СброситьКэшНаСервере();
КонецПроцедуры	

#Область include_core2_vo2_ВнешнийИнтерфейс_ЗапускИУстановка
#КонецОбласти

#Область include_core2_vo2_ВнешнийИнтерфейс_Актуальные
#КонецОбласти

#Область include_core2_vo2_ВнешнийИнтерфейс_УстаревшиеМетоды
#КонецОбласти

#Область include_core2_vo2_ВнешнийИнтерфейс_private
#КонецОбласти
