...
&НаКлиенте
Функция ТипФормируемогоДокумента(Кэш, ПараметрыРасчета) Экспорт
	
	ИниТекущегоРаздела = ПараметрыРасчета.ИниТекущегоРаздела;
	
	ТипПакета = "ДокОтгрИсх";
	ТипыПакета = Кэш.ОбщиеФункции.РассчитатьЗначение("РеестрСБИС_Тип", ИниТекущегоРаздела, Кэш);
	МассивТипов = Кэш.ОбщиеФункции.РазбитьСтрокуВМассивНаКлиенте(ТипыПакета, ",");
	Если МассивТипов.Количество() Тогда
		ТипПакета = МассивТипов[0];
	КонецЕсли;
	ПодтипПакета = Кэш.ОбщиеФункции.РассчитатьЗначение("РеестрСБИС_Подтип", ИниТекущегоРаздела, Кэш);
