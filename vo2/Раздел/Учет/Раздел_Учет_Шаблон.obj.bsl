...
Процедура УстановитьВидимостьЭлементовВформеПросмотра(ФормаПросмотра, СоставПакета, КэшПарам) Экспорт
	...
	ПоляТабличнаяЧастьСопоставлениеНоменклатуры = "НоменклатураПоставщика
	...
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ФормаПросмотра,
														"ТабличнаяЧастьСопоставлениеНоменклатуры.Идентификатор").Видимость = Ложь;
	
	Для НомерПоля = 1 По СтрЧислоСтрок(ПоляТабличнаяЧастьСопоставлениеНоменклатуры) Цикл
	
		ПутьКПолю = СтрЗаменить("ТабличнаяЧастьСопоставлениеНоменклатуры.{1}",
								"{1}",
								СтрПолучитьСтроку(ПоляТабличнаяЧастьСопоставлениеНоменклатуры, НомерПоля));
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ФормаПросмотра, ПутьКПолю).Видимость = Истина;
	
	КонецЦикла;
	
	МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ФормаПросмотра, "СинонимДокумента").Видимость = Истина;
	
	Если СоставПакета.Свойство("Этап") И СоставПакета.Этап.Количество()
		И СоставПакета.Этап[0].Свойство("Действие")
		И СоставПакета.Этап[0].Действие.Количество() Тогда
