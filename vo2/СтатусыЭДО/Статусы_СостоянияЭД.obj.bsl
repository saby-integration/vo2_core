...
Функция ЗаполнитьПараметрыЭДПоИсточнику(Источник, ФорматCML = Ложь, ВидЭД = Неопределено, БыстрыйОбмен = Ложь) Экспорт
	...
	Исключение  
		...
		ВызватьИсключение МодульОбъектаСерверБезКонтекста().СбисИсключение_ВСтрокуСервер(
			МодульОбъектаСерверБезКонтекста().НовыйСбисИсключениеСервер(, "Статусы_СостоянияЭД.ЗаполнитьПараметрыЭДПоИсточнику",,, СообщениеОбОшибке));
