...
функция ДублироватьСостояние(СоставПакета, ДокСсылка, XMLДокумента=неопределено, СтруктураФайла=неопределено) экспорт
	...
	если значениеЗаполнено(ПараметрыЭД.Организация) И значениеЗаполнено(ПараметрыЭД.Контрагент)тогда
		...
		СпособОбменаЭД= Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО;     // 1_6_2
		...
		Если ТаблицаПриглашений.Количество() = 0 Тогда	// создать приглашение
			...
			приглашение.НомерСтроки = ТаблицаПриглашений.Количество(); 
			...
			Выполнить("ОбменСКонтрагентамиСлужебный.ОбработатьТаблицуПриглашений(ТаблицаПриглашений)");
		конецесли;
		НастройкиЭД = Неопределено;
		...
		Если не СоставПакета.Свойство("Направление")  Тогда
			СоставПакета.Вставить("Направление", ПараметрыЭД.НаправлениеЭД);
		КонецЕсли;
		СсылкаНаВладельца = СоздатьЭлектронныйДокумент(ДокСсылка, СоставПакета, СпособОбменаЭД,
				...
				СостояниеВерсииЭД, XMLДокумента, СтруктураФайла);
...
&НаСервереБезКонтекста       
Функция ИнициализироватьТаблицуДанныхУчастниковОбмена()		// 1.6.4
...
Функция СоздатьЭлектронныйДокумент(ДокСсылка, СоставПакета, СпособОбменаЭД,
				...
				XMLДокумента, СтруктураФайла) экспорт
	Если СоставПакета.Направление =  "Исходящий" Тогда
		ЭлектронныйДокумент = "ЭлектронныйДокументИсходящий";
	Иначе
		ЭлектронныйДокумент = "ЭлектронныйДокументВходящий";
	КонецЕсли;
	запрос=новый запрос("ВЫБРАТЬ
	                    |	ЭД.Ссылка КАК Ссылка
	                    |ИЗ
	                    |	Документ."+ЭлектронныйДокумент+" КАК ЭД
	                    |ГДЕ
	...
	иначе	//	ОбменСКонтрагентамиСлужебный.СоздатьЭлектронныйДокумент(СтруктураЭД);
		НовыйЭлектронныйДокумент = Документы[ЭлектронныйДокумент].СоздатьДокумент();
	...
	Если ЗначениеЗаполнено(СтруктураФайла) И СтруктураФайла.Свойство("Файл") 
		...
		Для Каждого СтрокаТовара Из СтруктураФайла.Файл.Документ.ТаблДок.СтрТабл Цикл
			Если СтрокаТовара.Свойство("Номенклатура") И СтрокаТовара.Свойство("Название") Тогда
				...
				Исключение
				КонецПопытки;
				ОписаниеНоменклатурыКонтрагента = ОбменСКонтрагентамиКлиентСервер.НоваяНоменклатураКонтрагента(
													?(Партнеры, Контрагент.Партнер, Контрагент), Идентификатор);
		...
		КонецЦикла;		
	КонецЕсли;
	Если не ЗначениеЗаполнено(XMLДокумента)  Тогда
		ПрисоединенныйФайл = Ложь;
		Выполнить("ПрисоединенныйФайл = ОбменСКонтрагентамиСлужебный.ПрисоединенныйФайл(СсылкаНаВладельца)");
		Если Не ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
			XMLДокумента="<Файл/>";
		КонецЕсли;
	КонецЕсли; // alo Загрузка маркировки >>
