...
Процедура УстановитьПараметрыМодуля(Кэш, ПараметрыУстановки = Неопределено) Экспорт
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("ЕстьЕдиницыНаФормеПросмотра", Ложь);	
	
КонецПроцедуры 

// Получает контрагента 1С по структуре Контрагента СБИС, вызывает функцию поиска номенклатуры на сервере 
&НаКлиенте
Функция НайтиНоменклатуруПоставщикаПоТабличнойЧасти(стрКонтрагент, знач мТаблДок, КаталогНастроек, Ини, ДопПараметры = Неопределено) Экспорт
	...
	Если стрКонтрагент.Свойство("Контрагент1С") Тогда 
		
		Контрагент	= стрКонтрагент.Контрагент1С;  
	...
	Если Контрагент = Неопределено ИЛИ Контрагент = Ложь Тогда 
...
&НаКлиенте
Функция НайтиНоменклатуруПоставщика(стрКонтрагент, знач стрНоменклатураПоставщика, КаталогНастроек, Ини, ДопПараметры = Неопределено) Экспорт
// получает контрагента 1С по структуре Контрагента СБИС, вызывает функцию поиска номенклатуры на сервере 
	Если МестныйКэш = Неопределено Тогда
		МестныйКэш = сбисПолучитьФорму("ФормаГлавноеОкно").Кэш;
	КонецЕсли;
	...
	Возврат НайтиНоменклатуруПоставщикаНаСервере(стрНоменклатураПоставщика, Ини.Конфигурация, ДопПараметры);	
КонецФункции

// Процедура - получает контрагента 1С по структуре Контрагента СБИС, вызывает процедуру установки соответствия номенклатуры на сервере
//
// Параметры:
//  стрКонтрагент				 - Структура	 - структура для поиска контрагента и, если нашли, ссылка на контрагента 1С
//  стрНоменклатураПоставщика	 - Структура	 - структура для поиска номенклатуры
//  КаталогНастроек				 - Структура	 - путь к каталогу с настройками обработки
//  Ини							 - Структура	 - список ини файлов
//
&НаКлиенте
...
КонецПроцедуры
&НаСервереБезКонтекста
Функция НайтиНоменклатуруПоставщикаНаСервере(стрНоменклатураПоставщика, ИниКонфигурация, ДопПараметры = Неопределено) Экспорт
// ищет запись с определенной номенклатурой поставщика по реквизитам, указанным в файле настроек
// возвращает структуру с полями Номенклатура и Характеристика	
	...
	Если НоменклатураПоставщика<>Неопределено Тогда
		...
		Если ЗначениеЗаполнено(НоменклатураПоставщика[ИмяРеквизитаНоменклатуры]) Тогда
			...
			Если ИниКонфигурация.Свойство("НоменклатураПоставщиков_Характеристика") и ИниКонфигурация.НоменклатураПоставщиков_Характеристика.Значение<>"''" Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если НЕ ДопПараметры = Неопределено И ДопПараметры.Свойство("ВыгружатьЕдиницу") Тогда
				ДанныеЕдиницы = Новый Структура("Ссылка, ОКЕИ, Название");
				ДанныеЕдиницы.Вставить("Ссылка", НоменклатураПоставщика.Номенклатура[ДопПараметры.ИмяРеквизитаЕдиницы]);
				ДанныеЕдиницы.Вставить("ОКЕИ", НоменклатураПоставщика.Номенклатура[ДопПараметры.ИмяРеквизитаЕдиницы].Код);
				ДанныеЕдиницы.Вставить("Название", НоменклатураПоставщика.Номенклатура[ДопПараметры.ИмяРеквизитаЕдиницы].Наименование);
				Результат.Вставить("Единица", ДанныеЕдиницы);
			КонецЕсли;
			Возврат Результат;
		Иначе
...
// получает запись справочника или регистра сведений номенклатуры поставщика по реквизитам, указанным в файле настроек	
	...
	Если (Не стрНоменклатураПоставщика.Свойство("Идентификатор") или Не ЗначениеЗаполнено(стрНоменклатураПоставщика.Идентификатор))
        ...
        стрНоменклатураПоставщика.Вставить("Идентификатор", ?(стрНоменклатураПоставщика.Свойство("Название"),Лев(стрНоменклатураПоставщика.Название, 110), ""));
    КонецЕсли;
...
// Процедура устанавливает/удаляет соответствие номенклатуры
	...
	Если Не ЗначениеЗаполнено(стрНоменклатураПоставщика.Идентификатор) Или стрНоменклатураПоставщика.Идентификатор = "-" Тогда
		стрНоменклатураПоставщика.Идентификатор = Лев(стрНоменклатураПоставщика.Название, 110);
	КонецЕсли;
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		Ошибка = МодульОбъектаСервер().НовыйСбисИсключение(779301, "СопоставлениеНоменклатуры_Регистры.УстановитьСоответствиеНоменклатурыНаСервере",,
		"Не найден контрагент в ИС","Не удалось записать сопоставление номенклатуры");
		МодульОбъектаСервер().СбисСообщить(Ошибка);
	КонецПопытки
КонецПроцедуры
&НаСервереБезКонтекста
...
// Функция ищет идентификатор номенклатуры контрагента
//
// Параметры:
//  стрКонтрагент	 - Структура	 -  Класс стороны
//  стрНоменклатура	 - Структура	 -  Ссылки на справочники номенклатуры и характеристики
//  КаталогНастроек	 - Строка		 -  Путь к текущему каталогу настроек
//  Ини				 - Структура	 -  Список установленных ини файлов
// 
...
Функция ПолучитьИдентификаторНоменклатурыПоставщика(стрКонтрагент, стрНоменклатура, КаталогНастроек, Ини) Экспорт
	...
	ПараметрыПоискаКарточки	= Новый Структура("ТипСтороны, Размер, ТекстЗапроса, ТолькоПоИНН", "Контрагент", Неопределено, Неопределено, Истина);
	Контрагент = МодульОбъектаКлиент().Сторона_НайтиКарточки1С(стрКонтрагент, ПараметрыПоискаКарточки);   
