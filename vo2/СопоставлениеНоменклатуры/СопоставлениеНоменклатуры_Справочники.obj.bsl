	...
	Если НоменклатураПоставщика<>Неопределено Тогда
		...
		Если ЗначениеЗаполнено(НоменклатураПоставщика[ИмяРеквизитаНоменклатуры]) Тогда
			...
			Если НЕ ДопПараметры = Неопределено 
				И ДопПараметры.Свойство("ВыгружатьЕдиницу")
				И НЕ Результат.Номенклатура = Неопределено Тогда
				
				ДанныеЕдиницы = Новый Структура("Ссылка, ОКЕИ, Название");
				ДанныеЕдиницы.Вставить("Ссылка", Результат.Номенклатура[ДопПараметры.ИмяРеквизитаЕдиницы]);
				ДанныеЕдиницы.Вставить("ОКЕИ", Результат.Номенклатура[ДопПараметры.ИмяРеквизитаЕдиницы].Код);
				ДанныеЕдиницы.Вставить("Название", Результат.Номенклатура[ДопПараметры.ИмяРеквизитаЕдиницы].Наименование);
...
// получает запись справочника или регистра сведений номенклатуры поставщика по реквизитам, указанным в файле настроек	
	...
	Если (Не стрНоменклатураПоставщика.Свойство("Идентификатор") или Не ЗначениеЗаполнено(стрНоменклатураПоставщика.Идентификатор))
        ...
        стрНоменклатураПоставщика.Вставить("Идентификатор", ?(стрНоменклатураПоставщика.Свойство("Название"),Лев(стрНоменклатураПоставщика.Название, 300), ""));
    КонецЕсли;
...
// Процедура устанавливает/удаляет соответствие номенклатуры	
	...
	Если Не ЗначениеЗаполнено(стрНоменклатураПоставщика.Идентификатор) Или стрНоменклатураПоставщика.Идентификатор = "-"  Тогда
		стрНоменклатураПоставщика.Идентификатор = Лев(стрНоменклатураПоставщика.Название, 300);
	КонецЕсли;
