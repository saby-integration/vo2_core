...
Функция		СбисПолучитьНастройки(Кэш = Неопределено, Отказ = Ложь, ПродолжитьВыполнение = Истина, ДопПараметры = Неопределено)	Экспорт
	...
	Если Не РежимРаботы = "ЗагрузкаИзКаталога" Тогда // При загрузке из каталога не требуется
...
Функция		СбисПолучитьНастройкиСБИС(Кэш, Отказ=Ложь, ПродолжитьВыполнение=Истина)						Экспорт 
	...
	Если Кэш.КэшНастроек.Свойство("ВыбранныеНастройки", КлассКонекшенаПрименить) Тогда
	...
	Если МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ДоступныСерверныеНастройки") Тогда                      
		...
		КлассКонекшенаПрименить = ФайлыНастроекСервер.НовыйIntegrationConnection(ВремКэш, Новый Структура("Идентификатор, ИнитКоннекшен, ИнициироватьДанные", СбисПолучитьИдентификаторНастроекСБИС(Кэш), Истина, Ложь), Отказ);
		
		Если Не Отказ Тогда
			
			ФайлыНастроекСервер.КлассIntegrationConnection_СохранитьПодключение(ВремКэш, КлассКонекшенаПрименить, Новый Структура("Отказ, ПодолжитьВыполнение", Ложь, Истина), Отказ);
...
Функция		СбисПолучитьСтруктуруКонфигурации(Кэш, СтруктураКонфигураций, ЗначениеКонфигурации)						Экспорт
	...
	Для Каждого СтрокаСтруткур из СтруктураКонфигураций Цикл
		Если СтрокаСтруткур.Значение.НазываниеКонфигурации = ЗначениеКонфигурации Тогда
			СписокИниКонфигурации = ДобавитьИмяИниДляОтображения(СтрокаСтруткур.Значение.СтруктураКонфигурации, Кэш.Ини);
...
&НаСервере
Функция		ПолучитьМассивУстановленныхИниФайлов(Каталог, ДополнительныеПараметры, ИниИзКэш = Неопределено)
	...
	Для Каждого Элемент из СписокУстановленныхИниФайлов Цикл 
		...
		МассивУстановленныхИниФайлов.Добавить(НоваяСтрока);
	КонецЦикла;
	МассивУстановленныхИниФайлов = ДобавитьИмяИниДляОтображения(МассивУстановленныхИниФайлов, ИниИзКэш);
...
Функция		СбисРаспаковатьАрхивНаСервере(Знач ПараметрыРаспаковки, Отказ)
	...
	Если Не СбисФайлТест.Существует() Тогда
		Отказ = Истина;
		ОшибкаРаспаковки = МодульОбъектаСервер().НовыйСбисИсключение(779108,"ФайлыНастроекКаталог.СбисРаспаковатьАрхивНаСервере",,
		,"По пути " + ПутьКАрхиву + " отсутствует файл " + ИмяАрхива);
		Возврат ОшибкаРаспаковки;
	КонецЕсли;
...
&НаСервере
Функция ДобавитьИмяИниДляОтображения(ДоступныеИниФайлы, ИниИзКэш) Экспорт
	Перем ЗначениеИни;        
...
Функция		сбисПолучитьМассивУстановленныхИниФайлов(Кэш, сбисДополнительныеПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	КонецЕсли;
	Возврат ПолучитьМассивУстановленныхИниФайлов(Кэш.Парам.ИдентификаторНастроек, 
			Новый Структура("Автообновление", Кэш.Парам.НастройкиАвтообновление), Кэш.Ини);
