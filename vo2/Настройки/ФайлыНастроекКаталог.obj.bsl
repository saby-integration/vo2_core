...
&НаСервереБезКонтекста
Процедура	УдалитьФайлыНаСервере(Путь, Маска = Неопределено)// Удаляет выбранные файлы настроек на сервере
...
Функция		СбисОбновитьМассивУстановленныхИниФайлов(Кэш, МассивУстановленныхИни)									Экспорт
	...
	Возврат ОписаниеИниФайлов(ВыбранныеНастройки).МассивУстановленныхИниФайлов;
...
Функция		СбисПолучитьНастройки(Кэш = Неопределено, Отказ = Ложь, ПродолжитьВыполнение = Истина, ДопПараметры = Неопределено)	Экспорт
	...
	Попытка
		...
		Иначе	
			...
			МодульОбъектаКлиент().ИзменитьПараметрSaby("ВыбранныеНастройки", КоннекшенSABY, "Настройки");
...
Процедура	СбисСформироватьИниФайлыКлиента(Кэш, СписокФайловПроверки, ВыгружаемыеИни)								Экспорт
	...
	Если		РежимРаботы	= "ВыгрузкаВКаталог"	Тогда
		сбисСформироватьИниФайлыКлиентаВКонтекстеКлиента(Кэш, СписокФайловПроверки, ВыгружаемыеИни, Кэш.Парам.КаталогНастроек, РежимРаботы);
...
Функция		СбисУстановитьВыбранныеФайлыНастроек(МассивФайловДляУстановки, Кэш, КаталогНастроек, Отказ)				Экспорт
	...
	ПараметрыДляПолучения	= Новый Структура("Кэш, Тип", Кэш, "Настройки");
	ПодключениеSaby			= МодульОбъектаКлиент().ПараметрSaby("ВыбранныеНастройки", ПараметрыДляПолучения);
	...
	Попытка
		...
		СтруктураИниДополнить = СформироватьСтруктуруИни(УстановленныеИни, Новый Структура);
		МодульОбъектаКлиент().КоннекшенSABY_Установить(ПодключениеSaby, "Ини", СтруктураИниДополнить);
...
Функция		ПолучитьМассивУстановленныхИниФайлов(Каталог, ДопПараметры)
	...
	ПараметрыПолучить = Новый Структура("КаталогНастроек, ВключеныДоработки, Автообновление", Каталог, Истина, ДопПараметры.Автообновление);
...
Функция		СбисПолучитьУстановленныеНастройки(Кэш, ПараметрыУстановки, Отказ)
	...
	ПараметрыДляПОлучения	= Новый Структура("Кэш, Тип", Кэш, "Настройки");
	КоннекшенSaby			= МодульОбъектаКлиент().ПараметрSaby("ВыбранныеНастройки", ПараметрыДляПОлучения);
	
	// Установим параметры SABY на подключение из каталога.
	СтрукутраПараметровSaby = КоннекшенSaby_ПрочитатьПараметрыSaby(КоннекшенSaby);
	Если СтрукутраПараметровSaby.Свойство("DisableCustomAlgorithms", ПараметрПроверить) Тогда
		
		МодульОбъектаКлиент().КоннекшенSABY_Установить(КоннекшенSABY, "ОтключеныДоработки", ПараметрПроверить);
		
	КонецЕсли;

	Попытка

		КоннекшенSABY_Прочитать(КоннекшенSABY);
	...
	УстановленныеИниФайлы = МодульОбъектаКлиент().КоннекшенSABY_Получить(КоннекшенSABY, "ОписаниеИни").Установлены;
...
Функция		СбисПолучитьНастройки_ОбработатьПараметрыРаботы(Кэш, Отказ, ПродолжитьВыполнение)
	...
	Если ПараметрыРаботы.Свойство("ВыполнитьПроверку", ПараметрРаботы) Тогда				
		...
		Если ПараметрРаботы Тогда
			...
			МодульОбъектаКлиент().КоннекшенSABY_Очистить(ВыбранныеНастройки, "Данные");
			МодульОбъектаКлиент().КоннекшенSABY_Очистить(ВыбранныеНастройки, "ОписаниеИни");
...
&НаКлиенте
Функция	СбисПолучитьНастройкиДляТП(ПараметрыВыгрузки) Экспорт
	
	Возврат СбисПолучитьНастройкиССервера_Клиент(ПараметрыВыгрузки);

КонецФункции

&НаКлиенте
Функция	СбисПолучитьНастройкиССервера_Клиент(ПараметрыВыгрузки) Экспорт
	...
	Если ПараметрыВыгрузки.ВариантОбработки = 4 Тогда
		...
		Если Отказ Тогда
			...
			Возврат МодульОбъектаКлиент().НовыйСбисИсключение(РезультатОбработки, ЭтаФорма.ИмяФормы + "." + "ПолучитьНастройкиССервера_Клиент")
	...
	Если Отказ Тогда
		...
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(АдресАрхива, ЭтаФорма.ИмяФормы + "." + "ПолучитьНастройкиССервера_Клиент");
	...
	Исключение
		...
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), ЭтаФорма.ИмяФормы + "." + "ПолучитьНастройкиССервера_Клиент");
		
	КонецПопытки;
	
	Если Не ПараметрыВыгрузки.Распаковать Тогда
		Возврат ПараметрыВыгрузки.КаталогНаКлиенте + ПараметрыВыгрузки.ИмяАрхива;
	КонецЕсли;
	Кэш.ГлавноеОкно.СбисПоказатьСостояние("Распаковка", Кэш.ГлавноеОкно);
	РезультатВыполнения = РаспаковатьНастройкиНаКлиенте(Кэш, ПараметрыВыгрузки, Отказ);
	Если Отказ Тогда
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(РезультатВыполнения, ЭтаФорма.ИмяФормы + "." + "ПолучитьНастройкиССервера_Клиент");
	КонецЕсли;
	
	Возврат КаталогНастроекКлиент;
...
Процедура	ПриЗапускеРедактораНастроек(ПараметрыЗапускаРедактораВходящие, Кэш) Экспорт
	...
	Отказ = Ложь;
	МодульОбъектаКлиент().КоннекшенSABY_Установить(ВыбранныеНастройки, "ЧтениеПоТребованию", Ложь);	
