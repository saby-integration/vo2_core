...
Функция		ОпределитьТипНастроекПоПрефиксу(ПрефиксСервис, ПрефиксПолный)							Экспорт
	...
	ИначеЕсли ВРег(Лев(Префикс,2)) = "АА" Тогда //bav 11.09.2019
		СбисКонфигурация = "Альфа-Авто, редакция 5.1";
		Версия = Сред(Префикс, 3);
...
Функция		УстановитьИниРазделов(Кэш, сбисСтруктураРазделов, сбисДополнительныеПараметры=Неопределено, Отказ) Экспорт
	...
	ИниРаздела = ?(ЗначениеЗаполнено(Кэш.Текущий.ОписаниеРаздела.Ини),
					Кэш.ФормаНастроек.Ини(Кэш, Кэш.Текущий.ОписаниеРаздела.Ини),
					Неопределено);
	ПараметрыАккордеона = Новый Структура("ТекущийРазделУдален",
		ИниРаздела = Неопределено ИЛИ МодульОбъектаКлиент().ЭтоТипСбис(ИниРаздела, "СбисИсключение"));
	Кэш.ГлавноеОкно.СформироватьСтруктуруАккордеона(Кэш.ГлавноеОкно.Аккордеон, ПараметрыАккордеона);
	Если ПараметрыАккордеона.ТекущийРазделУдален Тогда
		Кэш.ГлавноеОкно.ПерейтиВРаздел("");
	Иначе
		Кэш.ГлавноеОкно.ПерейтиВРаздел(Кэш.Текущий.Имя);
	КонецЕсли;
КонецФункции
