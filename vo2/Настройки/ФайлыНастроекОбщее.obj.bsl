...
#Область include_core_vo2_Настройки_Общее_Интерфейс  
#КонецОбласти
...
Функция		УстановитьИниКонфигурации(Кэш, ИниКонфигурации, ДополнительныеПараметры=Неопределено, Отказ) Экспорт
	...
	Иначе
		МодульОбъектаКлиент().ИзменитьПараметрСбис("ИспользоватьВходящийКонтекст", Ложь); 
	КонецЕсли;
...
Функция	РазделЗаполнить(Кэш, НазваниеРаздела, ДанныеРаздела, СтрРаздела)
	...
	Для Каждого сбисТипДокумента Из ДанныеРаздела.Список Цикл
		...
		Если 	сбисТипДокумента.Ключ = "КонтрагентыСБИС"
			И	Не МодульОбъектаКлиент().ПолучитьЗначениеФичи(Новый Структура("НазваниеФичи", "НовыеКонтрагенты")) Тогда 
...
Функция	сбисОтключитьФайлыНастроек(ФайлыУдалить = Неопределено) Экспорт
	...
	Попытка
		СбисУстановитьНастройки(Новый Структура("ПринудительнаяПроверка, СписокИни", Истина, УстановленныеИни), МестныйКэш);
	Исключение
...
#Область include_core_vo2_Настройки_Общее_СбисПараметры
#КонецОбласти
