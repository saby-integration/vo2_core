...
Функция		УстановитьИниКонфигурации(Кэш, ИниКонфигурации, ДополнительныеПараметры=Неопределено, Отказ) Экспорт
	...
	Если Кэш.Парам.СпособХраненияНастроек = 1 Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ИначеЕсли МодульОбъектаКлиент().ПолучитьЗначениеФичи(Новый Структура("НазваниеФичи", "РасширенныйФункционалСопоставленияНоменклатуры")) Тогда
		//TODO #2
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("ИмяМодуляРаботыСНоменкатурой",	ФормаРаботыСНоменклатурой);
...
&НаКлиенте
Процедура ФильтрУстановитьТипыПолей(ИниКонфигурации, Кэш) Экспорт
	Если Кэш.ПараметрыСистемы.Клиент.УправляемоеПриложение Тогда
		ПутьКЭлементам = "Элементы";
		ФормаФильтра = Кэш.ГлавноеОкно.СбисПолучитьФорму("ФормаУстановкиФильтра");
	Иначе
		ПутьКЭлементам = "ЭлементыФормы";
		ФормаФильтра = Кэш.ГлавноеОкно;
	КонецЕсли;
	
	ИмяСправочника = Кэш.ОбщиеФункции.РассчитатьЗначение("Организации", ИниКонфигурации);
	Если	ЗначениеЗаполнено(ИмяСправочника)
		И	НРег(Лев(ИмяСправочника, 10)) = "справочник" Тогда
		ИмяСправочника = СокрЛП(Сред(ИмяСправочника, Найти(ИмяСправочника, ".")+1))
	Иначе
	...
	ИмяСправочника = Кэш.ОбщиеФункции.РассчитатьЗначение("Контрагенты", ИниКонфигурации);
	Если	ЗначениеЗаполнено(ИмяСправочника)
		И	НРег(Лев(ИмяСправочника, 10)) = "справочник" Тогда
		ИмяСправочника = СокрЛП(Сред(ИмяСправочника, Найти(ИмяСправочника, ".")+1))
	Иначе
	...
	ИмяСправочника = Кэш.ОбщиеФункции.РассчитатьЗначение("Ответственные", ИниКонфигурации);
	...
	Исключение
	КонецПопытки;
	Если НЕ ЗначениеЗаполнено(Кэш.ГлавноеОкно.ФильтрОтветственный) Тогда
		Кэш.ГлавноеОкно.ФильтрОтветственный = ФормаФильтра.ФильтрОтветственный;
	КонецЕсли;   
	Если НЕ ЗначениеЗаполнено(Кэш.ГлавноеОкно.ФильтрКонтрагент) Тогда
		Кэш.ГлавноеОкно.ФильтрКонтрагент = ФормаФильтра.ФильтрКонтрагент;  
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(Кэш.ГлавноеОкно.ФильтрОрганизация) Тогда
		Кэш.ГлавноеОкно.ФильтрОрганизация = ФормаФильтра.ФильтрОрганизация;
	КонецЕсли;
КонецПроцедуры
...
Функция 	ПереключитьСпособХраненияНастроекВСбис(Аргумент, ДопПараметры) Экспорт
	...
	ИначеЕсли Не МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("СерверныеНастройки") Тогда
...
Процедура СпособХраненияНастроекПриИзменении(Элемент)
	...
	ИначеЕсли	СпособХраненияНастроек	= 1
			И	МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("СерверныеНастройки") Тогда
