...
#Область include_core2_vo2_Настройки_Общее_Интерфейс  
#КонецОбласти
...
Функция		УстановитьИниРазделов(Кэш, сбисСтруктураРазделов, сбисДополнительныеПараметры=Неопределено, Отказ) Экспорт
	...
	Если Не Кэш.Свойство("СБИС", КэшСБИС) Тогда
		Возврат Ложь;
	КонецЕсли;
	СтруктураРазделовАккордеона = СформироватьСтруктуруРазделов(Кэш);
...
&НаКлиенте                                  
Функция	СформироватьСтруктуруРазделов(Кэш)
...
Функция		УстановитьИниКонфигурации(Кэш, ИниКонфигурации, ДопПараметры=Неопределено, Отказ) Экспорт
	...
	УстановитьТипыПолейВКэш(ИниКонфигурации, Кэш);
	...
	ФильтрУстановитьТипыПолей(ИниКонфигурации, Кэш);
	...
	СправочникСклады = Кэш.ОбщиеФункции.РассчитатьЗначение("СправочникСклады", ИниКонфигурации); 
	Кэш.ФормаНастроек.ПараметрыРаботы.Вставить("СправочникСклады", СправочникСклады);
	Кэш.ГлавноеОкно.ВладелецКаталога = МодульОбъектаКлиент().ТипыСторонСопоставления().ДанныеОрганизации;
	
	СправочникРасСчет = Кэш.ОбщиеФункции.РассчитатьЗначение("СправочникРасСчет", ИниКонфигурации);
	Кэш.ФормаНастроек.ПараметрыРаботы.Вставить("СправочникРасСчет", СправочникРасСчет);
	...
	ТипМетаданныхШтрихкоды = Кэш.ОбщиеФункции.РассчитатьЗначение("ТипМетаданныхШтрихкоды", ИниКонфигурации);;
	ИмяМетаданныхШтрихкоды = Кэш.ОбщиеФункции.РассчитатьЗначение("ИмяМетаданныхШтрихкоды", ИниКонфигурации);;
	ИмяМетаданныхРеквизитШтрихкод = Кэш.ОбщиеФункции.РассчитатьЗначение("ИмяМетаданныхРеквизитШтрихкод", ИниКонфигурации);;
	ИмяМетаданныхРеквизитНоменклатура = Кэш.ОбщиеФункции.РассчитатьЗначение("ИмяМетаданныхРеквизитНоменклатура", ИниКонфигурации);;
...
Процедура	УстановитьИниКонфигурации_СопоставлениеНоменклатуры(ИниКонфигурации, ДопПараметры = Неопределено)
	...
	РасширенныйФункционалСопоставленияНоменклатуры = МодульТекущий.ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры");
	...
	Если РасширенныйФункционалСопоставленияНоменклатуры Тогда
		...
		МодульТекущий.ИзменитьПараметрСбис("СпособСопоставленияНоменклатуры",	МодульТекущий.ПолучитьЗначениеПараметраСбис("СпособСопоставленияНоменклатуры"));
		МодульТекущий.ИзменитьПараметрСбис("ЕстьЕдиницыНаФормеПросмотра",		Истина);
		...
		МодульОбъектаКлиент().ИзменитьПараметрТекущегоСеанса("ФормаРаботыСНоменклатуройOLD", ФормаРаботыСоСопоставлениемOLD);   
				
		// Временное решение до нормального решения в 25.2100 (с) Сыч
		УстановитьМетаданныеРасширенногоСопоставления(ИниКонфигурации, Кэш);
	...
	Иначе
		...
		Если Отказ Тогда
			...
			ИсключениеУстановки = МодульТекущий.НовыйСбисИсключение(ФормаРаботыСоСопоставлением, "ФайлыНастроекОбщее.УстановитьИниКонфигурации");
			ВызватьИсключение МодульТекущий.СбисИсключение_ВСтроку(ИсключениеУстановки);
		...
		МодульТекущий.ИзменитьПараметрСбис("ИмяМодуляРаботыСНоменклатурой",	ФормаРаботыСНоменклатурой);
		МодульТекущий.ИзменитьПараметрСбис("ЕстьЕдиницыНаФормеПросмотра", ЕстьЕдиницыНаФормеПросмотра);
		
		МодульТекущий.ИзменитьПараметрТекущегоСеанса("МодульСопоставлениеНоменклатуры", ФормаРаботыСоСопоставлением);
		МодульТекущий.ИзменитьПараметрТекущегоСеанса("ИмяМодуляСопоставленияНоменклатуры", ФормаРаботыСоСопоставлением.ИмяФормы);
...
Процедура УстановитьИниКонфигурации_ПараметрыСторон(ИниКонфигурации, ДопПараметры)
	...
	ПараметрыСторон.Организация.Вставить("КлючЗапроса",			"ЗапросОрганизации");
	...
	ПараметрыСторон.Контрагент.Вставить("КлючЗапроса",			"ЗапросКонтрагента");
...
//
&НаКлиенте
Процедура УстановитьТипыПолейВКэш(ИниКонфигурации, Кэш) Экспорт
	
	ТипыПолейРезультат = Новый Структура;
	
	// Организации
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("Организации", ИниКонфигурации);
	Если Не ЗначениеЗаполнено(ИмяОбъекта) Тогда
		ИмяОбъекта = МодульОбъектаКлиент().ПолучитьИмяОбъектаПоМетаданным("Организации");
	КонецЕсли;
	...
	// Контрагенты
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("Контрагенты", ИниКонфигурации);
	Если Не ЗначениеЗаполнено(ИмяОбъекта) Тогда
	...
	// Ответственные
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("Ответственные", ИниКонфигурации);
	Если	Не ЗначениеЗаполнено(ИмяОбъекта) Тогда
	...
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("КлассификаторЕдиницИзмерения", ИниКонфигурации);
	Если НЕ ЗначениеЗаполнено(ИмяОбъекта) Тогда
	...
	// Договоры 1С
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("Договоры", ИниКонфигурации);
	Если НЕ ЗначениеЗаполнено(ИмяОбъекта) Тогда
	...
	// Номенклатура  1С
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("Номенклатура", ИниКонфигурации);
	Если Не ЗначениеЗаполнено(ИмяОбъекта) Тогда
	...
	// Харктеристики  1С
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("Характеристика", ИниКонфигурации);
	Если Не ЗначениеЗаполнено(ИмяОбъекта) Тогда
	...
	// Тип номенклатуры  1С
	ВидыСправочниковНоменклатуры = Кэш.ОбщиеФункции.РассчитатьЗначение("ВидыСправочниковНоменклатуры", ИниКонфигурации);
	...
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("ТипНоменклатуры", ИниКонфигурации);
	Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
		ДопПараметры = Новый Структура("СообщениеПриОшибке");
	...
	// Тип типа цен  1С
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("ТипЦен", ИниКонфигурации);
	Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
		ДопПараметры = Новый Структура("СообщениеПриОшибке");
	...
	// Тип типа склад  1С
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("СправочникСклады", ИниКонфигурации);
	Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
		ДопПараметры = Новый Структура("СообщениеПриОшибке");
...
// Процедура устанавливает типы полей фильров для форм с фильтрами
//
//
// Параметры:
//  ИниКонфигурации	- Структура - ини конфигурации
//  Кэш	 			- Структура - Кэш
// 
//	ТипыПолейФильтра = МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("ТипыПолейФильтра")
//	
&НаКлиенте
Процедура ФильтрУстановитьТипыПолей(ИниКонфигурации, Кэш)
	...
	ОписаниеТипов = ФильтрУстановитьТипыПолей_ПолучитьОписаниеТипов(МассивТиповФильтров, Кэш);

	#Если ТолстыйКлиентОбычноеПриложение Тогда
		//Временный костыль для ОФ
		ФильтрУстановитьТипыПолей_ПривестиПоле("ФильтрОрганизация", 	ОписаниеТипов);
		ФильтрУстановитьТипыПолей_ПривестиПоле("ФильтрКонтрагент",		ОписаниеТипов);
		ФильтрУстановитьТипыПолей_ПривестиПоле("ФильтрОтветственный",	ОписаниеТипов);
	#КонецЕсли
	
	МодульОбъектаКлиент().ИзменитьПараметрТекущегоСеанса("ТипыПолейФильтра", ОписаниеТипов);
	
КонецПроцедуры

&НаКлиенте
Функция ФильтрУстановитьТипыПолей_ПолучитьОписаниеТипов(МассивТипов, Кэш)
	
	СтруктураТиповПолей = МодульОбъектаКлиент().ПолучитьЗначениеПараметраТекущегоСеанса("ТипыПолей1С");
	
	ОписаниеТиповФильтров = Новый Структура;
	
	Для Каждого СтрокаТипа Из МассивТипов Цикл
		
		ТипСтрокой = СтрЗаменить(СтрокаТипа, "Фильтр", "");
		
		Если СтруктураТиповПолей.Свойство(ТипСтрокой) Тогда
			ОписаниеТипа = СтруктураТиповПолей[ТипСтрокой].ОписаниеТипа;
		Иначе
			ОписаниеТипа = Новый ОписаниеТипов;
		КонецЕсли;
		
		ОписаниеТиповФильтров.Вставить(СтрокаТипа, ОписаниеТипа);
		
	КонецЦикла;
	
	Возврат ОписаниеТиповФильтров;
	
КонецФункции

&НаКлиенте
Процедура ФильтрУстановитьТипыПолей_ПривестиПоле(КлючФильтра, ОписаниеТиповФильтра)
...
&НаКлиенте 
Процедура УстановитьМетаданныеРасширенногоСопоставления(ИниКонфигурации, Кэш)
	
	// Вариантов всего два, так что без извращений
	ИмяРегистра = ОпределитьРегистрДопСведенийПоМетаданным();
	МодульОбъектаКлиент().ИзменитьПараметрТекущегоСеанса("ИмяРегистраДополнительныхСведений", ИмяРегистра);
	...
	ИмяРеквизитаЕдиницИзмерения = ОпределитьИмяРеквизитаЕдиницИзмерения(ТипСправочникаНоменклатуры);
...
&НаСервереБезКонтекста
Функция ОпределитьИмяРеквизитаЕдиницИзмерения(ТипСправочникаНоменклатуры)
...
Функция		СбисДобавитьПечатныеФормы(Кэш, Отказ) Экспорт
	...
	КонецЕсли;
	ПараметрыФормированияНаСервере = Новый Структура("ПометкаУдаления, УправляемоеПриложение, ВидДопОбработокПечатнаяФорма", Ложь);
	ПараметрыФормированияНаСервере.УправляемоеПриложение		= Кэш.ПараметрыСистемы.Клиент.УправляемоеПриложение;
	ПараметрыФормированияНаСервере.ВидДопОбработокПечатнаяФорма	= ВидДопОбработокПечатнаяФорма;
	...
	Попытка
		...
		КонецЕсли;
		ДанныеОбработки			= Новый Структура("ОбработкаХранитсяВСправочнике, ДвоичныеДанныеОбработки, Версия", ОбработкаХранитсяВСправочнике, ДвоичныеДанныеОбработки);
		ДанныеОбработки.Версия	= Кэш.ПараметрыСистемы.Обработка.Версия;
...
Процедура	СбисДобавитьПечатныеФормыНаСервере(ДанныеОбработки, ПараметрыФормированияНаСервере) Экспорт
	Перем ИмяСправочникаОбработок, ИдентификаторКоманды, ХранилищеОбработки;
	...
	СбисПредставлениеПФСтарое = "Отправить через sbis";
	СбисПредставлениеПФНовое = "Отправить через saby";
	// << alo Меркурий
	
	Если ПараметрыФормированияНаСервере.свойство("ПредставлениеПФ") Тогда
		
		СбисПредставлениеПФНовое = ПараметрыФормированияНаСервере.ПредставлениеПФ;
		
	Конецесли;
	
	ПараметрыФормированияНаСервере.Свойство("ИдентификаторКоманды", ИдентификаторКоманды);
	...
	СбисОбработкаНовая		= СправочникВОМенеджер.НайтиПоНаименованию(СбисПредставлениеПФНовое);
	Если	СбисОбработкаСтарая = Неопределено
		Или	СбисОбработкаСтарая.Пустая() Тогда
		
		сбисОбработка = СбисОбработкаНовая;
		
	Иначе
		
		Если	СбисОбработкаНовая = Неопределено
			Или	СбисОбработкаНовая.Пустая() Тогда
	...
	Если СбисОбработка = Неопределено Или СбисОбработка.Пустая() Тогда
		...
		СбисОбработка.Наименование = СбисПредставлениеПФНовое;
		Если ПараметрыФормированияНаСервере.УправляемоеПриложение Тогда
			...
			НоваяКоманда.Представление = СбисПредставлениеПФНовое;
	...
	Если ДанныеОбработки.ОбработкаХранитсяВСправочнике Тогда
		...
		НаборМестПоиска = Новый Массив;
		НаборМестПоиска.Добавить("Электронный обмен документами в Saby");
		
		Если ПараметрыФормированияНаСервере.УправляемоеПриложение Тогда
			
			НаборМестПоиска.Добавить("Отправить документ через online.saby.ru");
			НаборМестПоиска.Добавить("Отправить документ через online.sbis.ru");
			
		Иначе
			
			НаборМестПоиска.Добавить("Электронный обмен документами в СБиС");
			
		КонецЕсли;
		
		Для Каждого МестоПоиска Из НаборМестПоиска Цикл 
			
			ОткрытаяОбработкаВСправочнике = СправочникВОМенеджер.НайтиПоНаименованию(МестоПоиска);
		...
		Если ХранилищеОбработки = Неопределено Тогда
			...
			ВызватьИсключение "При обновлении печатной формы обработкой из справочника возникла проблема: Объект обрабтки СБИС не найден в справочнике!";
...
&НаКлиенте
Функция		СбисДобавитьКомандыПечатиНаФормы(Кэш, Отказ = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
	Попытка
		ПараметрыФормированияКомандПечати = Новый Структура();
		ПараметрыФормированияКомандПечати.Вставить("УправляемоеПриложение",Кэш.ПараметрыСистемы.Клиент.УправляемоеПриложение);
		
		Если ЗначениеЗаполнено(ДополнительныеПараметры) Тогда
			Для каждого Параметр Из ДополнительныеПараметры Цикл
				ПараметрыФормированияКомандПечати.Вставить(Параметр.Ключ, Параметр.Значение);
			КонецЦикла;	
		КонецЕсли;
		
		Если НЕ ПараметрыФормированияКомандПечати.Свойство("СписокДокументов") Тогда
...
Процедура	СбисДобавитьКомандыПечатиНаФормыНаСервере(ПараметрыФормированияКомандПечати) Экспорт
	...
	Иначе//Нет справочника доп. обработок
		Возврат;
	КонецЕсли;
	СбисПредставлениеПФ = "Отправить через saby";
	// << alo Меркурий
...
Функция		ПроверитьЗначениеПараметра(знач ИмяПараметра, знач ЗначениеПараметра, СтруктураИниФайла, СлужебнаяСтруктура, ПараметрыФильтра)
			...
			Попытка
				...
				ИначеЕсли Метаданные.РегистрыСведений.Найти(Объект1С.Имя)<> Неопределено и (Лев(нрег(Объект1СПолноеИмя),15)="регистрсведений" или Лев(нрег(Объект1СПолноеИмя),19)="informationregister") Тогда
					...
					Исключение
						...
						КонецПопытки;
					КонецПопытки;
				ИначеЕсли Метаданные.РегистрыНакопления.Найти(Объект1С.Имя)<> Неопределено и (Лев(нрег(Объект1СПолноеИмя),17)="регистрнакопления" или Лев(нрег(Объект1СПолноеИмя),20)="accumulationregister") Тогда
					Попытка
						Возврат Метаданные.РегистрыНакопления[Объект1С.Имя].Ресурсы[имяРек];
					Исключение
						Попытка
							Возврат Метаданные.РегистрыНакопления[Объект1С.Имя].Измерения[имяРек];
						Исключение
							Попытка
								Возврат Метаданные.РегистрыНакопления[Объект1С.Имя].СтандартныеРеквизиты[имяРек];
							Исключение
								Возврат сбисЗаписатьОшибкуИниФайла(ИмяПараметра,ЗначениеПараметра,"Не найден реквизит """+ИмяРек+""".",СтруктураИниФайла, СлужебнаяСтруктура, Объект1С,Объект1СПолноеИмя,Объект1СПолноеИмя+"."+ИмяРек);
							КонецПопытки;
						КонецПопытки;
	...
	ИначеЕсли Найти(ЗначениеПараметра,"РегистрыНакопления")=1 Тогда	
		ИмяРек=прав(ЗначениеПараметра,СтрДлина(ЗначениеПараметра)-19);
		Если Метаданные.РегистрыНакопления.Найти(ИмяРек) = Неопределено Тогда
			Возврат сбисЗаписатьОшибкуИниФайла(ИмяПараметра,ЗначениеПараметра,"Не найден регистр накопления: """+ЗначениеПараметра+""".",СтруктураИниФайла, СлужебнаяСтруктура, Метаданные.Перечисления,"РегистрыСведений");
		КонецЕсли;	
		Возврат Метаданные.РегистрыНакопления[ИмяРек];		
...
Функция СбисУстановитьВыбранныеНастройки(Кэш, ПараметрыУстановки, ДополнительныеПараметры, Отказ) Экспорт
	...
	Исключение
		...
		Отказ = Истина;
		РезультатУстановки = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "ФайлыНастроекОбщее.СбисУстановитьВыбранныеНастройки", 608);
...
#Область include_core2_vo2_Настройки_Общее_СбисПараметры
#КонецОбласти
...
Функция Ини_Выгрузка(Кэш, ДокументСсылка, ДокументПредставление, СбисРеестр) Экспорт
	...
	Иначе
		...
		Если	ЗначениеЗаполнено(СбисРеестр)
			И	ЗначениеЗаполнено(СбисРеестр.ОписаниеРаздела)
			И	ЗначениеЗаполнено(СбисРеестр.ОписаниеРаздела.Ини) Тогда
			
			// Обработка идёт "от реестра", проверить инишку на переадресацию по типу
			ИниРеестра = Кэш.ФормаНастроек.Ини(Кэш, СбисРеестр.ОписаниеРаздела.Ини);
