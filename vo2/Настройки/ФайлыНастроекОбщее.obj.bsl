...
Функция	ПолучитьТипыПоляСОписаниемТипов(ИмяОбъекта, ДопПараметры = Неопределено)
	...
	Попытка
		...
		ТипыМассивСтрок = МодульОбъектаКлиент().СбисРазложитьСтрокуВМассивПодстрокНаСервереБезКонтекста(ИмяОбъекта, ",");
...
Процедура СбисПолучитьИУстановитьНастройкиВКэш(ПараметрыУстановкиВходящие, ЛокальныйКэш) Экспорт
	...
	Попытка
		...
		Если Отказ Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ПараметрыУстановкиНастроек = Новый Структура("СписокИни, ПринудительнаяПроверка", УстановленныеИниФайлы.СписокФайлов, ПринудительнаяПроверка);
