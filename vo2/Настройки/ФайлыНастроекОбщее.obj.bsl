...
Функция		НайтиСтандартнуюКонфигурациюНастроек(ПараметрыКонфигурацииВходящие=Неопределено)			Экспорт
	...
	СписокТиповНастроекДляКонфигурации.СортироватьПоПредставлению();
	ИндексТекущий = СписокТиповНастроекДляКонфигурации.Индекс(СписокТиповНастроекДляКонфигурации.НайтиПоЗначению("Текущая конфигурация"));
	Если СписокТиповНастроекДляКонфигурации.Количество() > 1 Тогда
...
Функция		УстановитьИниКонфигурации(Кэш, ИниКонфигурации, ДополнительныеПараметры=Неопределено, Отказ) Экспорт
	...
	ПараметрыСторон = Новый Структура("Организация, Контрагент", Новый Структура, Новый Структура);
	...
	ПараметрыСторон.Организация.Вставить("Тип",					МодульОбъектаКлиент().СтроковоеЗначениеУзлаИни(ИниКонфигурации.Организации,		ПараметраПолучитьРеквизиты));
	...
	ПараметрыСторон.Контрагент.Вставить("Тип",					МодульОбъектаКлиент().СтроковоеЗначениеУзлаИни(ИниКонфигурации.Контрагенты,		ПараметраПолучитьРеквизиты));
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("РеквизитыСторонДляПоиска",	ПараметрыСторон);
...
Функция	ПолучитьТипыПоляСОписаниемТипов(ИмяОбъекта, ДопПараметры = Неопределено)
	...
	Попытка
		...
		ТипыМассивСтрок = МодульОбъектаКлиент().СбисРазложитьСтрокуВМассивПодстрокНаКлиенте(ИмяОбъекта, ",");
...
Функция	сбисОтключитьФайлыНастроек(ФайлыУдалить = Неопределено) Экспорт
	...
	Попытка
		СбисУстановитьНастройки(Новый Структура("ПринудительнаяПроверка, СписокИни", Ложь, УстановленныеИни), МестныйКэш);
	Исключение
