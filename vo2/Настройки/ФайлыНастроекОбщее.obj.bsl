...
Функция		НайтиСтандартнуюКонфигурациюНастроек(ПараметрыКонфигурацииВходящие=Неопределено)			Экспорт
	...
	Для Каждого Элемент Из СписокДоступныхКонфигураций Цикл
		Если	Не	(	Элемент.Значение.ПараметрыКонфигурации.subsystem = ПрефиксДляПоиска
					Или	(	Элемент.Значение.ПараметрыКонфигурации.subsystem	= ""
						И	Элемент.Значение.ПараметрыКонфигурации.version		= "0.0.0.1")) Тогда
			Продолжить;
		КонецЕсли;
	...
	КонецЦикла;
	СписокТиповНастроекДляКонфигурации.Добавить("Текущая конфигурация", СбисЗаполнитьПрефиксНулями(ПараметрыКонфигурацииВходящие.version, ".") + "-");
	СписокТиповНастроекДляКонфигурации.СортироватьПоПредставлению();
	ИндексТекущий = СписокТиповНастроекДляКонфигурации.Индекс(СписокТиповНастроекДляКонфигурации.НайтиПоЗначению("Текущая конфигурация"));
...
Процедура УстановитьТипыПолейВКэш(ИниКонфигурации, Кэш) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
		...
		ТипыПолейРезультат.Вставить("ДоговорКонтрагента", СтруктураТипыПоляСОписаниемТипов);
	КонецЕсли;
	...
	МодульОбъектаКлиент().ИзменитьПараметрТекущегоСеанса("ОписаниеКонфигурации.ТипыПолей1С", ТипыПолейРезультат);
...
Процедура ФильтрУстановитьТипыПолей(ИниКонфигурации, Кэш)
	...
	МодульОбъектаКлиент().ИзменитьПараметрТекущегоСеанса("ОписаниеКонфигурации.ТипыПолейФильтра", ОписаниеТипов);
...
Процедура СбисПолучитьИУстановитьНастройкиВКэш(ПараметрыУстановкиВходящие, ЛокальныйКэш) Экспорт
	...
	Попытка
		...
		Если Отказ Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ПараметрыУстановкиНастроек = Новый Структура("СписокИни, ПринудительнаяПроверка", УстановленныеИниФайлы, ПринудительнаяПроверка);
