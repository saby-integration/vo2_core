...
Функция		УстановитьИниКонфигурации(Кэш, ИниКонфигурации, ДопПараметры=Неопределено, Отказ) Экспорт
	...
	РасширенныйФункционалСопоставленияНоменклатуры = МодульОбъектаКлиент().ПолучитьЗначениеФичи("РасширенныйФункционалСопоставленияНоменклатуры");
	...
	Если КаталогПоУмолчаниюСервер = Неопределено Тогда
		КаталогПоУмолчаниюСервер = МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("КаталогНастроек");
	КонецЕсли;
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("СтрокаПодключенияDBF",		СтрокаПодключенияDBF);
	МодульОбъектаКлиент().ИзменитьПараметрСбис("КаталогПоУмолчанию_Сервер",	КаталогПоУмолчаниюСервер);
	...
	Если Отказ Тогда
		Возврат Кэш.ОбщиеФункции.СбисИсключение(ФормаРаботыСоСтатусамиЗначение, "ФайлыНастроекОбщее.УстановитьИниКонфигурации");
	КонецЕсли;
	
	Попытка
		ФормаРаботыСоСтатусамиЗначение.УстановитьПараметрыМодуля(Новый Структура("Каталог", КаталогСтатусыНаСервере), Кэш);
	Исключение
		МодульОбъектаКлиент().СообщитьСбисИсключение(ИнформацияОбОшибке(), "ФайлыНастроекОбщее.УстановитьИниКонфигурации");
	КонецПопытки;
	...
	Если Кэш.Парам.СпособХраненияНастроек = 1 Тогда
		Если РасширенныйФункционалСопоставленияНоменклатуры Тогда
			//TODO #1 Временный костыль. Спилить, когда определимся с тем, как будет переноситься номенклатура и подготовим рельсы для этого
			Кэш.Парам.СпособСопоставленияНоменклатуры = 0;
		...
		Иначе
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли РасширенныйФункционалСопоставленияНоменклатуры Тогда
		//TODO #2
		Если Кэш.Парам.СпособСопоставленияНоменклатуры = 0 Тогда
			ФормаРаботыСНоменклатурой = "СопоставлениеНоменклатуры_СБИС";
		Иначе
	...
	Если Отказ Тогда
		Возврат  МодульОбъектаКлиент().НовыйСбисИсключение(ФормаРаботыСоСопоставлением, "ФайлыНастроекОбщее.УстановитьИниКонфигурации");
	КонецЕсли;
	Попытка
	...
	МодульОбъектаКлиент().ИзменитьПараметрТекущегоСеанса("АвтоматическоеСозданиеНоменклатуры", АвтоматическоеСозданиеНоменклатуры);
	
	//+++ 1186694647 TODO Спилить и сделать видимым всегда - после завершения проекта по сопоставлению номенклатуры.
	ЕстьЕдиницыНаФормеПросмотра = Не РасширенныйФункционалСопоставленияНоменклатуры
								И Не	(	ФормаРаботыСНоменклатурой = "СопоставлениеНоменклатуры_Регистры"
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("ЕстьЕдиницыНаФормеПросмотра", ЕстьЕдиницыНаФормеПросмотра);
	...
	МодульОбъектаКлиент().ИзменитьПараметрСбис("ИмяМодуляРаботыСоСтатусами",	ФормаРаботыСоСтатусами);
	МодульОбъектаКлиент().ИзменитьПараметрСбис("ИмяМодуляРаботыСНоменклатурой",	ФормаРаботыСНоменклатурой);
	...
	УстановитьИниКонфигурации_ПараметрыСторон(ИниКонфигурации, ДопПараметры, Отказ);
...
&НаКлиенте                                  
Процедура УстановитьИниКонфигурации_ПараметрыСторон(ИниКонфигурации, ДопПараметры, Отказ)
...
Процедура УстановитьМетаданныеРасширенногоСопоставления(ИниКонфигурации, Кэш)
	...
	Если НЕ МодульОбъектаКлиент().ПолучитьЗначениеФичи(Новый Структура("НазваниеФичи", "РасширенныйФункционалСопоставленияНоменклатуры")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура	СбисДобавитьПечатныеФормыНаСервере(ДанныеОбработки, ПараметрыФормированияНаСервере) Экспорт
	Перем ИмяСправочникаОбработок, ИдентификаторКоманды;
...
Процедура	сбисУстановитьФайлыНастроекЗавершение(Ответ, ДопПараметры)								Экспорт
	...
	МестныйКэш.ФормаНастроек.сбисУстановитьВыбранныеФайлыНастроек(МассивФайловДляУстановки, МестныйКэш, МестныйКэш.Парам.ИдентификаторНастроек, Отказ);
...
Функция	сбисОтключитьФайлыНастроек(ФайлыУдалить = Неопределено) Экспорт
	...
	Для Каждого ИниФайл Из ФайлыУдалить Цикл
		...
		Если	ИниФайл.ЕстьИзменения
			И	МестныйКэш.Парам.СпособХраненияНастроек = 0 Тогда
			СписокИниКОтключению.Добавить(СтрЗаменить(СтрЗаменить(ИниФайл.ИмяФайла, ".sbis3", ""), ".0", ""));
		КонецЕсли;							
	КонецЦикла;
	
	УстановленныеИни = МестныйКэш.ФормаНастроек.сбисОтключитьВыбранныеФайлыНастроек(СписокИниКОтключению, МестныйКэш, МестныйКэш.Парам.ИдентификаторНастроек, Отказ);
	
	Попытка
		СбисУстановитьНастройки(Новый Структура("ПринудительнаяПроверка, НеобходимоОбновление, СписокИни", Ложь, Истина, УстановленныеИни), МестныйКэш);
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
		МестныйКэш.ГлавноеОкно.СбисСообщитьОбОшибке(МестныйКэш, МодульОбъектаКлиент().НовыйСбисИсключение(ИнфоОбОшибке, "ФайлыНастроекСервер.сбисОтключитьФайлыНастроек"), Новый Структура("ФормаВладелец", МестныйКэш.ФормаНастроекОбщее));		
	КонецПопытки;
...
Процедура СпособХраненияНастроекПриИзменении(Элемент)
	...
	ИзмениласьФормаНастроек = (МестныйКэш.Парам.СпособХраненияНастроек <> СпособХраненияНастроек);
	Если	СпособХраненияНастроек	= 0				Тогда
		...
		МестныйКэш.ГлавноеОкно.ПерезапуститьГлавноеОкно(ПараметрыИнтеграции, Ложь, ИзмениласьФормаНастроек);
...
Процедура КаталогНастроекНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ОтказОтВыбора = Ложь;
	КаталогНастроек = МестныйКэш.ГлавноеОкно.сбисВыбратьКаталог(КаталогНастроек, ОтказОтВыбора);
...
Процедура КаталогНастроекПриИзменении(Элемент = Неопределено) Экспорт
	...
	ПараметрыПриИзменении(МестныйКэш.ГлавноеОкно.сбисЭлементФормы(ЭтаФорма, "КаталогНастроек"));
...
Процедура НастройкиНазваниеПриИзменении(Элемент)
	...
	МестныйКэш.КэшНастроек.ИзмененияВНастройках = Истина;
	МестныйКэш.КэшНастроек.ВыбранныеНастройки.Название	= НастройкиНазвание;
	сбисОбновитьЭлементы_РазделНастроек();
...
Процедура НастройкиНазваниеНачалоВыбора(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	МестныйКэш.ФормаНастроек.сбисВыбратьСоединение(МестныйКэш);
...
Процедура ПроверкаНастроекНажатие(Команда = Неопределено) 
	...
	Попытка
		СбисПолучитьИУстановитьНастройкиВКэш(Новый Структура("ПринудительнаяПроверка, ПродолжитьУстановку", Истина, Истина), МестныйКэш); 
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
...
Процедура ДействияСНастройками_ЗагрузитьИзКаталогаНажатие(Команда)
	...
	ДопПараметры = Новый Структура("Кэш, Отказ, ОбновитьСоставНастроек, Тип", МестныйКэш, Ложь, Ложь, "Загрузка");
	ОписаниеЗавершения = МестныйКэш.ОбщиеФункции.СбисОписаниеОповещения(МестныйКэш, "ДействияСНастройками_ЗавершениеВызоваКоманды", ЭтаФорма, ДопПараметры);
	МестныйКэш.ФормаНастроек.СбисЗагрузитьНастройкиИзКаталога(МестныйКэш, Новый Структура("Кэш, ОписаниеОповещенияОЗавершении", МестныйКэш, ОписаниеЗавершения));
...
Процедура ДействияСНастройками_ВыгрузитьВКаталогНажатие(Команда)
	...
	ДопПараметры = Новый Структура("Кэш, Отказ, ОбновитьСоставНастроек, Тип", МестныйКэш, Ложь, Ложь, "Выгрузка");
	ОписаниеЗавершения = МестныйКэш.ОбщиеФункции.СбисОписаниеОповещения(МестныйКэш, "ДействияСНастройками_ЗавершениеВызоваКоманды", ЭтаФорма, ДопПараметры);
	МестныйКэш.ФормаНастроек.СбисВыгрузитьНастройкиВКаталог(МестныйКэш, Новый Структура("Кэш, ОписаниеОповещенияОЗавершении", МестныйКэш, ОписаниеЗавершения));
...
Процедура ДействияСНастройками_ПеречитатьНастройки(Команда)
	...
	ПараметрыПеречитать = Новый Структура("Элемент", МестныйКэш.ГлавноеОкно.СбисПолучитьЭлементФормы(ЭтаФорма, "ДействияСНастройками_ПеречитатьНастройки"));
	МестныйКэш.ФормаНастроек.СбисПеречитатьНастройки(ПараметрыПеречитать, МестныйКэш);
...
Процедура ДействияСНастройками_ЗавершениеВызоваКоманды(РезультатКоманды, ДопПараметры) Экспорт
	...
	Если МестныйКэш = Неопределено Тогда 
		МестныйКэш = ДопПараметры.Кэш;
	КонецЕсли;
		
	МестныйКэш.ГлавноеОкно.СбисСпрятатьСостояние(); 
	Если ДопПараметры.Отказ Тогда
		Если ТипЗнч(РезультатКоманды) = Тип("Строка") Тогда
			РезультатКоманды = МестныйКэш.ОбщиеФункции.СбисИсключение(РезультатКоманды, "ДействияСНастройками_ЗавершениеВызоваКоманды");
		КонецЕсли;
		МестныйКэш.ГлавноеОкно.СбисСообщитьОбОшибке(МестныйКэш, РезультатКоманды, Новый Структура("ФормаВладелец", МестныйКэш.ФормаНастроекОбщее));
	ИначеЕсли ЗначениеЗаполнено(РезультатКоманды)
		...
		ИначеЕсли ДопПараметры.Тип = "Выгрузка" Тогда
			...
			МестныйКэш.ГлавноеОкно.СбисОткрытьКаталог(РезультатКоманды);
...
Процедура УстановитьНастройки(Команда)
	...
	сбисУстановитьФайлыНастроек(МестныйКэш);
