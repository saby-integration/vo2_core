...
Функция		ОпределитьПараметрыИни(Кэш, ЗначениеИни, КлючИни) Экспорт
	...
	ТипИни			 = ОбщиеФункции.РассчитатьЗначение("ТипИни", ЗначениеИни);
	ИмяИни			 = КлючИни;
	ТипВложения		 = КлючИни;
	НаправлениеИни	 = "";
	ФорматТипа		 = "";
...
Процедура	УстановитьИниКонфигурации_СопоставлениеНоменклатуры(ИниКонфигурации, ДопПараметры = Неопределено)
	...
	МодульОбъектаКлиент().ОбновитьПараметрыСопоставленияНоменклатуры(ИниКонфигурации, ДопПараметры);
	
	// Временное решение до нормального решения в 25.2100 (с) Сыч
	УстановитьМетаданныеРасширенногоСопоставления(ИниКонфигурации, ДопПараметры = Неопределено);
...
Процедура УстановитьТипыПолейВКэш(ИниКонфигурации, Кэш) Экспорт
	...
	// ФизическоеЛицо
	ИмяОбъекта = Кэш.ОбщиеФункции.РассчитатьЗначение("Исполнители", ИниКонфигурации);
	Если	Не ЗначениеЗаполнено(ИмяОбъекта) Тогда
		ИмяОбъекта = МодульОбъектаКлиент().ПолучитьИмяОбъектаПоМетаданным("ФизическиеЛица");
	КонецЕсли;

	ДопПараметры = Новый Структура("СообщениеПриОшибке");
	ДопПараметры.СообщениеПриОшибке = "Не определен тип для указания физических лиц. Некоторые элементы форм могут работать некорректно.";
	СтруктураТипыПоляСОписаниемТипов = ПолучитьТипыПоляСОписаниемТипов(ИмяОбъекта, ДопПараметры);
	
	ТипыПолейРезультат.Вставить("ФизическиеЛица", СтруктураТипыПоляСОписаниемТипов);	
...
&НаКлиенте 
Процедура УстановитьМетаданныеРасширенногоСопоставления(ИниКонфигурации, ДопПараметры = Неопределено)
	Перем ИмяРеквизитаЕдиницИзмерения;
