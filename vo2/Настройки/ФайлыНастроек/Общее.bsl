...
#Область include_core2_vo2_Настройки_ФайлыНастроек_Общее_API
#КонецОбласти
...
#Область include_core2_vo2_Настройки_Общее_СборТехИнфо
#КонецОбласти
			...
			Попытка
				...
				Иначе
					...
					ИначеЕсли Не Объект1С = Неопределено Тогда
						...
						Если ТипЗнч(Объект1С) = Тип("ОбъектМетаданных") Тогда
							
							ОбъектМетаданных = Объект1С;
							
						Иначе
							
							// Объект 1С это ссылка. Например, предопределенное значение. Для проверки надо брать метаданные.
							ОбъектМетаданных = Объект1С.Метаданные();
...
Процедура ВыполнитьУстановкуНастроек(ПараметрыУстановкиВходящие, ЛокальныйКэш) Экспорт
	Перем СтруктураПараметровСбис, СтруктураПараметровСбисИсходная, СказатьЧтоВсеХорошо, СбисДополнительныеПараметры, ИдентификаторНастроек, ТекущееПодключение;
	...
	Если	Не	ПараметрыУстановкиВходящие.Свойство("ИдентификаторНастроек",	ИдентификаторНастроек)
		...
		ИдентификаторНастроек = МодульОбъектаКлиент().КоннекшенSABY_Получить(ТекущееПодключение, "Идентификатор");
	КонецЕсли;
	...
	ОтключеныДоработки			=	МодульОбъектаКлиент().КоннекшенSABY_Получить(ТекущееПодключение, "ОтключеныДоработки");
	...
	Если		НужнаПроверкаПоИзменениям
		И	Не	ОтключеныДоработки Тогда  
	...
	Иначе
		...
		// если не проверяем, то считаем, что с ини все хорошо если есть конфигурация
		ИниВПорядке = ОписаниеИни.ПолноеОписание.Свойство("Конфигурация");
		МодульОбъектаКлиент().ИзменитьПараметрSaby("ИниВПорядке", ИниВПорядке, "Настройки");
	...
	Если Не МодульОбъектаКлиент().ПараметрSaby("ИниВПорядке", "Настройки") Тогда
	...
	Если Отказ Тогда
		МодульОбъектаКлиент().ИзменитьПараметрSaby("ИниВПорядке", Ложь, "Настройки");
...
Процедура СбисПолучитьИУстановитьНастройкиВКэш(ПараметрыУстановкиВходящие, ЛокальныйКэш) Экспорт
	...
	Попытка
		...
		Если ПринудительнаяПроверка Тогда
			...
			// Отключить чтение ини по требованию для подключения, т.к. для проверки понадобятся полные данные
			МодульОбъектаКлиент().КоннекшенSABY_Установить(ЛокальныйКэш.КэшНастроек.ВыбранныеНастройки, "ЧтениеПоТребованию",	Ложь);
			ЛокальныйКэш.ФормаНастроек.ПараметрыРаботы.Вставить("ВыполнитьПроверку", Истина);
		...
		Если Отказ Тогда
			Если УстановленныеИниФайлы = Неопределено Тогда
				Отказ = Ложь;
				УстановленныеИниФайлы = Новый Структура("СписокФайлов", Новый СписокЗначений);
			Иначе
	...
	Исключение
		МодульОбъектаКлиент().ВызватьСбисИсключение(ИнформацияОбОшибке(), "ФайлНастроекОбщее.СбисПолучитьИУстановитьНастройкиВКэш",,,"Ошибка установки настроек");
	КонецПопытки;
КонецПроцедуры
...
Функция СбисУстановитьВыбранныеНастройки(Кэш, ПараметрыУстановки, ДополнительныеПараметры, Отказ) Экспорт
	...
	Если ПараметрыУстановки.НеобходимоОбновление Тогда
		...
		КонецЕсли;
		РезультатУстановки = ОбновитьДанныеИни(Кэш.Ини, Ложь);
		// Пока без проверки на успех обновления
		//Если Отказ Тогда
	...
	Если		Кэш.ини.Свойство("ДополнительныеСведения")
		И	Не	ОтключеныДоработки Тогда
		
		РезультатУстановки = МодульОбъектаКлиент().ИниПоПараметрам("ДополнительныеСведения");
	...
	Попытка
		...
		ФункцииДокументов = МодульОбъектаКлиент().ПараметрSaby("ФункцииДокументов", "Сеанс");
...
Функция		сбисСформироватьСтруктуруРазделов(Кэш, СтруктураНастроек, сбисДополнительныеПараметры) Экспорт
	...
	ЧтениеИниПоДанным1С			= МодульОбъектаКлиент().НовыйЧтениеИниПоДанным1С(Новый Структура);
	...
	Для Каждого ТипДокумента Из СтруктураНастроек Цикл
		...
		Если	ТипДокумента.Значение.Свойство("мФайл") Тогда
			...
			Если ИниДляОпределенияТипа.Количество() Тогда
				...
				МодульОбъектаКлиент().ЧтениеИниПоДанным1С_Вставить(ЧтениеИниПоДанным1С, "ОбъектРассчитать", ЗначениеВставить);
	...
	СоответствиеДанные	= МодульОбъектаКлиент().ЧтениеИниПоДанным1С_Прочитать(ЧтениеИниПоДанным1С);
