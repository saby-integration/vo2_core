...
Процедура		УстановитьсбисПараметры(Кэш, СтруктураНастроек)	Экспорт
	...
	Попытка
		...
		ПредВерсия = МодульОбъектаКлиент().ПолучитьЗначениеПараметраСбис("ПредВерсия");

		Если ЗначениеЗаполнено(ПредВерсия) И Кэш.ОбщиеФункции.ЭтоНоваяВерсия("2.49", ПредВерсия) Тогда    // Если предыдущая версия обработки меньше 2.49 читаем настройки статусов из параметров пользователя
...
Процедура	сбисОчиститьКэшНастроек(Кэш, ПараметрыОчистки) Экспорт
	...
	КлючиУдалить = "СписокЭлементовФильтра";
...
Функция		СбисОпределитьНеобходимостьПроверки(Кэш, СтруктураПараметров, сбисДополнительныеПараметры=Неопределено) Экспорт
	...
	ЕстьИзменения = МодульОбъектаКлиент().СбисНоваяВерсия(Лев(сбисВерсияОбработки,5), Лев(Кэш.ПараметрыСистемы.Конфигурация.Версия, 5));
