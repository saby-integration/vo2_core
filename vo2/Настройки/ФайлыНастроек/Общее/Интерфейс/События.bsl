...
Процедура ПередЗакрытиемОбработчик(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка = Истина)
	...
	ИначеЕсли Кэш.ФормаНастроек.ЕстьИзмененияНастроек(Кэш) Тогда
		...
		ИдЗаписанногоПодключения	= МодульОбъектаКлиент().ПараметрSaby("ИдентификаторПодключения", "Пользователь"); 
		ИдТекущегоПодключения		= МодульОбъектаКлиент().КоннекшенSaby_Получить(ВыбранныеНастройки, "Идентификатор");
		...
		Если ИдЗаписанногоПодключения = ИдТекущегоПодключения Тогда
