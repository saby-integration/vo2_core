...
Процедура ПередЗакрытием(Отказ, Параметр2=Неопределено, Параметр3=Неопределено, Параметр4=Неопределено)
	...
	Если Не МестныйКэш.КэшНастроек.ИниВПорядке Тогда
		...
		Отказ = Истина;
		Возврат;
	ИначеЕсли МестныйКэш.ФормаНастроек.ЕстьИзмененияНастроек(МестныйКэш) Тогда
		...
		Если МестныйКэш.ФормаНастроек.СбисПолучитьИдентификаторНастроек(МестныйКэш) = МодульОбъектаКлиент().ПолучитьИдКоннекшена() Тогда
