...
Процедура	КлассIntegrationConnection_ЗаполнитьРедакторИниПоКлассу(Кэш, КлассIntegrationConnection, ДопПараметры) Экспорт
	Перем ПредупреждениеПриоткрытии;
	...
	Если НашаКонфигурация = Неопределено Тогда
		МодульОбъектаКлиент().СбисСообщить("Не удалось определить основную конфигурацию.");
	Иначе
	...
	Если Кэш.КэшНастроек.ВыбранныеНастройки.ПараметрыРаботы.Свойство("Предупреждение", ПредупреждениеПриоткрытии) Тогда
		
		МодульОбъектаКлиент().СбисСообщить(ПредупреждениеПриоткрытии);
		Кэш.КэшНастроек.ВыбранныеНастройки.ПараметрыРаботы.Удалить("Предупреждение");
		
	КонецЕсли;	
	
	//Флаг ставится если есть изменения в настройках, либо изменился коннекшен для пользователя в сохранённых.
	ИдПодключенияСохранен = МодульОбъектаКлиент().ПараметрSaby("ИдентификаторПодключения", "Пользователь");
	
	Кэш.КэшНастроек.ИзмененияВНастройках = Кэш.КэшНастроек.ИзмененияВНастройках Или Не ИдПодключенияСохранен = КлассIntegrationConnection.Идентификатор;
