...
//
&НаКлиенте
Функция		НовыйIntegrationConnection(Кэш, ДопПараметры, Отказ) Экспорт
	...
	Если ДопПараметры.Свойство("ДанныеБЛ", СбисДанныеБЛ) Тогда
		
		ПроверитьНазвание = Ложь;
	...
	Если КлассIntegrationConnection = Неопределено Тогда
		...
		Если ИнитКоннекшен Тогда
			...
			КлассIntegrationConnection	= _НовыйIntegrationConnection_ДанныеИнитКоннекшен(Кэш, РезультатКоннешен, Отказ);
			ПроверитьНазвание = Ложь;
			
			//ПараметрыСозданияКоннекшена	= Новый Структура("Идентификатор, Перечитать", РезультатКоннешен.ConnectionId, Истина);
			//ПараметрыСозданияКоннекшена.Вставить("ДанныеИнитКоннекшен", РезультатКоннешен);
			//КлассIntegrationConnection	= НовыйIntegrationConnection(Кэш, ПараметрыСозданияКоннекшена, Отказ);
			//
		Иначе
	...
	Если ПроверитьНазвание Тогда
	
		//Определим, название создано по шаблону, или это ручное изменение.
		СгенерированноеНазвание	= КлассIntegrationConnection_СгенерироватьНазвание(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ);
		Если Отказ Тогда
			
			Возврат СгенерированноеНазвание;
			
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(КлассIntegrationConnection.Название) Тогда
			
			КлассIntegrationConnection.Название = СгенерированноеНазвание;
			НазваниеСгенерировано = Истина;
			
		Иначе
			
			НазваниеСгенерировано = КлассIntegrationConnection.Название = СгенерированноеНазвание;
			
		КонецЕсли;
		
	Иначе
		
		// Конструктор от данных БЛ. Название не проверять
		НазваниеСгенерировано = Ложь;
	...
	Если	Не	ДопПараметры.Свойство("Кэшировать")
		...
		Кэш.КэшНастроек.КэшIntegrationConnection.Вставить(КлассIntegrationConnection_Идентификатор(Кэш, КлассIntegrationConnection), КлассIntegrationConnection);
...
Функция		_НовыйIntegrationConnection_НаОсновеКласса(Кэш, ПараметрыКонструктора, Отказ)
	...
	Если ПараметрыКонструктора.Свойство("Ини", ДанныеИни) Тогда
		...
		Иначе
			...
			КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection, Новый Структура, Отказ);
		КонецЕсли;
		...
		Если Отказ Тогда
			Возврат Кэш.ОбщиеФункции.СбисИсключение(КлассДляСравнения, "ФайлыНастроекСервер.НовыйIntegrationConnection.НаОсновеКласса");
		КонецЕсли;
		РезультатУстановки = КлассIntegrationConnection_ДозапроситьИни(КлассДляСравнения, Новый Структура("СписокИни_ВО", КлассIntegrationConnection.Данные.Ини), Отказ);
		Если Отказ Тогда
			Возврат Кэш.ОбщиеФункции.СбисИсключение(РезультатУстановки, "ФайлыНастроекСервер.НовыйIntegrationConnection.НаОсновеКласса");
		КонецЕсли;
		//1190111968
		...
		Для Каждого УстановленныйИни Из ЧистыеИни Цикл
			...
			Если		КлассДляСравнения.Данные.Ини.Свойство(ИмяИни, ИниСравнения)
				И	Не	ИниСравнения = Неопределено Тогда
				
				// Если проблема с именем ини, то типовой считаем что нет.
...
&НаКлиенте
Функция		_НовыйIntegrationConnection_ДанныеИнитКоннекшен(Кэш, ДанныеИнитКоннекшен, Отказ)
	Перем КлассIntegrationConnection, ДанныеИни, ДанныеМеню;

	КлассIntegrationConnection	= _КлассIntegrationConnection_ОсновныеПоля(Кэш);
	
	ПараметрыКонфигурации			= ПараметрыСтандартнойКонфигурации(Кэш);
	ПараметрыКонфигурации.service	= Кэш.КэшНастроек.ПараметрыКонфигурации.service;
	ПараметрыКонфигурации.subsystem	= ДанныеИнитКоннекшен.ExtSysSettingsId;
	ПараметрыКонфигурации.version	= ДанныеИнитКоннекшен.ExtSysSettingsVersion;
	
	КлассIntegrationConnection.ПараметрыРаботы.ТипНастроек	= "Неизвестно";
	КлассIntegrationConnection.Идентификатор				= ДанныеИнитКоннекшен.ConnectionId;
	
	МодульОбъектаКлиентСервер().КоннекшенSABY_Установить(КлассIntegrationConnection, "ПараметрыКонфигурации",	ПараметрыКонфигурации);
	МодульОбъектаКлиентСервер().КоннекшенSABY_Установить(КлассIntegrationConnection, "Статус",					"Активен");
	
	ПараметрыМеню = СбисОписаниеИниМеню(Кэш);
	Если		ДанныеИнитКоннекшен.Свойство("ExtSysSettings", ДанныеИни)
		И		ДанныеИни.Свойство(ПараметрыМеню.name, ДанныеМеню)
		И	Не	ДанныеМеню = Неопределено Тогда
		
		КлассIntegrationConnection.Данные.Меню = ДанныеМеню.data;		
		
	Иначе
		
		// Меню в InitConnection нет, значит его в принципе нет.
	...
	Исключение
		...
		Отказ = Истина;
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "ФайлыНастроекСервер._НовыйIntegrationConnection_ДанныеИнитКоннекшен");
