...
Функция		КлассIntegrationConnection_СгенерироватьНазвание(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	Если	Не  ТипНастроекДляНазвания = "Стандартные"
		...
		Если Отказ Тогда
			...
			Возврат МодульОбъектаКлиент().НовыйСбисИсключение(КлассIntegrationШаблон, "ФайлыНастроекСервер.КлассIntegrationConnection_СгенерироватьНазвание");
...
&НаКлиенте
Функция		КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	Если ДопПараметры.Свойство("КонтролироватьИни") Тогда
		
		ПараметрыЗапросаИни.Вставить("КонтролироватьИни", ДопПараметры.КонтролироватьИни);
		
	КонецЕсли;
	
	Если ДопПараметры.Свойство("ИсправитьПроблемыИни") Тогда
		
		ПараметрыЗапросаИни.Вставить("ИсправитьПроблемыИни", ДопПараметры.ИсправитьПроблемыИни);
	...
	Если КлассIntegrationConnection.Данные.Ини = Неопределено Тогда
		// Инитим Данные если попали в дозапрос всех инишек, но меню нет (неподдерживаемая конфа, или что-то не так с меню)
...
Функция		КлассIntegrationConnection_ЕстьИзменения(Кэш, КлассIntegrationConnection, Отказ)
	...
	Для Каждого ЭлементКарты Из ДанныеСписка.Карта Цикл
		ИмяИни = ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, ЭлементКарты.Представление);
	...
	РезультатДозапроса = КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnectionДоИзменений, Новый Структура, Отказ);
	Если Отказ Тогда
...
Функция		КлассIntegrationConnection_Инициализировать(Кэш, ДопПараметры, Отказ)
	Перем КлассIntegrationConnection, ПарметрыКонфигурации, СписокИниПолучить, ДанныеБЛ, ДанныеМеню;
	...
	Если Не ДопПараметры.Свойство("ДанныеБЛ", ДанныеБЛ) Тогда
		Если ИнициироватьДанные Тогда
			...
			ЗапроситьМеню			= Ложь;
			ЗапроситьКонфигурацию	=		КлассIntegrationConnection.Данные.Ини = Неопределено
			...
			Если	Не	ДопПараметры.Свойство("ДанныеИнитКоннекшен", ДанныеМеню)
				Или	Не	ДанныеМеню.Свойство("ExtSysSettings", ДанныеМеню)
				Или		ДанныеМеню = Неопределено 
				Или	Не	ДанныеМеню.Свойство(ПараметрыМеню.name, ДанныеМеню)
				Или		ДанныеМеню = Неопределено Тогда
				
				// Не ИнитКоннекшен, либо нет меню. Для нетиповых систем меню (и конфигурацию) не запрашиваем
				ЗапроситьМеню			= ЗапроситьКонфигурацию И ЗначениеЗаполнено(КлассIntegrationConnection.ПараметрыКонфигурации.subsystem);
				ЗапроситьКонфигурацию	= ЗапроситьМеню;
				
			Иначе
				
				// В ответе ИнитКоннекшен есть меню. Значит Конфигурация заправшивается, т.к. это готовый и настроенный коннекшен.
				КлассIntegrationConnection.Данные.Меню = ДанныеМеню.data;
				ЗапроситьКонфигурацию = ЗапроситьКонфигурацию И ЗначениеЗаполнено(КлассIntegrationConnection.ПараметрыКонфигурации.subsystem);
			...
			Если	КлассIntegrationConnection.Данные.Меню = Неопределено Тогда
				...
				Иначе
					...
					// Меню нет. Сделать пустую структуру для перегенрации
					КлассIntegrationConnection.Данные.Меню = Новый Структура;
	...
	Если Отказ Тогда
		//Если РезультатОбработки.code = 610 Тогда
		//	// Отсутствует часть настроек из меню.
		//	КлассIntegrationConnection.ПараметрыРаботы.СбисМеню.Обновить = Истина;
		//	СообщитьПользователю = МодульОбъектаКлиент().НовыйСбисИсключение(	РезультатОбработки,
		//																		"ФайлыНастроекСервер.РезультатОбработки");
		//	МодульОбъектаКлиент().СбисСообщить(РезультатОбработки, Новый Структура("Отложенно", Истина));
		//Иначе
...
&НаКлиенте
Процедура	КлассIntegrationConnection_ИсправитьОшибкиЦелостности(КлассIntegrationConnection, ИсключениеЦелостности, ДопПараметры = Неопределено)
	Перем ЗначениеИниВКлассе;

	Отказ	= Ложь;
	Кэш		= МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш();
	
	ПроблемыЦелостности = ИсключениеЦелостности.dump;
	
	Если Не ПроблемыЦелостности.Неполучено = Неопределено Тогда
		
		Для Каждого КлючИЗначение Из ПроблемыЦелостности.Неполучено Цикл
			
			ИмяИни	= ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, КлючИЗначение.Значение.name);
			ТипИни	= КлючИЗначение.Значение.type;
			
			Если Кэш.КэшНастроек.ПараметрыНастроек.ДоступныеТипыИни.Найти(ТипИни) = Неопределено Тогда
				
				ПриемникИсправления = КлассIntegrationConnection.Данные.Прочие
				
			Иначе

				ПриемникИсправления = КлассIntegrationConnection.Данные.Ини;

			КонецЕсли;
			
			Если ПриемникИсправления = Неопределено Тогда
				
				Продолжить;
				
			КонецЕсли;
			
			// Если ини в приемнике есть и она неопределена, то удалим её из класса
			Если	ПриемникИсправления.Свойство(ИмяИни, ЗначениеИниВКлассе)
				И	ЗначениеИниВКлассе = Неопределено Тогда
				
				ПриемникИсправления.Удалить(ИмяИни);
	...
	Если Не ПроблемыЦелостности.Излишки = Неопределено Тогда
		
		ДанныеСписка = КлассIntegrationConnection_СписокИни(Кэш, КлассIntegrationConnection, Новый Структура, Отказ);
		Если Отказ Тогда
			
			ВызватьИсключение(МодульОбъектаКлиент().СбисИсключение_Представление(ДанныеСписка));
			
		КонецЕсли; 
		
		Для Каждого КлючИЗначение Из ПроблемыЦелостности.Излишки Цикл
			
			Если Не ДанныеСписка.Карта.НайтиПоЗначению(КлючИЗначение.Ключ) = Неопределено Тогда
				
				// Если излишняя ини включена, то ничего страшного.
				Продолжить;
			...
			// Если лишняя ини не должна была быть получена и отключена в меню, то удалить её из данных класса
			ПриемникИсправления.Удалить(ИмяИни);
	...
	// Пропатчить основной класс после переноса
	ОписаниеСообщения = Новый Структура("Текст, СтатусСообщения, ЭлементНазначения, ФормаВладелец");
	ОписаниеСообщения.Текст				= "В настройках исправлены ошибки. Требуется сохранить изменения.";
	ОписаниеСообщения.СтатусСообщения	= СтатусСообщения.Обычное;
	ОписаниеСообщения.ФормаВладелец		= Кэш.ФормаНастроекОбщее;
	
	// Выставить признаки, чтобы коннекшен перезаписался с перегенерацией меню, с учетом исправлений
	КлассIntegrationConnection.ПараметрыРаботы.СбисМеню.Обновить = Истина;
	КлассIntegrationConnection.ПараметрыРаботы.Изменен = Истина;
	КлассIntegrationConnection.ПараметрыРаботы.Вставить("Предупреждение", ОписаниеСообщения);
...
Функция		КлассIntegrationConnection_ПолучитьИни(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	ИначеЕсли МассивПолучитьИтого.Количество() Тогда
		...
		ПолучитьТолькоПользовательскиеИзменения =	ДопПараметры.Свойство("ТолькоПользовательские")
												И	ДопПараметры.ТолькоПользовательские;
		ИсправитьПроблемыИни					=	ДопПараметры.Свойство("ИсправитьПроблемыИни")
												И	ДопПараметры.ИсправитьПроблемыИни; 
												
		Если ДопПараметры.Свойство("КонтролироватьИни") Тогда
			
			// Если признак прокинут явно, то работать от него
			КонтролироватьЧтениеИни = ДопПараметры.КонтролироватьИни;
			
		Иначе
			
			// Автоопределение - контролируем всегда, если только это не запрос пользовательских изменений
			// Если получаем пользовательские изменения для выгрузки, то по-дефолту неизвестно, что есть в дельте.
			КонтролироватьЧтениеИни = Не ПолучитьТолькоПользовательскиеИзменения;			
			
		КонецЕсли;	
		
		// Что-то дозапрашиваем
		...
		ПараметрыПеренести = Новый Структура("НаборИниПеренести", МассивПолучитьИтого);
		
		Если Отказ Тогда
			
			Если	РезультатОбработки.code = 610
				И	ИсправитьПроблемыИни Тогда
				
				Попытка
					
					// Пропатчить класс, от которого было чтение исправлением структуры ини
					КлассIntegrationConnection_ИсправитьОшибкиЦелостности(КлассIntegrationConnection, РезультатОбработки);
					Отказ = Ложь;
					
				Исключение
					
					РезультатОбработки = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "ФайлыНастроекСервер.КлассIntegrationConnection_ПолучитьИни");
		...
		Иначе
			...
			КлассIntegrationConnection_ПолучитьИни_Перенести(КлассIntegrationConnection, КлассДляЧтения, ПараметрыПеренести);
			
			// После переноса взять от готового класса данные по списку
			Для Каждого ЭлементСписка Из МассивПолучитьИтого Цикл

				ИмяИни = ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, ЭлементСписка.name);
				Если	Кэш.КэшНастроек.ПараметрыНастроек.ДоступныеТипыИни.Найти(ЭлементСписка.type) = Неопределено Тогда
					
					ИсточникДанных = КлассIntegrationConnection.Данные.Прочие;
					
				Иначе
					
					ИсточникДанных = КлассIntegrationConnection.Данные.Ини;
					
				КонецЕсли;
				
				Если		ИсточникДанных = Неопределено
					Или	Не	ИсточникДанных.Свойство(ИмяИни, ЗначениеИни) Тогда
...
// Процедура - переносит ини по списку из одного класса в другой
//
// Параметры:
//  КлассIntegrationConnection	 - КлассIC		 - 
//  КлассДляЧтения				 - КлассIC		 - 
//  ДопПараметры				 - Структура	 - 
//
&НаКлиенте
Процедура	КлассIntegrationConnection_ПолучитьИни_Перенести(КлассIntegrationConnection, КлассДляЧтения, ДопПараметры)
	Перем ЗначениеВставить;
	
	Кэш					= МодульОбъектаКлиент().ПолучитьТекущийЛокальныйКэш();
	ОписаниеСбисМеню	= СбисОписаниеИниМеню(Кэш);
	МассивПолучитьИтого	= ДопПараметры.НаборИниПеренести;
	...
	Для Каждого ЭлементСписка Из МассивПолучитьИтого Цикл
		...
		Если ЭлементСписка.name = ОписаниеСбисМеню.name Тогда
			...
			// Меню живёт в отдельном поле
			Если КлассДляЧтения.Данные.Меню = Неопределено Тогда
				
				// Меню запрашивали, но его по какой-то причине нет. Может быть не поддерживаемая конфа.
				КлассДляЧтения.Данные.Меню = Новый Структура;
		...
		Если	Кэш.КэшНастроек.ПараметрыНастроек.ДоступныеТипыИни.Найти(ЭлементСписка.type) = Неопределено Тогда
			...
			ПриемникЗначения = КлассIntegrationConnection.Данные.Прочие;
			Если ПриемникЗначения = Неопределено Тогда
				
				//Инитим Данные только если есть ини, которые должны быть добавлены в класс
				ПриемникЗначения  = Новый Структура;
				КлассIntegrationConnection.Данные.Прочие = ПриемникЗначения;
				
			КонецЕсли;
			ИсточникЗначения = КлассДляЧтения.Данные.Прочие;
			
		Иначе
			
			ПриемникЗначения = КлассIntegrationConnection.Данные.Ини;
			Если ПриемникЗначения = Неопределено Тогда
				
				//Инитим Данные только если есть ини, которые должны быть добавлены в класс
				ПриемникЗначения  = Новый Структура;
				КлассIntegrationConnection.Данные.Ини = ПриемникЗначения;
				
			КонецЕсли;
			ИсточникЗначения = КлассДляЧтения.Данные.Ини;

		КонецЕсли;
		
		Если Не ИсточникЗначения.Свойство(ИмяИни, ЗначениеВставить)
			Или	ЗначениеВставить = Неопределено Тогда
			
			// Ини есть в меню, но отсутствует в данных. Пропускаем.
			Продолжить;
			
		КонецЕсли;
						
		ПриемникЗначения.Вставить(ИмяИни, ЗначениеВставить);
...
Функция		КлассIntegrationConnection_СохранитьПодключение(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ) Экспорт
	...
	Если КлассIntegrationConnection.ПараметрыРаботы.ТипНастроек = "Стандартные" Тогда
		//Для создания нового подключения с стандартными настройками обязательно нужно передать СбисПараметры с service4config
		КлассIntegrationConnection.ПараметрыРаботы.СбисПараметры.Обновить = Истина;
		// Переопределить параметры конфигурации от БЛ, т.к. изначально стандартные настройки генерируются от текущей конфы
		КлассIntegrationConnection.ПараметрыКонфигурации = Кэш.ФормаНастроекОбщее.НайтиСтандартнуюКонфигурациюНастроек(КлассIntegrationConnection.ПараметрыКонфигурации);	
...
Функция		КлассIntegrationConnection_СписокИни(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
    Перем Искать, ТипыИни, Данные;
	Если НЕ КлассIntegrationConnection.Свойство("Данные", Данные)
		ИЛИ Данные.Меню = Неопределено Тогда
...
Функция		КлассIntegrationConnection_СформироватьСбисМеню(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	РезультатДополучения = КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection, ДопПараметры, Отказ);
	Если Отказ Тогда
...
Функция		КлассIntegrationConnection_Установить(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ) Экспорт
	Перем	ТихийРежим,				// Флаг управления выводом статуса установки
	        ...
	        ПрименитьXSLT,			// Необходимость получения и применения xslt. Необходимо для возможности установить пользовательскую
	...
	Если ПолучитьДанныеИни Тогда
		РезультатДозапроса = КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection, ПараметрыДозапросаИни, Отказ);
		Если Отказ Тогда
