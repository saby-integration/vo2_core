...
Функция		КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	МодульОбъектаКлиент().КоннекшенSABY_Очистить(КлассIntegrationConnection, "ОписаниеИни");
...
Функция		КлассIntegrationConnection_Инициализировать(Кэш, ДопПараметры, Отказ)
	...
	Если ДопПараметры.Свойство("ДанныеБЛ", ДанныеБЛ) Тогда
	...
	Если ИнициироватьДанные Тогда
		...
		Попытка
			...
			КлассIntegrationConnection_ИнициироватьИниПоУмолчанию(Кэш, КлассIntegrationConnection);
			
		Исключение
			
			Отказ = Истина;
			Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "ФайлыНастроекСервер.КлассIntegrationConnection_Инициализировать");
...
Процедура	КлассIntegrationConnection_ИсправитьОшибкиЦелостности(КлассIntegrationConnection, ИсключениеЦелостности, ДопПараметры = Неопределено)
	...
	ПроблемыЦелостности = ИсключениеЦелостности.data;
	...
	Если Не ПроблемыЦелостности.Излишки = Неопределено Тогда
		...
		Если Отказ Тогда
			...
			ВызватьИсключение(МодульОбъектаКлиент().СбисИсключение_Выгрузить(ДанныеСписка));
...
Процедура	КлассIntegrationConnection_ПолучитьИни_Перенести(КлассIntegrationConnection, КлассДляЧтения, ДопПараметры)
	...
	Для Каждого ЭлементСписка Из МассивПолучитьИтого Цикл
		...
		Если Не ИсточникЗначения = Неопределено И Не ИсточникЗначения.Свойство(ИмяИни, ЗначениеВставить)
...
&НаКлиенте
Функция		КлассIntegrationConnection_СформироватьСбисМеню(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ) Экспорт
...
Функция		КлассIntegrationConnection_Установить(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ) Экспорт
	...
	// Изменить выбранные настройки не глобально, в локальный кэш.
	ПараметрыИзменить = Новый Структура("Кэш, Глобально", Кэш, Ложь);
	МодульОбъектаКлиент().ИзменитьПараметрSaby("ВыбранныеНастройки", КлассIntegrationConnection, "Настройки", ПараметрыИзменить);
	...
	ОтключеныДоработки	= МодульОбъектаКлиент().КоннекшенSaby_Получить(КлассIntegrationConnection, "ОтключеныДоработки");
	ПолучитьДанныеИни	=	ПолучитьДанныеИни
						Или ОтключеныДоработки;
