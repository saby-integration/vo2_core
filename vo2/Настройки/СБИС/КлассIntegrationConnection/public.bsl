...
Функция		КлассIntegrationConnection_Инициализировать(Кэш, ДопПараметры, Отказ)
	...
	Если Не ДопПараметры.Свойство("ДанныеБЛ", ДанныеБЛ) Тогда
		Если ИнициироватьДанные
			И ЗначениеЗаполнено(КлассIntegrationConnection.ПараметрыКонфигурации.subsystem) Тогда
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(КлассIntegrationConnection.ПараметрыКонфигурации.subsystem)
			И КлассIntegrationConnection.Данные.Меню = Неопределено Тогда
			ПараметрыЧтенияИни = Новый Структура("СписокИни", СписокИниПолучить);
