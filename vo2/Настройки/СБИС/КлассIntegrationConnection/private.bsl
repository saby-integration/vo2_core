...
Функция		_КлассIntegrationConnection_ЗаполнитьПоДанным(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	Если	КонтролироватьЧтениеИни Тогда
		...
		Если	КонтрольНаПрочитать.Количество() Тогда
			...
			СтрокаОшибки	= СтрокаОшибки + Символы.ПС + "Не получены запрошенные файлы ини {Непрочитано}. Требуется отключить лишние файлы настроек для продолжения работы";
			Отказ			= Истина;
		...
		Если ЗначениеЗаполнено(КонтрольНаИзлишки) Тогда
			...
			СтрокаОшибки = СтрокаОшибки + Символы.ПС + "Неизвестная ини {Излишнее} в ответе. Обратитесь в техническую поддержку.";
