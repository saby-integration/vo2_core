...
Функция		_КлассIntegrationConnection_Записать(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	Если ЕстьИниКЗаписи
		И (КлассIntegrationConnection.ПараметрыРаботы.ТипНастроек = "Шаблон") Тогда // Ини только на шаблон!!!
		...
		Для Каждого ИмяИниВО Из КлассIntegrationConnection.ПараметрыРаботы.СбисИни.СписокОтправить Цикл
			...
			ИмяИниJson	= ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, ИмяИниВО, "во_json");
	...
	Если ФормированиеМеню Тогда
		ЗаписьИни = СбисОписаниеИниМеню(Кэш, Кэш.РаботаСJSON.ПреобразоватьЗначениеВJSON(КлассIntegrationConnection.Данные.Меню));
		ini_array.Добавить(ЗаписьИни);
...
Функция		_КлассIntegrationConnection_ЗаполнитьПоДанным(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	Перем ДанныеИниНаЧтение, СтрокаJson, ТипИни, КонтрольНаИзлишки, КонтрольНаПрочитать;
	
	СбисДанныеБЛ = ДопПараметры.ДанныеБЛ;
	//Если указано, какие ини читали, то проконтролировать, что всё получено в полном объеме и ничего лишнего не пришло.
	КонтролироватьЧтениеИни = ДопПараметры.Свойство("КонтрольИни", КонтрольНаПрочитать);
	...
	Если Не КлассIntegrationConnection.ПараметрыРаботы.ИнициированаШапка Тогда
		...
		Если СбисДанныеБЛ.Свойство("parent") Тогда
			
			МодульОбъектаКлиентСервер().КоннекшенSABY_Установить(КлассIntegrationConnection, "ИдентификаторИнтеграции", СбисДанныеБЛ.parent);
			
		КонецЕсли;

		Если СбисДанныеБЛ.Свойство("update_date", КлассIntegrationConnection.ПараметрыРаботы.ДатаИзмененияНастроек) 
				И ЗначениеЗаполнено(КлассIntegrationConnection.ПараметрыРаботы.ДатаИзмененияНастроек) Тогда//Дата последней правки настроек
	...
	Если	СбисДанныеБЛ.Свойство("data", ДанныеИниНаЧтение)
			...
			Попытка
				...
				Для	Каждого КлючИЗначение Из Соединениеdata Цикл
					...
					Если	КонтролироватьЧтениеИни Тогда
						...
						ЗначениеКонтроля	= КонтрольНаПрочитать.Получить(КлючКонтроля);
						Если ЗначениеКонтроля = Неопределено Тогда
							
							// Пришло что-то, чего мы не ожидали
							Если КонтрольНаИзлишки = Неопределено Тогда
								
								КонтрольНаИзлишки = Новый Соответствие;
								
							КонецЕсли;
							
							КонтрольНаИзлишки.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
							Продолжить;

						КонецЕсли;
						КонтрольНаПрочитать.Удалить(КлючКонтроля);
					КонецЕсли;
		...
		Если ЗначениеЗаполнено(ДанныеИниНаЧтение) Тогда
			...
			Для Каждого КлючИЗначениеИни Из ДанныеИни Цикл
				...
				ИмяИни	= ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, КлючИЗначениеИни.Ключ);
				
				Если КлючИЗначениеИни.Значение.Свойство("ТипИни", ТипИни) Тогда
					
					ТипИни	= ТипИни.Значение;
					
				Иначе
					
					// Тип на ини не задан явно, взять его от имени
					ТипИни	= СтрЗаменить(КлючИЗначениеИни.Ключ, "_", Символы.ПС);
					ТипИни	= СтрПолучитьСтроку(ТипИни, 1);
					
				КонецЕсли;
				
				Если Кэш.КэшНастроек.ПараметрыНастроек.ДоступныеТипыИни.Найти(ТипИни) = Неопределено Тогда
					
					// Это неизветсный тип. Возможно, если принудительное чтение ини, или БЛ вернул что-то левое.
	...
	Если	КлассIntegrationConnection.Данные.Параметры.Свойство("DisableCustomAlgorithms", ДанныеИниНаЧтение)
		И	ДанныеИниНаЧтение Тогда
		
		// Установить признаки на Коннекшен об отключенных ини
		МодульОбъектаКлиент().КоннекшенSABY_Установить(КлассIntegrationConnection, "ОтключеныДоработки",	Истина);
		
	КонецЕсли;
		
	Если	КонтролироватьЧтениеИни Тогда
		
		СтрокаОшибки	= КлассIntegrationConnection.Название + ":";
		ПараметрыФормат	= Новый Структура("Непрочитано, Излишнее", "", "");
		Если	КонтрольНаПрочитать.Количество() Тогда
			
			СтрокаОшибки	= СтрокаОшибки + Символы.ПС + "Не получены запрошенные файлы ини {Непрочитано}.";
			Отказ			= Истина;
			Для Каждого  ЭлементНаКонтроль Из КонтрольНаПрочитать Цикл
				
				ПараметрыФормат.Непрочитано	= ПараметрыФормат.Непрочитано
											+ ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, ЭлементНаКонтроль.Значение.name, "json_во") + ", ";
				
			КонецЦикла;
			
			ПараметрыФормат.Непрочитано = Лев(ПараметрыФормат.Непрочитано, СтрДлина(ПараметрыФормат.Непрочитано) - 2);
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(КонтрольНаИзлишки) Тогда
			...
			СтрокаОшибки = СтрокаОшибки + Символы.ПС + "Неизвестная ини {Излишнее} в ответе.";
			
			Для Каждого  ЭлементНаКонтроль Из КонтрольНаИзлишки Цикл
				
				ПараметрыФормат.Излишнее	= ПараметрыФормат.Излишнее
											+ ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, ЭлементНаКонтроль.Ключ, "json_во") + ", ";
				
				
			КонецЦикла;
			
			ПараметрыФормат.Излишнее = Лев(ПараметрыФормат.Излишнее, СтрДлина(ПараметрыФормат.Излишнее) - 2);
		...
		Если Отказ Тогда
			...
			ДампОшибкиЦелостности = Новый Структура("Неполучено, Излишки", КонтрольНаПрочитать, КонтрольНаИзлишки);
			ОшибкаЦелостности = МодульОбъектаКлиент().НовыйСбисИсключение(,
																			"ФайлыНастроекСервер.КлассIntegrationConnection.ЗаполнитьПоДанным",
																			610, "Возникла проблема разбора файлов настроек.",
																			МодульОбъектаКлиент().ПрименитьФорматКСтроке(СтрокаОшибки, ПараметрыФормат),
																			ДампОшибкиЦелостности);
																			
			Возврат ОшибкаЦелостности;
			// Блокирующая ошибка, должно всё быть точно как в аптеке.
			//МодульОбъектаКлиент().СбисСообщить(ОшибкаЦелостности, Новый Структура("Отложенно", Истина));
...
Функция		_КлассIntegrationConnection_ВыполнитьЧтениеСБЛ(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	Иначе
		...
		Запрос_Фильтр.Вставить("requested_ini_format", Кэш.КэшНастроек.ПараметрыНастроек.ПрефиксСервис);
		...
		Если		ДопПараметры.Свойство("ВключеныДоработки")
			И	Не	ДопПараметры.ВключеныДоработки Тогда
			//Если получаем только пользовательские изменения
			Запрос_Фильтр.Вставить("read_custom", Ложь);
		КонецЕсли;	
...
Функция		_КлассIntegrationConnection_ИнитКоннекшен(Кэш, ДопПараметры) Экспорт
	...
	Если Кэш.ПараметрыСистемы.Конфигурация.Свойство("УИДИнтеграцииСтарый") Тогда
		ПараметрыВызова1.Вставить("ExtSysUidOld", Кэш.ПараметрыСистемы.Конфигурация.УИДИнтеграцииСтарый);		
	КонецЕсли;
	Если ДопПараметры.Свойство("Идентификатор") Тогда
		ПараметрыВызова1.ConnectionId		= ДопПараметры.Идентификатор;
	КонецЕсли;
	
	Если	Не ЗначениеЗаполнено(ПараметрыВызова1.ConnectionId)
		И	Не ЗначениеЗаполнено(ПараметрыВызова1.ExtSysUid) Тогда
		
		ОшибкаВызова = МодульОбъектаКлиент().НовыйСбисИсключение(,
											"КлассIntegrationConnection.ИнитКоннекшен",
											510701,,
											"Запуск невозможен, так как у пользователя отсутствует доступ к хранилищу общих настроек.");
		ВызватьИсключение МодульОбъектаКлиент().СбисИсключение_Выгрузить(ОшибкаВызова); 
...
#Область include_core2_vo2_Настройки_СБИС_КлассIntegrationConnection_private_ЗначенияПоУмолчанию
#КонецОбласти
