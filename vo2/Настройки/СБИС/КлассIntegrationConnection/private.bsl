...
Функция		_КлассIntegrationConnection_ЗаполнитьПоДанным(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	Если	КонтролироватьЧтениеИни
		...
		// Сообщить о проблеме, не блокировать работу
		Если Не ДопПараметры.Свойство("КонтролироватьИни") Тогда
			ОшибкаЦелостности = МодульОбъектаКлиент().НовыйСбисИсключение(,
																			...
																			КлассIntegrationConnection.Название + СтрЗаменить(": не получены запрошенные файлы настроек {0}. Требуется отключить лишние файлы настроек для продолжения работы", "{0}", СтрокаОшибки));
			//Отказ = Истина;
