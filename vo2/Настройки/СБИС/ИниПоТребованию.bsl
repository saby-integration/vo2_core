...
Функция		Ини(Кэш, ИмяИни, ДопПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	Если 	Не	ДопПараметры = Неопределено Тогда
		...
		Если	ДопПараметры.Свойство("Тип", ТипИни)
			...
			Если ТипИни = "ПроверкаРасхождения" Тогда
				...
				ПараметрыЧТения.Вставить("КонтролироватьИни", Ложь);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	КонецЕсли;
	Результат = КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection, ПараметрыЧТения, Отказ);
    Если Отказ Тогда
		Возврат Кэш.ОбщиеФункции.СбисИсключение(Результат, "ФайлыНастроекСервер.Ини");
	КонецЕсли;
	
	Если Результат.Свойство(ИмяИни, ЗначениеИни) Тогда
		Если Не Принудительно Тогда
	...
	Отказ = Истина;
	Возврат Кэш.ОбщиеФункции.СбисИсключение(779, "ФайлыНастроекСервер.Ини",,,СтрЗаменить("Ини ""{}"" не найдена в Saby Меню.", "{}", ИмяИни));
...
Функция		СбисОбновитьДанныеПоНастройкам(Кэш, ДополнительныеПараметры, Отказ) Экспорт
	...
	// На случай локального экземпляра настроек, передадим кэш для извлечения объект
	ПараметрыПолучить	= Новый Структура("Кэш, Тип", Кэш, "Настройки");
	ВыбранныеНастройки	= МодульОбъектаКлиент().ПараметрSaby("ВыбранныеНастройки", ПараметрыПолучить);
	Если ВыбранныеНастройки = Неопределено Тогда
		
		Отказ = Истина;
		Возврат МодульОбъектаКлиент().NewExtException(,"ФайлыНастроекСервер.СбисОбновитьДанныеПоНастройкам", 615,,"Отсуствует установленное подключение к настройкам");
		
	КонецЕсли;
	
	ИдНастроек = МодульОбъектаКлиент().КоннекшенSaby_Получить(ВыбранныеНастройки, "Идентификатор");
	Если ЗначениеЗаполнено(ИдНастроек) Тогда
		
		РезультатОбновления = КлассIntegrationConnection_ДозапроситьИни(Кэш.КэшНастроек.ВыбранныеНастройки, Новый Структура, Отказ);
		Если Отказ Тогда
			
			Возврат МодульОбъектаКлиент().NewExtException(РезультатОбновления,"ФайлыНастроекСервер.СбисОбновитьДанныеПоНастройкам");
...
Функция		СбисПроверитьУстановленныеНастройки(Кэш, ПараметрыУстановкиВходящие, СтруктураИни, СтруктураПараметров, ИдентификаторНастроек) Экспорт	
	Отказ = Ложь;
	КлассIntegrationConnection	= Кэш.КэшНастроек.ВыбранныеНастройки;
	КоннекшенSaby_Прочитать(КлассIntegrationConnection);
	РезультатДозапроса			= КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection,Новый Структура,Отказ);
	Если Отказ Тогда
	...
	ПараметрыУстановкиВходящие.Вставить("СписокИни", ПолучитьМассивУстановленныхИниФайлов(СтруктураИни, КлассIntegrationConnection, Отказ));
...
Функция		СбисПолучитьМассивУстановленныхИниФайлов(Кэш, СбисДополнительныеПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	РезультатДозапроса = КлассIntegrationConnection_ДозапроситьИни(ВыбранныеНастройки, Новый Структура, Отказ);
	Если Отказ Тогда
...
Функция		СформироватьСтруктуруТекущихНастроек(Кэш, ДопПараметры, Отказ) Экспорт
	...
	Для Каждого КлючИЗначение Из Кэш.Ини Цикл
		Если КлючИЗначение.Значение = Неопределено Тогда
			ВыбранныеНастройки.Данные.Ини.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение)
		Иначе
			...
			Если ПараметрыИни.Тип = "Загрузка" Тогда
				КлючИни = Кэш.СБИС.МодульОбъектаКлиент.ПрименитьФорматКСтроке("{Тип}_{Направление}_{Название}", ПараметрыИни);
			Иначе
	...
	РезультатДозапроса = КлассIntegrationConnection_ДозапроситьИни(ВыбранныеНастройки, ДопПараметры, Отказ);
	Если Отказ Тогда
