...
Функция		ПолучитьСписокConnection(Кэш, ПараметрыПолучения, Отказ)
	...
	КонецЕсли;
	Если ПараметрыПолучения.ДополнительныеПараметры.Свойство("ExtSysUid") Тогда
		ПараметрыЗапроса.Вставить("ExtSysUid", ПараметрыПолучения.ДополнительныеПараметры.ExtSysUid);
	Иначе
		ПараметрыЗапроса.Вставить("ExtSysUid", Кэш.ПараметрыСистемы.Конфигурация.УИДИнтеграции);
	КонецЕсли;
	...
	ПолучитьСОшибками	= ПараметрыПолучения.Свойство("ПолучитьСОшибками")
						И ПараметрыПолучения.ПолучитьСОшибками;
...
Функция		ПолучитьСписокConfig(Кэш, ПараметрыПолучения, Отказ)
	...
	ПараметрыСообщения.Вставить("service", Кэш.КэшНастроек.ПараметрыНастроек.ПрефиксСервис);
	ПараметрыСообщения.Вставить("ProductVersion", Кэш.ПараметрыСистемы.Обработка.Версия);	
