...
Функция		ЗагрузитьНастройкиИзКаталогаНаСервер(Кэш, КаталогСНастройками, Отказ)
	...
	Если МодульОбъектаКлиент().ПолучитьЗначениеФичи("ВыгрузкаНастроекНаШаблон") Тогда
...
&НаКлиенте
Функция		ПолучитьИмяИни(ПараметрыНастроек, КлючИни, Направление="json_во")
...
Функция		ПрочитатьНастройкиЧерезИнитКоннекшен(Кэш, ДопПараметры = Неопределено)
	...
	Попытка
		...
		СбисЗаписатьИдентификаторНастроек(Кэш, КлассIntegrationConnection_Идентификатор(Кэш, КлассКонекшенаПрименить));
