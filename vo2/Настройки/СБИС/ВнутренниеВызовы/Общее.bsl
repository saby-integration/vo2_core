...
Функция		ЗагрузитьНастройкиИзКаталогаНаСервер(Кэш, КаталогСНастройками, Отказ)
	Перем ЗначениеИни, УстановленныеИниФайлы;
	...
	ПараметрыНастройкиПОлучены = ВременныйКэш.ФормаНастроек.СбисПолучитьНастройки(ВременныйКэш, Отказ);
	ДатаИзмененияНастроек = ТекущаяДата();
	
	Если		ПараметрыНастройкиПОлучены = Неопределено
		Или	Не	ТипЗнч(ПараметрыНастройкиПолучены) = Тип("Структура")
		Или	Не	ПараметрыНастройкиПолучены.Свойство("СписокФайлов", УстановленныеИниФайлы) Тогда
		Отказ	= Истина; 
	...
	// Прогон два раза: для шаблона и подключения (ини могут отличаться на обоих коннекшенах)
	...
	КлассIntegrationConnectionШаблонБыло = НовыйIntegrationConnection(Кэш, Новый Структура("Идентификатор", КлассIntegrationConnectionБыло.ПараметрыРаботы.Шаблон), Отказ);
	КлассIntegrationConnection_Очистить(Кэш, КлассIntegrationConnectionШаблонБыло, Новый Структура("Ини", Истина), Отказ);
	КлассIntegrationConnectionШаблонНовый = НовыйIntegrationConnection(Кэш, Новый Структура("БазовыйКласс, Ини", КлассIntegrationConnectionШаблонБыло, ВременныйКэш.Ини), Отказ);
	Кэш.КэшНастроек.Вставить("КоннекшенИнтеграции", КлассIntegrationConnectionШаблонНовый);
...
Функция		ИнициироватьРежимДиалога(Кэш, ДопПараметры, Отказ)
	...
	УправляемоеПриложение = Кэш.ПараметрыСистемы.Клиент.УправляемоеПриложение;
	
	Если УправляемоеПриложение Тогда
		
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "user").Видимость		= Истина;
		МодульОбъектаКлиент().ПолучитьЭлементФормыОбработки(ЭтаФорма, "account").Видимость	= Истина;    
...
&НаКлиенте
Функция		ПолучитьИмяИни(ПараметрыНастроек, КлючИни, Направление="json_во") Экспорт
	...
	ИмяИни	= КлючИни;
	Для Каждого Соответствие Из ПараметрыНастроек.СоответствиеТипов	Цикл
		Если Лев(ИмяИни, СтрДлина(Соответствие[КлючНаправления1])) = Соответствие[КлючНаправления1] Тогда
			...
			Иначе
				...
				ЧистоеИмя	= Сред(ИмяИни, СтрДлина(Соответствие[КлючНаправления1]) + ?(ПустаяСтрока(Соответствие[КлючНаправления1]), 0,  2));
				Если ПараметрыНастроек.СоответствиеНазваний.Свойство(ЧистоеИмя) Тогда
					ЧистоеИмя = ПараметрыНастроек.СоответствиеНазваний[ЧистоеИмя]; 
				КонецЕсли;
				ИмяИни		= Соответствие[КлючНаправления2] +
							?(ПустаяСтрока(Соответствие[КлючНаправления2]), "", "_") + //переводим имена по соответствию имен, убирая старый префикс.
							  ЧистоеИмя;
			КонецЕсли;
			Прервать;
		КонецЕсли;
...
Функция		СформироватьТаблицуДоступныхНастроек(Кэш, ДопПараметры, Отказ)
	...
	ПараметрыПолучитьСписок		= Новый Структура("ПараметрыКонфигурации, ДополнительныеПараметры, СписокИни", ПараметрыКонфигурацииСписка, ДопПараметры);
	ПараметрыПолучитьСписок.Вставить("ПолучитьСОшибками", Истина);
	РезультатЗапроса = ПолучитьСписокConnection(Кэш, ПараметрыПолучитьСписок, Отказ);
	Если Отказ Тогда
	...
	Для Каждого СтрокаПолучено Из РезультатЗапроса Цикл
		...
		Если МодульОбъектаКлиент().ЭтоТипСбис(СтрокаПолучено, "СбисИсключение") Тогда
			
			МодульОбъектаКлиент().СбисСообщить(СтрокаПолучено);
			Продолжить;
	...
	Если ДопПараметры.ТипКоннекшена = "account" Тогда
		...
		Если Отказ Тогда
			Если		ПустойКлассПодключения.code = 779 Тогда
				
				// Подключение отсутствует
				Отказ = Ложь;
			
			ИначеЕсли	ПустойКлассПодключения.code = 756 Тогда
				
				// Есть на другом аккаунте
				Отказ = Ложь;
	...
	Если СоздатьСтандартныйКласс Тогда
		...
		Если Отказ Тогда
			...
			Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ПустойКлассПодключения, "ФайлыНастроекСервер.СформироватьТаблицуДоступныхНастроек");
...
Функция		ОтключитьФайлыНастроекНаСервере(СписокИниКОтключению, Кэш, Идентификатор, Отказ)
	...
	СтруктураВыбранныхНастроек.ПараметрыРаботы.Вставить("ПроверитьИниПриСохранении", Истина);
...
Функция		ПолучитьМассивУстановленныхИниФайлов(КэшИни, ВыбранныеНастройки=Неопределено, Отказ=Ложь, ПрефиксИни="")
	...
	Если Не ВыбранныеНастройки = Неопределено Тогда
		
		МодульОбъектаКлиент().КоннекшенSABY_Установить(ВыбранныеНастройки, "ОписаниеИни", СписокИниФайлов);

	КонецЕсли;
	Возврат СписокИниФайлов;
	
КонецФункции

&НаКлиенте
Функция		ПараметрыСтандартнойКонфигурации(Кэш, ПараметрыКонфигурации = Неопределено)
	
	ПараметрыСтандартнойКонфигурации = Новый Структура("service, subsystem, version");
	Если Не ПараметрыКонфигурации = Неопределено Тогда
...
Процедура	СбисПеречитатьТекущиеНастройки(ПараметрыПеречитать, Кэш) Экспорт
	...
	ИдПодключения = МодульОбъектаКлиент().ПараметрSaby("ИдентификаторПодключения", "Пользователь");
	
	ПараметрыВосстановить = Новый Структура;
	ПараметрыВосстановить.Вставить("Идентификатор",			ИдПодключения);
...
&НаКлиенте
Функция		СбисОписаниеИни(Кэш, data_ini = Неопределено, type_ini = Неопределено, name_ini = Неопределено)
	
	РезультатОписание = Новый Структура("active, format, data, type, name", Истина, Кэш.КэшНастроек.ПараметрыКонфигурации.service);
	РезультатОписание.data = data_ini;
	РезультатОписание.type = type_ini;
	РезультатОписание.name = name_ini;
...
Функция		СбисПолучитьНастройки_ОбработатьПараметрыРаботы(Кэш, Отказ, ПродолжитьВыполнение)
	...
	Если	ПараметрыРаботы.Свойство("ВыполнитьПроверку", ПараметрРаботы)
		...
		СписокИниФайлов =  МодульОбъектаКлиент().КоннекшенSABY_Получить(ВыбранныеНастройки, "ОписаниеИни");
		Если Отказ Тогда
			Возврат СписокИниФайлов;
		КонецЕсли;
		Возврат	Новый Структура("СписокФайлов", СписокИниФайлов);
	КонецЕсли;
...
Функция		ПрочитатьНастройкиЧерезИнитКоннекшен(Кэш, ДопПараметры = Неопределено)
	...
	Попытка
		...
		КлассКонекшенаПрименить = НовыйIntegrationConnection(Кэш, Новый Структура("Идентификатор, ИнитКоннекшен", ИдПодключения, Истина), ОшибкаУстановки);
...
// Получает в кэш структуру XSLT при её отсутствии там
&НаКлиенте
...
//  НазваниеXSLT	 - Строка		- Полное название файла XSLT (с расширением)
//  Отказ			 - Булево
// 
...
//
&НаКлиенте
Функция		СбисПолучитьXSLTПоНазванию(Кэш, Идентификатор, НазваниеXSLT, Отказ) Экспорт
	
	Если Не Кэш.ТихийРежим Тогда
		Кэш.ГлавноеОкно.СбисПоказатьСостояние("Получение XSLT", Кэш.ГлавноеОкно);
	КонецЕсли;
	
	ДопПараметрыЗапроса	= Новый Структура("ВернутьОшибку, ЕстьРезультат", Истина, Истина);
	
	РезультатЗапроса = Кэш.Интеграция.ПолучитьXslt(Кэш, НазваниеXSLT, ДопПараметрыЗапроса, Отказ);
...
Функция		СформироватьСпискиИзМеню(ТипыИни, СбисМеню, Искать, СписокИни = Неопределено)	
	...
	Для	Каждого	КлючИЗначение Из СбисМеню Цикл
		...
		Если КлючИЗначение.Значение.Свойство("enable", enable) Тогда	
			...
			ТипИни				= Неопределено;
			ПользовательскаяИни	= Неопределено;
			...
			Если Не	Карта.НайтиПоЗначению(КлючКарты) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если Не КлючИЗначение.Значение.Свойство("custom", ПользовательскаяИни) Тогда
				ПользовательскаяИни = 0;
			КонецЕсли;
			Раздел.Добавить(Новый Структура("type,name,custom", ТипИни,ИмяИни,ПользовательскаяИни));
...
Процедура	ЧтениеИни_ОбработатьРезультат(Кэш, Результат, ПараметрыКонвертации = Неопределено)
	...
	Если Не Результат.Данные.Ини = Неопределено Тогда
		Для Каждого КлючИЗначение Из Результат.Данные.Ини Цикл
			...
			Иначе
				ИмяИни = ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, КлючИЗначение.Ключ);
