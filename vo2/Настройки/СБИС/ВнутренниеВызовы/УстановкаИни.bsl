...
Функция		УстановитьФайлыНастроекНаСервере(МассивФайловДляУстановки, Кэш, СтруктураВыбранныхНастроек, НашаКонфигурация, Отказ)
	...
	КонецЕсли;
	ИниКУстановке = КлассIntegrationConnection_ДозапроситьИни(КлассIntegrationConnection, Новый Структура("СписокИни_ВО", МассивДозапрос), Отказ);
	Если Отказ Тогда
	...
	СтруктураВыбранныхНастроек.ПараметрыРаботы.ПроверитьИниПриСохранении = Ложь;
