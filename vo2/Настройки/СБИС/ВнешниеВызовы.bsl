...
Процедура	ДублироватьСобытияСБИСПриИзменении(Кэш, НовоеЗначение) Экспорт
	ЗаписатьОбщиеНастройкиПодключения(Кэш, Новый Структура("СостояниеЭД", НовоеЗначение), Кэш.Парам.ИдентификаторНастроек);	// ДУБЛИРОВАТЬСТАТУСЫСБИС ЧЕКАП
КонецПроцедуры
...
Функция		ЕстьИзмененияНастроек(Кэш) Экспорт 
	...
	Возврат Кэш.КэшНастроек.ИзмененияВНастройках
		Или	Кэш.КэшНастроек.ВыбранныеНастройки.ПараметрыРаботы.Изменен;
...
Процедура	ИнициализироватьКэшНастроек(Кэш, ПараметрыИнициализации=Неопределено, Отказ=Ложь) Экспорт
	...
	Если ТипЗнч(ПараметрыИнициализации) = Тип("Структура") Тогда
		// флаг "Обновить" используется для установки кэша серверных настроек при изменении параметров 		
		Если	ПараметрыИнициализации.Свойство("Обновить")  
			И	ПараметрыИнициализации.Обновить Тогда
			УстановитьПараметрыХранящиесяНаШаблоне(Кэш, ПараметрыИнициализации.Параметр);
			Возврат;
		КонецЕсли;
		
		// флаг "ИнициализацияВременногоКэша" используется для установки кэша серверных настроек при получении Коннекшена для каталожных ини
		ЭтоОбщийКэш	=	Не ПараметрыИнициализации.Свойство("ИнициализацияВременногоКэша")  
					Или Не ПараметрыИнициализации.ИнициализацияВременногоКэша;
	КонецЕсли;
				
	Если ЭтоОбщийКэш Тогда			
		Кэш.ФормаНастроекОбщее.Кэш = Кэш;
	КонецЕсли;
	...
	// Привести название сломанных ини к нормальному виду.
	СоответствиеНазваний = Новый Структура;
	СоответствиеНазваний.Вставить("СчФактКор_3_01", "счФактКор_3_01");

	СоответствиеТипов	= Новый Массив;
	ДоступныеТипыИни	= Новый Массив;
	СоответствиеТипов.Добавить(Новый Структура("json, во", "Конфигурация",		"Конфигурация"));
	СоответствиеТипов.Добавить(Новый Структура("json, во", "сбисМеню",			"сбисМеню"));
	СоответствиеТипов.Добавить(Новый Структура("json, во", "СинхВыгрузка", 		"СинхВыгрузка"));
	СоответствиеТипов.Добавить(Новый Структура("json, во", "СинхЗагрузка", 		"СинхЗагрузка"));
	СоответствиеТипов.Добавить(Новый Структура("json, во", "ЗагрузкаИсходящих",	"Загрузка_Исходящий"));
	СоответствиеТипов.Добавить(Новый Структура("json, во", "Загрузка", 			"Загрузка_Входящий"));
	СоответствиеТипов.Добавить(Новый Структура("json, во", "ВыгрузкаВходящих", 	"Выгрузка_Входящий"));
	СоответствиеТипов.Добавить(Новый Структура("json, во", "Выгрузка", 			""));
	//Так, как в меню могут быть различные типы, не относящиеся к обработке, делаем фильтр.
	ДоступныеТипыИни.Добавить("Система");
	Для Шаг = 4 По СоответствиеТипов.Количество()-1 Цикл
		ДоступныеТипыИни.Добавить(СоответствиеТипов[Шаг].json);
	КонецЦикла;
	ПараметрыНастроек.Вставить("СоответствиеНазваний",	СоответствиеНазваний);//Шаблон перевода имен названий сервис-обработка
	ПараметрыНастроек.Вставить("СоответствиеТипов",		СоответствиеТипов);//Шаблон перевода имен названий сервис-обработка
	...
	СписокПараметровХранящихсяНаШаблоне.Добавить("состояниеэд"); 
	СписокПараметровХранящихсяНаШаблоне.Добавить("НеОтображатьНовостьПриЗапуске"); //bodybr
...
Процедура	КонфигурацияПриИзменении(ПараметрыКонфигурацииВходящие, Кэш) Экспорт
	...
	ИначеЕсли	Не ВыбранныеНастройки.ПараметрыРаботы.Статус = "Новый" Тогда
		...
		СбисИсключение = МодульОбъектаКлиент().НовыйСбисИсключение(719102, "ФайлыНастроекСервер.КонфигурацияПриИзменении",,,"Требуется создать новый шаблон настроек перед сменой конфигурации настроек");
...
Процедура	КонфигурацияПриИзменении_ПослеДиалога(РезультатДиалога, ПараметрыКонфигурацииВходящие) Экспорт
	...
	Попытка
		...
		РезультатДозапроса = КлассIntegrationConnection_ДозапроситьИни(НовыйКлассIntegrationConnection, Новый Структура, Отказ);
		Если Отказ Тогда
...
Процедура	ПриЗапускеРедактораНастроек(ПараметрыЗапускаРедактораВходящие, Кэш) Экспорт
	...
	ВыбранныеНастройки = МодульОбъектаКлиент().ПараметрSaby("ВыбранныеНастройки", "Настройки");
	МодульОбъектаКлиент().КоннекшенSaby_Установить("ВыбранныеНастройки", "ЧтениеПоТребованию", Ложь);
	КлассIntegrationConnection_ЗаполнитьРедакторИниПоКлассу(Кэш, ВыбранныеНастройки, Новый Структура)
...
Функция		КлючиНастроекСБИС(Кэш) Экспорт
	...
	ПараметрыПрименить = Кэш.СБИС.ПараметрыИнтеграции;

	Результат = Новый Массив;
	КлючДобавить = МодульОбъектаКлиент().ПрименитьФорматКСтроке(ШаблонКлюча, ПараметрыПрименить);
	...
	Если ЗначениеЗаполнено(КлючНастроек) Тогда
		...
		Результат.Добавить(МодульОбъектаКлиент().ПрименитьФорматКСтроке(ШаблонКлюча, ПараметрыПрименить));
...
Процедура	СбисПодключитьЗаплатки(Кэш, ДопПараметрыПодключения = Неопределено) Экспорт
	...
	Если Не	ДопПараметрыПодключения = Неопределено
		И	ДопПараметрыПодключения.Свойство("Отладка") Тогда
...
Процедура	СбисВыбратьСоединение(Кэш, ДопПараметры=Неопределено) Экспорт
	...
	ТекстПредупреждения = "Не удалось получить список настроек Saby!";
...
Функция		СбисОтключитьВыбранныеФайлыНастроек(Ини, Кэш, Идентификатор, Отказ=Ложь) Экспорт
	...
	Иначе
		...
		Возврат СбисПолучитьМассивУстановленныхИниФайлов(Кэш, Кэш.КэшНастроек.ВыбранныеНастройки, Отказ);
...
Функция		СбисОпределитьНеобходимостьПроверки(Кэш, УстановленныеИниФайлы, СтруктураПараметров) Экспорт
	...
	Если	Не НужноПроверять
		И	Не СбисПоследнееИзменение = ВыбранныеНастройки.ПараметрыРаботы.ДатаИзмененияНастроек 
		И   Не ВыбранныеНастройки.ПараметрыРаботы.ПроверитьИниПриСохранении Тогда
...
&НаКлиенте
Процедура	СбисПеречитатьНастройки(КоннекшенSABY = Неопределено, ДопПараметры = Неопределено) Экспорт
	
	СбисПараметрыДиалога	= Новый Структура("КоннекшенSABY, Кэш", КоннекшенSABY, ДопПараметры.Кэш);
	ЗадатьВопрос			= Истина;
	Если Не ДопПараметры = Неопределено Тогда
		
		Если ДопПараметры.Свойство("Элемент") Тогда
			
			СбисПараметрыДиалога.Вставить("Элемент", ДопПараметры.Элемент);
			
		КонецЕсли;
		
		Если ДопПараметры.Свойство("Принудительно") Тогда
			
			ЗадатьВопрос = Не ДопПараметры.Принудительно;
	...
	Если КоннекшенSABY = Неопределено Тогда
		
		МодульОбъектаКлиент().ВыполнитьСбисОписаниеОповещения(ОбработчикДиалога, КодВозвратаДиалога.Да);
		Возврат;
		
	КонецЕсли;
	
	//Ничего не менялось
	Если Не МодульОбъектаКлиент().КоннекшенSaby_Получить(КоннекшенSABY, "Изменен") Тогда
		
		ЭлементНазначения = Неопределено;
		СбисПараметрыДиалога.Свойство("Элемент", ЭлементНазначения);
		МодульОбъектаКлиент().СбисСообщить(Новый Структура("Текст, ЭлементНазначения", "Изменения отсутствуют.", ЭлементНазначения));
		Возврат;
...
Процедура	СбисПеречитатьНастройки_ПослеДиалога(РезультатОтвета, ДопПараметры) Экспорт
	Перем НастройкиДоПеречитывания;
	...
	НастройкиДоПеречитывания = ДопПараметры.КоннекшенSABY;
	ЛокальныйКэш = ДопПараметры.Кэш;
	
	РезультатОповестить = Новый Структура("Коннекшен", НастройкиДоПеречитывания);
	
	Попытка
		
		КоннекшенSaby_Прочитать(НастройкиДоПеречитывания, ДопПараметры);
		
	Исключение
		
		СбисИсключение = МодульОбъектаКлиент().НовыйСбисИсключение(	ИнформацияОбОшибке(),
																	"ФайлыНастроекСервер.СбисПеречитатьНастройки.ПослеДиалога");
...
Функция		СбисПолучитьДанныеСервисаИнтеграции(Кэш, ПараметрыЗапроса, Отказ) Экспорт
	...
	ИначеЕсли Не Лев(ТипЗапроса,4) = "List" Тогда
		Для Каждого ПараметрЗапросаИни Из СписокИни Цикл
			ИмяИни = ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, ПараметрЗапросаИни.name);
...
Функция		СбисПолучитьСтруктуруКонфигурации(Кэш, СтруктураКонфигураций, ЗначениеКонфигурации) Экспорт
		...
		Если	Не Параметры_Предыдущее.subsystem = ВыбранныеНастройки.ПараметрыКонфигурации.subsystem
			...
			Если	ЗначениеЗаполнено(ВыбранныеНастройки.ПараметрыКонфигурации.subsystem)
				...
				Для Каждого КлючИЗначение Из ВыбранныеНастройки.Данные.Ини Цикл
					СписокИскать.Добавить(ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, КлючИЗначение.Ключ, "во_json"));
				КонецЦикла;
	...
	Для	Каждого	Ини	Из	ДанныеСписка.Карта	Цикл
		ИмяИни = ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, Ини.Представление);
...
Функция	СбисПолучитьНастройкиДляТП(ПараметрыВыгрузки) Экспорт
	...
	Если Отказ Тогда
		...
		Если СтруктураТекущихНастроек.code = 765 Тогда//Не нашли типовые настройки. Вообще всё пропало!
			Возврат СтруктураТекущихНастроек;
		КонецЕсли;
		Отказ = Ложь;
		СбисПеречитатьНастройки();
	...
	ПараметрыВыгрузки.Вставить("ВыборДляДиалога", Новый ФиксированнаяСтруктура("Значение", 2)); // Заранее поставить вариант выгрузки ини - раздельно.
...
Функция	СбисПолучитьНастройкиССервера_Клиент(ПараметрыВыгрузки)  Экспорт
	...
	Если Не ЗначениеЗаполнено(ВыбранныеНастройки.Данные.Ини) Тогда
		Возврат "Отсутствуют файлы настроек для выгрузки."
	КонецЕсли;
	...
	ВременныйКэшНастроек = МодульОбъектаКлиент().НовыйКэшНастроек();
	ЗаполнитьЗначенияСвойств(ВременныйКэшНастроек, Кэш.КэшНастроек);
	ВременныйКэшНастроек.Вставить("ВыбранныеНастройки", Кэш.КэшНастроек.ВыбранныеНастройки);
	...
	ВременныйКэш.Вставить("КэшНастроек",		ВременныйКэшНастроек);
	...
	Если Не ВременныйКэш.Парам.СпособХраненияНастроек = 0 Тогда
		...
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение("Не удалось установить форму работы с настройками!", "ФайлыНастроекСервер.СбисПолучитьНастройкиССервера_Клиент");
	...
	Если Отказ Тогда
		...
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ДанныеСписка, "ФайлыНастроекСервер.СбисПолучитьНастройкиССервера_Клиент");
	...
	Для Каждого КлючИЗначение Из ИниКВыгрузке Цикл
		ИмяИни = ПолучитьИмяИни(Кэш.КэшНастроек.ПараметрыНастроек, КлючИЗначение.Ключ, "во_json");
	...
	ПараметрыВыгрузки.Вставить("СбисПараметры",			ВыбранныеНастройки.Данные.Параметры);
	
	Если ПараметрыВыгрузки.Свойство("ВыборДляДиалога") Тогда
		
		Возврат СбисПолучитьНастройкиССервера_Клиент_ОбработчикПослеДиалога(ПараметрыВыгрузки.ВыборДляДиалога, ПараметрыВыгрузки);
...
Процедура	СбисПолучитьНастройкиССервера_Клиент_ПослеДиалога(РезультатВвода, ПараметрыВыгрузки) Экспорт
	...
	Результат = СбисПолучитьНастройкиССервера_Клиент_ОбработчикПослеДиалога(РезультатВвода, ПараметрыВыгрузки);
	Если МодульОбъектаКлиент().ЭтоТипСбис(Результат, "СбисИсключение") Тогда
		
		ПараметрыВыгрузки.ОписаниеОповещенияОЗавершении.ДополнительныеПараметры.Отказ = Истина;
		
	КонецЕсли;	
	МодульОбъектаКлиент().ВыполнитьСбисОписаниеОповещения(Результат, ПараметрыВыгрузки.ОписаниеОповещенияОЗавершении);
	
КонецПроцедуры

&НаКлиенте
Функция	СбисПолучитьНастройкиССервера_Клиент_ОбработчикПослеДиалога(РезультатВвода, ПараметрыВыгрузки)
	...
	Если РежимВыгрузки = 1 Тогда
		...
		КлассДляВыгрузки = МодульОбъектаКлиент().НовыйКоннекшенSABY(ПараметрыВыгрузки.КлассНастроек);
		МодульОбъектаКлиент().КоннекшенSABY_Установить(КлассДляВыгрузки, "Идентификатор", ПараметрыВыгрузки.КаталогНаКлиенте);
		ВременныйКэш.КэшНастроек.Вставить("ВыбранныеНастройки", КлассДляВыгрузки);
		
		ОписаниеИниВыгрузить = МодульОбъектаКлиент().КоннекшенSABY_Получить(КлассДляВыгрузки, "ОписаниеИни");
		//УстановленныеИни = ОписаниеИниВыгрузить.МассивУстановленныхИниФайлов;  
		УстановленныеИни = ПолучитьМассивУстановленныхИниФайлов(ВременныйКэш.ини, , Отказ, ПараметрыВыгрузки.Префикс);
		//УстановленныеИни = ПолучитьМассивУстановленныхИниФайлов(ВременныйКэш.ини, ВременныйКэш.КэшНастроек.ПараметрыВыгрузки.КлассНастроек, Отказ, ПараметрыВыгрузки.Префикс);
	...
	Иначе
		Если ПараметрыВыгрузки.Типовые.Количество() Тогда
			...
			Если Отказ Тогда
				...
				Возврат МодульОбъектаКлиент().НовыйСбисИсключение(ИниКВыгрузке, "ФайлыНастроекСервер.СбисПолучитьНастройкиССервера.Клиент.ОбработчикПослеДиалога");
			...
			УстановленныеИни = ПолучитьМассивУстановленныхИниФайлов(ВременныйКэш.ини, КлассIntegrationConnection, Отказ,ПараметрыВыгрузки.Префикс);
			Для Каждого УстановленнаяИни Из УстановленныеИни Цикл
				СбисРеестрПоУмолчанию = Кэш.ОбщиеФункции.РассчитатьЗначение("РеестрПоУмолчанию", ВременныйКэш.Ини[УстановленнаяИни.Ини_ИмяИни]);
				Если СбисРеестрПоУмолчанию = "0" Тогда
					УстановленнаяИни.Ини_ИмяБезРасширения = УстановленнаяИни.Ини_ИмяБезРасширения + ".0";
		...
		Если ПараметрыВыгрузки.Пользовательские.Количество() Тогда	
			...
			Иначе//Получаем с сервера только пользовательские изменения
				...
				КлассIntegrationConnection_ДозапроситьИни(ПользовательскиеИзменения, Новый Структура("ТолькоПользовательские", Истина), Отказ);
	...
	Если ПараметрыВыгрузки.Свойство("СбисПараметры") Тогда
		ПараметрыСбис = Новый Структура("СбисПараметры", ПараметрыВыгрузки.СбисПараметры);      
		ЭлементМассива = Новый Структура("Имяфайла, ИмяфайлаБезРасширения, Ини_ИмяБезРасширения, Ини_ИмяИни, ЕстьИзменения, ПутьДоИниФайла",
		"СбисПараметры", "СбисПараметры", "СбисПараметры", "СбисПараметры", Ложь, "");
		МассивПараметров = Новый Массив;
		МассивПараметров.Добавить(ЭлементМассива);
		ВременныйКэш.ФормаНастроек.СохранитьПараметрыСБИС(Кэш, ПараметрыСбис, ПараметрыВыгрузки.КаталогНаКлиенте,, Истина);
	КонецЕсли;
	
	Возврат ВременныйКэш.Парам.КаталогНастроек;
...
Процедура	СбисЗапускПримененияНастроекСФормыПослеЗакрытия() Экспорт
	...
	Если	СбисДанныеДиалога.Параметры.РежимВыбора = "ВыборПодключения" Тогда
		Если		КлассIntegrationConnection.ПараметрыРаботы.Шаблон = Неопределено
			...
			ДопПараметрыДиалога = Новый Структура("ТипКоннекшена, Скрыть, ЗапретНовых, Заголовок, ОбработчикРезультата", "account", Новый Массив, Ложь, "Укажите шаблон для нового подключения:", ОписаниеОповещенияЗавершение);
...
Процедура	СбисСохранитьВыбранныеНастройки(Кэш, ДопПараметры) Экспорт
	...
	Если КлассIntegrationConnection.ПараметрыРаботы.ПроверитьИниПриСохранении Тогда  
		КлассIntegrationConnection.ПараметрыРаботы.Вставить("ДатаИзмененияНастроек", ТекущаяДата());
		КлассIntegrationConnection.ПараметрыРаботы.Вставить("ПроверитьИниПриСохранении", Ложь);    
		
		ПараметрыРаботы.Вставить("ВыполнитьПроверку", Истина);
		Попытка
			Кэш.ФормаНастроекОбщее.СбисПолучитьИУстановитьНастройкиВКэш(Новый Структура("ПринудительнаяПроверка, ПродолжитьУстановку", Истина, Истина), Кэш); 
		Исключение
			ИнфоОбОшибке = ИнформацияОбОшибке();
	...
	Иначе
		...
		КлассIntegrationConnectionШаблон = Неопределено;
		Если Кэш.КэшНастроек.Свойство("КоннекшенИнтеграции", КлассIntegrationConnectionШаблон) Тогда

			КлассIntegrationConnectionШаблон.ПараметрыРаботы.СбисПараметры.Обновить = Ложь;
			КлассIntegrationConnectionШаблон.ПараметрыРаботы.СбисПараметры.Отправлять = Ложь;
			КлассIntegrationConnectionШаблон.ПараметрыРаботы.СбисМеню.Обновить = Истина;
			КлассIntegrationConnectionШаблон.ПараметрыРаботы.СбисМеню.Отправлять = Ложь;
			_КлассIntegrationConnection_Записать(Кэш, КлассIntegrationConnectionШаблон, ДопПараметры, ДопПараметры.Отказ);
			Кэш.КэшНастроек.Удалить("КоннекшенИнтеграции");
    ...
    // Установить выбранные настройки после записи глобально.
	МодульОбъектаКлиент().ИзменитьПараметрSaby("ВыбранныеНастройки", КлассIntegrationConnection, "Настройки");
...
Процедура	СбисУстановитьВыбранныеФайлыНастроек(Ини, Кэш, Идентификатор, Отказ) Экспорт
    Перем СтруктураВыбранныхНастроек;
...
Функция		СбисЗаполнитьНеполученныеНастройки(Кэш, Отказ) Экспорт
	...
	Возврат КлассIntegrationConnection_ДозапроситьИни(Кэш.КэшНастроек.ВыбранныеНастройки,Новый Структура,Отказ);
...
Функция СформироватьПрефиксНабораНастроек(Кэш) Экспорт 
	...
	Если Кэш.КэшНастроек.Свойство("ВыбранныеНастройки") Тогда
		ВыбранныеНастройки = Кэш.КэшНастроек.ВыбранныеНастройки;
		Возврат Кэш.ФормаНастроекОбщее.СформироватьПрефиксСистемыКлиент(ВыбранныеНастройки.ПараметрыКонфигурации);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
