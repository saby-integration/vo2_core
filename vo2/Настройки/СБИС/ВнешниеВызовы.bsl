...
Процедура	КонфигурацияПриИзменении(ПараметрыКонфигурацииВходящие, Кэш) Экспорт
	...
	ИначеЕсли	Не ВыбранныеНастройки.ПараметрыРаботы.Статус = "Новый" Тогда
		...
		СбисИсключение = МодульОбъектаКлиент().НовыйСбисИсключение(, "ФайлыНастроекСервер.КонфигурацияПриИзменении",,,"Требуется создать новый шаблон настроек перед сменой конфигурации настроек");
...
Процедура	СбисПолучитьНастройкиССервера_Клиент(ПараметрыВыгрузки)  Экспорт
	...
	ВременныйКэш.Вставить("КэшНастроек",		Кэш.КэшНастроек);
