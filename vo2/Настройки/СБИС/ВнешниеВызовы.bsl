...
Процедура	ПриЗапускеРедактораНастроек(ПараметрыЗапускаРедактораВходящие, Кэш) Экспорт
	...
	Если		КэшироватьТекущие Тогда
		...
		Если Отказ Тогда
			...
			КонецЕсли;
			Отказ = Ложь;
			СбисПеречитатьТекущиеНастройки(Новый  Структура, Кэш);
...
Процедура	СбисСохранитьВыбранныеНастройки(Кэш, ДопПараметры) Экспорт
	...
	Иначе
		...
		Если Кэш.КэшНастроек.Свойство("КоннекшенИнтеграции", КлассIntegrationConnectionШаблон)
			И МодульОбъектаКлиент().ПолучитьЗначениеФичи("ВыгрузкаНастроекНаШаблон") Тогда
