...
Функция		ПрочитатьОбщуюНастройкуПодключения(ОписаниеНастройкиВходящее, Кэш) Экспорт
	...
	ИнтеграцияSABY = МодульОбъектаКлиент().КоннекшенSaby_Получить(КлассПодключенияЗаписи, "Интеграция");
	ИдЧтенияПараметров = МодульОбъектаКлиент().КоннекшенSaby_Получить(ИнтеграцияSABY, "Идентификатор");
...
Функция		ЗаписатьОбщиеНастройкиПодключения(Кэш, СтруктураНастроек, ИдентификаторНастроек, Отказ=Ложь) Экспорт
	...
	КлассIntegrationConnection_СохранитьСбисПараметры(Кэш, ИнтеграцияSABY, Новый Структура("ПараметрыДописать", СтруктураНастроек), Отказ);
	Если Отказ Тогда
		Возврат МодульОбъектаКлиент().НовыйСбисИсключение(, "ФайлыНастроекСервер.ЗаписатьОбщиеНастройкиПодключения", 616301);
	КонецЕсли;

	//МодульОбъектаКлиент().ИнтеграцияSABY_Установить(ИнтеграцияSABY, "ПараметрыSaby", СтруктураНастроек);
	//РезультатЗаписи = ИнтеграцияSABY_Записать(ИнтеграцияSABY, "Параметры", Отказ);
