...
Функция сбисВыполнитьКоманду(Кэш, МассивИд, ИмяКоманды, ДополнительныеПараметры = Неопределено) Экспорт	
	ГлавноеОкно			= Кэш.ГлавноеОкно;
	МодульВызоваКоманды	= ГлавноеОкно.сбисНайтиФормуФункции("сбисВыполнитьКоманду",			Кэш.ИнтеграцияИмя,			"",Кэш);
	МодульСтатусов		= ГлавноеОкно.сбисНайтиФормуФункции("СбисРазобратьИдентификатор",	Кэш.ФормаРаботыСоСтатусами,	"",Кэш);
	...
	Для Каждого Идентификатор Из МассивИд Цикл
		...
		Иначе
			...
			ИдПакета	= ОписаниеИд.ИдДокумента;
		КонецЕсли;
		КомандаВыполнена	= МодульВызоваКоманды.сбисВыполнитьКоманду(Кэш, ИдПакета, ИмяКоманды, "");
...
Процедура сбисЗапуститьОтправку(СписокДокументовКОтправке, ДопПараметры) Экспорт
	...
	ИначеЕсли	Не ВладелецФормы.Кэш.Текущий.Свойство("Форма") Тогда
		...
		//Установить раздел в Продажа по-дефолту, если ничего не передано
...
#Область include_core_vo2_ПубличныйИнтерфейс
#КонецОбласти
