	...
	Если Действие.ТребуетПодписания="Да" Тогда
		...
		Исключение
			...
			Если МодульТекущий.ПолучитьЗначениеПараметраСБИС("ТихийРежим") Тогда
				...
				ВызватьИсключение МодульТекущий.СбисИсключение_Представление(ОшибкаСертификата);
...
Функция ОтправитьПакетыДокументов(Кэш, МассивПакетов, РезультатОтправки=Неопределено) Экспорт
	...
	Для Каждого СтруктураДок Из documenty Цикл
		...
		Иначе
			...
			Если МассивПакетов[0].Параметры.Свойство("НеЗапускатьВДокументооборот") и МассивПакетов[0].Параметры.НеЗапускатьВДокументооборот = Истина Тогда   // если не запускаем в документооборот, то сразу проставляем состояния на документах 1С и больше ничего не делаем
...
Функция АвторизоватьсяПоЛогинуПаролюНаСервере(Кэш, Логин, Пароль, Отказ=Ложь) Экспорт 	
	...
	СтруктураПараметровЗапроса = Новый Структура("Логин, Пароль", Логин, Пароль);
	ПараметрыРезультат = Новый Структура("Метод, Параметры, ДополнительныеПараметры", ИмяМетода, СтруктураПараметровЗапроса, ДопПараметры);
	ПараметрыРезультат.Вставить("ПоСертификату", Ложь);
	...
	Если Отказ Тогда
		Если РезультатОтвет.code = (-32000) Тогда
			...
			Если	Найти(РезультатОтвет.message, "СБИС.Диск")
				...
				РезультатОтвет = Кэш.ОбщиеФункции.СбисИсключение(РезультатОтвет, "API.СбисОтправитьИОбработатьКоманду", 301, "Сервис Saby.Диск временно недоступен", РезультатОтвет.details);
			Иначе
...
Функция СбисВыполнитьКомандуНаСервере(Кэш, ПараметрыКоманды, ДопПараметры, Отказ)
	...
	Попытка
		Если Кэш.СовместимостьМетодов.HTTPСоединение.ФункцияПолучить Тогда
			...
			Исключение
				Отказ 			= Истина;
				СбисИсключение	= МодульОбъектаСервер().НовыйСбисИсключение(,"API.СбисВыполнитьКомандуНаКлиенте",
				...
				МодульОбъектаСервер().СообщитьСбисИсключение(СбисИсключение);		
		...
		Иначе
			...
			Исключение
				...
				ОшибкаВыполненияЗапроса = МодульОбъектаСервер().НовыйСбисИсключение(ИнформацияОбОшибке(), "Соединение.ОтправитьДляОбработки", 300);
				Отказ = Истина;
...
&НаКлиентеНаСервереБезКонтекста
Функция СбисСформироватьЗаголовки(КэшПарам, ДопПараметры)
	Заголовки = Новый Соответствие;
	Если	ДопПараметры.Свойство("ИдСессии")
		И	ЗначениеЗаполнено(ДопПараметры.ИдСессии) Тогда
		Заголовки.Вставить("X-SBISSessionID", ДопПараметры.ИдСессии);
	КонецЕсли;
	Если ДопПараметры.Свойство("ТаймАут") Тогда
		Заголовки.Вставить("X-Timeout", Формат(ДопПараметры.ТаймАут * 1000, "ЧН=0; ЧГ=0"));
	КонецЕсли;
...
Функция СбисОтправитьИОбработатьКомандуGETНаСервере(КэшСервер, ПараметрыЗапроса, Отказ) Экспорт
	...
	Если Не ПараметрыЗапроса.Свойство("Заголовки", Заголовки) Тогда
		...
		КонецЕсли;
		ПараметрыЗаголовки = Новый Структура("ИдСессии, ТаймАут", Session_ID, ДопПараметрыКоманды.X_Timeout);
		Заголовки	= СбисСформироватьЗаголовки(КэшСервер.Парам, ПараметрыЗаголовки);
		ПараметрыЗапроса.Вставить("Заголовки", Заголовки);
	КонецЕсли;
