...
Функция СбисОтправитьИОбработатьКомандуНаСервере(Знач Кэш, Знач Метод, Знач ПараметрыМетода, Знач ДопПараметры, Отказ) Экспорт
	...
	Возврат МодульОбъектаСервер().API_ОтправитьИОбработатьКомандуНаСервере(Кэш, Метод, ПараметрыМетода, ДопПараметры, Отказ);
	...
	Попытка
		...
		Иначе
			...
			Исключение
				...
				ОшибкаВыполненияЗапроса = МодульОбъектаКлиент().НовыйСбисИсключение(ИнформацияОбОшибке(), "Соединение.ОтправитьДляОбработки", 300);
				Отказ = Истина;
