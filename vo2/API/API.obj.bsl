...
Функция сбисПолучитьПараметрыАктуальнойВерсии(Кэш, ПараметрыОбновления, Отказ) Экспорт
	...
	УРЛОбновления = МодульОбъектаКлиент().СбисФормат(УРЛОбновления, Новый Структура("Заменить", ПараметрыОбновления));
...
Функция сбисСохранитьВФайлПоСсылке(Кэш, СбисПараметрыФайла, Отказ) Экспорт
	...
	Если			СбисПараметрыФайла.Свойство("URL",	УРЛОбновления) Тогда
		...
		Если Не Лев(УРЛОбновления, 8) = "https://" Тогда   //передан полный url
			...
			УРЛОбновления		= МодульОбъектаКлиент().СбисФормат(УРЛОбновленияШаблон, Новый Структура("Заменить", СбисПараметрыФайла));
		КонецЕсли;
...
Функция СбисОтправитьИОбработатьКоманду(Кэш, Метод, ПараметрыМетода, ДопПараметры, Отказ) Экспорт
	...
	Если Отказ Тогда
		Если РезультатОтвет.code = (-32000) Тогда
			...
			Иначе	
				РезультатОтвет = Кэш.ОбщиеФункции.СбисИсключение(РезультатОтвет, "API.СбисОтправитьИОбработатьКоманду", 300);
			КонецЕсли;
		КонецЕсли;
...
Функция СбисОтправитьИОбработатьКомандуНаСервере(КэшПарам, Метод, ПараметрыМетода, ДопПараметры, Отказ) Экспорт
	...
	Если Отказ Тогда
		...
		РезультатОтвет = ОбработкаОбъект.НовыйСбисИсключениеСервер(РезультатКоманды, "API.сбисВыполнитьКомандуНаСервере");
	Иначе
		...
		КонецЕсли;
		Попытка
			РезультатВыполнения = ОбработкаОбъект.СбисПрочитатьJSON_НаСервере(РезультатКоманды,,,ПреобразовыватьДаты);
		Исключение
			Отказ = Истина;
			Если Лев(СокрЛП(РезультатКоманды),1)="{" Тогда     // кривой json не смогли разобрать
				РезультатОтвет = ОбработкаОбъект.НовыйСбисИсключениеСервер(, "РаботаСJSON.СбисПрочитатьJSON_НаСервере", 785, "Ошибка во входящих данных", "Ошибка при разборе ответа от сервера: " + ОписаниеОшибки());
			Иначе    // в РезультатКоманды вернулась ошибка строкой
				РезультатОтвет = ОбработкаОбъект.НовыйСбисИсключениеСервер(, , , РезультатКоманды, РезультатКоманды);
			КонецЕсли;
		КонецПопытки;
		Если Не Отказ Тогда
			...
			Если РезультатВыполнения.Свойство("error", РезультатОтвет) Тогда
				Отказ = Истина;
				РезультатОтвет = ОбработкаОбъект.НовыйСбисИсключениеСервер(РезультатОтвет, Метод);
			ИначеЕсли Не РезультатВыполнения.Свойство("result", РезультатОтвет) Тогда
				Если	ДопПараметры.Свойство("ЕстьРезультат", Checkparam)
					И	Checkparam Тогда
					Отказ = Истина;
					РезультатОтвет = ОбработкаОбъект.НовыйСбисИсключениеСервер(, "API.сбисОтправитьИОбработатьКоманду", 775, "Не удалось получить ответ сервера", "Отсутствует результат выполнения метода " + Метод + "()");
				Иначе
...
#Область include_core_vo2_СпособыОбмена_API_ВнешниеОбертки
#КонецОбласти

#Область include_core_vo2_СпособыОбмена_API_ВнешниеОбертки_АсинхроннаяОтправка
#КонецОбласти 

#Область include_core_vo2_СпособыОбмена_API_ВнешниеОбертки_АсинхронноеВыполнениеДействия
#КонецОбласти

#Область include_core_vo2_СпособыОбмена_API_ВнешниеОбертки_СопоставлениеНоменклатуры
#КонецОбласти